...
export interface TestCaseDetailEditorReadOnlyProps {
  readonly version: TestCaseVersionRecord;
}

export function TestCaseDetailEditorReadOnly({
  version,
}: TestCaseDetailEditorReadOnlyProps): ReactNode {
  const hasDescription =
    version.description !== undefined && version.description.length > 0;
  return (
    <div className="grid gap-3">
      <TextField label="Title" value={version.title} onChange={NOOP} disabled />
      <DisabledTextArea label="Objective" value={version.objective} rows={3} />
      {hasDescription && (
        <DisabledTextArea
          label="Description"
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div className="grid gap-1">
          <span className="text-xs font-medium text-fg-default">Priority</span>
