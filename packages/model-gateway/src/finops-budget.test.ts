...
void test("ALLOWED_FINOPS_ROLES enumerates the known roles", () => {
  assert.deepEqual(
...
void test("ALLOWED_FINOPS_JOB_OUTCOMES includes both completed and budget_exceeded", () => {
...
void test("ALLOWED_FINOPS_BUDGET_BREACH_REASONS covers wall-clock, retries, replay-cache miss rate", () => {
...
void test("DEFAULT_FINOPS_BUDGET_ENVELOPE has no role limits", () => {
...
void test("cloneEuBankingDefaultFinOpsBudget returns a deep clone", () => {
...
void test("cloneFinOpsBudgetEnvelope drops unknown role keys (defensive)", () => {
  const envelope = {
    budgetId: "x",
    budgetVersion: "1.0.0",
    roles: {
...
void test("validateFinOpsBudgetEnvelope accepts the EU-banking default", () => {
  const result = validateFinOpsBudgetEnvelope(EU_BANKING_DEFAULT_FINOPS_BUDGET);
  assert.equal(result.valid, true);
  assert.deepEqual(result.errors, []);
});

void test("validateFinOpsBudgetEnvelope rejects empty budgetId", () => {
...
void test("validateFinOpsBudgetEnvelope rejects negative wall-clock", () => {
  const result = validateFinOpsBudgetEnvelope({
    budgetId: "x",
    budgetVersion: "1",
    maxJobWallClockMs: -1,
...
void test("validateFinOpsBudgetEnvelope rejects miss rate outside [0, 1]", () => {
...
void test("validateFinOpsBudgetEnvelope rejects unknown role keys", () => {
  const envelope = {
    budgetId: "x",
    budgetVersion: "1",
    roles: { unknown_role: {} },
...
void test("validateFinOpsBudgetEnvelope rejects image fields on test_generation", () => {
  const result = validateFinOpsBudgetEnvelope({
    budgetId: "x",
    budgetVersion: "1",
    roles: {
...
void test("validateFinOpsBudgetEnvelope rejects maxFallbackAttempts on visual_primary", () => {
  const result = validateFinOpsBudgetEnvelope({
    budgetId: "x",
    budgetVersion: "1",
    roles: {
...
void test("validateFinOpsBudgetEnvelope rejects zero token budgets but allows zero retries", () => {
...
void test("resolveFinOpsRequestLimits returns empty for undefined budget", () => {
  assert.deepEqual(resolveFinOpsRequestLimits(undefined), {});
});

void test("resolveFinOpsRequestLimits maps the four request fields", () => {
...
void test("resolveWallClockBudget applies the default elastic coefficients", () => {
...
void test("Epic #2167 P0 scenario — T7l7 11-case 2-judge 1-adversarial visual run clears the observed 194 879 ms breach", () => {
...
void test("resolveWallClockBudget clamps to the configured hard ceiling", () => {
  const budget = resolveWallClockBudget({
    caseCount: 500,
    judgePanelSize: 4,
    adversarialRounds: 8,
...
void test("resolveTestGenerationWallClockBudget preserves explicit constant overrides", () => {
...
void test("gateway: maxWallClockMs breach is fail-closed (retryable=false, attempts=1)", async () => {
...
void test("gateway: timeout without maxWallClockMs stays retryable", async () => {
  let dispatchCount = 0;
  const fetchImpl: typeof fetch = async (_url, init) => {
    dispatchCount += 1;
    return new Promise((_resolve, reject) => {
...
void test("gateway: maxWallClockMs covers delayed response body reads", async () => {
  let dispatchCount = 0;
  const fetchImpl: typeof fetch = async (_url, init) => {
    dispatchCount += 1;
    const signal = (init as RequestInit | undefined)?.signal;
...
void test("gateway: per-request maxRetries caps total attempts to min(config, request)", async () => {
  let dispatchCount = 0;
...
void test("gateway: per-request maxRetries=0 disables retry entirely", async () => {
  let dispatchCount = 0;
  const fetchImpl: typeof fetch = async () => {
    dispatchCount += 1;
    return new Response("server error", { status: 503 });
...
void test("gateway: invalid maxRetries / maxWallClockMs surface as schema_invalid", async () => {
  const fetchImpl: typeof fetch = async () =>
...
void test("gateway: maxOutputTokens requires provider support", async () => {
  let dispatchCount = 0;
  const fetchImpl: typeof fetch = async () => {
    dispatchCount += 1;
...
void test("gateway: reported output token overrun fails closed", async () => {
  const fetchImpl: typeof fetch = async () =>
    new Response(
      JSON.stringify({
        choices: [
...
void test("PRODUCTION_FINOPS_BUDGET_ENVELOPE validates clean", () => {
  const result = validateFinOpsBudgetEnvelope(
    PRODUCTION_FINOPS_BUDGET_ENVELOPE,
  );
...
void test("PRODUCTION_FINOPS_BUDGET_ENVELOPE pins the calibrated production limits", () => {
...
void test("PRODUCTION_FINOPS_BUDGET_ENVELOPE is non-permissive vs DEFAULT (every field tighter or equal)", () => {
...
void test("cloneProductionFinOpsBudgetEnvelope returns a fresh, mutable copy", () => {
