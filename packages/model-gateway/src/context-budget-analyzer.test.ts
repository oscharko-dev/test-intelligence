...
void test("context-budget analyzer returns none when the full prompt fits", () => {
...
void test("context-budget analyzer compacts prompt payloads before dropping context", () => {
  const result = analyzeContextBudget({
    ...baseInput(),
    maxInputTokens: 800,
  });
  ...
  assert.equal(
    result.report.categories.find(
      (category) => category.kind === "repair_history",
    )?.status,
    "compacted",
  );
...
void test("context-budget analyzer drops optional context when compaction is insufficient", () => {
  const result = analyzeContextBudget({
    ...baseInput(),
    maxInputTokens: 650,
    categories: [
  ...
  assert.equal(
    result.report.categories.find(
      (category) => category.kind === "source_context",
    )?.status,
    "dropped",
  );
...
void test("context-budget analyzer returns needs_review when required context still cannot fit", () => {
  const result = analyzeContextBudget({
    ...baseInput(),
    maxInputTokens: 100,
    categories: [
...
void test("context-budget analyzer report excludes raw prompt payload", () => {
  const result = analyzeContextBudget({
    ...baseInput(),
...
void test("context-budget analyzer hash ignores volatile execution job ids", () => {
  const first = analyzeContextBudget({
    ...baseInput(),
    jobId: "ti-cli-111",
    parentJobId: "parent-111",
    maxInputTokens: 3_000,
    categories: [
      {
        kind: "business_intent_ir",
  ...
  const second = analyzeContextBudget({
    ...baseInput(),
    jobId: "ti-cli-222",
    parentJobId: "parent-222",
    maxInputTokens: 3_000,
    categories: [
      {
        kind: "business_intent_ir",
  ...
  assert.equal(first.contextBudgetHash, second.contextBudgetHash);
  assert.equal(first.report.jobId, "ti-cli-111");
  assert.equal(second.report.jobId, "ti-cli-222");
});
