...
void test("mock: default success envelope is deterministic across calls", async () => {
...
void test("mock: responder controls outcome", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "gpt-oss-120b",
    modelRevision: "rev",
...
void test("mock: image guard rejects payload to test_generation without invoking responder", async () => {
  let invoked = 0;
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "gpt-oss-120b",
...
void test("mock: input budget rejects oversized prompts without invoking responder", async () => {
  let invoked = 0;
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "gpt-oss-120b",
...
void test("mock: visual roles accept image payloads", async () => {
  const client = createMockLlmGatewayClient({
    role: "visual_primary",
...
void test("mock: recordedRequests returns defensive snapshots", async () => {
  const client = createMockLlmGatewayClient({
    role: "visual_primary",
    deployment: "llama-4-maverick-vision",
    modelRevision: "rev",
...
void test("mock: rejects test_generation role declaring imageInputSupport", () => {
  assert.throws(
    () =>
      createMockLlmGatewayClient({
        role: "test_generation",
...
void test("mock: circuit breaker opens after configured transient failures", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "gpt-oss-120b",
    modelRevision: "rev",
...
void test("mock: non-retryable transport failures do not open the circuit breaker", async () => {
  let calls = 0;
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "gpt-oss-120b",
...
void test("mock: retryable flags on non-transient classes do not open the circuit breaker", async () => {
  let calls = 0;
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "gpt-oss-120b",
...
void test("mock: reset clears recorded requests and breaker state", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "gpt-oss-120b",
    modelRevision: "rev",
...
void test("createMockLlmGatewayClientFromConfig copies identity and config", async () => {
