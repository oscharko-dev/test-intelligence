...
void test("circuit-breaker: starts closed and admits requests", () => {
...
void test("circuit-breaker: opens after failureThreshold transient failures", () => {
...
void test("circuit-breaker: rejects requests while open", () => {
  const breaker = createLlmCircuitBreaker({
    failureThreshold: 1,
    resetTimeoutMs: 1000,
  });
...
void test("circuit-breaker: transitions to half_open after reset timeout", () => {
  const clock = fakeClock();
  const transitions: LlmCircuitTransitionEvent[] = [];
  const breaker = createLlmCircuitBreaker({
    failureThreshold: 1,
...
void test("circuit-breaker: half_open success closes; half_open failure re-opens", () => {
  const clock = fakeClock();
  const breaker = createLlmCircuitBreaker({
    failureThreshold: 1,
    resetTimeoutMs: 100,
...
void test("circuit-breaker: non-transient outcome resets the breaker", () => {
  const breaker = createLlmCircuitBreaker({
    failureThreshold: 5,
    resetTimeoutMs: 1000,
  });
...
void test("circuit-breaker: rejects invalid configuration", () => {
  assert.throws(
...
void test("circuit-breaker: restores an open breaker from persisted state", () => {
