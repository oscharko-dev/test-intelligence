...
void test("constants: TASK_COMPLEXITY_TIERS is the closed low/mid/high vocabulary", () => {
  assert.deepEqual(
    [...TASK_COMPLEXITY_TIERS],
...
void test("constants: classifier identity is stable", () => {
  assert.equal(TASK_CLASSIFIER_ROLE_ID, "task_classifier");
  assert.equal(TASK_CLASSIFIER_VERSION, "1.0.0");
});

void test("constants: default sampling + regression threshold are sane", () => {
...
void test("ordering: compareTaskComplexityTier ranks tiers low<mid<high", () => {
...
void test("ordering: maxTaskComplexityTier returns the more capable tier", () => {
...
void test("classifyTask: rejects empty taskId", () => {
  assert.throws(
    () => classifyTask({ taskId: "" }),
...
void test("classifyTask: simple_ui_validation routes to tier-low when small", () => {
...
void test("classifyTask: standard_business_logic defaults to tier-mid", () => {
  const decision = classifyTask({
    taskId: "task-2",
...
void test("classifyTask: complex_calculation forces tier-high", () => {
  const decision = classifyTask({
    taskId: "task-3",
...
void test("classifyTask: regulatory_inference forces tier-high", () => {
  const decision = classifyTask({
    taskId: "task-4",
    taskKind: "standard_business_logic",
...
void test("classifyTask: vision routes to at least tier-mid", () => {
  const decision = classifyTask({ taskId: "task-5", taskKind: "vision" });
  assert.equal(decision.tier, "tier-mid");
});

void test("classifyTask: large input/output tokens escalate from low to mid", () => {
  const decision = classifyTask({
    taskId: "task-6",
    taskKind: "simple_ui_validation",
...
void test("classifyTask: missing constrained decoding never picks tier-low", () => {
  const decision = classifyTask({
    taskId: "task-7",
    taskKind: "simple_ui_validation",
...
void test("classifyTask: role-based fallback maps generator → standard_business_logic → tier-mid", () => {
...
void test("classifyTask: role-based fallback maps adversarial_critic → tier-mid", () => {
  const decision = classifyTask({
    taskId: "task-9",
...
void test("classifyTask: same input always yields the same decision (determinism)", () => {
  const input = {
    taskId: "task-stable",
...
void test("classifyTask: outputs are deeply frozen", () => {
  const decision = classifyTask({
    taskId: "task-10",
    taskKind: "simple_ui_validation",
  });
...
void test("classifyTaskBatch: preserves input order and produces a frozen array", () => {
...
void test("taskClassificationGroupKey: stable kind::tier key", () => {
  const decision = classifyTask({
    taskId: "task-11",
    taskKind: "vision",
  });
...
void test("vocabulary: every TASK_CLASSIFIER_TASK_KIND can be classified", () => {
