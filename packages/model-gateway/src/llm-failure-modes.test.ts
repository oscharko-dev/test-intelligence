...
void test("llm-failure-modes: malformed JSON response (truncated) marks validation blocked", () => {
...
void test("llm-failure-modes: mock gateway responder returning malformed JSON produces schema_invalid errorClass", async () => {
  ...
  assert.equal(result.retryable, false, "schema_invalid must not be retryable");
});

void test("llm-failure-modes: incomplete JSON (valid prefix, missing closing brace) is schema_invalid", async () => {
...
void test("llm-failure-modes: shell-injection-shaped step content is flagged as semantic_suspicious_content (gap closed by Issue #1413)", () => {
...
void test("llm-failure-modes: refusal from mock gateway propagates to export pipeline as policy_blocked_cases_present", async () => {
...
void test("llm-failure-modes: timeout errorClass is retryable=true", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "gpt-oss-120b",
    modelRevision: "rev",
...
void test("llm-failure-modes: timeout failures open the circuit breaker after threshold", async () => {
...
void test("llm-failure-modes: timeout batch ends with no successful generations", async () => {
...
void test("llm-failure-modes: rate_limited is retryable but does NOT count toward refusal classification", async () => {
  ...
  assert.equal(r1.errorClass, "rate_limited");
  assert.equal(r2.errorClass, "rate_limited");
...
void test("llm-failure-modes: rate_limited opens circuit after threshold", async () => {
...
void test("llm-failure-modes: oversized response does not crash pipeline", () => {
...
void test("llm-failure-modes: gateway transport caps response bytes (#1414)", async () => {
