...
void test("memory cache: miss → store → hit cycle skips the mocked LLM client", async () => {
...
void test("replay execution: hit skips LLM and returns audit cacheHit=true", async () => {
  const cache = createMemoryReplayCache();
  const { cacheKey, cacheKeyDigest } = compileForFixture("job-1");
...
void test("memory cache: lookup returns a deep clone (caller cannot poison cache)", async () => {
  const cache = createMemoryReplayCache();
...
void test("memory cache: nested array mutations on a hit do not bleed into the cached entry", async () => {
  ...
  const first = await cache.lookup(cacheKey);
  assert.equal(first.hit, true);
  const tc1 = first.entry.testCases.testCases[0]!;
  tc1.steps.push({ index: 99, action: "POISONED" });
  tc1.expectedResults.push("POISONED");
  tc1.figmaTraceRefs.push({ screenId: "POISONED", nodeId: "POISONED" });
...
void test("memory cache: lookup returns a structurally-equal but reference-distinct entry", async () => {
...
void test("filesystem cache: miss → store → hit cycle persists JSON on disk", async () => {
...
void test("filesystem cache: rejects writing an invalid GeneratedTestCaseList", async () => {
  const root = await mkdtemp(join(tmpdir(), "wsd-replay-fs-"));
  try {
    const cache = createFileSystemReplayCache(root);
    const { cacheKey } = compileForFixture("job-1");
...
void test("filesystem cache: rejects reading a corrupted file", async () => {
  const root = await mkdtemp(join(tmpdir(), "wsd-replay-fs-"));
  try {
    const cache = createFileSystemReplayCache(root);
    const { cacheKey } = compileForFixture("job-1");
...
void test("memory cache: rejects writing an invalid GeneratedTestCaseList", async () => {
  const cache = createMemoryReplayCache();
  const { cacheKey } = compileForFixture("job-1");
  const invalid = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
...
void test("filesystem cache: stored JSON file is parseable and self-describes", async () => {
  const root = await mkdtemp(join(tmpdir(), "wsd-replay-fs-"));
  try {
    const cache = createFileSystemReplayCache(root);
    const { cacheKey } = compileForFixture("job-1");
...
void test("cache key digest: stable for the same key inputs", () => {
