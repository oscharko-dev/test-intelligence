...
const expectedOpenAiTokens = (widthPx: number, heightPx: number): number => {
  ...
  return (
    tiles * LLM_IMAGE_OPENAI_TOKENS_PER_TILE + LLM_IMAGE_OPENAI_BASE_TOKENS
  );
};
...
const expectedLlamaTokens = (widthPx: number, heightPx: number): number => {
  ...
  const tiles = Math.ceil((widthPx * heightPx) / tileArea);
  return tiles * LLM_IMAGE_LLAMA_TOKENS_PER_TILE + LLM_IMAGE_LLAMA_BASE_TOKENS;
};

void test("default image strategy is the OpenAI-aligned tile formula", () => {
  assert.equal(DEFAULT_LLM_IMAGE_TOKEN_STRATEGY, "openai_tiles");
});

void test("estimateLlmInputBytes counts only text payload (system + user + schema)", () => {
...
void test("estimateTextTokens uses the shared bytes-per-token heuristic", () => {
...
void test("text-only request matches the legacy `bytes / 4` heuristic", () => {
  const request = buildRequest({
...
void test("openai_tiles: 1280×720 screenshot estimates ≤ 1000 tokens (Issue #1930 AC)", () => {
  const widthPx = 1280;
  const heightPx = 720;
...
void test("openai_tiles: small 256×256 thumbnail collapses to a single tile", () => {
  const widthPx = 256;
  const heightPx = 256;
...
void test("openai_tiles: oversized 4K screenshot scales linearly in tile count", () => {
  const widthPx = 3840;
  const heightPx = 2160;
  const tokens = estimateLlmInputTokens(
    buildRequest({
...
void test("llama_tiles: 1280×720 screenshot uses the Llama tile/base constants", () => {
  const widthPx = 1280;
  const heightPx = 720;
  const tokens = estimateLlmInputTokens(
    buildRequest({
...
void test("raw_bytes: explicit override falls back to the legacy byte-based rule", () => {
...
void test("missing pixel dimensions force raw_bytes fallback even with a tile strategy", () => {
...
void test("non-positive pixel dimensions fall back to raw_bytes", () => {
...
void test("mixed payload sums text tokens + per-image tile tokens", () => {
...
void test("estimateImageInputTokens helper matches the request-level estimate per image", () => {
...
void test("default strategy applies when options are omitted entirely", () => {
  const widthPx = 1024;
  const heightPx = 768;
  const tokens = estimateLlmInputTokens(
    buildRequest({
