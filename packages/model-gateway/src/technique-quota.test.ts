...
import {
  ...
  computeTierElasticUseCaseQuota,
...
void test("tier-elastic formula: tier bands follow the 4/5/9/20 cutovers", () => {
...
void test("Epic #2167 P0 scenario — xr6Nf 32-field screen clears the documented quota miss-by-4", () => {
...
void test("tier-elastic use-case quotas cap planner overshoot by field count", () => {
  assert.equal(computeTierElasticUseCaseQuota(0).quota, 0);
  assert.equal(computeTierElasticUseCaseQuota(2).quota, 4);
  assert.equal(computeTierElasticUseCaseQuota(8).quota, 8);

  const plan = buildPlan({
    screenId: "1:usecase",
    fieldCount: 2,
    extras: [{ technique: "use_case", minCount: 14 }],
  });
  const elastic = resolveTechniqueQuotas(plan, { mode: "tier-elastic" }).find(
    (quota) => quota.technique === "use_case",
  );
  assert.equal(elastic?.requiredCount, 4);
  assert.equal(elastic?.formula, "tier-elastic-use-case:max(4, fields)");

  const fixed = resolveTechniqueQuotas(plan, { mode: "fixed" }).find(
    (quota) => quota.technique === "use_case",
  );
  assert.equal(fixed?.requiredCount, 14);
});

void test("Issue #2171: tier-elastic quotas accept caller-supplied tiers and keep the canonical tier table exposed", () => {
...
void test("buildPerScreenFieldCounts derives unique element counts from CoveragePlan.perElement", () => {
...
void test("Issue #2068: K0 evidence — 9 fields × tier-elastic mode resolves EP quota to 9", () => {
  const plan = buildPlan({
    screenId: "1:11309",
    fieldCount: 9,
    plannerEpMin: 12,
...
void test("Issue #2068: fixed mode preserves the planner's 12-EP minimum verbatim", () => {
  const plan = buildPlan({
    screenId: "1:11309",
    fieldCount: 9,
    plannerEpMin: 12,
...
void test("Issue #2026: shared EP cases satisfy the quota when they cover every field target on the screen", () => {
  const plan = buildPlan({
    screenId: "s-shared",
    fieldCount: 3,
    plannerEpMin: 3,
...
void test("Issue #2068: tier-elastic mode never synthesises an EP row when the planner publishes none", () => {
...
void test("Issue #2068: non-EP planner quotas are preserved in tier-elastic mode", () => {
  const plan = buildPlan({
    screenId: "s-1",
    fieldCount: 7,
    plannerEpMin: 12,
...
void test("Issue #2171: technique-quota report is sorted, deterministic, and carries the tier audit trail", () => {
  const plan = buildPlan({
    screenId: "s-1",
    fieldCount: 19,
    plannerEpMin: 12,
...
void test("Issue #2171: fixed technique-coverage mode still enforces the planner's EP minimum verbatim", () => {
  const plan = buildPlan({
    screenId: "s-fixed",
    fieldCount: 19,
    plannerEpMin: 12,
...
void test("Issue #2068: undefined policy defaults to tier-elastic (legacy callers stay safe)", () => {
  const plan = buildPlan({
    screenId: "1:11309",
    fieldCount: 9,
    plannerEpMin: 12,
...
void test("writeTechniqueQuotaReport persists a byte-deterministic JSON artifact", async () => {
  const plan = buildPlan({
    screenId: "s-1",
    fieldCount: 9,
    plannerEpMin: 12,
