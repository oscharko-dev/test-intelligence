...
void test("computeGatewayIdempotencyHmac is deterministic for same inputs+secret", async () => {
...
void test("computeGatewayIdempotencyHmac changes when any input changes", async () => {
...
void test("computeGatewayIdempotencyHmac differs across secrets", async () => {
  const a = await computeGatewayIdempotencyHmac(sampleInputs(), "secret-1");
  const b = await computeGatewayIdempotencyHmac(sampleInputs(), "secret-2");
  assert.notEqual(a, b);
});

void test("computeGatewayIdempotencyHmac rejects empty secret", async () => {
...
void test("computeGatewayIdempotencyHmac is order-independent for same fields", async () => {
...
void test("createLlmGatewayIdempotencyCache rejects missing/empty secret", () => {
  assert.throws(
    () =>
      createLlmGatewayIdempotencyCache({
        hmacSecret: "",
...
void test("createLlmGatewayIdempotencyCache rejects bad ttl/maxEntries", () => {
  assert.throws(
...
void test("cache.computeKey rejects malformed inputs", async () => {
  const cache = createLlmGatewayIdempotencyCache({ hmacSecret: "secret-1" });
  await assert.rejects(cache.computeKey(sampleInputs({ jobId: "" })), /jobId/);
  await assert.rejects(
...
void test("cache.lookup miss returns the computed key for storage", async () => {
  const cache = createLlmGatewayIdempotencyCache({ hmacSecret: "secret-1" });
...
void test("cache.store + cache.lookup hit returns identical structured result", async () => {
  const cache = createLlmGatewayIdempotencyCache({ hmacSecret: "secret-1" });
  const inputs = sampleInputs();
...
void test("TTL expiry: a cached entry past ttlMs misses and increments ttlExpirations", async () => {
  let now = 1_000;
...
void test("maxEntries: oldest entry is evicted under bounded memory", async () => {
  const cache = createLlmGatewayIdempotencyCache({
    hmacSecret: "secret-1",
    maxEntries: 2,
  });
...
void test("disk persistence: file is content-addressable, redacted, has no raw body or secret", async () => {
...
void test("worker-crash-mid-call: a fresh process+cache rehydrates from disk and avoids second LLM call", async () => {
...
void test("gateway: idempotency hit short-circuits dispatch when inputs+cache present", async () => {
  let upstreamCalls = 0;
  const cache = createLlmGatewayIdempotencyCache({ hmacSecret: "secret-1" });
...
void test("gateway: requests without idempotency input still dispatch every call", async () => {
  let upstreamCalls = 0;
  const cache = createLlmGatewayIdempotencyCache({ hmacSecret: "secret-1" });
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
...
void test("gateway: failed dispatch is NOT cached (next call re-dispatches)", async () => {
  let upstreamCalls = 0;
  const cache = createLlmGatewayIdempotencyCache({ hmacSecret: "secret-1" });
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
...
void test("gateway: getIdempotencyMetrics is undefined when no cache wired", async () => {
  const client = createLlmGatewayClient(baseConfig, {
...
void test("gateway: secret provider callback is invoked, value is never persisted", async () => {
...
void test("gateway: malformed disk file is ignored without throwing", async () => {
...
void test("gateway: tampered disk file with mismatched hmac is rejected", async () => {
...
void test("DEFAULT_GATEWAY_IDEMPOTENCY_TTL_MS is 24h", () => {
  assert.equal(DEFAULT_GATEWAY_IDEMPOTENCY_TTL_MS, 24 * 60 * 60 * 1000);
});

void test("gatewayIdempotencyCachePath refuses non-hex hmac (defence-in-depth)", () => {
  assert.throws(
    () => gatewayIdempotencyCachePath("/tmp/x", "../escape"),
    RangeError,
  );
  assert.throws(
    () => gatewayIdempotencyCachePath("/tmp/x", "0".repeat(63)),
    RangeError,
  );
});
