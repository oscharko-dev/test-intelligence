...
void test("registry: covers every AgentHarnessRole exactly once", () => {
...
void test("registry: every profile is deeply frozen", () => {
  for (const profile of listAgentRoleProfiles()) {
    assert.equal(
      Object.isFrozen(profile),
      true,
      `profile ${profile.role} not frozen`,
    );
...
void test("boundary: no llm_role profile may declare propose_changes", () => {
  for (const profile of listAgentRoleProfiles()) {
...
void test("boundary: every llm_role profile pins promptVersion + modelBinding + positive token budgets", () => {
  for (const profile of listAgentRoleProfiles()) {
    if (profile.roleKind !== "llm_role") continue;
...
void test("boundary: deterministic services do not declare promptVersion or modelBinding", () => {
  for (const profile of listAgentRoleProfiles()) {
...
void test("boundary: maxAttempts is in the closed allowed set", () => {
  for (const profile of listAgentRoleProfiles()) {
    assert.ok(
...
void test("boundary: capability and finOpsGroup are members of the closed sets", () => {
  for (const profile of listAgentRoleProfiles()) {
    assert.ok(
      (AGENT_ROLE_CAPABILITIES as readonly string[]).includes(
        profile.capability,
      ),
    );
    assert.ok(
      (AGENT_ROLE_FINOPS_GROUPS as readonly string[]).includes(
        profile.finOpsGroup,
      ),
    );
    assert.ok(
      (AGENT_ROLE_KINDS as readonly string[]).includes(profile.roleKind),
    );
  }
});

void test("serialization: every profile round-trips through canonical JSON", () => {
  for (const profile of listAgentRoleProfiles()) {
...
void test("serialization: modelBinding round-trips with optional ictRegisterRef", () => {
  ...
  const parsed = JSON.parse(
    serializeAgentRoleProfile(sample),
  ) as AgentRoleProfile;
...
void test("getAgentRoleProfile: returns the same frozen instance for every call", () => {
  const a = getAgentRoleProfile("generator");
  const b = getAgentRoleProfile("generator");
  assert.equal(a, b);
});

void test("generator diversity profiles: multi-pass mode returns frozen, distinct pass profiles", () => {
...
void test("generator diversity profiles: single-pass mode returns no diversity overrides", () => {
  assert.deepEqual(listGeneratorDiversityPassProfiles(1), []);
});

void test("generator guidance: technique quota rule is a hard requirement", () => {
  assert.match(
    GENERATOR_TECHNIQUE_QUOTA_RULE,
  ...
  assert.match(
    GENERATOR_TECHNIQUE_QUOTA_RULE,
    /minCount > 0 MUST be represented/u,
  );
});

void test("invariants: rejects llm_role with propose_changes", () => {
  assert.throws(
    () =>
      assertAgentRoleProfileInvariants({
        schemaVersion: AGENT_ROLE_PROFILE_SCHEMA_VERSION,
...
void test("invariants: rejects llm_role missing promptVersion", () => {
  assert.throws(
    () =>
      assertAgentRoleProfileInvariants({
        schemaVersion: AGENT_ROLE_PROFILE_SCHEMA_VERSION,
...
void test("invariants: rejects llm_role missing modelBinding", () => {
  assert.throws(
    () =>
      assertAgentRoleProfileInvariants({
        schemaVersion: AGENT_ROLE_PROFILE_SCHEMA_VERSION,
...
void test("invariants: rejects deterministic_service with promptVersion or modelBinding", () => {
  assert.throws(
    () =>
      assertAgentRoleProfileInvariants({
        schemaVersion: AGENT_ROLE_PROFILE_SCHEMA_VERSION,
...
void test("invariants: rejects negative or non-integer token budgets", () => {
  assert.throws(
    () =>
      assertAgentRoleProfileInvariants({
        schemaVersion: AGENT_ROLE_PROFILE_SCHEMA_VERSION,
...
void test("invariants: rejects empty modelBinding fields", () => {
  assert.throws(
    () =>
      assertAgentRoleProfileInvariants({
        schemaVersion: AGENT_ROLE_PROFILE_SCHEMA_VERSION,
...
void test("type guards: accept registered values, reject others", () => {
...
void test("listAgentRoleProfiles: stable alphabetical order", () => {
