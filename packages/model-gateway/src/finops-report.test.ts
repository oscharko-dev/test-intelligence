...
void test("recorder: empty snapshot lists every role with zero counters", () => {
...
void test("recorder: aggregates success and failure observations per role", () => {
  const recorder = createFinOpsUsageRecorder();
...
void test("recorder: failed visual sidecar attempts are counted in FinOps totals", () => {
  const recorder = createFinOpsUsageRecorder();
  recorder.recordAttempt({
    role: "visual_primary",
    deployment: "llama-4-maverick-vision",
...
void test("recorder: per-source entries retain circuit-breaker states in call order", () => {
  const recorder = createFinOpsUsageRecorder();
  recorder.recordAttempt({
    role: "visual_primary",
...
void test("recorder: breaker-open skips preserve state without adding source calls", () => {
  const recorder = createFinOpsUsageRecorder();
...
void test("recorder: rejects unknown roles", () => {
  const recorder = createFinOpsUsageRecorder();
  assert.throws(() => {
    recorder.recordAttempt({
...
void test("recorder: rejects unknown cache and budget breach dimensions", () => {
  const recorder = createFinOpsUsageRecorder();
  assert.throws(() => {
...
void test("recorder: explicit budget breaches are copied into the report", () => {
  const recorder = createFinOpsUsageRecorder();
  recorder.recordBudgetBreach({
...
void test("recorder: cache hits do NOT increment any token or attempt counter", () => {
  const recorder = createFinOpsUsageRecorder();
...
void test("recorder: estimatedCost combines token rates + per-attempt fixed cost", () => {
...
void test("buildFinOpsBudgetReport: bySource.judge_primary records the judge deployment, not the generator deployment, when they differ (Issue #1932)", () => {
  const recorder = createFinOpsUsageRecorder();
...
void test("buildFinOpsBudgetReport: bySource entries omit deployment when no source label was supplied (Issue #1932 — backwards compat)", () => {
  const recorder = createFinOpsUsageRecorder();
  recorder.recordAttempt({
    role: "test_generation",
    deployment: "gpt-oss-120b",
...
void test("buildFinOpsBudgetReport: includes deterministic bySource attribution", () => {
  const costRates: FinOpsCostRateMap = {
    currencyLabel: "USD",
    rates: {
      test_generation: {
...
void test("buildFinOpsBudgetReport: stamps schema/contract versions and negative invariants", () => {
  const recorder = createFinOpsUsageRecorder();
  const report = buildFinOpsBudgetReport({
    jobId: JOB_ID,
    generatedAt: GENERATED_AT,
...
void test("buildFinOpsBudgetReport: outcome=completed_cache_hit when only cache hits, no attempts", () => {
  const recorder = createFinOpsUsageRecorder();
  recorder.recordCacheHit({ role: "test_generation" });
  const report = buildFinOpsBudgetReport({
    jobId: JOB_ID,
...
void test("buildFinOpsBudgetReport: detects per-role token budget breach", () => {
  const recorder = createFinOpsUsageRecorder();
  recorder.recordAttempt({
    role: "test_generation",
    deployment: "gpt-oss-120b",
...
void test("buildFinOpsBudgetReport: detects job-level wall-clock breach", () => {
  const recorder = createFinOpsUsageRecorder();
  recorder.recordAttempt({
    role: "test_generation",
    deployment: "gpt-oss-120b",
...
void test("buildFinOpsBudgetReport: detects replay-cache miss-rate breach", () => {
  const recorder = createFinOpsUsageRecorder();
...
void test("buildFinOpsBudgetReport: detects fallback-attempt breach on visual_fallback", () => {
  const recorder = createFinOpsUsageRecorder();
  recorder.recordAttempt({
    role: "visual_fallback",
    deployment: "phi-4-multimodal-poc",
...
void test("buildFinOpsBudgetReport: detects estimated-cost cap breach", () => {
  const costRates: FinOpsCostRateMap = {
    currencyLabel: "USD",
    rates: {
      test_generation: {
...
void test("buildFinOpsBudgetReport: detects missing operational rules in deterministic order", () => {
  const recorder = createFinOpsUsageRecorder();
  recorder.recordAttempt({
    role: "visual_primary",
    deployment: "llama-4-maverick-vision",
...
void test("buildFinOpsBudgetReport: sorts equal-rule breaches by role and observed value", () => {
  const recorder = createFinOpsUsageRecorder();
  recorder.recordBudgetBreach({
    rule: "max_attempts",
    role: "visual_primary",
...
void test("buildFinOpsBudgetReport: outcomeOverride takes precedence over auto-derived outcome", () => {
  const recorder = createFinOpsUsageRecorder();
  recorder.recordAttempt({
    role: "test_generation",
    deployment: "gpt-oss-120b",
...
void test("buildFinOpsBudgetReport: outcomeOverride beats live-smoke budget breaches", () => {
...
void test("buildFinOpsBudgetReport: byte-stable for identical input", () => {
  const buildOnce = () => {
    const recorder = createFinOpsUsageRecorder();
    recorder.recordAttempt({
      role: "test_generation",
...
void test("buildFinOpsBudgetReport: roles array is sorted alphabetically", () => {
  const recorder = createFinOpsUsageRecorder();
  recorder.recordAttempt({
    role: "visual_primary",
    deployment: "llama-4-maverick-vision",
...
void test("buildFinOpsBudgetReport: throws on empty jobId or generatedAt", () => {
  const recorder = createFinOpsUsageRecorder();
  assert.throws(
    () =>
      buildFinOpsBudgetReport({
...
void test("writeFinOpsBudgetReport: persists under finops/ subdirectory", async () => {
...
void test("writeFinOpsBudgetReport: preserves region attestation summaries when present", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      onDisk.bySource.generator.regionAttestation,
      regionAttestation,
    );
  } finally {
    await rm(dir, { recursive: true, force: true });
...
void test("writeFinOpsBudgetReport: artifact never embeds prompt text or image bytes", async () => {
...
void test("buildFinOpsBudgetReport: redacts token-shaped report labels", () => {
...
void test("buildFinOpsBudgetReport: cache-hit job exposes zero LLM call usage and outcome=completed_cache_hit", () => {
  const recorder = createFinOpsUsageRecorder();
  recorder.recordCacheHit({
    role: "test_generation",
    deployment: "gpt-oss-120b",
...
void test("Issue #2016: attributionMode='audit' attempts do not count toward role-level attempts/tokens", () => {
  const recorder = createFinOpsUsageRecorder();
...
void test("Issue #2016: attributionMode='audit' alone produces no role-level attempts even with success result", () => {
  const recorder = createFinOpsUsageRecorder();
  recorder.recordAttempt({
    role: "test_generation",
    source: "judge_primary",
...
void test("Issue #2016: attributionMode='audit' does not inflate max_attempts / max_total_output_tokens breaches", () => {
  const recorder = createFinOpsUsageRecorder();
...
void test("Issue #2016: attributionMode default of 'primary' preserves legacy accounting (backwards compat)", () => {
  const recorder = createFinOpsUsageRecorder();
