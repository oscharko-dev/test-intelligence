...
void test("eu-banking default policy: declares cross-family diversity slots from Epic #2099 mapping", () => {
  ...
  const faithfulnessFallback = findRoute(
    policy,
    "faithfulness_judge",
    "fallback",
  );
...
void test("eu-banking default policy: cross-family diversity is canonically expressed via mistral + openai families", () => {
  const policy = EU_BANKING_DEFAULT_MODEL_ROUTING_POLICY;
...
void test("model routing policy: override changes the digest and targeted route metadata", () => {
...
void test("model routing policy: runtime policy preserves non-primary default slots and stamps live identities", () => {
...
void test("production topology policy: coverage planner config uses its own routed deployment", () => {
...
void test("production topology configs do not cap FinOps retry budgets below production defaults", () => {
