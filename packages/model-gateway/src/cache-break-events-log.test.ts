...
void test("buildCacheBreakEventsLog sorts deterministically and dedupes", () => {
...
void test("buildCacheBreakEventsLog emits empty payload for empty input", () => {
  const built = buildCacheBreakEventsLog({ entries: [] });
  assert.equal(built.entries.length, 0);
  assert.equal(built.serialized, "");
});

void test("isCacheBreakEventLogEntry rejects malformed entries", () => {
...
void test("buildCacheBreakEventsLog rejects invalid entries up front", () => {
  assert.throws(
    () =>
      buildCacheBreakEventsLog({
...
void test("writeCacheBreakEventsLog persists byte-stably and round-trips", async () => {
...
void test("parseCacheBreakEventsLog rejects non-trailing-newline payloads", () => {
...
void test("parseCacheBreakEventsLog accepts the empty payload", () => {
  const result = parseCacheBreakEventsLog("");
  assert.ok(result !== undefined);
  assert.equal(result!.length, 0);
});

void test("cache-break-events-log: golden line for the canonical example", () => {
  const built = buildCacheBreakEventsLog({
    entries: [
      baseEntry({
        ts: "2026-05-04T08:00:00.000Z",
  ...
  const golden =
    '{"cacheCreationTokens":200,"cacheReadTokens":12,"contractVersion":"1.39.0",' +
