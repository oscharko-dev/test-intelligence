...
import { createFileSystemA11yJudgeCache } from "@oscharko-dev/ti-production-runner";
import { createFileSystemFaithfulnessJudgeCache } from "@oscharko-dev/ti-quality";
import {
  deriveBusinessTestIntentIr,
  type IntentDerivationFigmaInput,
} from "@oscharko-dev/ti-core-engine";
import { createFileSystemLogicJudgeCache } from "@oscharko-dev/ti-quality";
...
void test("adversarial: tenant A cannot read tenant B's replay-cache entry even with identical input hash", async () => {
...
void test("adversarial: same tenant + different environment must not share replay-cache entries", async () => {
...
void test("adversarial: same tenant + same env + different project must not share replay-cache entries", async () => {
...
void test("adversarial: omitted projectId is normalised to 'default' and isolated from explicit 'default' siblings", async () => {
...
void test("adversarial: replay-cache loader rejects path-traversal tenant scope segments", () => {
...
void test("adversarial: logic-judge cache loader rejects path-traversal tenant scope segments", () => {
  assert.throws(
    () =>
...
void test("adversarial: faithfulness-judge cache loader rejects path-traversal tenant scope segments", () => {
  assert.throws(
    () =>
...
void test("adversarial: a11y-judge cache loader rejects path-traversal tenant scope segments", () => {
  assert.throws(
    () =>
...
void test("adversarial: tenant A cannot read tenant B's logic-judge cache entry even with identical key", async () => {
  const root = await mkdtemp(join(tmpdir(), "wsd-logic-judge-adv-"));
  try {
    const cacheA = createFileSystemLogicJudgeCache(root, {
      tenantScope: TENANT_A,
    });
    const cacheB = createFileSystemLogicJudgeCache(root, {
...
void test("adversarial: tenant A cannot read tenant B's faithfulness-judge cache entry even with identical key", async () => {
...
void test("adversarial: tenant A cannot read tenant B's a11y-judge cache entry even with identical key", async () => {
  const root = await mkdtemp(join(tmpdir(), "wsd-a11y-judge-adv-"));
  try {
    const cacheA = createFileSystemA11yJudgeCache(root, {
      tenantScope: TENANT_A,
    });
    const cacheB = createFileSystemA11yJudgeCache(root, {
      tenantScope: TENANT_B,
    });
...
void test("adversarial: tenant scope must NOT be folded into the replay-cache key digest", () => {
