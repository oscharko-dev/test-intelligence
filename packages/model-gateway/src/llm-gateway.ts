/**
 * LLM gateway client surface for Wave 1 validation (Issue #1363).
 *
 * Goals enforced here:
...
export const extractFirstJsonObjectOrArray = (raw: string): unknown => {
  ...
  const fenced = extractFirstMarkdownFence(raw);
  if (fenced !== undefined && fenced.length > 0) {
    try {
