...
const cancellableResponse = ({
  ...
  return new Response(stream, { status, ...(headers ? { headers } : {}) });
};

void test("config validation: rejects test_generation with imageInputSupport", () => {
  assert.throws(
    () =>
      createLlmGatewayClient({
        ...baseConfig,
...
void test("config validation: rejects empty deployment / bad sha", () => {
  assert.throws(
...
void test("config validation: rejects blank ictRegisterRef", () => {
  assert.throws(
...
void test("LlmGatewayError discriminates errorClass / retryable / attempt", () => {
...
void test("LLM_GATEWAY_ERROR_CLASSES set contains all allowed classes", () => {
  for (const cls of ALLOWED_LLM_GATEWAY_ERROR_CLASSES) {
    assert.ok(LLM_GATEWAY_ERROR_CLASSES.has(cls));
  }
});

void test("isLlmGatewayErrorRetryable: only transient classes retry", () => {
...
void test("4xx mapping: 401/403 surface as protocol (not schema_invalid)", async () => {
...
void test("4xx mapping: 400/409/422 surface as schema_invalid", async () => {
...
void test("abortSignal: caller-cancellation surfaces canceled (not timeout, not retryable)", async () => {
...
void test("abortSignal: cancellation mid-flight surfaces canceled", async () => {
  const controller = new AbortController();
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: (_url: string | URL | Request, init?: RequestInit) =>
      new Promise<Response>((_resolve, reject) => {
...
void test("guards image payloads on test_generation role without making a network call", async () => {
  let calls = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
      calls += 1;
...
void test("guards oversized input budgets before making a network call", async () => {
  let calls = 0;
  let keyReads = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
...
void test("rejects malformed maxInputTokens with schema_invalid", async () => {
  let calls = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
      calls += 1;
...
void test("input budget at the cap succeeds; one byte over fails closed", async () => {
  let calls = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
      calls += 1;
...
void test("response-size ceiling: Content-Length over default cap is rejected", async () => {
...
void test("response-size ceiling: streaming guard aborts when chunk total exceeds cap", async () => {
  const cap = 4 * 1024;
...
void test("response-size ceiling: streaming guard catches lying Content-Length", async () => {
  const cap = 1024;
...
void test("response-size ceiling: response just at the cap boundary succeeds", async () => {
  const cap = 4 * 1024;
...
void test("config validation: rejects non-positive / non-integer maxResponseBytes", () => {
...
void test("response_too_large is registered in the error-class enum and is non-retryable", () => {
...
void test("structured-output success: parses JSON content and strips raw text", async () => {
...
void test("structured-output success: accepts message.content text-part arrays", async () => {
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () =>
      okJsonResponse({
        choices: [
...
void test("structured-output success: accepts provider-parsed payloads when message.content is empty", async () => {
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () =>
      okJsonResponse({
        choices: [
...
void test("structured-output missing message.content is retried as a gateway failure", async () => {
  let attempts = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
      attempts += 1;
...
void test("structured-output success: accepts reasoning_content only when it satisfies the schema", async () => {
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () =>
      okJsonResponse({
        choices: [
...
void test("plain-text success is allowed when no response schema is requested", async () => {
...
void test("forwards api-key header from provider; never echoes it back", async () => {
...
void test("bearer auth mode uses Authorization: Bearer header", async () => {
...
void test("none auth mode: no auth headers attached", async () => {
  let observed: Headers | undefined;
  const client = createLlmGatewayClient(
...
void test("api_key mode without apiKeyProvider returns transport error and never calls fetch", async () => {
  let calls = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
      calls += 1;
...
void test("non-retryable auth failures do not open the circuit breaker", async () => {
  const client = createLlmGatewayClient(
    {
      ...baseConfig,
...
void test("apiKeyProvider returning empty string is rejected", async () => {
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => okJsonResponse({}),
    apiKeyProvider: () => "",
  });
...
void test("rate-limited 429 surfaces rate_limited and retries", async () => {
  let attempts = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
      attempts += 1;
...
void test("oversized 429 body preserves rate_limited retry classification", async () => {
  let attempts = 0;
  let cancellations = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
...
void test("5xx surfaces transport-class and retries up to max", async () => {
  let attempts = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
      attempts += 1;
...
void test("oversized 5xx body preserves transport retry classification", async () => {
  let attempts = 0;
  let cancellations = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
...
void test("408 response body is cancelled while preserving timeout classification", async () => {
  let cancelled = false;
  const client = createLlmGatewayClient(
...
void test("4xx body surfaces schema_invalid and does NOT retry", async () => {
  let attempts = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
      attempts += 1;
...
void test("content_filter finish_reason maps to refusal and does NOT retry", async () => {
  let attempts = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
      attempts += 1;
...
void test("explicit message.refusal field maps to refusal", async () => {
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () =>
      okJsonResponse({
        choices: [
...
void test("length finish_reason maps to incomplete", async () => {
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () =>
...
void test("incomplete finish_reason triggers bounded output-token expansion and retries", async () => {
  let attempts = 0;
...
void test("tool_calls finish_reason returns an explicit unsupported response", async () => {
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () =>
      okJsonResponse({
        choices: [
...
void test("non-JSON content body surfaces schema_invalid", async () => {
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () =>
      okJsonResponse({
        choices: [
...
void test("non-JSON structured content is retried before surfacing schema_invalid", async () => {
  let calls = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
      calls += 1;
...
void test("valid JSON that violates response schema surfaces schema_invalid", async () => {
  const client = createLlmGatewayClient(baseConfig, {
...
void test("response missing choices surfaces schema_invalid", async () => {
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => okJsonResponse({}),
    apiKeyProvider: () => "k",
    sleep: async () => undefined,
...
void test("AbortError-class fetch failure becomes timeout error", async () => {
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
...
void test("fetch throwing non-abort error becomes transport error and retries", async () => {
  let attempts = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
      attempts += 1;
...
void test("token-shaped strings in error messages are redacted", async () => {
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
      throw new Error(
...
void test("circuit breaker opens after enough transient failures and short-circuits subsequent requests", async () => {
  const client = createLlmGatewayClient(
    {
      ...baseConfig,
      maxRetries: 0,
...
void test("structured outputs flag drives response_format inclusion", async () => {
  let observedBody: string | undefined;
  const client = createLlmGatewayClient(
    {
      ...baseConfig,
...
void test("URL composition preserves trailing-slash baseUrl and adds chat/completions", async () => {
...
void test("seed, reasoning_effort, and max_output_tokens flags only forward when declared", async () => {
...
void test("mistral deployments forward maxOutputTokens as max_tokens", async () => {
  let observedBody: string | undefined;
  const client = createLlmGatewayClient(
    {
      ...baseConfig,
...
void test("wireStructuredOutputMode: defaults to json_schema when omitted", async () => {
  let observedBody: string | undefined;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async (_u, init) => {
      observedBody = init?.body as string | undefined;
...
void test('wireStructuredOutputMode: "json_object" emits weaker wire format but still validates schema in-process', async () => {
  let observedBody: string | undefined;
  const client = createLlmGatewayClient(
...
void test('wireStructuredOutputMode: "json_object" still surfaces schema_invalid for content that violates the schema', async () => {
  const client = createLlmGatewayClient(
    { ...baseConfig, wireStructuredOutputMode: "json_object" },
    {
...
void test('wireStructuredOutputMode: "none" omits response_format but still parses + schema-validates JSON content', async () => {
  let observedBody: string | undefined;
  const client = createLlmGatewayClient(
...
void test('wireStructuredOutputMode: "none" surfaces schema_invalid when free-form content is not parseable JSON', async () => {
  const client = createLlmGatewayClient(
    { ...baseConfig, wireStructuredOutputMode: "none" },
    {
...
void test('Epic #2167 Q0: "none" mode recovers JSON wrapped in a ```json markdown fence', async () => {
...
void test('Epic #2167 Q0: "none" mode recovers JSON from a prose prelude', async () => {
...
void test('Epic #2167 Q0: "none" mode recovery still rejects JSON that violates the schema', async () => {
...
void test("in-flight dedup: concurrent identical keys collapse to one gateway call and increment FinOps bySource hit counter", async () => {
...
void test("in-flight dedup: different policyProfileHash values do not collapse", async () => {
  let dispatches = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
      dispatches += 1;
...
void test("in-flight dedup: different inputHash values do not collapse", async () => {
  let dispatches = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
      dispatches += 1;
...
void test("in-flight dedup: malformed key surfaces schema_invalid before transport", async () => {
  let dispatches = 0;
  const client = createLlmGatewayClient(baseConfig, {
    fetchImpl: async () => {
      dispatches += 1;
...
void test("config validation: rejects invalid wireStructuredOutputMode value", () => {
  assert.throws(
    () =>
      createLlmGatewayClient(
        {
          ...
          wireStructuredOutputMode:
            "raw_text" as unknown as LlmGatewayWireStructuredOutputMode,
        },
        { apiKeyProvider: () => "k" },
      ),
...
void test('wireStructuredOutputMode: "none" plus structuredOutputs:false still omits response_format (no double-write)', async () => {
  let observedBody: string | undefined;
  const client = createLlmGatewayClient(
    {
      ...baseConfig,
