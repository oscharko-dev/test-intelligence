 ...
 * compares the generated case list against it. The quotas for field-driven
 * `equivalence_partitioning` and `use_case` techniques are tier-elastic by
 * default: they scale with the screen's coverage-relevant field count
...
export const computeTierElasticUseCaseQuota = (
  fieldCount: number,
): {
  quota: number;
  formula: string;
  ...
  if (safeCount === 0) {
    return {
      quota: 0,
      formula: "tier-elastic-use-case:fields=0",
      formulaTier: "fields=0: no field-driven use-case floor",
      formulaMultiplier: null,
    };
  }
  return {
    quota: Math.max(4, safeCount),
    formula: "tier-elastic-use-case:max(4, fields)",
    formulaTier: "fields>=1: max(4, fields)",
    formulaMultiplier: 1,
  };
};
 ...
 *   - `tier-elastic` mode replaces field-driven equivalence-partitioning
 *     and use-case quotas with capped formulas; rows for other techniques
 *     (accessibility, decision-table, boundary-value-analysis, …) keep the
 *     planner's published `minCount`. Conservative scope is unchanged from
 *     Issue #2068: when the planner omits a quota row entirely, the gate
    ...
    for (const quota of screenQuotas) {
      ...
      if (
        ...
        resolved.push({
          ...
          formula:
            effective === tier.quota ? tier.formula : FIXED_FORMULA_LABEL,
          formulaTier: tier.formulaTier,
          formulaMultiplier: tier.formulaMultiplier,
        });
        continue;
      }
      if (mode === "tier-elastic" && quota.technique === "use_case") {
        const tier = computeTierElasticUseCaseQuota(fieldCount);
        const effective = Math.min(quota.minCount, tier.quota);
        resolved.push({
          screenId: screen.screenId,
          technique: quota.technique,
...
export const writeTechniqueQuotaReport = async (
  ...
  const filePath = join(input.runDir, TECHNIQUE_QUOTA_REPORT_ARTIFACT_FILENAME);
