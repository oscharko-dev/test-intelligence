...
void test("checkJiraApiQuota: passes when planned calls are under the EU-banking cap", () => {
  const result = checkJiraApiQuota(EU_BANKING_DEFAULT_FINOPS_BUDGET, 1);
  assert.equal(result.ok, true);
  assert.equal(result.breachReason, undefined);
});

void test("checkJiraApiQuota: passes at exactly the cap", () => {
...
void test("checkJiraApiQuota: breaches when planned calls exceed the cap", () => {
  const result = checkJiraApiQuota(
    EU_BANKING_DEFAULT_FINOPS_BUDGET,
...
void test("checkJiraPasteQuota: passes when paste bytes are under the EU-banking cap", () => {
...
void test("checkJiraPasteQuota: breaches when paste bytes exceed the cap", () => {
  const result = checkJiraPasteQuota(
    EU_BANKING_DEFAULT_FINOPS_BUDGET,
    MAX_JIRA_PASTE_BYTES_PER_JOB + 1,
  );
...
void test("checkCustomContextQuota: passes when input bytes are under the EU-banking cap", () => {
...
void test("checkCustomContextQuota: breaches when input bytes exceed the cap", () => {
  const result = checkCustomContextQuota(
    EU_BANKING_DEFAULT_FINOPS_BUDGET,
    MAX_CUSTOM_CONTEXT_BYTES_PER_JOB + 1,
  );
...
void test("source-quota helpers: pass with an empty envelope (no caps configured)", () => {
...
void test("validateFinOpsBudgetEnvelope: accepts a valid sourceQuotas block", () => {
...
void test("validateFinOpsBudgetEnvelope: rejects negative sourceQuotas fields", () => {
  const envelope: FinOpsBudgetEnvelope = {
...
void test("validateFinOpsBudgetEnvelope: rejects non-integer sourceQuotas fields", () => {
  const envelope: FinOpsBudgetEnvelope = {
...
void test("EU_BANKING_DEFAULT_FINOPS_BUDGET.sourceQuotas matches the contract constants", () => {
...
void test("createFinOpsUsageRecorder: accepts recordIngestBytes for ingest roles", () => {
...
void test("breach reason exhaustiveness: the 3 new Wave 4.I breach reasons appear in ALLOWED_FINOPS_BUDGET_BREACH_REASONS", () => {
...
void test("breach reason exhaustiveness: checkJiraApiQuota produces a reason that is in the allowed list", () => {
  const result = checkJiraApiQuota(
    EU_BANKING_DEFAULT_FINOPS_BUDGET,
    MAX_JIRA_API_REQUESTS_PER_JOB + 1,
  );
...
void test("breach reason exhaustiveness: checkJiraPasteQuota produces a reason that is in the allowed list", () => {
  const result = checkJiraPasteQuota(
    EU_BANKING_DEFAULT_FINOPS_BUDGET,
    MAX_JIRA_PASTE_BYTES_PER_JOB + 1,
  );
...
void test("breach reason exhaustiveness: checkCustomContextQuota produces a reason that is in the allowed list", () => {
  const result = checkCustomContextQuota(
    EU_BANKING_DEFAULT_FINOPS_BUDGET,
    MAX_CUSTOM_CONTEXT_BYTES_PER_JOB + 1,
  );
...
void test("checkJiraApiQuota: boundary — exactly at cap passes (not strictly less-than)", () => {
...
void test("checkJiraPasteQuota: zero-byte paste always passes", () => {
  const result = checkJiraPasteQuota(EU_BANKING_DEFAULT_FINOPS_BUDGET, 0);
  assert.equal(result.ok, true);
});

void test("checkCustomContextQuota: zero-byte input always passes", () => {
