...
void test("resource-budget: maxOutputTokens is forwarded to the gateway client", async () => {
...
void test("resource-budget: mock gateway enforces a synthetic maxOutputTokens ceiling", async () => {
...
void test("resource-budget: maxInputTokens cap rejects oversized prompts before gateway work", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "gpt-oss-120b",
    modelRevision: "rev",
...
void test("resource-budget: AbortController wall-clock timeout produces errorClass=timeout", async () => {
...
void test("resource-budget: MAX_VISUAL_SIDECAR_INPUT_BYTES enforced before gateway call", () => {
...
void test("resource-budget: preflight rejects before any gateway call (gateway invocation count = 0)", async () => {
...
void test("resource-budget: writeVisualSidecarResultArtifact writes <100 KB and contains no base64 image data", async () => {
...
void test("resource-budget: capture-count fairness — primary attempt count = N, fallback attempt count = N", async () => {
  ...
  const fallbackSucceed: MockResponder = (_req, attempt) => {
    fallbackCalls += 1;
    return {
      outcome: "success",
      content: {
...
void test("resource-budget: empty captures short-circuit before gateway — zero invocations", async () => {
...
void test("resource-budget: duplicate screenIds short-circuit before gateway — zero invocations", async () => {
  let gatewayInvoked = 0;
  const countingResponder: MockResponder = (_req, attempt) => {
    gatewayInvoked += 1;
    return {
...
void test("resource-budget: preflightCaptures detects empty_screen_capture_set directly", () => {
  const result = preflightCaptures([]);
  assert.equal(result.ok, false);
  assert.equal(result.failure.failureClass, "empty_screen_capture_set");
});

void test("resource-budget: preflightCaptures detects duplicate_screen_id directly", () => {
