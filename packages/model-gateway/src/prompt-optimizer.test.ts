...
const buildEvalSet = (): GeneratedTestCase[] => [
  ...
  buildCase({
    ...
    type: "negative",
    expectedResults: ["An error message is shown indicating an invalid IBAN"],
    figmaTraceRefs: [{ screenId: "s-loan", nodeId: "n-iban" }],
...
void test("bootstrap: filters runs below the quality gate", () => {
...
void test("bootstrap: deduplicates content-equivalent cases", () => {
...
void test("bootstrap: rejects out-of-range qualityGate", () => {
  assert.throws(() =>
    bootstrapExemplars({
      acceptedRuns: buildAcceptedRuns(),
      qualityGate: 200,
...
void test("cycle: deterministic — identical inputs yield identical reports", () => {
  const inputA = {
    jobId: JOB_ID,
    datasetId: DATASET_ID,
...
void test("cycle: empirical lift >= 3 points on the eval set", () => {
...
void test("cycle: respects the FinOps budget cap (5x baseline default)", () => {
  const report = runPromptOptimizationCycle({
    jobId: JOB_ID,
    datasetId: DATASET_ID,
    roleStepId: ROLE_STEP_ID,
...
void test("cycle: tight budget skips candidates without throwing", () => {
  const report = runPromptOptimizationCycle({
    jobId: JOB_ID,
    datasetId: DATASET_ID,
    roleStepId: ROLE_STEP_ID,
...
void test("cycle: rejects out-of-range hyperparameters", () => {
  const baseInput = {
    jobId: JOB_ID,
    datasetId: DATASET_ID,
    roleStepId: ROLE_STEP_ID,
  ...
  assert.throws(() => runPromptOptimizationCycle({ ...baseInput, seed: -1 }));
  assert.throws(() =>
...
void test("cycle: provenance node records base template + optimizer activity", () => {
  const report = runPromptOptimizationCycle({
    jobId: JOB_ID,
    datasetId: DATASET_ID,
    roleStepId: ROLE_STEP_ID,
...
void test("cycle: report carries pinned schema + optimizer + contract versions", () => {
  const report = runPromptOptimizationCycle({
    jobId: JOB_ID,
    datasetId: DATASET_ID,
    roleStepId: ROLE_STEP_ID,
  ...
  assert.equal(report.schemaVersion, PROMPT_OPTIMIZER_REPORT_SCHEMA_VERSION);
...
void test("cycle: only directive ids from the closed registry are emitted", () => {
  const report = runPromptOptimizationCycle({
    jobId: JOB_ID,
    datasetId: DATASET_ID,
    roleStepId: ROLE_STEP_ID,
...
void test("write: persists a byte-stable canonical-JSON artifact", async () => {
...
void test("lock-file: appends optimizedTemplates additively without touching the base pin", async () => {
  ...
  try {
    ...
    const after = JSON.parse(await readFile(lockPath, "utf8")) as {
      version: string;
      promptCompilerSha256: string;
      description: string;
      optimizedTemplates: PromptOptimizationLockEntry[];
    };
...
void test("lock-file: refuses to write when the base-template pin is missing", async () => {
...
void test("standard runs: prompt-compiler.ts is untouched (no version bump in this PR)", async () => {
