...
void test("constants: defaults are 0.05 / 0.05", () => {
  assert.equal(ROUTING_QUALITY_DEFAULT_TOLERANCE, 0.05);
  assert.equal(ROUTING_QUALITY_DEFAULT_THRESHOLD, 0.05);
});

void test("sampleTierLowDecisions: rejects out-of-range sample rate", () => {
  assert.throws(
    () =>
      sampleTierLowDecisions({
...
void test("sampleTierLowDecisions: returns empty when no tier-low decisions", () => {
...
void test("sampleTierLowDecisions: deterministic for the same seed", () => {
...
void test("sampleTierLowDecisions: different seeds give different orderings", () => {
  const decisions = tierLowDecisions(50);
...
void test("sampleTierLowDecisions: respects minimum sample size floor", () => {
...
void test("sampleTierLowDecisions: ceil ensures non-empty when rate * count is small", () => {
...
void test("sampleTierLowDecisions: filters out non-tier-low decisions", () => {
...
void test("evaluateTierLowRegression: clean run reports no regression", () => {
...
void test("evaluateTierLowRegression: regression rate past threshold fails the gate", () => {
  const verdicts = [
...
void test("evaluateTierLowRegression: tolerance hides small drift", () => {
  const verdicts = Array.from({ length: 10 }, (_, i) => ({
    taskId: `t-${i}`,
    baselineScore: 0.9,
    routedScore: 0.87,
...
void test("evaluateTierLowRegression: clamps scores to [0,1] and entries are sorted", () => {
  const verdicts = [
...
void test("evaluateTierLowRegression: empty sample is a passing report", () => {
  const report = evaluateTierLowRegression({
    jobId: "job-empty",
    generatedAt: "2026-05-08T00:00:00Z",
    verdicts: [],
...
void test("evaluateTierLowRegression: rejects empty jobId", () => {
  assert.throws(
    () =>
      evaluateTierLowRegression({
        jobId: "",
...
void test("evaluateTierLowRegression: end-to-end with the sampler", () => {
  const decisions = tierLowDecisions(20);
...
void test("assertRoutingQualityNotRegressed: noop on a passing report", () => {
  const report = evaluateTierLowRegression({
    jobId: "job-ok",
    generatedAt: "2026-05-08T00:00:00Z",
...
void test("classifyTask integration: tier-low decisions propagate through the sampler unchanged", () => {
