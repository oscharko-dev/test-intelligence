...
void test("buildRoutingSavingsReport: produces a deterministic, byte-stable report", () => {
...
void test("buildRoutingSavingsReport: per-tier breakdown sums match totals", () => {
...
void test("buildRoutingSavingsReport: tier-low tasks save vs tier-high baseline", () => {
  const report = buildRoutingSavingsReport({
    jobId: "job-3",
    generatedAt: "2026-05-08T00:00:00Z",
    table: EU_BANKING_DEFAULT_ROUTING_TABLE,
...
void test("buildRoutingSavingsReport: tier-high tasks save nothing vs baseline", () => {
  const report = buildRoutingSavingsReport({
    jobId: "job-4",
    generatedAt: "2026-05-08T00:00:00Z",
    table: EU_BANKING_DEFAULT_ROUTING_TABLE,
...
void test("buildRoutingSavingsReport: large workload meets >=50% savings target", () => {
...
void test("buildRoutingSavingsReport: throws on missing usage", () => {
  assert.throws(
    () =>
      buildRoutingSavingsReport({
        jobId: "job-bad",
...
void test("buildRoutingSavingsReport: rejects empty jobId", () => {
  assert.throws(
    () =>
      buildRoutingSavingsReport({
        jobId: "",
...
void test("buildRoutingSavingsReport: rejects rates missing a tier", () => {
...
void test("assertRoutingSavingsAtLeast: passes when savings meet target", () => {
  const decisions = classifyTaskBatch([
...
void test("assertRoutingSavingsAtLeast: throws when savings fall short", () => {
  const decisions = classifyTaskBatch([
...
void test("assertRoutingSavingsAtLeast: rejects out-of-range minimum", () => {
  const decisions = classifyTaskBatch([
...
void test("writeRoutingSavingsReport: persists the artifact atomically with canonical JSON", async () => {
