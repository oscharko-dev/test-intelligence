...
import type { AgentModelBinding } from "@oscharko-dev/ti-contracts";
import {
...
void test("constants: routing-table profile + environment vocabularies are stable", () => {
  assert.deepEqual(
    [...ROUTING_TABLE_PROFILES],
    ["eu-banking-default", "standard-default", "permissive-default"],
  );
  assert.deepEqual([...ROUTING_TABLE_ENVIRONMENTS], ["dev", "staging", "prod"]);
});

void test("type guards: detect known profiles + environments", () => {
...
void test("registry: every profile has a default table at the right schema version", () => {
...
void test("registry: built-in tables and helpers are deeply frozen", () => {
...
void test("getDefaultRoutingTable: returns the same frozen instance for repeat calls", () => {
...
void test("validateRoutingTable: built-in tables pass validation", () => {
  for (const table of [
    EU_BANKING_DEFAULT_ROUTING_TABLE,
    STANDARD_DEFAULT_ROUTING_TABLE,
    PERMISSIVE_DEFAULT_ROUTING_TABLE,
...
void test("validateRoutingTable: rejects bad schema version", () => {
...
void test("validateRoutingTable: rejects empty providerId", () => {
  const cloned = cloneRoutingTable(STANDARD_DEFAULT_ROUTING_TABLE);
  (cloned.environments.prod as Record<"tier-low", AgentModelBinding>)[
    "tier-low"
  ] = {
    providerId: "",
...
void test("validateRoutingTable: rejects unknown environment key", () => {
  const cloned = cloneRoutingTable(
    STANDARD_DEFAULT_ROUTING_TABLE,
  ) as unknown as {
...
void test("validateEuResidencyConstraint: passes for built-in EU table", () => {
...
void test("validateEuResidencyConstraint: flags non-EU bindings under eu-banking-default", () => {
  const cloned = cloneRoutingTable(EU_BANKING_DEFAULT_ROUTING_TABLE);
  (cloned.environments.prod as Record<"tier-low", AgentModelBinding>)[
    "tier-low"
  ] = {
...
void test("validateEuResidencyConstraint: skips non-EU profiles", () => {
  const result = validateEuResidencyConstraint(
    PERMISSIVE_DEFAULT_ROUTING_TABLE,
  );
  assert.equal(result.valid, true);
...
void test("resolveRoutingBinding: returns the right binding for the decision tier", () => {
...
void test("resolveRoutingBinding: tier-low routes to the cheap binding", () => {
  const decision = classifyTask({
    taskId: "task-y",
...
void test("cloneRoutingTable: produces a mutable copy", () => {
  const cloned = cloneRoutingTable(EU_BANKING_DEFAULT_ROUTING_TABLE);
  assert.notEqual(cloned, EU_BANKING_DEFAULT_ROUTING_TABLE);
  (cloned.environments.dev["tier-low"] as { providerId: string }).providerId =
    "custom";
...
void test("freezeRoutingTableExternal: returned table is deeply frozen", () => {
  const cloned = cloneRoutingTable(STANDARD_DEFAULT_ROUTING_TABLE);
