...
void test("probe: declared capabilities marked supported when responder always succeeds", async () => {
...
void test("probe: probes are sorted alphabetically and deterministic", async () => {
  const client = createMockLlmGatewayClient({
    role: "visual_primary",
...
void test("probe: undeclared capability is recorded as untested without invoking client", async () => {
  let calls = 0;
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "gpt-oss-120b",
...
void test("probe: gateway returning schema_invalid produces unsupported outcome", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "gpt-oss-120b",
    modelRevision: "rev",
...
void test("probe: transient failure produces probe_failed outcome", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "gpt-oss-120b",
    modelRevision: "rev",
...
void test("probe: serializes deterministically via canonicalJson", async () => {
...
void test("probe: writeLlmCapabilitiesArtifact persists exact bytes atomically", async () => {
...
void test("probe: persisted artifact never contains api keys or auth headers", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "gpt-oss-120b",
    modelRevision: "rev",
