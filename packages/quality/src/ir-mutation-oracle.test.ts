...
void test("computeIrMutationCoverageStrength covers all five mutation kinds deterministically", () => {
...
void test("computeIrMutationCoverageStrength treats navigation-only coverage as a transition kill", () => {
  const report = computeIrMutationCoverageStrength({
    model: buildModel(),
    list: buildList([
...
void test("computeIrMutationCoverageStrength does not kill by substring-only token overlap", () => {
  const report = computeIrMutationCoverageStrength({
    model: buildModel(),
    list: buildList([
...
void test("computeIrMutationCoverageStrength surfaces surviving mutations for repair when three cases kill none", () => {
  const report = computeIrMutationCoverageStrength({
    model: buildModel(),
    list: buildList([
...
void test("writeIrMutationCoverageStrengthArtifact persists canonical bytes", async () => {
  const report = computeIrMutationCoverageStrength({
    model: buildModel(),
    list: buildList([]),
  });
