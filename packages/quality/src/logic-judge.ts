...
import { detectCalculationConstraintViolation } from "@oscharko-dev/ti-core-engine";

const RESPONSE_SCHEMA_NAME = "test-intelligence-logic-judge-v1" as const;
const SYSTEM_PROMPT = [
  "You are the production logic judge for Test Intelligence.",
...
export interface LogicJudgeReplayCache {
  lookup(
    ...
    { hit: false; key: string } | { hit: true; entry: LogicJudgeCacheEntry }
  >;
...
const validateLogicJudgeResponse = (
  ...
  const findingsRaw = record["findings"] as ReadonlyArray<
    Record<string, unknown>
  >;
  const repairInstructionsRaw = record["repairInstructions"] as ReadonlyArray<
    Record<string, unknown>
  >;
...
const resolveJudgeFindingAnchor = (
  ...
  if (scope === "job") {
    if (testCaseId !== undefined && testCaseId !== JOB_LEVEL_TEST_CASE_ID) {
  ...
  if (scope === "test_case") {
    ...
    if (
      testCaseId === JOB_LEVEL_TEST_CASE_ID ||
      !testCaseIds.includes(testCaseId)
    ) {
      const path = `${basePath}.testCaseId`;
      return {
        ok: false,
...
const sanitizeShortMessage = (value: string): string => truncateMessage(value);
...
const findFirstHallucinatedId = (
  ...
  | { kind: "field" | "action" | "validation" | "navigation"; id: string }
  | undefined => {
...
const computeCoverageRatios = (
  ...
  return {
    fieldRatio:
      ir.fieldIds.size === 0 ? 1 : coveredFields.size / ir.fieldIds.size,
    actionRatio:
  ...
  for (const testCase of cases) {
    ...
    pushFinding(
      acc,
      {
        scope: "test_case",
        testCaseId: testCase.id,
        code: LOGIC_JUDGE_COVERAGE_HARD_GATE_FINDING_CODES.unsupportedUnresolvedValidationDetail,
        severity: "error",
  ...
  for (const testCase of cases) {
    ...
    pushFinding(
      acc,
      {
        scope: "test_case",
        testCaseId: testCase.id,
        code: LOGIC_JUDGE_COVERAGE_HARD_GATE_FINDING_CODES.financialCalculationConstraintBreach,
        severity: "error",
...
export const applyCoverageHardGate = (
  ...
  const upgradedVerdict: LogicJudgeVerdictLabel =
    verdict.verdict === "accept" && acc.hasNewError
      ? "repair"
      : verdict.verdict;
