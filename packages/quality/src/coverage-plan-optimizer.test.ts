...
void test("constants: schema versions and gate code are well-formed", () => {
...
void test("constants: AC#4 thresholds match the issue specification", () => {
  assert.equal(COVERAGE_PLAN_OPTIMIZER_KILL_RATE_FLOOR, 0.95);
  assert.equal(COVERAGE_PLAN_OPTIMIZER_COST_CEILING, 0.8);
});

void test("constants: objective list & directions are closed and well-formed", () => {
  assert.deepEqual(
    [...COVERAGE_PLAN_OPTIMIZER_OBJECTIVES],
    ["mutationKillRate", "techniqueCoverage", "tokenCost", "latencyMs"],
  );
  assert.equal(
...
void test("constants: methodology disclaimer is verbatim and non-empty", () => {
  assert.match(
    COVERAGE_PLAN_OPTIMIZER_METHODOLOGY_DISCLAIMER,
...
void test("DEFAULT_NSGA_II_CONFIG passes its own validator", () => {
...
void test("evaluatePlan: zero-min fixture with zero quota yields zero objectives", () => {
...
void test("evaluatePlan: more quota never decreases kill rate (monotonic)", () => {
...
void test("evaluatePlan: kill rate stays in [0,1] and coverage in [0,1]", () => {
  const plan = {
...
void test("evaluatePlan: clamps quotas outside [minQuota, maxQuota]", () => {
...
void test("evaluatePlan: pure — same input ⇒ byte-identical output", () => {
...
void test("optimizeFixture: deterministic — same seed ⇒ byte-identical result", () => {
...
void test("optimizeFixture: returns a non-empty Pareto front", () => {
...
void test("optimizeFixture: Pareto front is internally non-dominated", () => {
  const result = optimizeFixture({
...
void test("optimizeFixture: front is sorted by ascending token cost", () => {
  const result = optimizeFixture({
    fixture: LARGE_FIXTURE,
    config: SMALL_CONFIG,
    seed: 7,
...
void test("optimizeFixture: every plan obeys per-technique [minQuota, maxQuota]", () => {
  const result = optimizeFixture({
    fixture: LARGE_FIXTURE,
    config: SMALL_CONFIG,
    seed: 7,
...
void test("optimizeFixture: recommended plan satisfies AC #4 for representative fixtures", () => {
  const result = optimizeFixture({
    fixture: LARGE_FIXTURE,
    config: SMALL_CONFIG,
    seed: 7,
...
void test("optimizeFixture: rejects fixture with currentQuota outside [min, max]", () => {
...
void test("selectRecommendedPlan: picks the lowest-cost compliant member", () => {
...
void test("selectRecommendedPlan: empty front ⇒ empty_front reason", () => {
...
void test("selectRecommendedPlan: all below kill floor ⇒ no_kill_rate_above_floor", () => {
  const result = selectRecommendedPlan({
    front: [FAKE_FRONT[0]!],
    fixture: LARGE_FIXTURE,
  });
...
void test("selectRecommendedPlan: all above cost ceiling ⇒ no_cost_below_ceiling", () => {
  const result = selectRecommendedPlan({
    front: [FAKE_FRONT[2]!],
    fixture: LARGE_FIXTURE,
  });
...
void test("buildCoveragePlanOptimizerReport: empty fixtures rejected", () => {
  assert.throws(
    () =>
      buildCoveragePlanOptimizerReport({
        fixtures: [],
...
void test("buildCoveragePlanOptimizerReport: duplicate fixtureId rejected", () => {
  assert.throws(
    () =>
      buildCoveragePlanOptimizerReport({
...
void test("buildCoveragePlanOptimizerReport: invalid seed rejected", () => {
  assert.throws(
    () =>
      buildCoveragePlanOptimizerReport({
...
void test("buildCoveragePlanOptimizerReport: invalid generatedAt rejected", () => {
  assert.throws(
    () =>
      buildCoveragePlanOptimizerReport({
        fixtures: [SMALL_FIXTURE],
...
void test("buildCoveragePlanOptimizerReport: fixtures sorted by id", () => {
...
void test("buildCoveragePlanOptimizerReport: deterministic across two calls", () => {
...
void test("renderParetoFrontierSvg: deterministic, well-formed SVG", () => {
  const result = optimizeFixture({
    fixture: SMALL_FIXTURE,
    config: SMALL_CONFIG,
    seed: 11,
...
void test("renderParetoFrontierSvg: handles null recommended plan without throwing", () => {
...
void test("renderParetoFrontierSvg: escapes XML metacharacters in fixture id", () => {
...
void test("writeCoveragePlanOptimizerReport: writes byte-stable canonical JSON", async () => {
...
void test("FixtureOptimizationResult.paretoPlotSvg is present and well-formed for every fixture", () => {
  const report = buildCoveragePlanOptimizerReport({
...
void test("REFERENCE_BENCHMARK_FIXTURES: every fixture passes its own validator", () => {
...
void test("buildCoveragePlanOptimizerBaselineReport: AC #4 holds for every reference fixture", () => {
...
void test("committed baseline.json bytes match the freshly built report (G_COVERAGE_OPTIMIZER_BASELINE_PASS)", async () => {
...
void test("committed pareto-*.svg bytes match the freshly built plots", async () => {
  const report = buildCoveragePlanOptimizerBaselineReport();
  for (const fixture of report.fixtures) {
...
void test("computeCoveragePlanOptimizerReportDigest is byte-stable", () => {
...
void test("optimizeFixture: different seeds explore different Pareto fronts", () => {
  const a = optimizeFixture({
    fixture: LARGE_FIXTURE,
    config: SMALL_CONFIG,
    seed: 1,
