...
void test("sovereign profile carries its own canonical id distinct from the default", () => {
  assert.equal(
...
void test("sovereign profile inherits the default rule set byte-for-byte", () => {
...
void test("sovereign profile is deep-frozen", () => {
  assert.ok(Object.isFrozen(EU_BANKING_SOVEREIGN_POLICY_PROFILE));
});

void test("clone is mutable and isolated from both frozen profiles", () => {
...
void test("clone honours allowedHostingRegions override and rejects empty list", () => {
...
void test("clone preserves the default allow-list when no override is supplied", () => {
