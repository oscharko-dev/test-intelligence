...
void test("judge-panel vocabularies are closed and alphabetical", () => {
  assert.deepEqual(
    [...JUDGE_PANEL_JUDGE_IDS],
  ...
  assert.deepEqual(
    [...JUDGE_PANEL_AGREEMENT_LABELS],
    ["both_fail", "both_pass", "disagree"],
  );
  assert.deepEqual(
    [...JUDGE_PANEL_RESOLVED_SEVERITIES],
    ["critical", "downgraded_disagreement", "major", "minor"],
  );
  assert.deepEqual(
    [...JUDGE_PANEL_ESCALATION_ROUTES],
    ["accept", "downgrade", "needs_review"],
  );
...
void test("type guards round-trip the closed vocabularies", () => {
...
void test("both-pass verdict is deterministic and routes to accept/minor", () => {
...
void test("both-fail verdict is deterministic and routes to needs_review/critical", () => {
  const verdicts = buildJudgePanelVerdicts({
    samples: [
...
void test("AT-022: disagree downgrades severity by default and is auditable", () => {
  const verdicts = buildJudgePanelVerdicts({
    samples: [
...
void test("AT-022: disagreementRoute=needs_review escalates instead of downgrading", () => {
  const verdicts = buildJudgePanelVerdicts({
    samples: [
...
void test("disagree never maps to severity outside downgraded_disagreement", () => {
...
void test("calibration is per-judge empirical CDF (monotonic, distribution-aware)", () => {
  ...
  assert.ok((cPrimary.calibratedScore as number) >= JUDGE_PASS_THRESHOLD);
  assert.equal(cPrimary.verdict, "pass");
});

void test("calibration is byte-stable for byte-identical inputs", () => {
...
void test("no length normalisation: short and long reasons score identically given equal raw score", () => {
  ...
  for (const judgeId of JUDGE_PANEL_JUDGE_IDS) {
    const shortJudge: JudgePanelPerJudgeVerdictRecord | undefined =
      shortV.perJudge.find((r) => r.judgeId === judgeId);
    const longJudge: JudgePanelPerJudgeVerdictRecord | undefined =
      longV.perJudge.find((r) => r.judgeId === judgeId);
    assert.equal(shortJudge?.calibratedScore, longJudge?.calibratedScore);
    assert.equal(shortJudge?.verdict, longJudge?.verdict);
  }
});

void test("rejects reasons containing line-separator codepoints (U+000A/U+000D/U+2028/U+2029)", () => {
  for (const ch of ["\n", "\r", "\u2028", "\u2029"]) {
    assert.throws(
      () =>
        buildJudgePanelVerdicts({
...
void test("rejects over-long reasons", () => {
...
void test("accepts a reason exactly at the JUDGE_PANEL_REASON_MAX_CHARS boundary", () => {
  ...
  assert.equal(verdicts.length, 1);
  assert.equal(
    verdicts[0]?.perJudge[0]?.reason.length,
    JUDGE_PANEL_REASON_MAX_CHARS,
  );
});
...
void test("rejects empty samples array", () => {
  assert.throws(
...
void test("rejects an unknown judgeId", () => {
  assert.throws(
    () =>
      buildJudgePanelVerdicts({
        samples: [
...
void test("rejects a duplicate (testCaseId, criterion, judgeId) triple", () => {
  assert.throws(
    () =>
      buildJudgePanelVerdicts({
        samples: [
...
void test("rejects an incomplete panel (missing judge for a case)", () => {
  assert.throws(
    () =>
      buildJudgePanelVerdicts({
...
void test("rejects an out-of-range raw score", () => {
  assert.throws(
    () =>
      buildJudgePanelVerdicts({
        samples: [
...
void test("rejects unknown disagreementRoute policy", () => {
  assert.throws(
    () =>
      buildJudgePanelVerdicts({
        samples: [
...
void test("assertJudgePanelVerdictInvariants accepts a builder-produced verdict", () => {
...
void test("assertJudgePanelVerdictInvariants refuses agreement/severity inconsistency", () => {
  const v = buildJudgePanelVerdicts({
    samples: [
...
void test("assertJudgePanelVerdictInvariants refuses unsorted perJudge entries", () => {
  const v = buildJudgePanelVerdicts({
    samples: [
      sample({ judgeId: "judge_primary", reason: "p" }),
      sample({
...
void test("assertJudgePanelVerdictInvariants refuses bad schemaVersion", () => {
  const v = buildJudgePanelVerdicts({
    samples: [
      sample({ judgeId: "judge_primary", reason: "p" }),
      sample({
  ...
  const bad: JudgePanelVerdict = {
    ...v,
    schemaVersion:
      "0.0.0" as unknown as typeof JUDGE_PANEL_VERDICT_SCHEMA_VERSION,
  };
...
void test("writeJudgePanelVerdicts persists canonical JSON to <runDir>/judge-panel-verdicts.json", async () => {
  await withRunDir(async (runDir) => {
    ...
    const parsed = JSON.parse(onDisk) as readonly JudgePanelVerdict[];
...
void test("writeJudgePanelVerdicts refuses an empty verdict array", async () => {
  await withRunDir(async (runDir) => {
...
void test("writeJudgePanelVerdicts refuses unsorted verdict input", async () => {
  await withRunDir(async (runDir) => {
    const verdicts = buildJudgePanelVerdicts({
      samples: [
        sample({
...
void test("writeJudgePanelVerdicts atomically replaces an existing artifact", async () => {
  await withRunDir(async (runDir) => {
...
void test("serializeJudgePanelVerdicts preserves canonical-JSON ordering", () => {
  const verdicts = buildJudgePanelVerdicts({
    samples: [
  ...
  const parsed: readonly JudgePanelVerdict[] = JSON.parse(
    serialised,
  ) as readonly JudgePanelVerdict[];
