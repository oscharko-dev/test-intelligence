...
void test("buildJudgeDisagreementReport produces a sorted, frozen, valid report", () => {
  ...
  assert.deepEqual(
    report.judges.map((j) => j.judgeId),
    ["a11y_judge", "faithfulness_judge", "logic_judge"],
  );
  ...
  assert.deepEqual(
    report.costByFamily.map((c) => c.family),
    ["anthropic", "google", "openai"],
  );
  assert.doesNotThrow(() => assertJudgeDisagreementReportInvariants(report));
});

void test("buildJudgeDisagreementReport surfaces zero rates on a unanimous panel", () => {
  const report = buildJudgeDisagreementReport({
    jobId: "job-unanimous",
    generatedAt: "2026-05-08T12:00:00Z",
    bindings: [
...
void test("buildJudgeDisagreementReport normalises a per-family cost rollup", () => {
  const report = buildJudgeDisagreementReport({
    jobId: "job-cost",
    generatedAt: "2026-05-08T12:00:00Z",
    bindings: [
...
void test("serializeJudgeDisagreementReport is byte-stable across runs", () => {
  const input = {
    jobId: "job-stable",
    generatedAt: "2026-05-08T12:00:00Z",
    bindings: [
  ...
  const a = serializeJudgeDisagreementReport(
    buildJudgeDisagreementReport(input),
  );
  const b = serializeJudgeDisagreementReport(
    buildJudgeDisagreementReport(input),
  );
  assert.equal(a, b);
});

void test("writeJudgeDisagreementReport persists the canonical artifact atomically", async () => {
  ...
  try {
    ...
    const reloaded = JSON.parse(fileContents) as JudgeDisagreementReport;
...
void test("buildJudgeDisagreementReport refuses cost rollup with a family not in the run", () => {
  assert.throws(
    () =>
      buildJudgeDisagreementReport({
...
void test("assertJudgeDisagreementReportInvariants enforces matrix totals and ordering", () => {
  const report = buildJudgeDisagreementReport({
    jobId: "job-validate",
    generatedAt: "2026-05-08T12:00:00Z",
    bindings: [
  ...
  ) as JudgeDisagreementReport;
  (
    tampered as unknown as { perFamilyAgreement: Array<{ dissents: number }> }
  ).perFamilyAgreement[0]!.dissents = 5;
  assert.throws(
...
void test("assertJudgeDisagreementReportInvariants refuses non-ISO generatedAt", () => {
  const report = buildJudgeDisagreementReport({
    jobId: "job-iso",
    generatedAt: "2026-05-08T12:00:00Z",
    bindings: [
...
void test("buildJudgeDisagreementReport refuses an unknown family", () => {
  assert.throws(
    () =>
      buildJudgeDisagreementReport({
