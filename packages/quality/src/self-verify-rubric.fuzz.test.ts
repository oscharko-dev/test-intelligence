...
void test("property: validated rubric scores stay in [0, 1]", () =>
  fc.assert(
...
void test("property: aggregate jobLevelRubricScore stays in [0, 1]", () =>
  fc.assert(
...
void test("property: scores outside [0, 1] are always rejected", () =>
  fc.assert(
    fc.property(
      fc.oneof(
...
void test("property: prompt + schema hashes are deterministic", () =>
  fc.assert(
