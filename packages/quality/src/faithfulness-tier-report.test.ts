...
void test("classifyFaithfulnessStepTier flags numeric data as concrete_data", () => {
...
void test("classifyFaithfulnessStepTier flags label-only steps without data", () => {
  const tier = classifyFaithfulnessStepTier({
    index: 1,
...
void test("stepPassesTierThreshold enforces label_only `match` strictness", () => {
  assert.equal(stepPassesTierThreshold("label_only", "match"), true);
  assert.equal(stepPassesTierThreshold("label_only", "evidence_partial"), true);
  assert.equal(stepPassesTierThreshold("label_only", "mismatch"), false);
});

void test("stepPassesTierThreshold enforces concrete_data 0.80 floor", () => {
...
void test("buildFaithfulnessTierReport aggregates evidence_partial above 0.80", () => {
...
void test("buildFaithfulnessTierReport tags concrete-data steps and rejects mismatches", () => {
  const list = buildList([
    buildCase({
      steps: [
        {
...
void test("buildFaithfulnessTierReport defaults absent step verdicts to match", () => {
  const list = buildList([
    buildCase({
      steps: [
        { index: 1, action: "Open the form" },
...
void test("buildFaithfulnessTierReport refuses to build from a refused verdict", () => {
...
void test("buildFaithfulnessTierReport unblocks K0-shape: five label-only evidence_partial steps clear 0.80", () => {
  const list = buildList([
    buildCase({
      steps: [
...
void test("Issue #2170: classifyFaithfulnessStepTier promotes label-only steps of a state_transition case to the state_transition tier", () => {
  const tier = classifyFaithfulnessStepTier(
...
void test("Issue #2170: classifyFaithfulnessStepTier keeps concrete-data steps strict even on a state_transition case", () => {
  const tier = classifyFaithfulnessStepTier(
...
void test("Issue #2170: state_transition tier accepts evidence_partial at the 0.65 floor", () => {
  ...
  assert.equal(stepPassesTierThreshold("state_transition", "mismatch"), false);
});

void test("Issue #2170: label_only evidence_partial threshold tightens to 0.85", () => {
  // The verdict score for evidence_partial is 0.85 — exactly at the
  // tightened threshold, so it still passes.
  assert.equal(stepPassesTierThreshold("label_only", "evidence_partial"), true);
});

void test("Issue #2170: tier report flags partial-majority cases at >= 60 % evidence_partial", () => {
  const list = buildList([
    buildCase({
      id: "tc-majority",
      steps: [
...
void test("Issue #2170: tier report partialMajorityCaseIds is sorted ascending by id", () => {
  const list = buildList([
    buildCase({
      id: "tc-zeta",
...
void test("Issue #2170: state_transition case with evidence_partial steps still clears the 0.80 aggregate floor", () => {
  const list = buildList([
    buildCase({
      id: "tc-workflow",
...
void test("Issue #2116: persisted tier report carries evaluationMode='per_step'", () => {
  const list = buildList([
    buildCase({
      steps: [
        {
