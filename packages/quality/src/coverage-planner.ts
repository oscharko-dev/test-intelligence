...
import { selectTestDesignHeuristics } from "@oscharko-dev/ti-core-engine";
...
export interface BuildCoveragePlanWithAugmentationInput extends BuildCoveragePlanInput {
...
const DECISION_SIGNAL_PATTERN =
  /\b(if|when|unless|otherwise|depends|only if|required when|either|one of|all of|wenn|falls|ansonsten|abh[aä]ngig|entweder|alle)\b|\band\/or\b/i;
...
const COVERAGE_PLANNER_RESPONSE_SCHEMA_NAME =
  "test-intelligence-coverage-planner-v1" as const;
...
const semanticCoverageText = (element: {
  label?: string;
  kind?: string;
}): string =>
  normalizeCoverageText(`${element.label ?? ""} ${element.kind ?? ""}`);

const isResultDisplayElementLike = (element: {
  label?: string;
  kind?: string;
}): boolean => RESULT_DISPLAY_HINT_PATTERN.test(semanticCoverageText(element));

const isSelectableOptionElementLike = (element: {
  label?: string;
  kind?: string;
}): boolean =>
  SELECTABLE_OPTION_HINT_PATTERN.test(semanticCoverageText(element));
...
const isAcceptanceCriterionBoundary = (
  requirement: TestDesignRequirement,
): boolean => BOUNDARY_SIGNAL_PATTERN.test(requirement.text);

const isAcceptanceCriterionDecision = (
  requirement: TestDesignRequirement,
): boolean => DECISION_SIGNAL_PATTERN.test(requirement.text);

const isAcceptanceCriterionInteraction = (
  requirement: TestDesignRequirement,
): boolean =>
  /\b(open|opened|expand|expanded|collapse|collapsed|click|submit|cancel|save|navigate|aufklappbar|oeffnen|offnen|abbrechen|anlegen|speichern)\b/iu.test(
    requirement.text,
  );

const selectRequirementTechnique = (
  requirement: TestDesignRequirement,
): CoveragePlanTechnique => {
  if (isAcceptanceCriterionBoundary(requirement)) return "boundary_value";
  if (isAcceptanceCriterionDecision(requirement)) return "decision_table";
  if (isAcceptanceCriterionInteraction(requirement)) return "state_transition";
  if (requirement.verificationMode === "manual_review") return "error_guessing";
  if (requirement.verificationMode === "visual") return "initial_state";
...
const hasSupportingContextSection = (
...
const pushRiskRequirement = ({
  ...
  const screen =
    riskSignal.screenId === undefined
      ? undefined
      : screens.get(riskSignal.screenId);
  requirements.push(
    buildRequirement({
      technique: "error_guessing",
      reasonCode: riskSignal.text.startsWith("Multi-source ")
        ? "source_reconciliation_probe"
        : "risk_regression",
      ...(riskSignal.screenId !== undefined
        ? { screenId: riskSignal.screenId }
        : {}),
...
const compareTechniqueQuotas = (
  ...
  generatedTechniqueRank(left.technique) -
    generatedTechniqueRank(right.technique) || left.minCount - right.minCount;
...
const buildPerScreenPlan = (input: {
  ...
  const countsByScreen = new Map<string, Map<TestCaseTechnique29119, number>>();
  ...
  for (const requirement of [...input.minimumCases]) {
...
const mergeCoveragePlanAugmentation = (input: {
  ...
  const mergedPerScreen = input.plan.perScreen.map((screen) => {
    ...
    for (const [technique, minCount] of normalizeTechniqueQuotaRecord(
      ...
      nextCounts.set(
        technique,
        Math.max(nextCounts.get(technique) ?? 0, minCount),
      );
    }
    return {
      screenId: screen.screenId,
...
export const buildCoveragePlan = (
  ...
  for (const action of input.workflowTopology?.actions ?? []) {
    recommendedCases.push(
      buildRequirement({
        ...
        visualRefs: screens.get(action.screenId)?.visualRefs ?? [],
      }),
    );
  }
  ...
  for (const requirement of model.requirements ?? []) {
    const screen =
      requirement.screenId === undefined
        ? undefined
        : screens.get(requirement.screenId);
    minimumCases.push(
      buildRequirement({
        technique: selectRequirementTechnique(requirement),
        reasonCode: "acceptance_criterion",
        ...(requirement.screenId !== undefined
          ? { screenId: requirement.screenId }
          : {}),
        targetIds: [requirement.requirementId],
        sourceRefs: requirement.sourceRefs,
        visualRefs: screen?.visualRefs ?? allVisualRefs,
      }),
    );
  }
    ...
    if (hasPairwiseEvidence(screen)) {
      recommendedCases.push(
        buildRequirement({
          ...
          screenId: screen.screenId,
          targetIds: interactiveElements.map((element) => element.elementId),
          sourceRefs: screen.sourceRefs,
          visualRefs: screen.visualRefs,
        }),
      );
  ...
  for (const rule of model.businessRules) {
    const screen =
      rule.screenId === undefined ? undefined : screens.get(rule.screenId);
    minimumCases.push(
      buildRequirement({
...
export const buildCoveragePlanWithAugmentation = async (
  ...
  const request = {
    ...
    systemPrompt: [
      "You are the optional Coverage-Planner augmentation model for Test Intelligence.",
...
export const writeCoveragePlanArtifact = async (input: {
  ...
  if (typeof input.runDir !== "string" || input.runDir.length === 0) {
    throw new TypeError(
      "writeCoveragePlanArtifact: runDir must be a non-empty string",
    );
  }
