...
void test("coverage-baseline-drift: threshold and axis surface are stable", () => {
  ...
  assert.deepEqual([...COVERAGE_BASELINE_AXES].sort(), [
    "actionCoverage",
    "fieldCoverage",
    "navigationCoverage",
    "validationCoverage",
...
void test("coverage-baseline-drift: path is per-tenant under <runtimeRoot>/coverage-baselines/<tenantId>/<archetype>.json", () => {
  const path = coverageBaselinePath({
    runtimeRoot: "/var/lib/test-intelligence",
    tenantId: TENANT_ID,
    archetype: ARCHETYPE,
  });
  assert.equal(
    path,
    `/var/lib/test-intelligence/${COVERAGE_BASELINES_DIRNAME}/${TENANT_ID}/${ARCHETYPE}.json`,
  );
});

void test("coverage-baseline-drift: path rejects traversal segments and empty inputs", () => {
  assert.throws(() =>
    coverageBaselinePath({
      runtimeRoot: "/var/lib/test-intelligence",
      tenantId: "../escape",
...
void test("coverage-baseline-drift: extractCoverageRatiosFromReport pulls every tracked axis", () => {
...
void test("coverage-baseline-drift: evaluation with no baseline returns seeded=true and no findings", () => {
...
void test("coverage-baseline-drift: in-tolerance candidate yields no findings", () => {
...
void test("coverage-baseline-drift: drift > 10% on any axis trips the gate", () => {
...
void test("coverage-baseline-drift: drift exactly at threshold is tolerated", () => {
  const baseline = buildCoverageBaselineRecord({
    tenantId: TENANT_ID,
    archetype: ARCHETYPE,
    policyProfileId: POLICY_PROFILE_ID,
...
void test("coverage-baseline-drift: relativeDelta is null when baseline ratio is 0", () => {
  const baseline = buildCoverageBaselineRecord({
    tenantId: TENANT_ID,
    archetype: ARCHETYPE,
    policyProfileId: POLICY_PROFILE_ID,
...
void test("coverage-baseline-drift: drift on multiple axes is reported deterministically", () => {
  const baseline = buildCoverageBaselineRecord({
    tenantId: TENANT_ID,
    archetype: ARCHETYPE,
    policyProfileId: POLICY_PROFILE_ID,
...
void test("coverage-baseline-drift: build...Violation emits warning + needs_review-class outcome", () => {
  const baseline = buildCoverageBaselineRecord({
    tenantId: TENANT_ID,
    archetype: ARCHETYPE,
    policyProfileId: POLICY_PROFILE_ID,
...
void test("coverage-baseline-drift: build...Violation returns undefined when not exceeded", () => {
  const evaluation = evaluateCoverageBaselineDrift({
...
void test("coverage-baseline-drift: writeCoverageBaseline persists canonical JSON atomically", async () => {
...
void test("coverage-baseline-drift: loadCoverageBaseline returns undefined when missing (first run)", async () => {
  const runtimeRoot = await setupTempRuntimeRoot();
...
void test("coverage-baseline-drift: loadCoverageBaseline rejects identity mismatch", async () => {
  const runtimeRoot = await setupTempRuntimeRoot();
  const path = coverageBaselinePath({
    runtimeRoot,
    tenantId: TENANT_ID,
...
void test("coverage-baseline-drift: loadCoverageBaseline rejects schema-version drift", async () => {
  const runtimeRoot = await setupTempRuntimeRoot();
  const path = coverageBaselinePath({
    runtimeRoot,
    tenantId: TENANT_ID,
...
void test("coverage-baseline-drift: syncCoverageBaselineForJob seeds on first run (no drift)", async () => {
  const runtimeRoot = await setupTempRuntimeRoot();
...
void test("coverage-baseline-drift: syncCoverageBaselineForJob in check mode flags drift > 10%", async () => {
  const runtimeRoot = await setupTempRuntimeRoot();
  // Seed first.
...
void test("coverage-baseline-drift: syncCoverageBaselineForJob in update mode rebases (drift skipped)", async () => {
  const runtimeRoot = await setupTempRuntimeRoot();
  // Seed at low ratios.
  await syncCoverageBaselineForJob({
    runtimeRoot,
...
void test("coverage-baseline-drift: syncCoverageBaselineForJob check mode does not rewrite when in tolerance", async () => {
  const runtimeRoot = await setupTempRuntimeRoot();
  await syncCoverageBaselineForJob({
    runtimeRoot,
    tenantId: TENANT_ID,
...
void test("coverage-baseline-drift: writeCoverageBaseline rejects record/path identity mismatch", async () => {
  const runtimeRoot = await setupTempRuntimeRoot();
  const record = buildCoverageBaselineRecord({
    tenantId: "tenant-A",
    archetype: ARCHETYPE,
...
void test("coverage-baseline-drift: evaluateCoverageBaselineDrift rejects negative threshold", () => {
  assert.throws(() =>
    evaluateCoverageBaselineDrift({
      baseline: undefined,
      tenantId: TENANT_ID,
