...
void test("rubric prompt hash is deterministic across calls", () => {
...
void test("rubric schema hash is deterministic across calls", () => {
...
void test("rubric input hash binds list + intent + visual together", () => {
...
void test("rubric cache key digest changes when input changes", () => {
  const intent = buildIntent();
  const list = buildList([buildCase({ id: "tc-1" })]);
...
void test("rubric cache key digest changes when deployment or compatibility changes", () => {
  const intent = buildIntent();
  const list = buildList([buildCase({ id: "tc-1" })]);
  const baseKey = {
    passKind: "self_verify_rubric" as const,
...
void test("rubric validation rejects non-object responses", () => {
...
void test("rubric validation rejects responses missing caseEvaluations", () => {
  const result = validateSelfVerifyRubricResponse({}, ["tc-1"], false);
  assert.equal(result.ok, false);
  assert.equal(result.refusal.code, "schema_invalid_response");
});

void test("rubric validation rejects out-of-range scores", () => {
  const result = validateSelfVerifyRubricResponse(
    {
      caseEvaluations: [
        {
...
void test("rubric validation rejects unexpected testCaseId", () => {
  const result = validateSelfVerifyRubricResponse(
    {
...
void test("rubric validation rejects duplicate testCaseId", () => {
  const result = validateSelfVerifyRubricResponse(
    {
      caseEvaluations: [
...
void test("rubric validation rejects missing testCaseId", () => {
  const result = validateSelfVerifyRubricResponse(
    { caseEvaluations: [] },
    ["tc-1"],
    false,
...
void test("rubric validation rejects visualSubscores when no visual was supplied", () => {
  const result = validateSelfVerifyRubricResponse(
    {
...
void test("rubric validation accepts visualSubscores when visual was supplied", () => {
  const result = validateSelfVerifyRubricResponse(
    {
      caseEvaluations: [buildPerfectCaseEvaluation("tc-1", true)],
    },
...
void test("rubric validation requires complete dimension set", () => {
  const result = validateSelfVerifyRubricResponse(
    {
      caseEvaluations: [
        {
...
void test("aggregate produces job-level mean and per-dimension means", () => {
...
void test("aggregate includes visualSubscores when any case has them", () => {
  const evaluations: SelfVerifyRubricCaseEvaluation[] = [
    {
      testCaseId: "tc-1",
      dimensions: [...ALLOWED_SELF_VERIFY_RUBRIC_DIMENSIONS]
...
void test("aggregate over empty list returns zero job score and zero dimensions", () => {
...
void test("projectSelfVerifyRubricToTestCaseQualitySignals emits a sorted per-case list", () => {
  const evaluations: SelfVerifyRubricCaseEvaluation[] = [
    {
      testCaseId: "tc-2",
      dimensions: [],
...
void test("runSelfVerifyRubricPass scores every test case on the happy path", async () => {
...
void test("runSelfVerifyRubricPass cache hit skips the LLM call", async () => {
  const list = buildList([buildCase({ id: "tc-1" })]);
  const intent = buildIntent();
...
void test("runSelfVerifyRubricPass refuses when the gateway returns an error", async () => {
  const list = buildList([buildCase({ id: "tc-1" })]);
  const intent = buildIntent();
...
void test("runSelfVerifyRubricPass refuses when client uses a non-test_generation role", async () => {
  const list = buildList([buildCase({ id: "tc-1" })]);
  const intent = buildIntent();
  const client = createMockLlmGatewayClient({
    role: "visual_primary",
...
void test("runSelfVerifyRubricPass refuses before LLM call when model binding mismatches client", async () => {
  const list = buildList([buildCase({ id: "tc-1" })]);
  const intent = buildIntent();
  const client = buildPerfectMockClient(false);
  const result = await runSelfVerifyRubricPass({
...
void test("runSelfVerifyRubricPass refuses non-openai_chat compatibility before LLM call", async () => {
  const list = buildList([buildCase({ id: "tc-1" })]);
  const intent = buildIntent();
  const client = createMockLlmGatewayClient({
    role: "test_generation",
...
void test("runSelfVerifyRubricPass redacts secret-like substrings from refusal messages", async () => {
  const list = buildList([buildCase({ id: "tc-1" })]);
  const intent = buildIntent();
  const leakyToken =
...
void test("runSelfVerifyRubricPass refuses on schema_invalid_response when content shape is wrong", async () => {
  const list = buildList([buildCase({ id: "tc-1" })]);
  const intent = buildIntent();
  const client = createMockLlmGatewayClient({
    role: "test_generation",
...
void test("filesystem rubric cache round-trips a stored entry", async () => {
...
void test("writeSelfVerifyRubricReportArtifact emits canonical JSON under testcases/", async () => {
...
void test("rubric response schema enforces full dimension list", () => {
  const schema = buildSelfVerifyRubricResponseSchema();
  const props = (schema as { properties: Record<string, unknown> }).properties;
  assert.ok(props["caseEvaluations"]);
});

void test("rubric user prompt embeds the test case ids", () => {
  const list = buildList([
...
void test("rubric user prompt redacts secret-like strings before the gateway call", () => {
  const secret =
    "sk-ant-api01-AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"; // pragma: allowlist secret
  const list = buildList([
    buildCase({
      id: "tc-secret",
...
void test("rubric request never carries imageInputs (image-payload guarantee)", async () => {
  const list = buildList([buildCase({ id: "tc-1" })]);
  const intent = buildIntent();
