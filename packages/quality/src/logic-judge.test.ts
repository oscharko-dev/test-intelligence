...
import { createMemoryLogicJudgeCache, runLogicJudge } from "./logic-judge.js";
...
void test("runLogicJudge happy path emits an accept verdict and prompt artifact", async () => {
  ...
  assert.equal(
    ...
    "test-intelligence-logic-judge-v1",
  );
  assert.equal(client.callCount(), 1);
});

void test("runLogicJudge reuses the replay cache on the second invocation", async () => {
...
void test("runLogicJudge converts a gateway refusal into a reject verdict", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "mistral-document-ai-2512",
    modelRevision: "mistral-document-ai-2512@test",
...
void test("runLogicJudge surfaces a repair verdict with findings and repair instructions", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "mistral-document-ai-2512",
    modelRevision: "mistral-document-ai-2512@test",
        ...
        repairInstructions: [
          {
            ...
            instruction:
              "populate from IR field identifiers traced by the steps",
          },
        ],
      },
      finishReason: "stop",
...
void test("runLogicJudge accepts a job-scoped finding without testCaseId and canonicalizes it to $job", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "mistral-document-ai-2512",
    modelRevision: "mistral-document-ai-2512@test",
...
void test("runLogicJudge accepts an implicit job-level /testCases repair instruction without scope or testCaseId", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "mistral-document-ai-2512",
    modelRevision: "mistral-document-ai-2512@test",
...
void test("runLogicJudge preserves the legacy $job placeholder finding and infers job scope", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "mistral-document-ai-2512",
    modelRevision: "mistral-document-ai-2512@test",
...
void test("runLogicJudge accepts a test-case-scoped finding with a real testCaseId", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "mistral-document-ai-2512",
    modelRevision: "mistral-document-ai-2512@test",
...
void test("runLogicJudge normalizes wire severity aliases to the internal severity contract", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "mistral-document-ai-2512",
    modelRevision: "mistral-document-ai-2512@test",
...
void test("runLogicJudge rejects a test-case-scoped finding without testCaseId", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "mistral-document-ai-2512",
    modelRevision: "mistral-document-ai-2512@test",
...
void test("runLogicJudge passes through a semantic reject verdict from the model", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "mistral-document-ai-2512",
    modelRevision: "mistral-document-ai-2512@test",
...
void test("runLogicJudge converts a token-limit overrun into a reject verdict", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "mistral-document-ai-2512",
    modelRevision: "mistral-document-ai-2512@test",
  ...
  assert.match(result.verdict.refusal!.message, /exceeds maxOutputTokens/u);
  assert.equal(result.verdict.findings[0]!.severity, "error");
});
...
for (const scenario of SCHEMA_VIOLATION_REPAIR_CASES) {
  void test(`runLogicJudge surfaces a schema_violation repair verdict for ${scenario.title}`, async () => {
    const client = createMockLlmGatewayClient({
      role: "test_generation",
      deployment: "mistral-document-ai-2512",
      modelRevision: "mistral-document-ai-2512@test",
...
void test("runLogicJudge converts gateway schema_invalid response-shape failures into repair", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "mistral-document-ai-2512",
    modelRevision: "mistral-document-ai-2512@test",
  ...
  assert.equal(result.verdict.repairInstructions[0]!.kind, "schema_violation");
  assert.equal(
    result.verdict.repairInstructions[0]!.path,
...
void test("runLogicJudge records truncatedInstructionCount when schema-repair guidance exceeds the shared limit", async () => {
  const client = createMockLlmGatewayClient({
    role: "test_generation",
    deployment: "mistral-document-ai-2512",
    modelRevision: "mistral-document-ai-2512@test",
  ...
  assert.equal(result.verdict.repairInstructions[0]!.instruction.length, 240);
