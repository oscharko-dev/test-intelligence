...
void test("WAVE4_DEFAULT_EVAL_THRESHOLDS: matches the documented production-readiness thresholds", () => {
...
void test("evaluateWave4ProductionReadiness: a single passing source-mix yields passed=true", () => {
...
void test("evaluateWave4ProductionReadiness: ok=false on the run produces passed=false", () => {
  const report = evaluateWave4ProductionReadiness({
    sourceMixResults: [
      {
        mixId: "figma_only",
...
void test("evaluateWave4ProductionReadiness: quotasPassed=false produces passed=false", () => {
  const report = evaluateWave4ProductionReadiness({
    sourceMixResults: [
      {
...
void test("evaluateWave4ProductionReadiness: missing source provenance fails against expected source count", () => {
  const report = evaluateWave4ProductionReadiness({
    sourceMixResults: [
      {
        mixId: "figma_plus_jira_plus_custom",
...
void test("evaluateWave4ProductionReadiness: conflict recall below threshold fails the gate", () => {
  const report = evaluateWave4ProductionReadiness({
    sourceMixResults: [
      {
...
void test("evaluateWave4ProductionReadiness: airgap fetch calls above threshold fails the gate", () => {
  const report = evaluateWave4ProductionReadiness({
    sourceMixResults: [
      {
        mixId: "jira_paste_only",
...
void test("evaluateWave4ProductionReadiness: empty sourceMixResults passes vacuously", () => {
...
void test("evaluateWave4ProductionReadiness: overall coverage is clamped to [0, 1]", () => {
  const report = evaluateWave4ProductionReadiness({
...
void test("evaluateWave4ProductionReadiness: markdownCustomContextCoverage reflects supplied counts", () => {
  const report = evaluateWave4ProductionReadiness({
    sourceMixResults: [
      {
...
void test("evaluateWave4ProductionReadiness: report.version matches the schema constant", () => {
  const report = evaluateWave4ProductionReadiness({
    sourceMixResults: [passingMix()],
  });
  assert.equal(
...
void test("writeWave4ProductionReadinessEvalReport: writes a deterministic JSON artifact", async () => {
...
void test("writeWave4ProductionReadinessEvalReport: throws TypeError when runDir is empty", async () => {
  const report = evaluateWave4ProductionReadiness({
    sourceMixResults: [passingMix()],
  });
