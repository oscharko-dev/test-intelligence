...
void test("ALLOWED_TEST_INTENT_SOURCE_MIX_KINDS covers all seven mix kinds", () => {
...
void test("ALLOWED_SOURCE_MIX_PLANNER_REFUSAL_CODES covers all eight codes", () => {
...
void test("isSourceMixPlannerRefusalCode rejects unknown strings", () => {
...
void test("planSourceMix rejects custom-only envelope with primary_source_required", () => {
...
void test("planSourceMix rejects custom_markdown-only envelope with primary_source_required", () => {
...
void test("planSourceMix: figma_only plan has correct shape", () => {
...
void test("planSourceMix: jira_rest_only has visualSidecarRequirement=not_applicable", () => {
...
void test("planSourceMix: jira_rest_only primarySourceIds contains jira source id", () => {
  const envelope = buildEnvelope([jiraRestRef("jira.0", "PAY-100")]);
  const result = planSourceMix(envelope);
  assert.equal(result.ok, true);
...
void test("planSourceMix: jira_paste_only maps correctly", () => {
...
void test("planSourceMix: figma_jira_rest includes reconciliation_report", () => {
...
void test("planSourceMix: figma_jira_paste maps correctly", () => {
  const envelope = buildEnvelope([
    figmaRef("fig.0"),
...
void test("planSourceMix: figma_jira_mixed with different issue keys succeeds", () => {
  const envelope = buildEnvelope([
    figmaRef("fig.0"),
...
void test("planSourceMix: jira_mixed (no figma) with different keys succeeds", () => {
  const envelope = buildEnvelope([
...
void test("planSourceMix: figma + custom_markdown includes custom_context_markdown section", () => {
  const envelope = buildEnvelope([
    figmaRef("fig.0"),
...
void test("planSourceMix: jira_rest + custom_markdown includes both sections", () => {
  const envelope = buildEnvelope([
...
void test("planSourceMix: custom_markdown without hashes raises custom_markdown_hash_required", () => {
...
void test("planSourceMix: custom_markdown with inputFormat raises custom_markdown_input_format_invalid", () => {
  const bad: TestIntentSourceRef = {
    sourceId: "md.bad",
    kind: "custom_markdown",
    contentHash: HEX("md.bad"),
...
void test("planSourceMix: REST + paste with same issue key raises duplicate_jira_issue_key", () => {
  const envelope = buildEnvelope([
...
void test("planSourceMix: REST + paste duplicate can be routed to conflict evidence", () => {
  const envelope = buildEnvelope([
    jiraRestRef("rest.0", "PAY-1234"),
    jiraPasteRef("paste.0", "PAY-1234"),
  ]);
...
void test("planSourceMix: two paste sources with same key do not raise duplicate_jira_issue_key", () => {
...
void test("planSourceMix: REST + paste with different keys succeeds", () => {
  const envelope = buildEnvelope([
...
void test("computeSourceMixPlanHash: identical inputs produce identical hashes", () => {
  const input = {
...
void test("computeSourceMixPlanHash: different mix kind produces different hash", () => {
  const base = {
...
void test("computeSourceMixPlanHash: source ID order is normalized (sorted)", () => {
  const base = {
...
void test("planSourceMix: identical envelopes produce identical plan hashes", () => {
...
void test("planSourceMix: different mix kind changes plan hash", () => {
...
void test("planSourceMix: same source IDs with different content hashes change plan hash", () => {
...
void test("planSourceMix: custom markdown redacted hashes participate in plan hash", () => {
...
void test("planSourceMix: rawJiraResponsePersisted is always false", () => {
  const r = planSourceMix(buildEnvelope([jiraRestRef("jira.0", "PAY-100")]));
  assert.equal(r.ok, true);
  assert.equal(r.plan.rawJiraResponsePersisted, false);
});

void test("planSourceMix: rawPasteBytesPersisted is always false", () => {
...
void test("writeSourceMixPlan: writes artifact and content is valid JSON", async () => {
  const envelope = buildEnvelope([figmaRef("fig.0")]);
  const result = planSourceMix(envelope);
  assert.equal(result.ok, true);
...
void test("writeSourceMixPlan: rejects empty runDir", async () => {
  const envelope = buildEnvelope([figmaRef("fig.0")]);
  const result = planSourceMix(envelope);
  assert.equal(result.ok, true);
...
void test("planSourceMix: figma_only has only figma_intent section", () => {
  const r = planSourceMix(buildEnvelope([figmaRef("fig.0")]));
  assert.equal(r.ok, true);
  assert.deepEqual(r.plan.promptSections, ["figma_intent"]);
});

void test("planSourceMix: jira_rest_only has only jira_requirements section", () => {
  const r = planSourceMix(buildEnvelope([jiraRestRef("jira.0", "PAY-100")]));
  assert.equal(r.ok, true);
  assert.deepEqual(r.plan.promptSections, ["jira_requirements"]);
});

void test("planSourceMix: figma + legacy custom_text includes custom_context section", () => {
...
void test("planSourceMix: figma + jira + markdown includes all four sections", () => {
  const r = planSourceMix(
    buildEnvelope([
      figmaRef("fig.0"),
