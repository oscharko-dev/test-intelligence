...
void test("property: sanitizeJqlFragment rejects curated JQL injection vectors", () => {
...
void test("property: sanitizeJqlFragment preserves safe identifiers and values", () => {
  fc.assert(
    fc.property(fc.stringMatching(/^[A-Z][A-Z0-9_]{1,8}-[1-9][0-9]{0,5}$/), (issueKey) => {
      ...
      assert.equal(result.sanitized, fragment);
    }),
    { numRuns: 100 }
  );
});
