...
void test("isValidJiraIssueKey: accepts canonical keys, rejects adversarial", () => {
...
void test("sanitizeJqlFragment: rejects injection-shaped tokens", () => {
  for (const bad of [
  ...
  assert.equal(ok.sanitized, "project = PAY AND status = Done");
});

void test("buildJiraIssueIr: rejects invalid issue key", () => {
  ...
  assert.equal(result.code, "jira_issue_key_invalid");
});

void test("buildJiraIssueIr: rejects oversize issue key with too_long code", () => {
  const result = buildJiraIssueIr({
  ...
  assert.equal(result.ok, false);
  assert.equal(result.code, "jira_issue_key_too_long");
});

void test("buildJiraIssueIr: collapses unknown issue type to 'other'", () => {
  const result = buildJiraIssueIr({
    issueKey: "PAY-1",
    issueType: "exploit",
    summary: "x",
  ...
  assert.equal(result.ir.issueType, "other");
});

void test("buildJiraIssueIr: redacts IBAN/PAN/email/full-name/account-id placeholders", () => {
  const result = buildJiraIssueIr({
    issueKey: "PAY-1",
    issueType: "story",
...
void test("buildJiraIssueIr: redacts internal hostnames in ADF text", () => {
  const result = buildJiraIssueIr({
    issueKey: "PAY-1",
    issueType: "story",
...
void test("buildJiraIssueIr: customer-name field escalates to customer_name_placeholder", () => {
  const result = buildJiraIssueIr({
    issueKey: "PAY-1",
    issueType: "task",
...
void test("buildJiraIssueIr: rejects ADF over byte cap as jira_description_invalid", () => {
...
void test("buildJiraIssueIr: descriptionPlain truncated to 32 KiB cap", () => {
...
void test("buildJiraIssueIr: comments excluded by default, dataMinimization records it", () => {
  const result = buildJiraIssueIr({
    issueKey: "PAY-1",
    issueType: "story",
    summary: "x",
...
void test("buildJiraIssueIr: comments included when opted in, body capped to 4 KiB", () => {
...
void test("buildJiraIssueIr: unknown custom fields excluded by default", () => {
  const result = buildJiraIssueIr({
    issueKey: "PAY-1",
    issueType: "story",
    summary: "x",
...
void test("buildJiraIssueIr: contentHash deterministic across rebuilds", () => {
  const input = {
  ...
  assert.equal(a.ir.contentHash, b.ir.contentHash);
});

void test("buildJiraIssueIr: aggregates acceptance criteria from configured field", () => {
  const result = buildJiraIssueIr({
    issueKey: "PAY-1",
    issueType: "story",
    summary: "AC-bearing",
...
void test("buildJiraIssueIr: rejects malformed custom-field id", () => {
  const result = buildJiraIssueIr({
    issueKey: "PAY-1",
    issueType: "story",
    summary: "x",
  ...
  assert.equal(result.ok, false);
  assert.equal(result.code, "jira_custom_field_id_invalid");
});

void test("buildJiraIssueIr: rejects malformed custom-field entries without throwing", () => {
...
void test("DEFAULT_JIRA_FIELD_SELECTION_PROFILE: data-minimised by default", () => {
...
void test("writeJiraIssueIr: persists artifact at sources/<sourceId>/jira-issue-ir.json", async () => {
...
void test("Persisted Jira IR: zero token-shaped substrings present", () => {
  // Token-shaped fixture literals are assembled at runtime so the
  // repository secret scanner never matches a verbatim source string.
  const awsToken = ["AKIA", "IOSFODNN7", "EXAMPLE"].join("");
  const result = buildJiraIssueIr({
    issueKey: "PAY-1",
    issueType: "story",
    ...
    description: adf([
      para(
        `ghp_AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA1234 and ${awsToken} plus xoxb-12345678-1234567890.`,
      ),
    ]),
    status: "Open",
    capturedAt: ISO,
  ...
  const tokens = ["sk-abcdefghij1234567890", awsToken];
...
void test("maskKind/postal_address: positive — DE address yields postal_address indicator", () => {
...
void test("maskKind/postal_address: negative — bare postal code does not yield postal_address indicator", () => {
...
void test("maskKind/date_of_birth: positive — labelled DOB yields date_of_birth indicator", () => {
  const result = buildIrForText(
...
void test("maskKind/date_of_birth: negative — bare timestamp does not yield date_of_birth indicator", () => {
  const result = buildIrForText(
...
void test("maskKind/account_number: positive — labelled customer id yields account_number indicator", () => {
...
void test("maskKind/account_number: negative — unlabelled digit run does not yield account_number indicator", () => {
...
void test("maskKind/national_id: positive — Swiss AHV yields national_id indicator", () => {
...
void test("maskKind/national_id: negative — random alnum run does not yield national_id indicator", () => {
...
void test("maskKind/special_category: positive — health keyword yields special_category indicator", () => {
...
void test("maskKind/special_category: negative — neutral business prose yields no special_category indicator", () => {
  const result = buildIrForText(
...
void test("maskKind: stage-1 progression — postal_address + account_number both surface as distinct indicators", () => {
...
void test("maskKind: stage-1 progression — date_of_birth + special_category both surface as distinct indicators", () => {
...
void test("maskKind: stage-1 progression — national_id + postal_address both surface as distinct indicators", () => {
