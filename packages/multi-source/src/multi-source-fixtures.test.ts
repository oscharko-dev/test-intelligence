...
const EXPECTED_NEGATIVE_FIXTURE_REFUSALS = {
  ...
  "release-multisource-custom-markdown-adversarial": "primary_source_required",
} as const;

void test("multi-source-fixtures: ships every Wave 4.I production-readiness fixture id", () => {
...
void test("multi-source-fixtures: isWave4ProductionReadinessFixtureId is a precise type guard", () => {
  assert.equal(
...
void test("multi-source-fixtures: loadWave4ProductionReadinessFixture refuses unknown ids", async () => {
...
for (const fixtureId of WAVE4_PRODUCTION_READINESS_FIXTURE_IDS) {
  void test(`multi-source-fixtures: ${fixtureId} loads with a valid envelope shape`, async () => {
...
for (const fixtureId of WAVE4_PRODUCTION_READINESS_FIXTURE_IDS) {
  const expectedRefusal: string | undefined = (
    EXPECTED_NEGATIVE_FIXTURE_REFUSALS as Record<string, string>
  )[fixtureId];

  void test(`multi-source-fixtures: ${fixtureId} has expected validator outcome`, async () => {
    const fixture = await loadWave4ProductionReadinessFixture(fixtureId);
...
void test("multi-source-fixtures: onboarding fixture exposes figma + jira-rest + custom JSON inputs", async () => {
...
void test("multi-source-fixtures: paste-only fixture omits Figma / REST inputs", async () => {
  const fixture = await loadWave4ProductionReadinessFixture(
...
void test("multi-source-fixtures: markdown adversarial fixture exposes markdown body only", async () => {
  const fixture = await loadWave4ProductionReadinessFixture(
    "release-multisource-custom-markdown-adversarial",
  );
  assert.equal(fixture.figmaJson, undefined);
...
void test("multi-source-fixtures: payment-with-conflict fixture exposes a Jira input (paste or REST)", async () => {
  const fixture = await loadWave4ProductionReadinessFixture(
    "release-multisource-payment-with-conflict",
  );
  const hasJiraInput =
...
void test("multi-source-fixtures: air-gap fixtures omit Figma JSON", async () => {
...
void test("multi-source-fixtures: onboarding fixture envelope has version 1.0.0 and >=1 source", async () => {
  const fixture = await loadWave4ProductionReadinessFixture(
    "release-multisource-onboarding",
  );
