...
import { JIRA_ISSUE_IR_SCHEMA_VERSION } from "@oscharko-dev/ti-contracts";
...
void test("Jira IR snapshot: canonical-JSON byte-stable for a fixed input", () => {
  ...
  const parsed = JSON.parse(json1) as typeof result.ir;
...
void test("Jira IR snapshot: persistedIR.json structural invariants", () => {
  const result = buildJiraIssueIr({
    issueKey: "PAY-1",
    issueType: "task",
