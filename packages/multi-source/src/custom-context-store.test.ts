...
void test("persistCustomContext refuses custom-only jobs before writing artifacts", async () => {
  ...
  try {
    ...
    assert.equal(result.code, "primary_source_required");
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});

void test("persistCustomContext rejects bare unsafe Markdown URLs without writing custom artifacts", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "custom-context-store-"));
  try {
    await writeFile(
...
void test("persistCustomContext writes redacted Markdown and structured artifacts with deterministic source refs", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "custom-context-store-"));
  try {
    await writeFile(
      path.join(dir, "business-intent-ir.json"),
...
void test("persistCustomContext rejects invalid capturedAt before writing artifacts", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "custom-context-store-"));
  try {
    await writeFile(
      path.join(dir, "business-intent-ir.json"),
...
void test("persistCustomContext discovers Jira primary sources and tolerates corrupt prior custom artifacts", async () => {
  ...
  try {
    ...
    await unlink(
      path.join(dir, "sources", jira.result.sourceId, "paste-provenance.json"),
    );
...
void test("sha256Hex remains available for source fixture hashes", () => {
