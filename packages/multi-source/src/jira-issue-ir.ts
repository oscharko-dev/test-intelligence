...
const maskKind = (value: string, kind: PiiKind): string => {
  switch (kind) {
    case "email":
      return replaceEmailLike(value, "_");
    case "iban":
      return replaceIbanLike(value, "_");
    case "bic":
    ...
    case "postal_address":
      return replacePostalAddressLike(value, "_");
    case "date_of_birth":
      return value.replace(
...
const redactValue = (value: string): string => {
  let out = value;
  // Email
  out = replaceEmailLike(out, redactPii("email"));
  // Phone (E.164)
  out = out.replace(
    /(?<![\dA-Za-z])\+\d{1,3}[\s-]\d{2,4}[\s-]\d{3,8}(?:[\s-]\d{3,4})?(?!\d)/gu,
    redactPii("phone"),
  ...
  // IBAN-like
  out = replaceIbanLike(out, redactPii("iban"));
  // BIC
  out = out.replace(
    /\b[A-Z]{4}(?:DE|AT|CH|FR|GB|US|NL|ES|IT|BE|LU|DK|SE|NO|FI|IE|PT|PL|CZ|SK|HU|RO|BG|GR|CY|MT|EE|LV|LT|SI|HR)[A-Z0-9]{2}(?:[A-Z0-9]{3})?\b/gu,
    redactPii("bic"),
...
const scanEmailBoundary = (
  ...
  while (cursor >= 0 && cursor < value.length) {
    const ch = value[cursor] as string;
...
const isEmailTokenChar = (ch: string): boolean => {
...
const findIbanLikeSpan = (
  ...
  for (let start = offset; start + 15 <= value.length; start += 1) {
    ...
    if (
      !isDigitChar(value[start + 2] ?? "") ||
      !isDigitChar(value[start + 3] ?? "")
    ) {
      continue;
    }
    let cursor = start + 4;
...
const isUpperAsciiAlphaNum = (ch: string | undefined): boolean =>
  isUpperAsciiLetter(ch) || (ch !== undefined && isDigitChar(ch));
...
const replacePostalAddressLike = (
  value: string,
  replacement: string,
): string => {
  const span = findPostalAddressSpan(value);
  if (span === undefined) return value;
  return `${value.slice(0, span.start)}${replacement}${value.slice(span.end)}`;
};

const findPostalAddressSpan = (
  value: string,
): { start: number; end: number } | undefined => {
  const lower = value.toLowerCase();
  for (let i = 0; i < value.length; i += 1) {
    if (!isDigitChar(value[i] ?? "")) continue;
    const digitStart = i;
    while (i < value.length && isDigitChar(value[i] ?? "")) i += 1;
    const digitCount = i - digitStart;
    if (digitCount !== 4 && digitCount !== 5) continue;
    const contextStart = Math.max(0, digitStart - 80);
    const contextEnd = Math.min(value.length, i + 40);
    const context = lower.slice(contextStart, contextEnd);
    if (!POSTAL_STREET_HINTS.some((hint) => context.includes(hint))) continue;
    return {
      start: findAddressSpanStart(value, contextStart, digitStart),
      end: findAddressSpanEnd(value, i, contextEnd),
    };
  }
  return undefined;
};

const findAddressSpanStart = (
  value: string,
  lowerBound: number,
  beforePostalCode: number,
): number => {
  let start = beforePostalCode;
  while (start > lowerBound) {
    const ch = value[start - 1] as string;
    if (ch === "\n" || ch === "—" || ch === ";" || ch === ".") break;
    start -= 1;
  }
  return start;
...
const findAddressSpanEnd = (
  value: string,
  afterPostalCode: number,
  upperBound: number,
): number => {
  let end = afterPostalCode;
  while (end < upperBound) {
    const ch = value[end] as string;
    if (ch === "\n" || ch === "—" || ch === ";" || ch === ".") break;
    end += 1;
  }
  return end;
...
const isDigitChar = (value: string): boolean => {
