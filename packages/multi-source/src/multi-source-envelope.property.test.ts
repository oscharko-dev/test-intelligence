...
void test("property: aggregate hash is invariant under source reordering for non-priority policies", () => {
  fc.assert(
    fc.property(
...
void test("property: aggregate hash changes when any source contentHash changes", () => {
  fc.assert(
    fc.property(
      fc
...
void test("property: validator round-trips for any well-formed primary-source envelope", () => {
  fc.assert(
    fc.property(
      fc.uniqueArray(arbPrimaryRef, {
        minLength: 1,
...
void test("property: any envelope without a primary source refuses with primary_source_required", () => {
...
void test("property: priority policy hash differs when priorityOrder is permuted (and the permutation is not a no-op)", () => {
  fc.assert(
    fc.property(
      fc.constantFrom(
...
void test("property: validator never accepts a zero-source envelope", () => {
  fc.assert(
    fc.property(
...
void test("property: arbAnyKind is exhaustive", () => {
