...
import { buildJiraAuthHeaders } from "./jira-capability-probe.js";
...
const DEFAULT_CONFIG: JiraGatewayConfig = {
  ...
  userAgent: "test-intelligence/1.0",
  allowedHostPatterns: ["example.atlassian.net"],
};

void test("buildJiraAuthHeaders generates Bearer header", () => {
...
void test("buildJiraAuthHeaders generates Basic header", () => {
  const headers = buildJiraAuthHeaders({
    ...DEFAULT_CONFIG,
...
void test("buildJiraAuthHeaders generates OAuth2 header", () => {
  const headers = buildJiraAuthHeaders({
    ...DEFAULT_CONFIG,
...
void test("JiraGatewayClient probes capability successfully", async () => {
  let fetchedUrl = "";
  ...
  assert.ok(result.ok);
  assert.equal(result.capability.deploymentType, "Cloud");
  assert.ok(fetchedUrl.endsWith("/rest/api/3/serverInfo"));
});

void test("JiraGatewayClient handles probe capability failure", async () => {
...
void test("JiraGatewayClient caches non-retryable failed capability probe once per session", async () => {
  let calls = 0;
  const mockFetch = async (): Promise<Response> => {
    calls += 1;
    return new Response("", { status: 401 });
...
void test("JiraGatewayClient retries retryable capability probe failures", async () => {
  let probeCalls = 0;
  let searchCalls = 0;
  const mockFetch = async (url: string): Promise<Response> => {
  ...
  const client = createJiraGatewayClient(
    ...
    { fetchImpl: mockFetch as unknown as typeof fetch, sleep: async () => {} },
  );
...
void test("JiraGatewayClient fetches JQL", async () => {
  const mockFetch = async (url: string): Promise<Response> => {
    if (url.endsWith("serverInfo")) {
      return new Response(
        JSON.stringify({ version: "10.0.0", deploymentType: "Cloud" }),
...
void test("JiraGatewayClient caches successful capability probe once per session", async () => {
  let probeCalls = 0;
  let searchCalls = 0;
  const mockFetch = async (url: string): Promise<Response> => {
    if (url.endsWith("serverInfo")) {
...
void test("JiraGatewayClient pushes issue-key batches and field selection into the request", async () => {
...
void test("JiraGatewayClient rejects invalid issue-key batches before search fetch", async () => {
  let calls = 0;
  const mockFetch = async (url: string): Promise<Response> => {
    calls += 1;
    if (url.endsWith("serverInfo")) {
...
void test("JiraGatewayClient rejects invalid request budgets before network", async () => {
  let calls = 0;
  const client = createJiraGatewayClient(DEFAULT_CONFIG, {
    fetchImpl: (async () => {
      calls += 1;
...
void test("JiraGatewayClient rejects invalid replay sourceId without throwing", async () => {
  let calls = 0;
  const client = createJiraGatewayClient(DEFAULT_CONFIG, {
    fetchImpl: (async () => {
      calls += 1;
  ...
  const result = await client.fetchIssues({
    query: { kind: "jql", jql: "project=TEST", maxResults: 10 },
    runDir: "/tmp/test-intelligence-test",
    sourceId: "../escape",
  });
...
void test("JiraGatewayClient fails closed when Jira response issue cannot build IR", async () => {
  const mockFetch = async (url: string): Promise<Response> => {
    if (url.endsWith("serverInfo")) {
      return new Response(
        JSON.stringify({ version: "10.0.0", deploymentType: "Cloud" }),
...
void test("JiraGatewayClient fetch rate limit retry", async () => {
  let attempts = 0;
  const mockFetch = async (url: string): Promise<Response> => {
    if (url.endsWith("serverInfo")) {
      return new Response(
  ...
  const client = createJiraGatewayClient(
    ...
    { fetchImpl: mockFetch as unknown as typeof fetch, sleep },
  );
...
void test("JiraGatewayClient refuses Retry-After that exceeds wall-clock budget", async () => {
  let attempts = 0;
  let sleeps = 0;
  const mockFetch = async (url: string): Promise<Response> => {
    if (url.endsWith("serverInfo")) {
...
void test("JiraGatewayClient fails closed on auth and oversized responses", async () => {
...
void test("JiraGatewayClient persists redacted Jira IR list and replay mode issues zero fetch calls", async () => {
...
void test("JiraGatewayClient replay mode fails closed when cached IR list is malformed", async () => {
...
void test("MockJiraGatewayClient returns deterministic static response", async () => {
  const staticResult = {
    issues: [],
    capability: {
      version: "mock",
...
void test("JiraGatewayClient rejects SSRF vectors", () => {
  const invalidUrls = [
...
void test("JiraGatewayClient validates auth-specific host shapes", () => {
  assert.doesNotThrow(() =>
...
void test("JiraGatewayClient: cumulative wall-clock cannot exceed maxWallClockMs across retries (#1666)", async () => {
  let now = 0;
  const clock = { now: () => now };
  let probeCalls = 0;
...
// code path.
void test("JiraGatewayClient: pre-attempt cumulative-budget guard fires after sleep overshoot (#1666)", async () => {
  let now = 0;
  const clock = { now: () => now };
  const fetchImpl = async (input: string | URL): Promise<Response> => {
    const url = typeof input === "string" ? input : input.toString();
...
void test("JiraGatewayClient: pre-attempt cumulative-budget guard refuses subsequent attempts", async () => {
  let now = 0;
  const clock = { now: () => now };
  const fetchImpl = async (input: string | URL): Promise<Response> => {
    const url = typeof input === "string" ? input : input.toString();
