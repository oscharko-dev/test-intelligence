...
import { MAX_JIRA_ADF_INPUT_BYTES } from "@oscharko-dev/ti-contracts";
...
void test("ADF parser: rejects unknown node type", () => {
...
void test("ADF parser: rejects unknown mark type", () => {
  const result = parseJiraAdfDocument(
...
void test("ADF parser: paragraph + heading + list emit deterministic plain text", () => {
  const doc = adf([
    {
      type: "heading",
      attrs: { level: 2 },
...
void test("ADF parser: code block carries language and text", () => {
  const doc = adf([
    {
      type: "codeBlock",
...
void test("ADF parser: code block text children count toward traversal cap", () => {
  const doc = adf([
    {
      type: "codeBlock",
...
void test("ADF parser: panel summary preserves nested structural blocks", () => {
  const doc = adf([
    {
      type: "panel",
      content: [
...
void test("ADF parser: mention/inlineCard/media render as opaque stubs", () => {
  const doc = adf([
    {
      type: "paragraph",
      content: [
...
void test("ADF parser: nested-depth bound rejects pathological input", () => {
...
void test("ADF parser: byte-stable across canonicalised re-runs", () => {
  const doc1 = adf([
    para("hello world"),
    {
      type: "heading",
...
void test("ADF parser: rejects inline node at block level", () => {
  const result = parseJiraAdfDocument(
