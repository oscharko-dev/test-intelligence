...
void test("custom context input: validates and canonicalizes structured attributes", () => {
...
void test("custom context input: redacts PII from structured attribute values before persistence", () => {
  const result = validateCustomContextInput({
    attributes: [
      {
        key: "dataClass",
...
void test("custom context input: rejects unsupported fields and malformed attributes", () => {
  const result = validateCustomContextInput({
...
void test("normalizeAttributeKey maps public camelCase labels to canonical wire keys", () => {
