...
import { buildSourceScopedValidationOpenQuestions } from "@oscharko-dev/ti-core-engine";
...
const cloneScreen = (
  ...
  trace: {
    ...(screen.trace.nodeId !== undefined
      ? { nodeId: screen.trace.nodeId }
      : {}),
...
const stableSlug = (value: string): string =>
  slugifyAscii(normalizeText(value)) || "value";
...
const inferRiskFromText = (value: string): TestCaseRiskCategory | undefined => {
  ...
  if (
    /(regulated|gdpr|pci|pii|personal data|kyc|aml|customer data)/.test(
      normalized,
    )
  ) {
...
const extractJiraFieldCandidates = (
  ...
  const figmaLabels =
    figmaIntent?.detectedFields.map((field) => field.label) ?? [];
  const texts = [
    issue.summary,
...
const extractValidationRulesFromText = (text: string): string[] => {
  ...
  for (const line of lines) {
    ...
    const regexTail = tailAfterKeyword(line, "regex");
    if (regexTail !== undefined) rules.add(`regex ${regexTail}`);
  }
  return [...rules].sort();
};

const tailAfterKeyword = (
  line: string,
  keyword: string,
): string | undefined => {
  ...
  while (index >= 0) {
    const before = index === 0 ? "" : (lower[index - 1] as string);
    const afterIndex = index + keyword.length;
    const after = lower[afterIndex] ?? "";
    const boundaryBefore = before.length === 0 || !isAsciiWordChar(before);
    const boundaryAfter = after.length === 0 || !isAsciiWordChar(after);
...
const isAsciiWordChar = (value: string): boolean => {
...
export const reconcileMultiSourceIntent = (
  ...
  const legacySource = legacySourceFromMultiSourceEnvelope(input.envelope) ?? {
  ...
  const baseIntent: BusinessTestIntentIr =
    input.figmaIntent !== undefined
      ? {
          ...
          detectedActions: input.figmaIntent.detectedActions.map((value) => ({
            ...
            trace: {
              ...
              ...(value.trace.sourceRefs !== undefined
                ? {
                    sourceRefs: value.trace.sourceRefs.map((ref) => ({
                      ...ref,
                    })),
                  }
                : {}),
          ...
          detectedNavigation: input.figmaIntent.detectedNavigation.map(
            (value) => ({
              ...value,
              trace: {
                ...value.trace,
      ...
      : {
          ...
          source: legacySource,
          screens: (input.jiraIssues ?? [])
            .map(buildJiraOnlyScreen)
  ...
  const recordContribution = (
    ...
    const existing =
      contributingSourcesPerCase.get(caseId) ?? new Set<string>();
  ...
  const jiraValidationCandidates = jiraIssues.flatMap((issue) => {
    ...
    const fields = jiraFieldCandidates.filter(
      (field) => field.sourceId === ref.sourceId,
    );
  ...
  for (const validation of baseIntent.detectedValidations) {
    const targetField = validation.targetFieldId
      ? baseIntent.detectedFields.find(
          (field) => field.id === validation.targetFieldId,
        )
      : undefined;
    ...
    if (existing === undefined)
      figmaValidationsBySemantic.set(key, [validation]);
  ...
  for (const candidate of jiraFieldCandidates) {
    ...
    for (const field of figmaMatches) {
      const fieldSourceIds =
        field.sourceRefs?.map((ref) => ref.sourceId) ?? figmaSourceIds;
      ...
      if (normalizeText(field.label) !== candidate.normalizedLabel) {
        const resolution = resolutionForPolicy(
          input.envelope.conflictResolutionPolicy,
        );
        ...
        if (
          ...
          !baseIntent.detectedFields.some(
            (existing) => existing.id === alternative.id,
          )
        ) {
  ...
  for (const candidate of jiraValidationCandidates) {
    ...
    for (const validation of figmaMatches) {
      ...
      const participatingSourceIds = [
        ...validationSourceIds,
        candidate.sourceId,
      ];
      if (
        normalizeValidationRule(validation.rule) !== candidate.normalizedRule
      ) {
        const conflict = buildConflict({
          kind: "validation_rule_mismatch",
          participatingSourceIds,
          normalizedValues: [validation.rule, candidate.rule],
          resolution: resolutionForPolicy(
            input.envelope.conflictResolutionPolicy,
          ),
        ...
        if (
          alternative !== undefined &&
          !baseIntent.detectedValidations.some(
            (existing) => existing.id === alternative.id,
          )
        ) {
  ...
  for (const ref of jiraSourceRefs) {
    ...
    if (existing === undefined)
      jiraKeys.set(ref.canonicalIssueKey, [ref.sourceId]);
...
const sourceRefForId = (
  ...
  const ref = envelope.sources.find(
...
const applyValidationConflictPolicy = (input: {
  ...
  if (input.policy === "reviewer_decides") {
    ...
    input.validation.trace.sourceRefs = input.validation.sourceRefs.map(
      (ref) => ({
        ...ref,
      }),
    );
...
const winnerSourceId = (
  ...
  const refs = sourceIds
    .map((sourceId) =>
      envelope.sources.find((ref) => ref.sourceId === sourceId),
    )
