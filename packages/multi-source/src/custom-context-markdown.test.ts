...
void test("custom context markdown: redacts PII across headings, tables, lists, blockquotes, and code fences", () => {
...
void test("custom context markdown: rejects executable and embedded content fail-closed", () => {
...
void test("custom context markdown: refuses unsafe URL host variants and accepts safe public links", () => {
  const unsafe = [
    "[x](file:///etc/passwd)",
    "[x](vbscript:msgbox(1))",
  ...
  const remainingHttpUrls = [
    ...safe.value.bodyMarkdown.matchAll(/https?:\/\/[^\s)<>]+/gu),
  ].map((match) => match[0]);
  assert.deepEqual(remainingHttpUrls, []);
  assert.equal(
    safe.value.bodyMarkdown.includes("about:blank#redacted-link"),
    true,
  );
...
void test("custom context markdown: validates input type and canonical byte budgets", () => {
...
void test("custom context markdown: canonical hashes are stable for semantic whitespace variants", () => {
