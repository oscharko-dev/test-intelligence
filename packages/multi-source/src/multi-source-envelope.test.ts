...
void test("ALLOWED_TEST_INTENT_SOURCE_KINDS covers all eight Wave 4.K kinds", () => {
...
void test("primary and supporting source-kind sets partition the universe", () => {
...
void test("ALLOWED_CONFLICT_RESOLUTION_POLICIES has the three contract values", () => {
...
void test("ALLOWED_TEST_INTENT_CUSTOM_INPUT_FORMATS exposes the three formats", () => {
...
void test("MULTI_SOURCE_TEST_INTENT_ENVELOPE_SCHEMA_VERSION is 1.0.0", () => {
  assert.equal(MULTI_SOURCE_TEST_INTENT_ENVELOPE_SCHEMA_VERSION, "1.0.0");
});

void test("buildMultiSourceTestIntentEnvelope produces a stable schema version", () => {
...
void test("buildMultiSourceTestIntentEnvelope omits priorityOrder for non-priority policies", () => {
  const envelope = buildMultiSourceTestIntentEnvelope({
    sources: [figmaRef("src.0", "a")],
    conflictResolutionPolicy: "keep_both",
...
void test("buildMultiSourceTestIntentEnvelope preserves valid sourceMixPlan hook", () => {
  const envelope = buildMultiSourceTestIntentEnvelope({
    sources: [figmaRef("src.0", "a")],
    conflictResolutionPolicy: "keep_both",
    sourceMixPlan: {
...
void test("aggregate hash is invariant under source reordering for non-priority policy", () => {
...
void test("aggregate hash differs when priority order differs", () => {
  const a = figmaRef("src.0", "alpha");
  const b = jiraRestRef("src.1", "beta");
...
void test("aggregate hash changes when source content changes", () => {
  const a = figmaRef("src.0", "alpha");
...
void test("validator accepts a Figma-only envelope", () => {
...
void test("validator accepts Jira REST + Jira paste + custom_text combination", () => {
  const env = buildMultiSourceTestIntentEnvelope({
    sources: [
...
void test("validator rejects a custom-only envelope with primary_source_required", () => {
  const env = buildMultiSourceTestIntentEnvelope({
...
void test("validator rejects empty source list with sources_empty", () => {
...
void test("validator rejects unknown source kind", () => {
  const result = validateMultiSourceTestIntentEnvelope({
    version: MULTI_SOURCE_TEST_INTENT_ENVELOPE_SCHEMA_VERSION,
    sources: [
      {
...
void test("validator rejects malformed content hash and bad capturedAt", () => {
  const result = validateMultiSourceTestIntentEnvelope({
    version: MULTI_SOURCE_TEST_INTENT_ENVELOPE_SCHEMA_VERSION,
    sources: [
      {
...
void test("validator rejects impossible calendar dates in capturedAt", () => {
  const env = buildMultiSourceTestIntentEnvelope({
    sources: [
      {
        ...figmaRef("src.0", "x"),
  ...
  assert.ok(
    result.issues.some(
      (i) =>
        i.code === "invalid_captured_at" && i.path === "sources[0].capturedAt",
    ),
  );
});

void test("validator rejects duplicate source IDs", () => {
  const env = buildMultiSourceTestIntentEnvelope({
...
void test("validator detects duplicate Jira paste-collision issue keys", () => {
  const env = buildMultiSourceTestIntentEnvelope({
    sources: [
...
void test("validator reports Jira paste-collision paths using original source indexes", () => {
  const env = buildMultiSourceTestIntentEnvelope({
    sources: [
...
void test("validator requires custom inputFormat for custom_text", () => {
  const result = validateMultiSourceTestIntentEnvelope({
    version: MULTI_SOURCE_TEST_INTENT_ENVELOPE_SCHEMA_VERSION,
    sources: [
      figmaRef("src.0"),
...
void test("validator rejects inputFormat on primary source", () => {
  const env = buildMultiSourceTestIntentEnvelope({
    sources: [
      {
        sourceId: "src.0",
  ...
  assert.equal(result.ok, false);
  assert.ok(
    result.issues.some((i) => i.code === "primary_source_input_format_invalid"),
  );
});

void test("validator rejects markdown metadata on non-markdown custom source", () => {
  const env = buildMultiSourceTestIntentEnvelope({
    sources: [
      figmaRef("src.0"),
      {
...
void test("validator accepts markdown metadata on markdown custom source", () => {
  const env = buildMultiSourceTestIntentEnvelope({
...
void test("validator requires deterministic hashes for markdown custom sources", () => {
  const env = buildMultiSourceTestIntentEnvelope({
    sources: [
      figmaRef("src.0"),
      {
...
void test("validator rejects Jira issue keys on non-Jira sources", () => {
  const env = buildMultiSourceTestIntentEnvelope({
...
void test("validator rejects non-canonical Jira issue keys", () => {
  const env = buildMultiSourceTestIntentEnvelope({
...
void test("validator catches aggregate hash mismatch", () => {
...
void test("validator requires priorityOrder under priority policy", () => {
  const env: MultiSourceTestIntentEnvelope = {
    version: MULTI_SOURCE_TEST_INTENT_ENVELOPE_SCHEMA_VERSION,
...
void test("validator rejects priorityOrder on non-priority policy", () => {
  const env: MultiSourceTestIntentEnvelope = {
    version: MULTI_SOURCE_TEST_INTENT_ENVELOPE_SCHEMA_VERSION,
    sources: [figmaRef("src.0")],
    aggregateContentHash: computeAggregateContentHash({
...
void test("validator rejects priorityOrder containing kinds not in sources", () => {
  const env = buildMultiSourceTestIntentEnvelope({
    sources: [figmaRef("src.0"), jiraRestRef("src.1", "j")],
    conflictResolutionPolicy: "priority",
...
void test("validator rejects priorityOrder with missing kinds", () => {
  const env = buildMultiSourceTestIntentEnvelope({
    sources: [figmaRef("src.0"), jiraRestRef("src.1", "j")],
    conflictResolutionPolicy: "priority",
    priorityOrder: ["figma_local_json"],
  ...
  const result = validateMultiSourceTestIntentEnvelope(env);
  assert.equal(result.ok, false);
  assert.ok(result.issues.some((i) => i.code === "priority_order_incomplete"));
});

void test("validator accepts a complete priority envelope round-trip", () => {
  const env = buildMultiSourceTestIntentEnvelope({
    sources: [figmaRef("src.0"), jiraRestRef("src.1", "j")],
    conflictResolutionPolicy: "priority",
...
void test("validator returns envelope_missing for non-objects", () => {
...
void test("validator returns envelope_version_mismatch on unknown version", () => {
  const env = {
    version: "9.9.9",
    sources: [figmaRef("src.0")],
    aggregateContentHash: "0".repeat(64),
  ...
  const result = validateMultiSourceTestIntentEnvelope(env);
  assert.equal(result.ok, false);
  assert.ok(result.issues.some((i) => i.code === "envelope_version_mismatch"));
});

void test("validator rejects malformed authorHandle", () => {
  const env = buildMultiSourceTestIntentEnvelope({
    sources: [
      {
...
void test("legacySourceFromMultiSourceEnvelope prefers Figma when present", () => {
  const env = buildMultiSourceTestIntentEnvelope({
    sources: [
      jiraRestRef("src.0", "j"),
      figmaRef("src.1", "f"),
...
void test("legacySourceFromMultiSourceEnvelope collapses non-Figma to hybrid", () => {
  const env = buildMultiSourceTestIntentEnvelope({
...
void test("canonicalizeMultiSourceEnvelope is deterministic", () => {
  const env = buildMultiSourceTestIntentEnvelope({
    sources: [figmaRef("src.0"), jiraRestRef("src.1", "j")],
    conflictResolutionPolicy: "reviewer_decides",
  });
...
void test("isMultiSourceEnvelopeRefusalCode and ...ModeGateRefusalCode guards", () => {
...
void test("evaluateMultiSourceModeGate allows when all four gates green", () => {
...
void test("evaluateMultiSourceModeGate refuses when test-intelligence parent disabled", () => {
  const decision = evaluateMultiSourceModeGate({
...
void test("evaluateMultiSourceModeGate fails closed on llmCodegenMode != deterministic", () => {
  const decision = evaluateMultiSourceModeGate({
    testIntelligenceEnvEnabled: true,
    testIntelligenceStartupEnabled: true,
    multiSourceEnvEnabled: true,
...
void test("enforceMultiSourceModeGate throws MultiSourceModeGateError on refusal", () => {
  assert.throws(
    () =>
      enforceMultiSourceModeGate({
        testIntelligenceEnvEnabled: false,
...
void test("enforceMultiSourceModeGate is silent when allowed", () => {
  enforceMultiSourceModeGate({
    testIntelligenceEnvEnabled: true,
    testIntelligenceStartupEnabled: true,
    multiSourceEnvEnabled: true,
...
void test("resolveTestIntelligenceMultiSourceEnvEnabled accepts known truthy values", () => {
...
void test("ALLOWED_MULTI_SOURCE_ENVELOPE_REFUSAL_CODES includes primary_source_required", () => {
  assert.ok(
...
void test("source-mix matrix coverage: every documented combination", () => {
