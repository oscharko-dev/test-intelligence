...
This suite covers untrusted-content hardening for prompt-injection carriers.
...
inline in `packages/core-engine/src/adversarial-2025.test.ts`, and is
mandatory in `pnpm --filter @oscharko-dev/ti-core-engine test`.

The carriers exercise every branch of `UntrustedContentNormalizer` used
by the production input pipeline. No fixture mutates the production code
...
- The fixtures contain **only** synthetic injection text. No real PII,
  ...
  to match the regex set in `untrusted-content-normalizer.ts` (mirrored
  from `test-design-model.ts`).
...
5. Confirm `pnpm --filter @oscharko-dev/ti-core-engine test` is green.
