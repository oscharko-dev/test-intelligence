...
| Export goldens              | `export-pipeline.golden.test.ts` and friends     | `*.expected.*`                            |
...
└── agent-lessons-eval-input.json       # required: approved-lesson fixture matrix
└── agent-lessons-eval-report.json      # required: deterministic lesson quality report
...
- `eval-baseline-<archetype>.json` stores the deterministic MA-0
  single-pass eval snapshot. The archetype token is
  ...
  air-gapped PR lane.
- `eval-ab-input.json` is the curated MA-5 review sample. It carries,
  for each archetype, the single-pass and
...
- `agent-lessons-eval-input.json` is the curated lesson fixture matrix.
  Each entry points at one shipped Wave-1 validation fixture,
  ...
  contribute.
- `agent-lessons-eval-report.json` is the deterministic lesson quality
  report. The builder compiles prompts with and without each
...
- `agent-online-eval-traces.json` is the curated MA-5 production-trace
  sample. Each entry carries a `traceId`, `runId`,
