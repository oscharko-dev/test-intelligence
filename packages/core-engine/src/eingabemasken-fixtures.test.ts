...
void describe("eingabemasken-fixtures (banking and insurance UI input masks)", () => {
  void test("registers exactly fifteen Eingabemaske archetype ids", () => {
  ...
  void test("every fixture has a tier in {1,2,3} and a domain", () => {
    for (const id of EINGABEMASKEN_ARCHETYPE_FIXTURE_IDS) {
      const tier: number = EINGABEMASKEN_FIXTURE_TIERS[id];
      assert.ok(
        tier === 1 || tier === 2 || tier === 3,
        `tier for ${id} must be 1, 2, or 3`,
      );
      const domain: string = EINGABEMASKEN_FIXTURE_DOMAINS[id];
      assert.ok(
        domain === "banking" ||
          domain === "insurance" ||
          domain === "compliance",
  ...
  void test("Issue #2108: every fixture has an applicable-invariants entry, and every entry resolves to a registered invariant id", () => {
    const registry = buildActiveDatasetInvariantRegistry();
    const registeredIds = new Set(registry.ids());
    for (const id of EINGABEMASKEN_ARCHETYPE_FIXTURE_IDS) {
      assert.ok(
        (EINGABEMASKEN_APPLICABLE_INVARIANTS[id] as unknown) !== undefined,
  ...
  void test("Issue #2108: the Eingabemasken benchmark exercises every applicable invariant across all tiers", () => {
  ...
  void test("the suite covers all three tiers and at least banking + insurance", () => {
  ...
  void test("rejects unknown archetype ids loudly", async () => {
  ...
  for (const archetypeId of EINGABEMASKEN_ARCHETYPE_FIXTURE_IDS) {
    void describe(archetypeId, () => {
      void test("loads, derives an IR, and matches the expected summary", async () => {
      ...
      void test("expected.summary.json is byte-stable canonical JSON", async () => {
      ...
      void test("compliance sidecar is well-formed and matches the archetype id", async () => {
        const loaded = await loadEingabemaskenArchetypeFixture(archetypeId);
      ...
      void test("compliance.json is byte-stable canonical JSON", async () => {
      ...
      void test("at-risk fixtures (high audit criticality) declare regulated risk", async () => {
        const loaded = await loadEingabemaskenArchetypeFixture(archetypeId);
        const c = loaded.compliance;
      ...
      void test("contains no raw screenshots and no obvious secrets", async () => {
        const loaded = await loadEingabemaskenArchetypeFixture(archetypeId);
      ...
      void test("default values do not leak realistic-looking PII", async () => {
        const loaded = await loadEingabemaskenArchetypeFixture(archetypeId);
