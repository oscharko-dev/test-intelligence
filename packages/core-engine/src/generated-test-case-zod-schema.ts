/**
 * Zod companion schema for `GeneratedTestCaseList`.
 *
 * The Zod schema is the single source of truth for the JSON Schema emitted
 * to structured-output gateways. It depends one-directionally on
 * `src/contracts/`.
 */
...
/** Stable identifier of the `GeneratedTestCaseList` JSON Schema. */
export const GENERATED_TEST_CASE_LIST_SCHEMA_NAME: string = `test-intelligence-generated-test-case-list-v${SCHEMA_NAME_VERSION_MAJOR}`;

/** Zod schema for a `GeneratedTestCaseList` payload. */
    ...
    testCases: z.array(
      z.strictObject({
        ...
        qualitySignals: z.strictObject({
          ...
          coveredRequirementIds: z.array(z.string()).optional(),
        ...
        audit: z.strictObject({
          ...
          // Optional additive audit field. Aligns the Zod schema with the
...
/**
 * Builds the JSON Schema for `GeneratedTestCaseList` from the Zod source.
 *
 * `$id` is set unconditionally from `GENERATED_TEST_CASE_LIST_SCHEMA_NAME`
 * because `z.toJSONSchema` does not reliably propagate `.meta({ id })` across
 * all Zod versions (confirmed absent in Zod 4.4.3).
 */
...
> => {
  const schema = z.toJSONSchema(generatedTestCaseListZodSchema) as Record<
    ...
    unknown
  >;
  schema["$id"] = GENERATED_TEST_CASE_LIST_SCHEMA_NAME;
  // Remove the `id` key in case a future Zod version starts emitting it.
  delete schema["id"];
  return schema;
};
