...
void test("coverage buckets reflect intent ids covered by accepted cases", () => {
  const cases = [
    buildCase({
      id: "tc-1",
      type: "functional",
...
void test("uncovered ids are sorted deterministically", () => {
...
void test("workflow topology overrides the action universe for action coverage", () => {
  const report = computeCoverageReport({
    jobId: "job-1",
    generatedAt: "2026-04-25T10:00:00.000Z",
    policyProfileId: EU_BANKING_DEFAULT_POLICY_PROFILE_ID,
...
void test("workflow topology field lifecycles define the lifecycle coverage universe", () => {
  const report = computeCoverageReport({
    jobId: "job-1",
    generatedAt: "2026-04-25T10:00:00.000Z",
    policyProfileId: EU_BANKING_DEFAULT_POLICY_PROFILE_ID,
...
void test("static typography and icon actions are excluded from mandatory field coverage totals", () => {
...
void test("semantic helper copy and headings do not inflate field coverage totals", () => {
  const report = computeCoverageReport({
    jobId: "job-1",
    generatedAt: "2026-04-25T10:00:00.000Z",
    policyProfileId: EU_BANKING_DEFAULT_POLICY_PROFILE_ID,
...
void test("acceptance criteria coverage is derived from coverage plan requirements", () => {
  const report = computeCoverageReport({
    jobId: "job-1",
    generatedAt: "2026-04-25T10:00:00.000Z",
    policyProfileId: EU_BANKING_DEFAULT_POLICY_PROFILE_ID,
  ...
  assert.deepEqual(report.acceptanceCriteriaCoverage, {
    total: 2,
    covered: 1,
    uncoveredIds: ["AC-002"],
    ratio: 0.5,
...
void test("rubric score is rounded and clamped", () => {
  const report = computeCoverageReport({
    jobId: "job-1",
    generatedAt: "2026-04-25T10:00:00.000Z",
    policyProfileId: EU_BANKING_DEFAULT_POLICY_PROFILE_ID,
...
void test("rubric score out of range throws", () => {
  assert.throws(
    () =>
