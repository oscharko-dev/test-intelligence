...
// Bounded `.{0,200}` instead of `.*` to avoid the polynomial-ReDoS pattern
// CodeQL flags as `js/polynomial-redos` (PR #93 / Issue #79). The 200-char
// window covers any realistic VAT-formula evidence text; longer inputs are
// truncated rather than scanned with unbounded backtracking.
const VAT_FORMULA_RE =
  /\bvat\b.{0,200}(?:\+|plus|included)|(?:\+|plus|included).{0,200}\bvat\b/i;
...
const caseTouchesFinancingNeed = (
  ...
  for (const screenId of relevantScreenIds) {
    ...
    if (
      arrayFromPartial(
        (screen as Partial<ScreenModel> | undefined)?.elements,
      ).some((element) => FINANCING_NEED_RE.test(element.label))
    ) {
      return true;
    }
  }
  ...
  for (const trace of testCase.figmaTraceRefs) {
    ...
    if (
      ...
      arrayFromPartial((screen as Partial<ScreenModel>).elements).some(
        (element) => FINANCING_NEED_RE.test(element.label),
      )
    ) {
      return screen;
    }
...
const computeVatExcludedFinancingNeedAmount = (
  ...
  ).find((element) => FINANCING_NEED_RE.test(element.label));
