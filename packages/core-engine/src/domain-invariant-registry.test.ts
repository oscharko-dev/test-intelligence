...
void test("registry: rejects malformed invariant ids", () => {
...
void test("registry: refuses duplicate invariant ids", () => {
  const registry = createInvariantRegistry();
...
void test("registry: list() and ids() are alphabetically sorted", () => {
  const registry = createInvariantRegistry();
...
void test("active-dataset registry exposes the documented invariant ids", () => {
...
void test("active-dataset registry has at least 20 invariants for Issue #2108", () => {
  const registry = buildActiveDatasetInvariantRegistry();
  assert.ok(
...
void test("Issue #2108 compliance invariants all carry a non-empty legalSource citation", () => {
  const registry = buildActiveDatasetInvariantRegistry();
...
void test("evaluateInvariants: case touching VAT-financing rule fires INV-VAT-01 violation", () => {
...
void test("evaluateInvariants: brutto/netto exclusivity is rejected on a single string", () => {
...
void test("evaluateInvariants: netto/brutto option-label accessibility text is not a financial-result violation", () => {
  const intent = buildIntentNoFinancing();
  const context = buildContext(intent);
...
void test("evaluateInvariants: netto/brutto focus order with financial field labels is not a result conflation", () => {
  const intent = buildIntentNoFinancing();
  const context = buildContext(intent);
...
void test("evaluateInvariants: optional-cost expectation without selection is a violation", () => {
  const intent = buildIntentNoFinancing();
  const context = buildContext(intent);
  const offending = buildCase({
    expectedResults: [
...
void test("evaluateInvariants: optional-cost selection in preconditions clears the case", () => {
  const intent = buildIntentNoFinancing();
  const context = buildContext(intent);
  const ok = buildCase({
    preconditions: [
...
void test("evaluateInvariants: violations are sorted deterministically", () => {
  const intent = buildIntentWithFinancing();
  const context = buildContext(intent);
  const cases = [
    buildCase({
...
void test("computeInvariantCoverageRatio: empty registry yields zeros", () => {
...
void test("computeInvariantCoverageRatio: rounds to six digits", () => {
  const ratio = computeInvariantCoverageRatio({
    registered: ["a", "b", "c"],
    cases: [],
...
void test("evaluateInvariants: registry with no scope match leaves invariantCoverage at zero", () => {
  const intent = buildIntentNoFinancing();
  const context = buildContext(intent);
  const evaluation = evaluateInvariants({
    registry: buildActiveDatasetInvariantRegistry(),
...
void test("INV-PSD2-SCA-01: high-value payment without 2FA fires; with SCA passes", () => {
...
void test("INV-PSD2-DYNLINK-01: SCA without dynamic linking to amount and payee fires", () => {
  expectInvariantViolation(
    "INV-PSD2-DYNLINK-01",
...
void test("INV-MIFID-SUITAB-01: securities order without suitability fires", () => {
  expectInvariantViolation(
    "INV-MIFID-SUITAB-01",
...
void test("INV-MIFID-APPROP-01: complex product warning required for execution-only", () => {
  expectInvariantViolation(
    "INV-MIFID-APPROP-01",
...
void test("INV-MIFID-COSTS-01: order without ex-ante costs disclosure fires", () => {
  expectInvariantViolation(
    "INV-MIFID-COSTS-01",
...
void test("INV-GWG-PEP-01: high-value transfer without PEP screening fires", () => {
  expectInvariantViolation(
    "INV-GWG-PEP-01",
...
void test("INV-AML-CUMUL-01: AML threshold checked on cumulative amount, not single", () => {
  expectInvariantViolation(
    "INV-AML-CUMUL-01",
...
void test("INV-DORA-ICT-01: outsourced workflow without ICT-third-party flag fires", () => {
  expectInvariantViolation(
    "INV-DORA-ICT-01",
...
void test("INV-GDPR-ART9-01: special-category data without explicit consent fires", () => {
  expectInvariantViolation(
    "INV-GDPR-ART9-01",
...
void test("INV-GDPR-ART15-01: account screen without Auskunftsrecht entry fires", () => {
  expectInvariantViolation(
    "INV-GDPR-ART15-01",
...
void test("INV-IDD-DEMANDS-01: insurance contract without demands-and-needs fires", () => {
  expectInvariantViolation(
    "INV-IDD-DEMANDS-01",
...
void test("INV-SOLV2-COOLOFF-01: long-term contract without cooling-off period fires", () => {
  expectInvariantViolation(
    "INV-SOLV2-COOLOFF-01",
...
void test("INV-FX-MARGIN-01: FX conversion with markup but no disclosure fires", () => {
  expectInvariantViolation(
    "INV-FX-MARGIN-01",
...
void test("INV-KYC-AGE-01: KYC onboarding without age-gate fires", () => {
  expectInvariantViolation(
    "INV-KYC-AGE-01",
...
void test("INV-EAA-KBD-01: payment flow without keyboard-only assertion fires", () => {
  expectInvariantViolation(
    "INV-EAA-KBD-01",
...
void test("INV-VAG-BERATUNG-01: Anlagevermittlung without Beratungsprotokoll fires", () => {
  expectInvariantViolation(
    "INV-VAG-BERATUNG-01",
