...
void test("validation-fixtures: ships at least one onboarding and one payment-style fixture", () => {
  assert.ok((ALL_FIXTURES.length as number) >= 2, "expected at least two validation fixtures");
  assert.ok(ALL_FIXTURES.includes("validation-onboarding"));
  assert.ok(ALL_FIXTURES.includes("validation-payment-auth"));
});

void test("validation-fixtures: isWave1ValidationFixtureId is a precise type guard", () => {
...
void test("validation-fixtures: loadWave1ValidationFixture refuses unknown ids", async () => {
...
for (const fixtureId of ALL_FIXTURES) {
  void test(`validation-fixtures: ${fixtureId} loads with figma + visual payloads`, async () => {
...
void test("validation-fixtures: ships at least one visual image or mask fixture", async () => {
