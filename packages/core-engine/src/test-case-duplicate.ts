...
const coverageTargetIds = (testCase: GeneratedTestCase): Set<string> =>
  new Set([
...
const fieldCoverageTargetIds = (testCase: GeneratedTestCase): Set<string> =>
  new Set(testCase.qualitySignals.coveredFieldIds);

const haveDisjointCoverageTargets = (
  left: GeneratedTestCase,
  right: GeneratedTestCase,
): boolean => {
  const leftFields = fieldCoverageTargetIds(left);
  const rightFields = fieldCoverageTargetIds(right);
  if (leftFields.size > 0 && rightFields.size > 0) {
    const smaller =
      leftFields.size <= rightFields.size ? leftFields : rightFields;
    const larger = smaller === leftFields ? rightFields : leftFields;
    for (const target of smaller) {
      if (larger.has(target)) return false;
    }
    return true;
  }

  const leftTargets = coverageTargetIds(left);
  const rightTargets = coverageTargetIds(right);
  if (leftTargets.size === 0 || rightTargets.size === 0) return false;
  const smaller =
    leftTargets.size <= rightTargets.size ? leftTargets : rightTargets;
  const larger = smaller === leftTargets ? rightTargets : leftTargets;
  for (const target of smaller) {
    if (larger.has(target)) return false;
  }
  return true;
...
export const detectDuplicateTestCases = (input: {
  ...
  for (let i = 0; i < cases.length; i++) {
    ...
    for (let j = i + 1; j < cases.length; j++) {
      ...
      if (haveDisjointCoverageTargets(left, right)) continue;
