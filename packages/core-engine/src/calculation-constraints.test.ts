...
void test("extractCalculationConstraints recognises the German VAT-exclusion phrasing for Issue #2013", () => {
...
void test("extractCalculationConstraints handles English exclusion phrasing without regression", () => {
  const model = buildModel({
    businessRules: [
      {
        ruleId: "rule-vat-en",
...
void test("extractCalculationConstraints sees German openQuestions about the financing-need formula", () => {
  const model = buildModel({
    openQuestions: [
      {
...
void test("buildSourceScopedCalculationAssumptions extracts German VAT exclusion statements", () => {
  const text =
...
void test("detectCalculationConstraintViolation does not treat input amounts as financing-need expectations", () => {
  const model = buildModel({
    screens: [
      {
        screenId: "screen-financing",
  ...
  const testCase = {
    ...
    steps: [
      ...
      },
      {
        ...
        expected:
          "Ein Ergebniswert wird ohne konkret erfundenen Betrag angezeigt.",
      },
    ],
