...
const expectedArchetypesWithCustomMarkdown: ReadonlySet<BaselineArchetypeFixtureId> =
  new Set(["baseline-multi-context"]);
...
void describe("baseline-fixtures (Issue #1762)", () => {
  void test("registers exactly the seven MA-0 archetype ids", () => {
  ...
  for (const archetypeId of BASELINE_ARCHETYPE_FIXTURE_IDS) {
    void describe(archetypeId, () => {
      void test("loads, derives an IR, and matches the expected summary", async () => {
        ...
        assert.ok(summary.notes.length > 0, "summary.notes must not be empty");

        const screenCount = figma.screens.length;
        const nodeCount = figma.screens.reduce((n, s) => n + s.nodes.length, 0);
      ...
      void test("expected.summary.json round-trips through canonicalJson", async () => {
      ...
      void test("multi-context jira.json is canonical-JSON when present", async () => {
        const loaded = await loadBaselineArchetypeFixture(archetypeId);
      ...
      void test("contains no raw screenshots and no obvious secrets", async () => {
        ...
        if (loaded.jiraPath !== undefined) candidatePaths.push(loaded.jiraPath);
  ...
  void test("figma.json files round-trip through JSON.parse without surprises", async () => {
    for (const archetypeId of BASELINE_ARCHETYPE_FIXTURE_IDS) {
