...
void test("computeIntentDelta: identical IRs produce empty entries + zero totals", () => {
...
void test("computeIntentDelta: detects added/removed/changed fields, actions, validations, navigation, screens", () => {
  const prior = baseIr({
    screens: [
      {
        screenId: "screen-a",
...
void test("computeIntentDelta: deterministic sort by (kind, elementId, changeType)", () => {
  const prior = baseIr({
    detectedFields: [
...
void test("computeIntentDelta: visual fixture hash change surfaces as visual_screen changed", () => {
  const ir = baseIr({});
  const delta = computeIntentDelta({
    jobId: "job-4",
    generatedAt: "2026-04-26T00:00:00.000Z",
...
void test("computeIntentDelta: visual confidence drop surfaces as confidence_dropped", () => {
...
void test("computeIntentDelta: visual ambiguity increase surfaces as ambiguity_increased", () => {
...
void test("computeIntentDelta: identical visual sidecars produce zero visual entries", () => {
...
void test("computeIntentDelta: rejects out-of-range confidence drift threshold", () => {
  assert.throws(
    () =>
      computeIntentDelta({
        jobId: "job-8",
...
void test("computeIntentDelta: schemaVersion + contractVersion stamped + invariants false", () => {
  const delta = computeIntentDelta({
    jobId: "job-10",
    generatedAt: "2026-04-26T00:00:00.000Z",
    prior: baseIr({}),
...
void test("computeIntentDelta: only allowed kinds and change types appear in entries", () => {
  const delta = computeIntentDelta({
    jobId: "job-11",
    generatedAt: "2026-04-26T00:00:00.000Z",
...
void test("writeIntentDeltaReport: persists deterministic canonical JSON atomically", async () => {
...
void test("INTENT_DELTA_DEFAULT_CONFIDENCE_DRIFT is a stable in-range constant", () => {
  assert.equal(typeof INTENT_DELTA_DEFAULT_CONFIDENCE_DRIFT, "number");
  assert.ok((INTENT_DELTA_DEFAULT_CONFIDENCE_DRIFT as number) > 0);
  assert.ok((INTENT_DELTA_DEFAULT_CONFIDENCE_DRIFT as number) <= 1);
});
