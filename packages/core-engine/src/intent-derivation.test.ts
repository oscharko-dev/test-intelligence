...
void test("derivation emits fields and first-class interactions for a basic form", () => {
...
void test("derivation models radio, select, and amount inputs as interactions", () => {
...
void test("derivation does not promote raw typography text into input fields", () => {
  const figma: IntentDerivationFigmaInput = {
    source: { kind: "figma_local_json" },
    screens: [
      {
...
void test("derivation is deterministic across two runs with the same input", () => {
  const a = deriveBusinessTestIntentIr({ figma: simpleForm });
  const b = deriveBusinessTestIntentIr({ figma: simpleForm });
  assert.equal(JSON.stringify(a), JSON.stringify(b));
});

void test("derivation source hash ignores volatile visual sidecar metadata", () => {
  const visualBase = {
    screenId: "screen-login",
...
void test("derivation output is stable when node order is shuffled", () => {
...
void test("derivation degrades gracefully when optional fields are missing", () => {
...
void test("derivation redacts PII in default values and records indicators", () => {
...
void test("derivation produces a trace.nodeId for every detected element", () => {
  const ir = deriveBusinessTestIntentIr({ figma: simpleForm });
