...
void test("new case (id absent in prior) gets `new` verdict + absent_in_prior reason", () => {
...
void test("identical case (same fingerprint, no IR delta touch) gets `unchanged`", () => {
...
void test("fingerprint change yields `changed` + fingerprint_changed reason", () => {
...
void test("trace screen changed in IR delta yields `changed` even with stable fingerprint", () => {
  const tc = buildCase({ id: "tc-1" });
...
void test("prior case whose every trace screen is absent from current IR is `obsolete` (NOT removed from QC)", () => {
...
void test("low visual confidence escalates `unchanged` to `requires_review`", () => {
  const tc = buildCase({ id: "tc-1" });
...
void test("visual conflict outcome escalates to `requires_review` with reconciliation_conflict", () => {
  const tc = buildCase({ id: "tc-1" });
  const visual: VisualSidecarValidationReport = {
    schemaVersion: VISUAL_SIDECAR_VALIDATION_REPORT_SCHEMA_VERSION,
    contractVersion: TEST_INTELLIGENCE_CONTRACT_VERSION,
...
void test("obsolete verdict is NOT downgraded by visual escalation (obsolete dominates)", () => {
  const prior = buildCase({
    id: "tc-1",
    figmaTraceRefs: [{ screenId: "screen-removed" }],
  });
...
void test("rows are sorted by testCaseId for deterministic output", () => {
  const out = classifyTestCaseDelta({
    jobId: "job-1",
    generatedAt: "2026-04-26T00:00:00.000Z",
    prior: list([]),
...
void test("visualConfidenceThreshold range guard rejects invalid values", () => {
  assert.throws(
    () =>
      classifyTestCaseDelta({
        jobId: "job-1",
...
void test("default visual confidence floor is in (0, 1]", () => {
  assert.ok((TEST_CASE_DELTA_DEFAULT_VISUAL_CONFIDENCE_FLOOR as number) > 0);
  assert.ok((TEST_CASE_DELTA_DEFAULT_VISUAL_CONFIDENCE_FLOOR as number) <= 1);
});

void test("invariant flags are stamped at runtime", () => {
  const out = classifyTestCaseDelta({
    jobId: "job-1",
    generatedAt: "2026-04-26T00:00:00.000Z",
    prior: list([]),
...
void test("schemaVersion is TEST_CASE_DELTA_REPORT_SCHEMA_VERSION", () => {
  const out = classifyTestCaseDelta({
    jobId: "job-1",
    generatedAt: "2026-04-26T00:00:00.000Z",
    prior: list([]),
...
void test("absent_in_current fires when case is dropped from generation but screens still exist", () => {
...
void test("absent_in_current does NOT fire for genuinely obsolete case (every trace screen removed)", () => {
  const prior = buildCase({
    id: "tc-1",
    figmaTraceRefs: [{ screenId: "screen-removed" }],
  });
...
void test("visual_ambiguity_increased fires when intent delta records ambiguity growth on a trace screen", () => {
  const tc = buildCase({
    id: "tc-1",
    figmaTraceRefs: [{ screenId: "screen-a" }],
  });
...
void test("visual_ambiguity_increased does NOT fire when ambiguity delta is on an unrelated screen", () => {
  const tc = buildCase({
    id: "tc-1",
    figmaTraceRefs: [{ screenId: "screen-a" }],
  });
...
void test("visual_ambiguity_increased does NOT fire when no ambiguity_increased entries in delta", () => {
  const tc = buildCase({
    id: "tc-1",
    figmaTraceRefs: [{ screenId: "screen-a" }],
  });
...
void test("writeTestCaseDeltaReport persists deterministic canonical JSON atomically", async () => {
