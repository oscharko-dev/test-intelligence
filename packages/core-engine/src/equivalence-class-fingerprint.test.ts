...
void test("Issue #2123: fingerprint sorts and de-duplicates covered ids", () => {
...
void test("Issue #2123: equivalenceClassKey is byte-stable across id ordering", () => {
...
void test("Issue #2123: oracle polarity collapses validation onto negative", () => {
...
void test("Issue #2123: redundant case in same class with no distinct coverage is flagged", () => {
...
void test("Issue #2123: identical text with different action subset is NOT redundant", () => {
  const a = buildCase({
    id: "tc-a",
    qualitySignals: {
      coveredFieldIds: ["fld"],
...
void test("Issue #2123: same class but different state path is NOT redundant", () => {
  const a = buildCase({
    id: "tc-a",
    figmaTraceRefs: [{ screenId: "s-1", nodePath: "root/form" }],
  });
...
void test("Issue #2123: same class but different oracle category is NOT redundant", () => {
...
void test("Issue #2123: boundary classifier can VETO a deterministic redundancy verdict", () => {
...
void test("Issue #2123: boundary classifier 'redundant' verdict records source=deterministic+classifier", () => {
  const classifier: IntraClassBoundaryClassifier = {
    identifier: "phi-4-mini-instruct@stub",
    classify: () => "redundant",
  };
...
void test("Issue #2123: classifier is NOT consulted when deterministic logic keeps the case", () => {
  let invoked = 0;
  const classifier: IntraClassBoundaryClassifier = {
    identifier: "phi-4-mini-instruct@stub",
    classify: () => {
...
void test("Issue #2123: A/B/A pattern marks the third case redundant when it collapses back onto an earlier kept case", () => {
  const a = buildCase({
    id: "tc-a",
    figmaTraceRefs: [{ screenId: "s-1", nodePath: "root/form" }],
  });
...
void test("Issue #2123: empty input yields zero ratio and no findings", () => {
...
void test("Issue #2123: single-case input never produces a finding", () => {
  const result = detectIntraClassRedundancy({
...
void test("Issue #2123: levenshteinCapped short-circuits beyond cap", () => {
...
void test("Issue #2123: detectExactNearDuplicateText flags Levenshtein-2 pairs", () => {
  const a = buildCase({
    id: "tc-a",
  ...
  assert.ok(findings[0].characterDistance <= 2);
});

void test("Issue #2123: detectExactNearDuplicateText respects the configured budget", () => {
...
void test("Issue #2123: redundancy findings are sorted deterministically", () => {
