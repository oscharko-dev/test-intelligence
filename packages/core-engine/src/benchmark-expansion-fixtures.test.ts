...
void describe("benchmark-expansion-fixtures (Issue #2115)", () => {
  void test("registers at least 28 expansion fixture ids and they round-trip the type guard", () => {
    assert.ok(
      (BENCHMARK_EXPANSION_FIXTURE_IDS.length as number) >= 28,
  ...
  void test("the corpus 7 + 15 + N >= 50 once the expansion lands", () => {
  ...
  void test("every per-stratum minimum from the sample plan ADR is satisfied", () => {
  ...
  void test("the adversarial subset has at least 10 fixtures across at least three adversarial kinds", async () => {
  ...
  void test("locale coverage spans DE, EN, FR, IT (Issue #2118 calibration)", async () => {
  ...
  for (const archetypeId of BENCHMARK_EXPANSION_FIXTURE_IDS) {
    void describe(archetypeId, () => {
      ...
      void test("high-criticality fixtures do not downgrade regulated risk to low", async () => {
        const loaded = await loadBenchmarkExpansionFixture(archetypeId);
        const c = loaded.compliance;
      ...
      void test("adversarial fixtures declare a non-empty adversarialKind", async () => {
        const loaded = await loadBenchmarkExpansionFixture(archetypeId);
        if (loaded.adversarial) {
          assert.ok(
