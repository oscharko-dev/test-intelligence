...
void test("prompt-injection figma fixture: derivation treats injection text as node data, not directives", async () => {
...
void test("prompt-injection figma fixture: compiled systemPrompt never contains injection text verbatim", async () => {
  // Arrange
  const raw = await readFile(
    join(FIXTURES_DIR, "adversarial-prompt-injection.figma.json"),
    "utf8",
...
void test("PII figma fixture: derivation detects email, PAN, phone, and populates piiIndicators", async () => {
  // Arrange
  const raw = await readFile(
...
void test("PII figma fixture: compiled userPrompt does not contain raw PII values", async () => {
  // Arrange
  const raw = await readFile(
    join(FIXTURES_DIR, "adversarial-pii.figma.json"),
    "utf8",
...
void test("visual sidecar adversarial fixture: validateVisualSidecar detects prompt_injection_like_text", async () => {
...
void test("visual sidecar adversarial fixture: blocked validation propagates to job-level error via runPolicyGate", async () => {
