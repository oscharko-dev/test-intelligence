...
void test("cosineSimilarity: identical vectors → 1", () => {
  assert.equal(cosineSimilarity([1, 0, 0], [1, 0, 0]), 1);
});

void test("cosineSimilarity: orthogonal vectors → 0", () => {
  assert.equal(cosineSimilarity([1, 0], [0, 1]), 0);
});

void test("cosineSimilarity: rejects mismatched lengths and non-finite components", () => {
...
void test("cosineSimilarity: zero-magnitude vector → 0", () => {
  assert.equal(cosineSimilarity([0, 0], [1, 0]), 0);
});

void test("air-gapped (no provider, no probe) → lexical-only path still flags duplicates", async () => {
...
void test("air-gapped: completely distinct cases → no duplicates", async () => {
  const a = buildCase({
    id: "tc-a",
    title: "abcde",
    riskCategory: "low",
...
void test("embedding provider: similar vectors above threshold flagged", async () => {
...
void test("embedding provider: orthogonal vectors below threshold → not flagged", async () => {
  const a = buildCase({ id: "tc-a", title: "Title A" });
  const b = buildCase({ id: "tc-b", title: "Title B" });
  const provider: EmbeddingProvider = {
...
void test("embedding provider: inconsistent vector dimensions throw", async () => {
...
void test("embedding provider: empty vector throws", async () => {
  const a = buildCase({ id: "tc-a" });
  const provider: EmbeddingProvider = {
    identifier: "empty",
...
void test("external probe disabled by default → state=disabled, cases=0", async () => {
  const out = await detectTestCaseDuplicatesExtended({
    jobId: "job-1",
    generatedAt: "2026-04-26T00:00:00.000Z",
...
void test("external probe configured & found → external_lookup finding surfaces", async () => {
...
void test("external probe unavailable on every case → state=unconfigured (fail-closed)", async () => {
...
void test("external probe disabled sentinel → state=executed, no findings", async () => {
...
void test("external probe throws on a case → partial_failure with sanitised note", async () => {
  const probe: ExternalDedupeProbe = {
...
void test("invariants are stamped + thresholds out of range throw", async () => {
  await assert.rejects(
    () =>
      detectTestCaseDuplicatesExtended({
        jobId: "job-1",
...
void test("schemaVersion + invariants set + sorted findings", async () => {
  const out = await detectTestCaseDuplicatesExtended({
    jobId: "job-1",
    generatedAt: "2026-04-26T00:00:00.000Z",
    testCases: [],
...
void test("perCase rows are sorted by testCaseId for determinism", async () => {
  const out = await detectTestCaseDuplicatesExtended({
    jobId: "job-1",
    generatedAt: "2026-04-26T00:00:00.000Z",
    testCases: [
...
void test("writeTestCaseDedupeReport persists deterministic canonical JSON atomically", async () => {
