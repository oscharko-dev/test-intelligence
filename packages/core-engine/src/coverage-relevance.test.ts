...
void test("coverage-relevance > filters single angle-bracket placeholders such as <Radio>", () => {
...
void test("coverage-relevance > filters compound structural placeholders that mix bracketed components", () => {
  assert.equal(
    isCoverageRelevantElementLike({
...
void test("coverage-relevance > filters standalone unit and currency labels", () => {
  assert.equal(isCoverageRelevantElementLike({ label: "EUR" }), false);
  assert.equal(isCoverageRelevantElementLike({ label: "€" }), false);
});

void test("coverage-relevance > filters value-only labels including the standard EUR/percent suffixes", () => {
...
void test("coverage-relevance > filters known decorative labels including Text and (optional)", () => {
...
void test("coverage-relevance > filters decorative kinds even when the label is non-empty", () => {
  assert.equal(
...
void test("coverage-relevance > keeps meaningful business labels intact", () => {
  assert.equal(
    isCoverageRelevantElementLike({
...
void test("coverage-relevance > distinguishes interactive controls from static typography", () => {
  assert.equal(
    isInteractiveCoverageElementLike({
      label: "Höhe des Kaufpreises (Netto)",
      kind: "text_input",
...
void test("coverage-relevance > filters bracketed placeholder action labels", () => {
  assert.equal(isCoverageRelevantActionLike({ label: "<Radio>" }), false);
});

void test("coverage-relevance > keeps meaningful actions", () => {
  assert.equal(
    isCoverageRelevantActionLike({
      label: "Berechnen",
      kind: "submit",
...
void test("coverage-relevance > normalizeCoverageText trims, collapses whitespace, and lowercases", () => {
