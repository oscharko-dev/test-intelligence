 ...
 * visual-evidence test-intelligence run is expected to handle. Each archetype
 ...
 * this baseline.
 *
 * On-disk layout under `packages/core-engine/src/fixtures/`:
 *
...
const tryReadOptional = async (path: string): Promise<string | undefined> => {
  try {
