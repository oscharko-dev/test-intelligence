...
void test("identical fingerprints have similarity 1", () => {
...
void test("disjoint fingerprints have similarity 0", () => {
...
void test("near-duplicate cases above threshold are reported deterministically", () => {
  const a = buildCase({
    id: "tc-aa",
  ...
  assert.ok(pairs[0].similarity >= 0.7);
});

void test("near-duplicate cases with disjoint coverage targets are not reported", () => {
  const a = buildCase({
    id: "tc-aa",
    title: "Enter valid field value",
    steps: [{ index: 1, action: "Enter a valid value", expected: "Accepted" }],
    qualitySignals: {
      coveredFieldIds: ["field-a"],
      coveredActionIds: ["shared-action"],
      coveredValidationIds: [],
      coveredNavigationIds: [],
      confidence: 0.9,
    },
...
void test("threshold validation rejects out-of-range values", () => {
  assert.throws(
...
void test("pairs are emitted in lexical id order regardless of input order", () => {
