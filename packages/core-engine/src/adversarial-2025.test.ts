...
import { normalizeUntrustedContent } from "@oscharko-dev/ti-production-runner";

const FIXTURES_DIR = new URL("./fixtures/adversarial-2025", import.meta.url)
  .pathname;
...
void test("adversarial-2025: figma-hidden-layer-injection drops the visible=false child and emits a risk signal", async () => {
...
void test("adversarial-2025: figma-zero-opacity-injection drops the opacity=0 child and emits a risk signal", async () => {
  const document = await readFixtureJson<unknown>(
...
void test("adversarial-2025: figma-off-canvas-injection drops the out-of-parent child and emits a risk signal", async () => {
  const document = await readFixtureJson<unknown>(
...
void test("adversarial-2025: figma-fontsize-zero-injection drops the zero-font-size child and emits a risk signal", async () => {
  const document = await readFixtureJson<unknown>(
...
void test("adversarial-2025: jira-adf-collapsed-node-injection rejects the document and yields empty plain text", async () => {
...
void test("adversarial-2025: custom-zero-width-unicode-injection emits zero-width + injection signals and routes to needs_review", async () => {
...
void test("adversarial-2025: repair-thought-injection-forged-validator-line emits a critical injection signal and routes to needs_review", async () => {
...
void test("adversarial-2025: normalization is byte-stable across repeated runs", async () => {
  const document = await readFixtureJson<unknown>(
    "figma-hidden-layer-injection.json",
  );
