...
void test("classifier table is exhaustive over the canonical 6-state lifecycle", () => {
  ...
  for (const from of states) {
    for (const to of states) {
      const tier: string = classifyFieldLifecycleTransitionPair(from, to);
      assert.ok(
...
void test("classifier matches the issue-spec mandatory_negative_path subset", () => {
...
void test("classifier matches the issue-spec recommended_positive_path subset", () => {
...
void test("Epic #2167 P0 scenario — `initial → validated|error|terminal` skip-state edges are NOT mandatory", () => {
...
void test("classifier marks every outgoing edge from `terminal` as state_transition_test_only", () => {
...
void test("classifyFieldLifecycleTransition matches classifyFieldLifecycleTransitionPair", () => {
...
void test("Issue #2168 property: only mandatory-tier transitions can produce severity:error", () => {
  fc.assert(
    fc.property(
      fc
        .integer({ min: 1, max: 4 })
...
void test("Issue #2168 property: covering every mandatory transition keeps the run unblocked", () => {
  fc.assert(
    fc.property(
      fc
        .integer({ min: 1, max: 3 })
