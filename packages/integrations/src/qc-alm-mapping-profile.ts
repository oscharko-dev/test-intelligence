...
const validateBaseUrlAlias = (alias: unknown): QcMappingProfileIssue[] => {
  ...
  if (startsWithUrlScheme(alias) || containsUserInfoLikeSegment(alias)) {
    return [
      issue(
        "/baseUrlAlias",
...
const startsWithUrlScheme = (value: string): boolean => {
...
const containsUserInfoLikeSegment = (value: string): boolean => {
  ...
  for (const ch of value) {
    ...
    if (ch === ":") segmentHasColon = true;
    if (ch === "@" && segmentHasColon) return true;
  }
  return false;
};
