...
void test("property: every builtin descriptor has six boolean capability flags", () => {
...
void test("property: every builtin descriptor has non-empty label and semver version", () => {
  for (const descriptor of BUILTIN_QC_PROVIDER_DESCRIPTORS) {
    assert.ok(
...
void test("property: descriptor.provider is in ALLOWED_QC_ADAPTER_PROVIDERS", () => {
...
void test("property: adapter presence matches capabilities.dryRun, except for custom slot", () => {
...
void test("property: getQcProviderDescriptor returns an independent copy", () => {
  const registry = createQcProviderRegistry();
  fc.assert(
    fc.property(providerArb, (provider) => {
...
void test("property: only the custom slot has registerCustom=true", () => {
  for (const descriptor of BUILTIN_QC_PROVIDER_DESCRIPTORS) {
...
void test("property: opentext_alm is the only provider with apiTransfer=true", () => {
  for (const descriptor of BUILTIN_QC_PROVIDER_DESCRIPTORS) {
