 ...
 *     `TEST_INTELLIGENCE_TMS_<NAME>_TOKEN` (PAT/bearer) or
 *     `TEST_INTELLIGENCE_TMS_<NAME>_OAUTH_ACCESS_TOKEN` (OAuth).
 ...
 * `TEST_INTELLIGENCE_TMS_<NAME>_TOKEN` for PAT/bearer and
 * `TEST_INTELLIGENCE_TMS_<NAME>_OAUTH_ACCESS_TOKEN` for OAuth 2.0.
 */
 ...
 *   1. `TEST_INTELLIGENCE_TMS_<NAME>_OAUTH_ACCESS_TOKEN` → `oauth2`
 *   2. `TEST_INTELLIGENCE_TMS_<NAME>_TOKEN`              → `pat`
 *   3. `TEST_INTELLIGENCE_TMS_<NAME>_BEARER`             → `bearer`
 */
export const loadTmsCredentialsFromEnv = (
  ...
  const oauthName = `TEST_INTELLIGENCE_TMS_${envSuffix}_OAUTH_ACCESS_TOKEN`;
  const patName = `TEST_INTELLIGENCE_TMS_${envSuffix}_TOKEN`;
  const bearerName = `TEST_INTELLIGENCE_TMS_${envSuffix}_BEARER`;

  const oauth = nonEmptyEnv(input.env, oauthName);
  if (oauth !== undefined) {
    const refreshName = `TEST_INTELLIGENCE_TMS_${envSuffix}_OAUTH_REFRESH_TOKEN`;
