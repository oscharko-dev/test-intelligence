...
void test("xray-adapter: connect probes /rest/api/3/myself", async () => {
...
void test("xray-adapter: validateProject ok on 200", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse({ status: 200, headers: {}, body: { accountId: "1" } });
...
void test("xray-adapter: validateProject project_not_found on 404", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse({ status: 200, headers: {}, body: { accountId: "1" } });
  fake.pushResponse({
    status: 404,
...
void test("xray-adapter: mapTestCase produces deterministic idempotency key", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse({ status: 200, headers: {}, body: { accountId: "1" } });
  const adapter = createXrayAdapter({ http: fake.client, sleep: async () => {} });
  const session = await adapter.connect({
...
void test("xray-adapter: pushTestCase records pushed verdict on 201", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse({ status: 200, headers: {}, body: { accountId: "1" } });
  fake.pushResponse({
    status: 201,
...
void test("xray-adapter: pushTestCase dryRun never issues request", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse({ status: 200, headers: {}, body: { accountId: "1" } });
  const adapter = createXrayAdapter({ http: fake.client, sleep: async () => {} });
  const session = await adapter.connect({
...
void test("xray-adapter: pushTestCase records skipped-dup on dedup response", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse({ status: 200, headers: {}, body: { accountId: "1" } });
  fake.pushResponse({
    status: 200,
...
void test("xray-adapter: pushTestCase records failed verdict on 422", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse({ status: 200, headers: {}, body: { accountId: "1" } });
  fake.pushResponse({
    status: 422,
...
void test("xray-adapter: pollSyncStatus returns issue state on 200", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse({ status: 200, headers: {}, body: { accountId: "1" } });
  fake.pushResponse({
    status: 200,
...
void test("xray-adapter: pullExecutions returns parsed envelope rows", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse({ status: 200, headers: {}, body: { accountId: "1" } });
  fake.pushResponse({
    status: 200,
...
void test("xray-adapter: pullExecutions rejects rows with foreign tenantId", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse({ status: 200, headers: {}, body: { accountId: "1" } });
  fake.pushResponse({
    status: 200,
...
void test("xray-adapter: disconnect releases session credentials", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse({ status: 200, headers: {}, body: { accountId: "1" } });
  const adapter = createXrayAdapter({ http: fake.client, sleep: async () => {} });
  const session = await adapter.connect({
