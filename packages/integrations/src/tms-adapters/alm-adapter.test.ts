...
void test("alm-adapter: connect performs LWSSO + QCSession handshake", async () => {
...
void test("alm-adapter: connect refuses malformed project id", async () => {
  const fake = buildFakeHttp();
  const adapter = createAlmAdapter({ http: fake.client, sleep: async () => {} });
...
void test("alm-adapter: pushTestCase lookup-then-create on miss", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse(lwssoResp());
  fake.pushResponse(sessionResp());
...
void test("alm-adapter: pushTestCase short-circuits to skipped-dup on lookup hit", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse(lwssoResp());
  fake.pushResponse(sessionResp());
  fake.pushResponse({
...
void test("alm-adapter: disconnect issues sign-out best-effort", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse(lwssoResp());
  fake.pushResponse(sessionResp());
