 ...
 * Endpoint resolution:
 *
 *   1. `TEST_INTELLIGENCE_TMS_<NAME>_<ALIAS>_BASE_URL` — alias-specific.
 *   2. `TEST_INTELLIGENCE_TMS_<NAME>_BASE_URL`         — adapter default.
 *
...
import { type TmsAdapterId } from "@oscharko-dev/ti-contracts";
  ...
  return {
    async request(req: TmsHttpRequest): Promise<TmsHttpResponse> {
      ...
      try {
        ...
        clearTimeout(timer);
        const detail = err instanceof Error ? err.message : "transport failure";
...
const resolveBaseUrl = (input: {
  ...
  const aliasUpper = input.endpointAlias
    ...
    .toUpperCase();
  const aliasName = `TEST_INTELLIGENCE_TMS_${suffix}_${aliasUpper}_BASE_URL`;
  const defaultName = `TEST_INTELLIGENCE_TMS_${suffix}_BASE_URL`;
