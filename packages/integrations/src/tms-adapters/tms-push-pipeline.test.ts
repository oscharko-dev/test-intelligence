...
const buildFakeAdapter = (overrides: {
  ...
  async pullExecutions(): Promise<TmsPullExecutionsResult> {
    return { evidence: [] };
  },
...
void test("tms-push-pipeline: refused on missing mapping preview", async () => {
...
void test("tms-push-pipeline: refused on empty preview", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "tms-pipeline-"));
  try {
...
void test("tms-push-pipeline: pushed entries sorted by testCaseId, counts correct", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "tms-pipeline-"));
  try {
    await writePreview(
      tempDir,
...
void test("tms-push-pipeline: validateProject failure records project_validation_failed", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "tms-pipeline-"));
  try {
    ...
    assert.equal(result.report.refused, true);
    assert.deepEqual(result.report.refusalCodes, ["project_validation_failed"]);
    assert.equal(result.report.failedCount, 1);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
...
void test("tms-push-pipeline: dryRun reports skipped-dup verdicts", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "tms-pipeline-"));
  try {
    await writePreview(tempDir, buildPreview([{ testCaseId: "tc-1" }]));
    const adapter = buildFakeAdapter({
