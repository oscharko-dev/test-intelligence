...
void test("qtest-adapter: connect probes /api/v3/users/current", async () => {
...
void test("qtest-adapter: pushTestCase pushes and records numeric id", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse({ status: 200, headers: {}, body: { id: 1 } });
...
void test("qtest-adapter: pushTestCase 409 → skipped-dup", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse({ status: 200, headers: {}, body: { id: 1 } });
  fake.pushResponse({
    status: 409,
...
void test("qtest-adapter: validateProject 404 returns project_not_found", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse({ status: 200, headers: {}, body: { id: 1 } });
  fake.pushResponse({
    status: 404,
...
void test("qtest-adapter: pollSyncStatus returns approve_status", async () => {
  const fake = buildFakeHttp();
  fake.pushResponse({ status: 200, headers: {}, body: { id: 1 } });
  fake.pushResponse({
    status: 200,
