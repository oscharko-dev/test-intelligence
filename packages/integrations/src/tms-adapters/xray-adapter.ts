...
export const createXrayAdapter = (
  ...
  return {
    ...
    async pullExecutions(args: {
      ...
      const evidence: TmsRawExecutionEvidence[] = parsed.evidence.map(
        (row) => ({
          ...row,
        }),
      );
    ...
    const tmsTestCaseId =
      typeof issueKey === "string" && issueKey.length > 0
        ...
        : typeof issueId === "string"
          ? issueId
          : "";
    const dedupedFlag = readJsonField<unknown>(
      value.body,
      "deduplicated",
      false,
    );
...
const readJsonField = <T>(source: unknown, field: string, fallback: T): T => {
  if (
