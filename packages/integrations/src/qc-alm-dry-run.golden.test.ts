...
void test("golden: dry-run adapter emits byte-identical dry-run-report.json", async () => {
  ...
  assert.equal(
    serialized,
    expected,
    `golden ${FIXTURE_NAME} drifted — re-run with TEST_INTELLIGENCE_GOLDEN_APPROVE=1`,
  );
});
