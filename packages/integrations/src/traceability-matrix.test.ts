...
void test("buildTraceabilityMatrix: minimal export-only mode joins Figma → IR → coverage", () => {
  ...
  assert.ok(step0);
  assert.equal(step0.stepIndex, 1);
  assert.deepEqual(step0.figmaScreenIds, ["screen-a"]);
  assert.deepEqual(step0.figmaNodeIds, ["node-1"]);
  assert.equal(row.reconciliationDecisions.length, 1);
  const decision0 = row.reconciliationDecisions[0];
  assert.ok(decision0);
  assert.equal(decision0.elementId, "screen-a::field::node-1");
  assert.equal(decision0.provenance, "figma_node");
  assert.equal(row.transferOutcome, undefined);
  assert.equal(matrix.totals.rows, 1);
  assert.equal(matrix.totals.transferred, 0);
});

void test("buildTraceabilityMatrix: with qcMapping populates externalIdCandidate + qcFolderPath", () => {
  ...
  const step0 = row.steps[0];
  assert.ok(step0);
  assert.equal(step0.qcDesignStepIndex, 1);
});

void test("buildTraceabilityMatrix: transfer-aware mode populates qcEntityId + transferOutcome", () => {
...
void test("buildTraceabilityMatrix: visual sidecar observations attached only for matching screens", () => {
  ...
  const obs0 = row.visualObservations[0];
  assert.ok(obs0);
  assert.equal(obs0.screenId, "screen-a");
  assert.equal(obs0.meanConfidence, 0.92);
  const step0 = row.steps[0];
  assert.ok(step0);
  assert.equal(step0.visualObservations.length, 1);
  const stepObs0 = step0.visualObservations[0];
  assert.ok(stepObs0);
  assert.equal(stepObs0.screenId, "screen-a");
});

void test("buildTraceabilityMatrix: validation report aggregates to per-case error/warning/ok", () => {
...
void test("buildTraceabilityMatrix: policy report populates policyDecision + sorted policyOutcomes", () => {
...
void test("buildTraceabilityMatrix: review snapshot populates reviewState", () => {
...
void test("buildTraceabilityMatrix: rows sorted by testCaseId for determinism", () => {
  const matrix = buildTraceabilityMatrix({
    jobId: "job-1",
    generatedAt: "2026-04-26T00:00:00.000Z",
    intent: intentWithFields(),
...
void test("buildTraceabilityMatrix: exportProfile + policyProfile passed through", () => {
  const matrix = buildTraceabilityMatrix({
    jobId: "job-1",
    generatedAt: "2026-04-26T00:00:00.000Z",
    intent: intentWithFields(),
...
void test("buildTraceabilityMatrix: ambiguity strings are sanitised (whitespace collapsed + clipped)", () => {
...
void test("buildTraceabilityMatrix: unknown coverage ids are dropped (no orphan reconciliation rows)", () => {
  const matrix = buildTraceabilityMatrix({
    jobId: "job-1",
    generatedAt: "2026-04-26T00:00:00.000Z",
    intent: intentWithFields(),
...
void test("writeTraceabilityMatrix persists deterministic canonical JSON atomically", async () => {
...
void test("QC_CREATED_ENTITIES_SCHEMA_VERSION reachable (sanity link)", () => {
...
void test("buildTraceabilityMatrix: 50×50 correctness — each test case resolves the right QC entry and transfer record", () => {
  const N = 50;
