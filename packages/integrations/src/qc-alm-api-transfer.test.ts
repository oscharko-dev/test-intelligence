...
type BaseInputOverrides = {
  [K in keyof RunOpenTextAlmApiTransferInput]?:
    | RunOpenTextAlmApiTransferInput[K]
    | undefined;
};
...
const baseInput = (
  ...
  const dryRun = Object.hasOwn(overrides, "dryRun")
    ? overrides.dryRun
    : buildDryRun(preview);
  const defaults: RunOpenTextAlmApiTransferInput = {
    jobId: JOB_ID,
    generatedAt: GENERATED_AT,
    profile: PROFILE,
    preview,
  ...
  if (dryRun !== undefined) {
    defaults.dryRun = dryRun;
  }
  // Overriding with `undefined` must clear the key on the merged result so
  // the transfer's "missing" detection (which checks `value === undefined`)
  // still fires. Use a Record view for assignment + delete to stay sound
  // under exactOptionalPropertyTypes without losing required-key checking
  // on `defaults`.
  ...
  return merged as unknown as RunOpenTextAlmApiTransferInput;
};

void test("api-transfer: contract enums round-trip the public membership", () => {
...
void test("api-transfer: feature gate disabled fails closed without any client call", async () => {
...
void test("api-transfer: admin gate disabled fails closed", async () => {
  const { client, log } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
...
void test("api-transfer: bearer token missing fails closed (token never configured)", async () => {
  const { client, log } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
    baseInput({
...
void test("api-transfer: bearer token mismatch fails closed (forged caller token)", async () => {
  const { client, log } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
    baseInput({
...
void test("api-transfer: timing-safe bearer compare matches configured token", async () => {
  const { client } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
    baseInput({
      configuredBearerToken: "secret-bearer",
...
void test("api-transfer: principal-bound credentials surface the matching principalId in audit", async () => {
  const { client } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
    baseInput({
      configuredBearerToken: undefined,
...
void test("api-transfer: missing dry-run report fails closed (every gate aggregated)", async () => {
  const { client } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
...
void test("api-transfer: refused dry-run report fails closed", async () => {
  const { client } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
    baseInput({
...
void test("api-transfer: dry-run profile mismatch fails closed", async () => {
  const { client } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
    baseInput({
      dryRun: buildDryRun(undefined, {
...
void test("api-transfer: provider mismatch on profile fails closed", async () => {
  const { client } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
    baseInput({
...
void test("api-transfer: invalid mapping profile fails closed before folder resolution", async () => {
  const { client, log } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
    baseInput({
      profile: {
...
void test("api-transfer: invalid per-entry target folder fails closed before resolution", async () => {
...
void test("api-transfer: out-of-root per-entry target folder fails closed before resolution", async () => {
  const preview = buildPreview([buildCase({})]);
  preview.entries[0] = {
    ...preview.entries[0]!,
...
void test("api-transfer: dot-only per-entry target folder segments fail closed before resolution", async () => {
...
void test("api-transfer: dry-run payload mismatch fails closed before resolution", async () => {
  const preview = buildPreview([buildCase({})]);
...
void test("api-transfer: empty preview fails closed (no_mapped_test_cases)", async () => {
  const { client } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
    baseInput({
...
void test("api-transfer: needs_review case fails closed (unapproved_test_cases_present)", async () => {
  const { client, log } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
    baseInput({
...
void test("api-transfer: policy-blocked case fails closed", async () => {
  const { client } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
    baseInput({
      reviewSnapshot: buildSnapshot([
...
void test("api-transfer: four-eyes pending case fails closed", async () => {
  const { client, log } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
    baseInput({
      reviewSnapshot: buildSnapshot([
...
void test("api-transfer: review_state_inconsistent — preview entry has no snapshot row", async () => {
  const { client } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
    baseInput({
      reviewSnapshot: buildSnapshot([]),
...
void test("api-transfer: forged review state with policy_decision=approved but state=rejected → fails closed", async () => {
...
void test("api-transfer: happy path — exported case is created with deterministic step ordering", async () => {
  const { client, log } = buildMockClient();
...
void test("api-transfer: resolves distinct target folders before deterministic transfer", async () => {
...
void test("api-transfer: idempotent re-run skips duplicate entities and never creates", async () => {
  const preview = buildPreview([buildCase({})]);
  ...
  assert.equal(result.refused, false);
  const record0 = result.report.records[0];
  assert.ok(record0);
  assert.equal(record0.outcome, "skipped_duplicate");
  assert.equal(record0.qcEntityId, "qc-already-existing");
...
void test("api-transfer: per-case create failure isolates and continues other cases", async () => {
  const cases = [buildCase({ id: "tc-1" }), buildCase({ id: "tc-2" })];
  const preview = buildPreview(cases);
...
void test("api-transfer: resumes design-step loop when tenant has incomplete entity", async () => {
  ...
  const record0 = result.report.records[0];
  assert.ok(record0);
  assert.equal(record0.outcome, "created");
  assert.equal(record0.qcEntityId, "qc-already-existing");
  assert.equal(record0.designStepsCreated, totalSteps - alreadyPersisted);
...
void test("api-transfer: lookup with matching design-step count short-circuits to skipped_duplicate", async () => {
  const preview = buildPreview([buildCase({ id: "tc-1" })]);
  const externalId = preview.entries[0]?.externalIdCandidate ?? "";
  const totalSteps = preview.entries[0]?.designSteps.length ?? 0;
  const { client, log } = buildMockClient({
...
void test("api-transfer: legacy lookup omitting existingDesignStepCount preserves skipped_duplicate fast path", async () => {
  const preview = buildPreview([buildCase({ id: "tc-1" })]);
  const externalId = preview.entries[0]?.externalIdCandidate ?? "";
  const { client, log } = buildMockClient({
    preexistingByExternalId: { [externalId]: "qc-already-existing" },
...
void test("api-transfer: per-step failure records partial designStepsCreated count", async () => {
  const { client } = buildMockClient({
  ...
  const record0 = result.report.records[0];
  assert.ok(record0);
  assert.equal(record0.outcome, "failed");
  assert.equal(record0.designStepsCreated, 1);
  assert.equal(record0.failureClass, "server_error");
...
void test("api-transfer: failure detail strips URLs and high-risk secrets", async () => {
  const { client } = buildMockClient({
    failOnCreate: {
      externalId:
...
void test("api-transfer: visual sidecar evidence missing for visual-driven case fails closed", async () => {
...
void test("api-transfer: blocked visual sidecar report fails closed", async () => {
  const preview = buildPreview([buildCase({})]);
  preview.entries[0] = {
    ...preview.entries[0]!,
    visualProvenance: {
...
void test("api-transfer: visual provenance without trace refs fails closed", async () => {
  const preview = buildPreview([buildCase({})]);
  preview.entries[0] = {
    ...preview.entries[0]!,
    sourceTraceRefs: [],
...
void test("api-transfer: stale visual sidecar evidence hash fails closed", async () => {
  const preview = buildPreview([buildCase({})]);
  preview.entries[0] = {
    ...preview.entries[0]!,
    visualProvenance: {
...
void test("api-transfer: stale same-screen visual report from different job fails closed", async () => {
...
void test("api-transfer: visual-driven case transfers when sidecar evidence matches", async () => {
  const preview = buildPreview([buildCase({})]);
  preview.entries[0] = {
    ...preview.entries[0]!,
    visualProvenance: {
...
void test("api-transfer: review event sink receives one transferred event per success", async () => {
...
void test("api-transfer: review event sink failure keeps outcome=created and notes failure_detail", async () => {
  const sink: TransferReviewEventSink = {
    appendTransferred: () => {
  ...
  assert.equal(result.refused, false);
  const sinkRecord0 = result.report.records[0];
  assert.ok(sinkRecord0);
  assert.equal(sinkRecord0.outcome, "created");
  assert.equal(sinkRecord0.failureDetail, "review_event_sink_failed");
});

void test("api-transfer: artifacts persist atomically under artifactRoot", async () => {
  ...
  try {
    ...
    const report = JSON.parse(reportRaw) as {
      ...
      transferUrlIncluded: boolean;
      audit: { dryRunReportId: string };
    };
    ...
    const traceability = JSON.parse(traceabilityRaw) as {
      rows: Array<{
        qcEntityId?: string;
    ...
    const tRow0 = traceability.rows[0];
    assert.ok(tRow0);
    assert.equal(tRow0.qcEntityId, "qc-id-1");
    assert.equal(tRow0.transferOutcome, "created");
    assert.equal(tRow0.steps.length, 2);
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});

void test("api-transfer: evidence reference overrides must be sha256 hashes", async () => {
  ...
  try {
    ...
    const parsedReport = JSON.parse(reportRaw) as {
      audit: {
        evidenceReferences: {
    ...
    const refs = parsedReport.audit.evidenceReferences;
...
void test("api-transfer: rollback guidance contains only created+skipped entries", async () => {
  const cases = [buildCase({ id: "tc-1" }), buildCase({ id: "tc-2" })];
  const preview = buildPreview(cases);
...
void test("api-transfer: unconfigured client throws permission_denied through QcApiTransferError", async () => {
...
void test("api-transfer: orchestrator throws programmer error when client missing", async () => {
  await assert.rejects(
    async () =>
      runOpenTextAlmApiTransfer({
        ...baseInput(),
...
void test("api-transfer: report ids are deterministic for fixed clock + idSource", async () => {
...
void test("api-transfer: multiple refusals are reported in one cycle (sorted, deduped)", async () => {
  const { client } = buildMockClient();
  const result = await runOpenTextAlmApiTransfer(
    baseInput({
