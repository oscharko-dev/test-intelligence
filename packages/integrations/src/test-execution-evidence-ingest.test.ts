...
const makeSignedEvidenceFixture = (
  ...
  const verifyingPublicKeyPem = publicKey.export({
    type: "spki",
    format: "pem",
  }) as string;
  const body = {
...
void test("ingestExecutionEvidence: accepts a single signed entry and persists it", async () => {
  ...
  assert.match(
    result.acceptedEvidencePaths[0]!,
    /\/2026-05\/[0-9a-f]{64}\.json$/,
  );
  const persisted = JSON.parse(
    await readFile(result.acceptedEvidencePaths[0]!, "utf8"),
  ) as {
    schemaVersion: string;
    signingKeyFingerprintSha256: string;
    evidence: { testCaseId: string };
  };
  assert.equal(persisted.schemaVersion, EXECUTION_EVIDENCE_SCHEMA_VERSION);
  assert.equal(
    persisted.signingKeyFingerprintSha256,
    result.report.signingKeyFingerprintSha256,
  );
...
void test("ingestExecutionEvidence: rejects entries with a tampered body (signature mismatch)", async () => {
  const fixture = makeSignedEvidenceFixture();
  const tenantDir = await makeTenantDir();
...
void test("ingestExecutionEvidence: rejects entries with mismatched tenantId", async () => {
  const fixture = makeSignedEvidenceFixture();
  const tenantDir = await makeTenantDir();
...
void test("ingestExecutionEvidence: rejects schema-invalid entries (bad verdict)", async () => {
  const fixture = makeSignedEvidenceFixture();
  const tenantDir = await makeTenantDir();
  const bad = {
    ...fixture.evidence,
...
void test("ingestExecutionEvidence: refuses missing signature with signature_invalid code", async () => {
  const fixture = makeSignedEvidenceFixture();
  const tenantDir = await makeTenantDir();
...
void test("ingestExecutionEvidence: dedupes via sha256 filename when re-ingesting", async () => {
  const fixture = makeSignedEvidenceFixture();
  const tenantDir = await makeTenantDir();
...
void test("loadPersistedExecutionEvidence: returns empty when no corpus exists", async () => {
  const tenantDir = await makeTenantDir();
  const records = await loadPersistedExecutionEvidence(
    join(tenantDir, "calibration-corpus"),
  );
...
void test("summarizeExecutionEvidenceForDossier: aggregates verdicts + conflicts", async () => {
  ...
  assert.equal(
    summary.reviewerConflictCounts.execution_pass_reviewer_rejected,
    1,
  );
...
void test("buildExecutionEvidenceSigningBytes + computeVerifyingKeyFingerprint round-trip", () => {
  const fixture = makeSignedEvidenceFixture();
...
void test("ExecutionEvidenceSignatureGateError: carries G12 hard-gate code", () => {
...
void test("ingestExecutionEvidence: produces deterministic, sorted accepted paths", async () => {
  const tenantDir = await makeTenantDir();
...
void test("persisted evidence file is canonical JSON (sorted keys)", async () => {
  const fixture = makeSignedEvidenceFixture();
  const tenantDir = await makeTenantDir();
  const result = await ingestExecutionEvidence({
    evidence: [fixture.evidence],
...
void test("default verifying key path layout matches the documented contract", async () => {
  const fixture = makeSignedEvidenceFixture();
  const tenantDir = await makeTenantDir();
