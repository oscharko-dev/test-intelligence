...
const sanitizeFolderSegment = (raw: string): string => {
  ...
  const collapsed = collapseFolderSegment(trimmed);
  return collapsed.length > 0 ? collapsed : "Unspecified";
};

const collapseFolderSegment = (value: string): string => {
  let out = "";
...
const trimTrailingChar = (value: string, char: string): string => {
  let end = value.length;
...
export const buildTargetFolderPath = (input: {
  ...
  const profileRoot = trimTrailingChar(input.profile.rootFolderPath, "/");
