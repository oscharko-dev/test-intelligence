...
void test("happy path produces manifest, summary, request/response/testcase per case", async () => {
...
void test("complete run emits a deterministic separate markdown file list without aggregate sub-task markdown", async () => {
  await withTempDir(async (outputDir) => {
    const cases = [
      buildTestCase({ id: "tc-b" }),
      buildTestCase({ id: "tc-a" }),
...
void test("rerun in same directory removes stale managed markdown files only", async () => {
  await withTempDir(async (outputDir) => {
    const firstCases = [
      buildTestCase({ id: "tc-a" }),
      buildTestCase({ id: "tc-b" }),
...
void test("dryRun=true surfaces in manifest.md content", async () => {
  await withTempDir(async (outputDir) => {
...
void test("dryRun=false surfaces in manifest.md content", async () => {
  await withTempDir(async (outputDir) => {
    const cases = [buildTestCase({ id: "tc-a" })];
...
void test("errors.md is emitted for successful runs with an empty failure list", async () => {
  await withTempDir(async (outputDir) => {
    const cases = [buildTestCase({ id: "tc-a" })];
...
void test("errors.md present when at least one case failed", async () => {
  await withTempDir(async (outputDir) => {
    const cases = [
      buildTestCase({ id: "tc-a" }),
      buildTestCase({ id: "tc-b" }),
...
void test("errors.md includes every partial failure for the run", async () => {
  await withTempDir(async (outputDir) => {
    const cases = [
      buildTestCase({ id: "tc-a" }),
      buildTestCase({ id: "tc-b" }),
...
void test("safeId is deterministic across calls", () => {
...
void test("safeId differs across distinct ids", () => {
  const a = buildJiraWriteMarkdownSafeId("tc-1");
...
void test("property: no Bearer/Authorization token leaks into any markdown file", async () => {
  await fc.assert(
    fc.asyncProperty(
      fc
...
void test("URL stripper redacts http/https URLs from failure detail", async () => {
  await withTempDir(async (outputDir) => {
...
void test("errors.md includes run correlation jobId as header (Issue #1484 AC)", async () => {
  await withTempDir(async (outputDir) => {
