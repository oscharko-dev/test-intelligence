...
void test("property: buildTraceabilityMatrix is deterministic", () => {
  fc.assert(
...
void test("property: rows always sorted by testCaseId", () => {
  fc.assert(
...
void test("property: hard invariants always stamped false", () => {
  fc.assert(
...
void test("property: totals.rows always equals rows.length", () => {
  fc.assert(
    fc.property(fc.uniqueArray(idArb, { maxLength: 6 }), (ids) => {
      const matrix = buildTraceabilityMatrix({
        jobId: "job-1",
