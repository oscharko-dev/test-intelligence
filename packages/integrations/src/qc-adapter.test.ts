...
void test("qc-adapter: ALLOWED_QC_ADAPTER_MODES surfaces the three modes", () => {
...
void test("qc-adapter: provider list pins forward-compatible adapters", () => {
...
void test("qc-adapter: openTextAlmDryRunAdapter declares correct discriminator", () => {
  const adapter = createOpenTextAlmDryRunAdapter();
  assert.equal(adapter.provider, "opentext_alm");
  assert.equal(adapter.version, "1.0.0");
});

void test("qc-adapter: validateProfile delegates to mapping-profile validator", () => {
  const adapter = createOpenTextAlmDryRunAdapter();
...
void test("qc-adapter: api_transfer mode throws QcAdapterModeNotImplementedError", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
...
void test("qc-adapter: export_only mode also throws (dry-run adapter is dry-run only)", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
  await assert.rejects(
    () =>
      adapter.dryRun({
...
void test("qc-adapter: isDryRunMode narrows the mode union", () => {
