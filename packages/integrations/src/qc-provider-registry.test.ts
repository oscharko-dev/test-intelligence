...
void test("qc-provider-registry: all 8 builtin descriptors are present", () => {
...
void test("qc-provider-registry: listQcProviderDescriptors returns sorted by provider id", () => {
  const registry = createQcProviderRegistry();
  const descriptors = listQcProviderDescriptors(registry);
...
void test("qc-provider-registry: BUILTIN_QC_PROVIDER_DESCRIPTORS is sorted", () => {
  const ids = BUILTIN_QC_PROVIDER_DESCRIPTORS.map((d) => d.provider);
  const sorted = [...ids].sort();
  assert.deepEqual(ids, sorted);
});

void test("qc-provider-registry: capability matrix per provider matches design", () => {
  const registry = createQcProviderRegistry();
...
void test("qc-provider-registry: opentext_alm resolves to a real adapter", () => {
  const registry = createQcProviderRegistry();
...
void test("qc-provider-registry: non-ALM/non-custom providers resolve to a stub adapter", () => {
  const registry = createQcProviderRegistry();
  for (const provider of ALL_PROVIDERS) {
...
void test("qc-provider-registry: custom slot resolves to null until registered", () => {
  const registry = createQcProviderRegistry();
  assert.equal(resolveQcProviderAdapter(registry, "custom"), null);
});

void test("qc-provider-registry: custom adapter registers, resolves, and survives sibling reads", async () => {
  ...
  assert.notStrictEqual(result.registry, initial);
  ...
  assert.ok(registeredDescriptor);
...
void test("qc-provider-registry: duplicate custom registration refuses", () => {
  const initial = createQcProviderRegistry();
...
void test("qc-provider-registry: shadowing a builtin refuses with register_custom_not_supported", () => {
  const initial = createQcProviderRegistry();
...
void test("qc-provider-registry: registering into builtin custom slot requires descriptor", () => {
  const initial = createQcProviderRegistry();
  const result = registerQcProviderAdapter({
    registry: initial,
    adapter: buildCustomAdapter(),
...
void test("qc-provider-registry: registering with descriptor whose provider differs refuses", () => {
  const initial = createQcProviderRegistry();
  const adapter = buildCustomAdapter();
...
void test("qc-provider-registry: ALLOWED_QC_PROVIDER_OPERATIONS matches capabilities keys", () => {
...
void test("qc-provider-registry: descriptors carry no provider-specific test-case fields", () => {
  const registry = createQcProviderRegistry();
...
void test("qc-provider-registry: getQcProviderEntry returns descriptor + adapter", () => {
  const registry = createQcProviderRegistry();
...
void test("qc-provider-registry: snapshot cannot mutate registry state", () => {
  const registry = createQcProviderRegistry();
...
void test("qc-provider-registry: opentext_alm carries the mappingProfileSeedId", () => {
  const registry = createQcProviderRegistry();
  const alm = getQcProviderDescriptor(registry, "opentext_alm");
  assert.equal(alm?.mappingProfileSeedId, "opentext-alm-default");
});

void test("qc-provider-registry: extraDescriptors only redefines the custom slot", () => {
  const registry = createQcProviderRegistry({
    extraDescriptors: [
      {
        provider: "custom",
