...
void test("qc-mapping: target folder path uses screen name + risk", () => {
...
void test("qc-mapping: target folder path falls back to _unmapped without trace", () => {
  const profile = cloneOpenTextAlmReferenceProfile();
  const folder = buildTargetFolderPath({
    profile,
...
void test("qc-mapping: external id candidate is deterministic", () => {
  const profile = cloneOpenTextAlmReferenceProfile();
...
void test("qc-mapping: external id varies with jobId and testCaseId", () => {
  const profile = cloneOpenTextAlmReferenceProfile();
  const a = computeExternalIdCandidate({
    jobId: "job-1",
    testCaseId: "tc-1",
...
void test("qc-mapping: preview entries sorted by testCaseId", () => {
  const list = wrapList([
    buildCase({ id: "tc-z" }),
    buildCase({ id: "tc-a" }),
    buildCase({ id: "tc-m" }),
...
void test("qc-mapping: blocking reasons combine case + policy violations", () => {
...
void test("qc-mapping: visual provenance derives from sidecar records", () => {
  const list = wrapList([buildCase({ id: "tc-1" })]);
