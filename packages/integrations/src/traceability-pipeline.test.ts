...
void test("persistExportTraceabilityMatrix: writes traceability-matrix.json deterministically", async () => {
...
void test("persistTransferTraceabilityMatrix: identical alias of the export helper", () => {
  assert.equal(
    persistTransferTraceabilityMatrix,
    persistExportTraceabilityMatrix,
  );
...
void test("persistExportTraceabilityMatrix: with transferReport populates qcEntityId in the persisted matrix", async () => {
  const dir = await mkdtemp(join(tmpdir(), "wd-trace-pipeline-"));
  try {
    ...
    assert.ok(row0);
    assert.equal(row0.qcEntityId, "qc-9999");
    assert.equal(row0.transferOutcome, "created");
...
void test("persistExportTraceabilityMatrix: empty list still emits a valid empty matrix", async () => {
  const dir = await mkdtemp(join(tmpdir(), "wd-trace-pipeline-"));
  try {
    const { matrix, paths } = await persistExportTraceabilityMatrix({
      jobId: "job-1",
