...
void test("qc-alm-dry-run: valid profile produces a successful report", async () => {
...
void test("qc-alm-dry-run: missing profile fields refuse with mapping_profile_invalid + diagnostics", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
...
void test("qc-alm-dry-run: invalid target folder path refuses with folder_resolution_failed", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
  const profile = cloneOpenTextAlmDefaultMappingProfile();
...
void test("qc-alm-dry-run: malformed profile target path refuses before folder resolution", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
  const profile = cloneOpenTextAlmDefaultMappingProfile();
...
void test("qc-alm-dry-run: provider mismatch refuses with provider_mismatch", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
  const profile = cloneOpenTextAlmDefaultMappingProfile();
...
void test("qc-alm-dry-run: missing-mapped-cases refuses without folder resolution", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
  let resolverCalls = 0;
  const result = await adapter.dryRun({
    jobId: "job-1368",
...
void test("qc-alm-dry-run: REGRESSION — folder resolver is read-only and not invoked for write-shaped paths", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
...
void test("qc-alm-dry-run: low-confidence visual sidecar flags the affected case", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
...
void test("qc-alm-dry-run: high-confidence visual sidecar produces no flags", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
...
void test("qc-alm-dry-run: visualConfidenceThreshold override works", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
...
void test("qc-alm-dry-run: completeness reflects unexportable cases as incomplete", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
  const cases = [
    buildCase({ id: "tc-ok" }),
    buildCase({
  ...
  assert.ok(incomplete.missingRequiredFields.length > 0);
});

void test("qc-alm-dry-run: resolver throwing degrades to folder_resolution_failed without exposing message", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
  const result = await adapter.dryRun({
    jobId: "job-1368",
    mode: "dry_run",
...
void test("qc-alm-dry-run: resolver evidence is redacted and bounded before report persistence", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
...
void test("qc-alm-dry-run: planned payloads carry visual provenance fields when sidecar matches (Issue #1374)", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
...
void test("qc-alm-dry-run: visual evidence refs use total deterministic ordering", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
...
void test("qc-alm-dry-run: planned payloads omit visual provenance keys when no sidecar records match (Issue #1374)", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
  const result = await adapter.dryRun({
    jobId: "job-1374",
    mode: "dry_run",
...
void test("qc-alm-dry-run: visual provenance confidence is rounded to 4 decimals (Issue #1374)", async () => {
  const adapter = createOpenTextAlmDryRunAdapter();
