...
const buildIctRegisterEntry = (input: {
  ...
  return {
    ...
    ictArrangement: {
      providerName: "Test Intelligence",
      serviceDescription:
...
const buildSummaryReport = (input: {
  ...
  lines.push("");
  lines.push(
    "Public-key fingerprints (record these — private keys are NEVER reprinted):",
  );
  lines.push(
  ...
  lines.push(`  2. Sanity-check the layout:`);
  lines.push(
  ...
  lines.push(`  3. Run a smoke test against a small mask:`);
  lines.push(
...
export const runTenantOnboarding = async (
  ...
  const generate =
    rawInput.generateEd25519KeyPair ?? defaultGenerateEd25519KeyPair;
  ...
  const regionAttestationSecret = random(
    REGION_ATTESTATION_HMAC_BYTES,
  ).toString("hex");
...
const checkRegionAttestationKey = async (
  ...
  if (
    ...
    return {
      ...
      ok: false,
      detail:
        "region-attestation key fingerprint does not match recorded fingerprint",
    };
  }
  return {
    name: "region-attestation-key",
