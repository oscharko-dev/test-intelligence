...
void test("tenant-isolation-proof: schema version + filename + gate code are pinned", () => {
...
void test("tenant-isolation-proof: build is deterministic for identical inputs", () => {
  ...
  assert.equal(
    serializeTenantIsolationProof(a),
    serializeTenantIsolationProof(b),
  );
  assert.equal(a.proofSha256, b.proofSha256);
});

void test("tenant-isolation-proof: proofSha256 is verifiable by re-derivation", () => {
...
void test("tenant-isolation-proof: methodology disclaimer + claim are stamped verbatim", () => {
  const proof = buildTenantIsolationProof({ generatedAt: fixedGeneratedAt });
  assert.equal(
...
void test("tenant-isolation-proof: cacheKey construction declares tenant-independence", () => {
  const proof = buildTenantIsolationProof({ generatedAt: fixedGeneratedAt });
...
void test("tenant-isolation-proof: storage namespace pins POSIX template + segment validation", () => {
  const proof = buildTenantIsolationProof({ generatedAt: fixedGeneratedAt });
  assert.equal(
...
void test("tenant-isolation-proof: every distinctness witness differs in at least one segment", () => {
  const proof = buildTenantIsolationProof({ generatedAt: fixedGeneratedAt });
  assert.ok(
...
void test("tenant-isolation-proof: side-channel analysis covers timing, eviction-order, error-disclosure", () => {
  const proof = buildTenantIsolationProof({ generatedAt: fixedGeneratedAt });
...
void test("tenant-isolation-proof: tenant commitments produce distinct sha256 hashes for distinct scopes", () => {
  const proof = buildTenantIsolationProof({ generatedAt: fixedGeneratedAt });
  ...
  assert.equal(
    unique.size,
    commitments.length,
    "every scope must hash uniquely",
  );
});

void test("tenant-isolation-proof: rejects malformed generatedAt", () => {
  assert.throws(
...
void test("tenant-isolation-proof: rejects malformed cache-key digest", () => {
  assert.throws(
    () =>
      buildTenantIsolationProof({
...
void test("tenant-isolation-proof: rejects fewer than two tenant scopes", () => {
  assert.throws(
    () =>
      buildTenantIsolationProof({
        generatedAt: fixedGeneratedAt,
...
void test("tenant-isolation-proof: rejects empty cacheKeyExamples", () => {
  assert.throws(
    () =>
      buildTenantIsolationProof({
        generatedAt: fixedGeneratedAt,
...
void test("tenant-isolation-proof: write produces an atomic, deterministic JSON file", async () => {
...
void test("tenant-isolation-proof: witness paths match runtime persistent-cache layout", async () => {
...
void test("pentest evidence: zero-leak attempts produce a 'pass' verdict", () => {
...
void test("pentest evidence: any leak produces 'fail' and assertTenantIsolationPentestPasses throws", () => {
  const evidence = buildTenantIsolationPentestEvidence({
    generatedAt: fixedGeneratedAt,
    attempts: [
      {
...
void test("pentest evidence: attempts are sorted for byte-stable bytes", () => {
  const evidence = buildTenantIsolationPentestEvidence({
    generatedAt: fixedGeneratedAt,
    attempts: [
      {
...
void test("pentest evidence: rejects empty attempts list", () => {
  assert.throws(
    () =>
      buildTenantIsolationPentestEvidence({
        generatedAt: fixedGeneratedAt,
...
void test("pentest evidence: serialize ends with newline + parses back identically", () => {
  const evidence = buildTenantIsolationPentestEvidence({
    generatedAt: fixedGeneratedAt,
    attempts: [
      {
  ...
  const parsed = JSON.parse(serialized) as { evidenceSha256: string };
...
void test("pentest scenario: tenant A's replay-cache directory is unreachable from tenant B's loader", async () => {
