...
void test("runTenantOnboarding lays down every required artifact", async () => {
  ...
  try {
    ...
    assert.match(
      ...
      /test-intelligence run --tenant-bundle/u,
    );
  } finally {
...
void test("runTenantOnboarding generates valid Ed25519 keys with fingerprints matching the ICT register", async () => {
  const outputRoot = await makeOutputRoot();
  try {
    const result = await runTenantOnboarding({
      tenantId: "acme-bank",
    ...
    assert.equal(
      result.fingerprints.auditDossierEd25519Sha256,
      expectedAuditFp,
    );
...
void test("runTenantOnboarding refuses to overwrite without --force", async () => {
  const outputRoot = await makeOutputRoot();
  try {
...
void test("runTenantOnboarding rejects unknown policy profile and bad tenant id", async () => {
  const outputRoot = await makeOutputRoot();
  try {
    await assert.rejects(
      () =>
...
void test("runTenantOnboardingDoctor passes on a freshly-onboarded tenant", async () => {
  const outputRoot = await makeOutputRoot();
  try {
    await runTenantOnboarding({
      tenantId: "acme-bank",
    ...
    assert.ok(report.checks.some((c) => c.name === "tenant-bundle" && c.ok));
    assert.ok(
      report.checks.some((c) => c.name === "audit-dossier-private-key" && c.ok),
    );
    assert.ok(
      report.checks.some((c) => c.name === "region-attestation-key" && c.ok),
    );
  } finally {
    await rm(outputRoot, { recursive: true, force: true });
  }
...
void test("runTenantOnboardingDoctor detects missing keys and orphaned files", async () => {
  const outputRoot = await makeOutputRoot();
  try {
    const result = await runTenantOnboarding({
      tenantId: "acme-bank",
...
void test("runTenantOnboardingDoctor flags a tenant-scope mismatch as a multi-tenant isolation violation", async () => {
  const outputRoot = await makeOutputRoot();
  try {
    const result = await runTenantOnboarding({
      tenantId: "acme-bank",
...
void test("runTenantOnboardingDoctor returns a single failure when the tenant directory does not exist", async () => {
  const outputRoot = await makeOutputRoot();
  try {
    const report = await runTenantOnboardingDoctor({
      tenantId: "ghost-bank",
