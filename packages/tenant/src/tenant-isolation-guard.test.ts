...
void test("constants are stable per Issue #2176 contract", () => {
...
void test("assertTenantScope: identical scopes do not throw", () => {
  assert.doesNotThrow(() =>
    assertTenantScope("op", tenantA, { ...tenantA }),
  );
});

void test("assertTenantScope: undefined projectId equals 'default'", () => {
...
void test("assertTenantScope: tenantId mismatch throws TenantIsolationViolation", () => {
  assert.throws(
...
void test("assertTenantScope: environmentId mismatch throws", () => {
  assert.throws(
    () =>
...
void test("assertTenantScope: projectId mismatch throws", () => {
  assert.throws(
    () =>
      assertTenantScope("read", tenantA, {
        ...tenantA,
...
void test("withTenantScope: nested same-scope is a no-op on context", async () => {
...
void test("withTenantScope: nested different-scope throws eagerly", async () => {
  await withTenantScope(tenantA, async () => {
...
void test("getCurrentTenantScope is undefined outside any block", () => {
  assert.equal(getCurrentTenantScope(), undefined);
});

void test("recordPersistentStoreRead: outside withTenantScope is a no-op", () => {
...
void test("recordPersistentStoreRead: matching scope appends to attestation buffer", async () => {
  await withTenantScope(tenantA, async () => {
...
void test("recordPersistentStoreRead: mismatched scope crashes the run", async () => {
  await withTenantScope(tenantA, async () => {
    assert.throws(
...
void test("recordTenantIdRead: matching tenantId records the read", async () => {
  await withTenantScope(tenantA, async () => {
...
void test("recordTenantIdRead: mismatched tenantId crashes the run", async () => {
  await withTenantScope(tenantA, async () => {
    assert.throws(
...
void test("recordTenantIdRead: outside scope is a no-op", () => {
  assert.doesNotThrow(() => recordTenantIdRead("op", "anything"));
});

void test("recordActiveTenantRead: appends scoped entry inside block", async () => {
  await withTenantScope(tenantA, async () => {
...
void test("recordActiveTenantRead: outside block is a no-op", () => {
  assert.doesNotThrow(() => recordActiveTenantRead("op"));
});

void test("buildTenantIsolationAttestation: byte-stable across identical inputs", async () => {
...
void test("buildTenantIsolationAttestation: refuses to emit when reads cross scopes", () => {
  assert.throws(
    () =>
      buildTenantIsolationAttestation({
        jobId: "job-1",
...
void test("buildTenantIsolationAttestation: digest is over canonical scope+reads only", () => {
  const reads = [
...
void test("buildTenantIsolationAttestation: read sequence is normalized 0..N-1", () => {
...
void test("withTenantScope: context isolation between concurrent scopes", async () => {
...
void test("property: every random tenant pair where tenantA !== tenantB throws", () => {
