...
void test("computeJudgeCalibrationMetrics: perfect accept stream produces accuracy 1.0 and zero FP/FN", () => {
...
void test("computeJudgeCalibrationMetrics: false positive (judge accept while human reject) lifts FPR and is captured in divergences", () => {
  const metrics = computeJudgeCalibrationMetrics([
...
void test("computeJudgeCalibrationMetrics: false negative (judge reject while human accept) lifts FNR", () => {
  const metrics = computeJudgeCalibrationMetrics([
    sample("a", { humanVerdict: "accept", predictedVerdict: "accept" }),
    sample("b", { humanVerdict: "accept", predictedVerdict: "accept" }),
...
void test("computeJudgeCalibrationMetrics: over-repair (judge repair while human accept) is captured but does NOT count as FP/FN", () => {
  const metrics = computeJudgeCalibrationMetrics([
    sample("a", { humanVerdict: "accept", predictedVerdict: "accept" }),
...
void test("computeJudgeCalibrationMetrics: finding precision and recall split TP/FP/FN cleanly", () => {
  const metrics = computeJudgeCalibrationMetrics([
    sample("a", {
      humanVerdict: "repair",
...
void test("computeJudgeCalibrationMetrics: empty sample list yields neutral metrics (accuracy 1, rates 0)", () => {
  const metrics = computeJudgeCalibrationMetrics([]);
...
void test("evaluateJudgeCalibrationVerdict: passes when all metrics are within thresholds", () => {
  const metrics = computeJudgeCalibrationMetrics([
    sample("a", { humanVerdict: "accept", predictedVerdict: "accept" }),
    sample("b", { humanVerdict: "accept", predictedVerdict: "accept" }),
...
void test("evaluateJudgeCalibrationVerdict: emits accuracy_below_threshold when accuracy < 0.85", () => {
  const metrics = computeJudgeCalibrationMetrics([
    sample("a", { humanVerdict: "accept", predictedVerdict: "accept" }),
    sample("b", { humanVerdict: "accept", predictedVerdict: "repair" }),
  ...
  assert.deepEqual(verdict.failures.map((failure) => failure.reason).sort(), [
    "accuracy_below_threshold",
  ]);
});

void test("evaluateJudgeCalibrationVerdict: emits false_positive_rate_above_threshold when FPR > 0.10", () => {
...
void test("evaluateJudgeCalibrationVerdict: emits false_negative_rate_above_threshold when FNR > 0.20", () => {
  const samples: JudgeCalibrationSample[] = [];
  for (let i = 0; i < 4; i += 1) {
    samples.push(
      sample(`accept-${i}`, {
...
void test("partitionSamplesByJudge splits samples into logic and faithfulness buckets", () => {
...
void test("loadJudgeCalibrationFixture round-trips a single fixture pair", async () => {
...
void test("every loaded fixture carries at least 2 reviewer verdicts (Issue #2109)", async () => {
...
void test("exactly two fixtures across the calibration set are adjudicated (Issue #2109)", async () => {
  const fixtures = await loadAllJudgeCalibrationFixtures();
...
void test("resolveJudgeCalibrationFixturePath produces stable paths for both kinds", () => {
...
void test("JUDGE_CALIBRATION_FIXTURE_INDEX contains exactly 10 logic + 10 faithfulness cases with the prescribed scenario mix", () => {
...
void test("loadAllJudgeCalibrationFixtures returns 20 fixtures aligned with the index", async () => {
  const fixtures = await loadAllJudgeCalibrationFixtures();
...
void test("calibration set passes the production hard-gate thresholds for both judges", async () => {
  const fixtures = await loadAllJudgeCalibrationFixtures();
...
void test("buildJudgeCalibrationEvalArtifact pins schema/contract versions and sorts samples", () => {
...
void test("buildJudgeCalibrationEvalArtifact rejects samples whose judge mismatches the requested judge", () => {
  assert.throws(
    () =>
      buildJudgeCalibrationEvalArtifact({
        judge: "logic",
...
void test("writeJudgeCalibrationEvalArtifact + readJudgeCalibrationEvalArtifact round-trip canonical JSON", async () => {
...
void test("appendJudgeCalibrationHistoryEntry appends a row, persists canonical JSON, and trims to maxEntries", async () => {
  ...
  try {
    ...
    assert.equal(path1, join(tmpDir, JUDGE_CALIBRATION_HISTORY_FILENAME));
    ...
    const parsed = JSON.parse(raw) as {
      entries: ReadonlyArray<{ recordedAt: string }>;
    };
...
void test("appendJudgeCalibrationHistoryEntry default maxEntries is conservative enough for routine drift tracking", () => {
  ...
  assert.ok((JUDGE_CALIBRATION_HISTORY_MAX_ENTRIES as number) >= 50);
  assert.ok((JUDGE_CALIBRATION_HISTORY_MAX_ENTRIES as number) <= 1000);
});

void test("JUDGE_CALIBRATION_EVAL_REPORT_DIRNAME points at the deployed Storybook eval-reports bundle", () => {
  assert.equal(
    JUDGE_CALIBRATION_EVAL_REPORT_DIRNAME,
