...
import { TEST_INTELLIGENCE_CONTRACT_VERSION } from "@oscharko-dev/ti-contracts";
...
void test("computeUncertaintyScore plateaus inside the half-band and tapers outside", () => {
  assert.equal(
    computeUncertaintyScore(
...
void test("computeUncertaintyScore takes the panel max and ignores entries without confidence", () => {
...
void test("computeDisagreementScore: agreement-shape table maps to canonical values", () => {
...
void test("computeDisagreementScore returns 1 when vetoBy is set, regardless of shape", () => {
  const score = computeDisagreementScore(
    candidate({
      caseId: "c-veto",
...
void test("computeDisagreementScore: high-confidence reject overrides a unanimous shape", () => {
  const score = computeDisagreementScore(
    candidate({
      caseId: "c-reject",
...
void test("computeDriftScore is binary on the flagged signal", () => {
  assert.equal(computeDriftScore({ flagged: false }), 0);
  assert.equal(computeDriftScore({ flagged: true }), 1);
});

void test("buildActiveLearningQueueArtifact selects top-N by composite score with deterministic tie-break", () => {
...
void test("buildActiveLearningQueueArtifact prioritises mandatory-risk cases regardless of composite score", () => {
  const candidates: ActiveLearningCandidateCase[] = [
    candidate({
      caseId: "case-uncertain",
      panel: buildPanel([{ judgeId: "j", verdict: "accept", confidence: 0.5 }]),
...
void test("buildActiveLearningQueueArtifact aggregate counts every selection reason", () => {
  const artifact = buildActiveLearningQueueArtifact({
...
void test("buildActiveLearningQueueArtifact rejects duplicate caseIds", () => {
  assert.throws(
    () =>
      buildActiveLearningQueueArtifact({
        cycleId: "cycle-dupe",
...
void test("buildActiveLearningQueueArtifact rejects weights that do not sum to 1", () => {
...
void test("buildActiveLearningQueueArtifact rejects out-of-range confidence", () => {
  assert.throws(
    () =>
      buildActiveLearningQueueArtifact({
        cycleId: "cycle-conf",
...
void test("runActiveLearningSampler writes a canonical, byte-stable artifact", async () => {
...
void test("runActiveLearningSampler is deterministic across two builds with identical inputs", async () => {
  const candidates: ActiveLearningCandidateCase[] = [
    candidate({
      caseId: "case-A",
      panel: buildPanel([{ judgeId: "j", verdict: "accept", confidence: 0.5 }]),
...
void test("buildActiveLearningGrowthLog sorts records by addedAt then cycleId, and dedupes case ids", () => {
...
void test("buildActiveLearningGrowthLog rejects duplicate cycleIds", () => {
  assert.throws(
    () =>
      buildActiveLearningGrowthLog({
        recordedAt: FIXTURE_GENERATED_AT,
...
void test("writeActiveLearningGrowthLog persists the canonical document", async () => {
...
void test("summariseActiveLearningQuarterlyGrowth counts unique cases inside the as-of quarter", () => {
  const log = buildActiveLearningGrowthLog({
    recordedAt: FIXTURE_GENERATED_AT,
    records: [
      {
...
void test("evaluateActiveLearningQuarterlyGate throws on quarterly deficit", () => {
  const log = buildActiveLearningGrowthLog({
    recordedAt: FIXTURE_GENERATED_AT,
    records: [
      {
...
void test("evaluateActiveLearningQuarterlyGate passes when threshold is met", () => {
...
void test("evaluateActiveLearningKappaGate routes new paired ratings through the inter-rater contract", () => {
...
void test("evaluateActiveLearningKappaGate fails when κ drops below the hard floor", () => {
  const ratings: CalibrationPairedRating[] = [];
...
void test("writeActiveLearningQueueArtifact uses the canonical filename and atomic-write pattern", async () => {
...
void test("growth-log records flagged as adjudicated still count for quarterly growth", () => {
