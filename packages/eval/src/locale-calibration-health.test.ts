...
const healthyArtifact = (
  locale: SupportedLocale,
): LoadedPerLocaleArtifacts => ({
  locale,
  heldOutEce: 0.05,
  heldOutKappa: 0.78,
  sampleCount: 30,
...
void test("G13: gate code is the stable string G13_LOCALE_CALIBRATION_HEALTHY", () => {
  assert.equal(
    G13_LOCALE_CALIBRATION_HEALTHY,
    "G13_LOCALE_CALIBRATION_HEALTHY",
  );
...
void test("G13: thresholds match the documented contract", () => {
...
void test("G13: healthy artifact passes all four invariants", () => {
...
void test("G13: κ below floor fails the gate", () => {
...
void test("G13: ECE above ceiling fails the gate", () => {
  const entry = evaluateLocaleCalibrationHealth({
...
void test("G13: sample count below 30 fails the gate", () => {
  const entry = evaluateLocaleCalibrationHealth({
...
void test("G13: fallbackToDefault=true fails the gate", () => {
  const entry = evaluateLocaleCalibrationHealth({
...
void test("G13: aggregate report passes when all five Issue #2188 locales are healthy", () => {
...
void test("G13: aggregate report reports the failing locale set", () => {
  const report = buildLocaleCalibrationHealthReport([
...
void test("G13: passes when artifact list is empty (no locales to gate)", () => {
  const report = buildLocaleCalibrationHealthReport([]);
  assert.equal(report.passed, true);
  assert.equal(report.failedLocales.length, 0);
});
...
void test("G13: malformed fixture raises a typed error", async () => {
...
void test("G13: assert throws LocaleCalibrationHealthError on unhealthy fixtures", async () => {
  const root = await mkdtemp(join(tmpdir(), "loc-cal-health-"));
  await mkdir(join(root, "PL-PL"), { recursive: true });
  await writeFile(
    join(root, "PL-PL", "platt-curve.json"),
  ...
  await assert.rejects(
    ...
    (error) => error instanceof LocaleCalibrationHealthError,
  );
});

void test("G13: locale mismatch between filename and payload is rejected", async () => {
  const root = await mkdtemp(join(tmpdir(), "loc-cal-health-"));
  await mkdir(join(root, "PL-PL"), { recursive: true });
  await writeFile(
    join(root, "PL-PL", "platt-curve.json"),
