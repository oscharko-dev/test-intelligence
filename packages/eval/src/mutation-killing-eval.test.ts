...
void test("catalog: rejects malformed mutation ids", () => {
...
void test("catalog: rejects unknown mutation classes", () => {
  const catalog = createMutationCatalog();
  assert.throws(() =>
    catalog.register({
      id: "MUT-UNKNOWN-01",
...
void test("catalog: rejects duplicate mutation ids", () => {
  const catalog = createMutationCatalog();
...
void test("catalog: list() and ids() are sorted alphabetically", () => {
  const catalog = createMutationCatalog();
  catalog.register({
    id: "MUT-ZZZ-01",
    mutationClass: "field-required-flipped",
...
void test("default catalog: every mutation class has at least one entry", () => {
...
void test("default catalog: every mutation id matches MUT-* pattern", () => {
  const catalog = buildDefaultMutationCatalog();
...
void test("registerDefaultMutations: composable on existing catalog", () => {
  const catalog = createMutationCatalog();
  registerDefaultMutations(catalog);
  assert.ok(catalog.list().length >= 12);
});

void test("evaluation: empty test-case list yields zero kill rate", () => {
...
void test("evaluation: rejects out-of-range threshold", () => {
  const intent = buildIntent();
  assert.throws(() =>
    evaluateMutationKillingSuite({
      jobId: "job-2041",
...
void test("evaluation: a single highly-specific case kills field-required + IBAN mutations", () => {
  const intent = buildIntent();
  const cases: GeneratedTestCase[] = [
    buildCase({
      ...
      objective:
        "Verify that submission is blocked when the required IBAN field is empty",
      ...
      steps: [
        { index: 1, action: "Leave IBAN empty" },
        {
          ...
          action: "Press Submit",
          expected:
            "Submission is blocked and an error message is displayed: Required field",
        },
      ],
      expectedResults: [
...
void test("evaluation: financial calculation case kills VAT + currency-rounding mutations", () => {
  const intent = buildIntent();
  const cases: GeneratedTestCase[] = [
    buildCase({
      id: "tc-vat-calc",
...
void test("evaluation: deterministic ordering across runs", () => {
  const intent = buildIntent();
  const cases = [
...
void test("evaluation: reports unkilled mutations sorted alphabetically", () => {
  const intent = buildIntent();
  const cases: GeneratedTestCase[] = [
...
void test("evaluation: a comprehensive negative + positive suite reaches the 0.85 KPI", () => {
  const intent = buildIntent();
  const cases: GeneratedTestCase[] = [
    buildCase({
      id: "tc-required",
    ...
    buildCase({
      ...
      expectedResults: [
        "After submit, navigate to the receipt confirmation page",
      ],
    }),
    buildCase({
      id: "tc-a11y",
    ...
    buildCase({
      ...
      expectedResults: ["Customer name PII is redacted in the audit log entry"],
    }),
    buildCase({
      id: "tc-four-eyes",
      type: "functional",
...
void test("buildMutationKillRateSummary: projects the report shape", () => {
  const intent = buildIntent();
...
void test("encodeCanonicalReportBytes: deterministic UTF-8 with trailing newline", () => {
  const intent = buildIntent();
  const report = evaluateMutationKillingSuite({
    jobId: "job-2041",
    generatedAt: GENERATED_AT,
  ...
  const parsed = JSON.parse(encoded.trimEnd()) as MutationReport;
...
void test("writeMutationReportArtifact: persists at the canonical filename", async () => {
  ...
  try {
    ...
    assert.equal(result.path, join(tmp, MUTATION_REPORT_ARTIFACT_FILENAME));
...
void test("FinOps cap constant is the documented 0.20 ceiling", () => {
  assert.equal(MUTATION_EVAL_TOKEN_BUDGET_RATIO_CAP, 0.2);
});

void test("default threshold matches the >=0.85 KPI from Issue #1753", () => {
