...
const FIXTURES_DIR = join(
  ...
  "../../core-engine/src/fixtures",
);
const TRACES_FIXTURE_PATH = join(FIXTURES_DIR, "agent-online-eval-traces.json");
...
void test("computeTraceSamplingScore yields a stable value in [0, 1)", () => {
...
void test("shouldSampleAgentTrace honours rate=0 and rate=1 boundaries", () => {
  assert.equal(
...
void test("shouldSampleAgentTrace rejects invalid sample rates", () => {
...
void test("sampling is deterministic for identical seed + sampleRate", () => {
...
void test("sampling rate is approximately respected over a large population", () => {
...
void test("different seeds select different subsets at the same rate", () => {
  const traces = synthTraces(500);
...
void test("redactProductionTrace masks PII in prompt, response, and metadata", () => {
...
void test("redactProductionTrace passes clean strings through unchanged", () => {
  const redacted = redactProductionTrace({
    traceId: "t-clean",
    runId: "r-clean",
...
void test("defaultAgentOnlineEvaluator classifies refusal, fail, and pass", () => {
...
void test("defaultAgentOnlineEvaluator down-scores verbose responses", () => {
...
void test("buildAgentOnlineEvalReport rejects an empty seed", () => {
  assert.throws(() =>
    buildAgentOnlineEvalReport({
...
void test("buildAgentOnlineEvalReport rejects duplicate traceIds", () => {
...
void test("buildAgentOnlineEvalReport is deterministic and canonical", async () => {
  ...
  assert.deepEqual(
    reportA,
    reportB,
    "trace input order must not affect output",
  );
...
void test("buildAgentOnlineEvalReport redacts PII before evaluation", async () => {
  const fixture = await loadTraceFixture();
...
void test("buildAgentOnlineEvalReport supports a custom evaluator", async () => {
  const fixture = await loadTraceFixture();
...
void test("buildAgentOnlineEvalReport rejects an out-of-range evaluator score", () => {
  const trace: AgentProductionTrace = {
    traceId: "t",
    runId: "r",
    prompt: "p",
...
void test("buildAgentOnlineEvalReport population block reports sample rate honestly", async () => {
  const fixture = await loadTraceFixture();
  const report = buildAgentOnlineEvalReport({
    traces: fixture.traces,
    seed: "fixture-seed",
...
void test("writeAgentOnlineEvalReport produces canonical, byte-stable output", async () => {
  const fixture = await loadTraceFixture();
  const report = buildAgentOnlineEvalReport({
    traces: fixture.traces,
    seed: "fixture-seed",
...
void test("runAgentOnlineEvalSampler builds + persists in one call", async () => {
  const fixture = await loadTraceFixture();
  const tempDir = await mkdtemp(join(tmpdir(), "agent-online-eval-"));
  try {
...
void test("default sample rate is 1% when none is supplied", async () => {
  const fixture = await loadTraceFixture();
  const report = buildAgentOnlineEvalReport({
    traces: fixture.traces,
    seed: "fixture-seed",
