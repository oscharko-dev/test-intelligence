...
void test("computeKlDivergence is zero for identical histograms and symmetric", () => {
  const a = [3, 7, 2, 0];
  const b = [6, 14, 4, 0];
...
void test("computeKlDivergence rejects mismatched lengths", () => {
  assert.throws(() => computeKlDivergence([1, 2], [1, 2, 3]));
});

void test("computeKlDivergence returns 0 for two empty histograms", () => {
  assert.equal(computeKlDivergence([], []), 0);
});

void test("recordInputDistributionSnapshot produces a deterministic snapshot", async () => {
...
void test("recordInputDistributionSnapshot rejects malformed fixtureSuiteId", async () => {
...
void test("evaluateDistributionShiftReport returns warming with no findings on first record", () => {
...
void test("evaluateDistributionShiftReport flags KL > threshold for shifted inputs", async () => {
...
void test("evaluateDistributionShiftReport stays quiet when current matches baseline", async () => {
  let state: DistributionShiftBaselineState =
    emptyDistributionShiftBaselineState({
      tenantId: TENANT,
      policyProfileId: PROFILE,
...
void test("recordInputDistributionSnapshot uses the embedding provider when supplied", async () => {
...
void test("evaluateDistributionShiftReport flags embedding centroid > 2σ", async () => {
...
void test("evaluateDistributionShiftReport flags centroid shift in σ-units when historical scatter is non-zero", async () => {
...
void test("appendDistributionShiftBaselineRecord trims to the history window", async () => {
  let state: DistributionShiftBaselineState =
    emptyDistributionShiftBaselineState({
      tenantId: TENANT,
      policyProfileId: PROFILE,
...
void test("baseline persistence round-trips byte-stable canonical JSON", async () => {
...
void test("loadDistributionShiftBaselineState returns an empty state when the file is missing", async () => {
...
void test("distributionShiftBaselinePath rejects unstable identifiers", () => {
  assert.throws(() =>
    distributionShiftBaselinePath({
      runtimeRoot: "/tmp",
      tenantId: "tenant a",
...
void test("buildDistributionShiftDashboard summarises the per-record KL trend", async () => {
  let state: DistributionShiftBaselineState =
    emptyDistributionShiftBaselineState({
      tenantId: TENANT,
      policyProfileId: PROFILE,
...
void test("artifact writers persist canonical JSON byte-stably", async () => {
  ...
  try {
    ...
    const reportParsed = JSON.parse(reportRaw) as {
    ...
    const dashboardParsed = JSON.parse(dashboardRaw) as {
      fixtureSuiteId: string;
    };
    assert.equal(dashboardParsed.fixtureSuiteId, SUITE);

    const alertsParsed = JSON.parse(alertsRaw) as {
      fixtureSuiteId: string;
