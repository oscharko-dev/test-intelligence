...
void test("computeCohensKappa: empty input returns vacuous κ=1 with degenerate=true", () => {
...
void test("computeCohensKappa: perfect agreement yields κ=1 (degenerate when one label dominates)", () => {
  const result = computeCohensKappa([
...
void test("computeCohensKappa: textbook 9/10 agreement on 3-class verdicts yields κ ≈ 0.84", () => {
...
void test("computeCohensKappa: full disagreement on a single label yields κ=-1 lower bound", () => {
  const result = computeCohensKappa([
...
void test("computeCohensKappa: degenerate cell with pe=1 and partial disagreement yields κ=0", () => {
...
void test("buildInterRaterAgreementReport: happy path passes per-judge gate at κ ≥ 0.8", () => {
...
void test("buildInterRaterAgreementReport: κ below 0.7 trips kappa_below_hard_floor failure", () => {
...
void test("buildInterRaterAgreementReport: missing paired ratings for a judge fails the gate", () => {
  const ratings: CalibrationPairedRating[] = [];
  for (let i = 0; i < 10; i += 1) {
    ratings.push(
...
void test("buildInterRaterAgreementReport: per-scenario gate is suppressed below 8 paired ratings", () => {
...
void test("buildInterRaterAgreementReport: reviewer-share above hard cap (>0.6) fails the gate", () => {
  const ratings: CalibrationPairedRating[] = [];
...
void test("buildInterRaterAgreementReport: rotation log enumerates assignment counts and shares deterministically", () => {
  const ratings: CalibrationPairedRating[] = [
...
void test("INTER_RATER_GATE_THRESHOLDS exposes the public gate constants verbatim", () => {
...
void test("formatInterRaterFailure / formatInterRaterWarning render scope and subject readably", () => {
...
void test("loaded calibration set passes the inter-rater gate at κ ≥ 0.8 for both judges (Issue #2109)", async () => {
...
void test("writeInterRaterAgreementArtifact persists canonical JSON under the canonical filename", async () => {
