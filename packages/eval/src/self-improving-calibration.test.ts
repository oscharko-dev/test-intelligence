...
import type { SupportedLocale } from "@oscharko-dev/ti-contracts";

// An ephemeral Ed25519 signing key is minted on first access and written to a
// temp PKCS8/PEM file the test then passes to `ratifyOrRollback({ signKeyPath })`.
// Committing a real operator private key into the public repo would be an
// irreversible secret leak; the operator-key fixture stays out of source
// control and is generated per test run instead.
let CACHED_OPERATOR_KEY_PATH: string | undefined;
const operatorKeyPath = async (): Promise<string> => {
  if (CACHED_OPERATOR_KEY_PATH !== undefined) {
    return CACHED_OPERATOR_KEY_PATH;
  }
  const dir = await mkdtemp(join(tmpdir(), "self-improving-calibration-key-"));
  const { privateKey } = generateKeyPairSync("ed25519");
  const pem = privateKey.export({ format: "pem", type: "pkcs8" }) as string;
  const path = join(dir, "operator-ed25519.private-key.pem");
  await writeFile(path, pem, "utf8");
  CACHED_OPERATOR_KEY_PATH = path;
  return path;
};
...
void test("self-improving-calibration: REGULATED_RISK_CLASSES is the exact regulated subset", () => {
  assert.deepEqual(
    [...REGULATED_RISK_CLASSES],
    ["high", "regulated_data", "financial_transaction"],
  );
});

void test("self-improving-calibration: hard gates carry the issue's published thresholds", () => {
...
void test("self-improving-calibration: parseCurveFilename round-trips production paths", () => {
...
void test("self-improving-calibration: parseCurveFilename rejects malformed names", () => {
...
void test("proposeCalibrationRefit: writes a proposal and computes a Platt fit", async () => {
...
void test("proposeCalibrationRefit: identical inputs produce identical proposalId and digest", async () => {
...
void test("proposeCalibrationRefit: rejects a riskClass outside the regulated subset", async () => {
  const dir = await makeTmpCurvesDir();
  try {
...
void test("proposeCalibrationRefit: rejects malformed proposedAt", async () => {
  const dir = await makeTmpCurvesDir();
  try {
    await assert.rejects(
      proposeCalibrationRefit({
...
void test("proposeCalibrationRefit: rejects under-powered cells below the minimum sample floor", async () => {
  const dir = await makeTmpCurvesDir();
  try {
    await assert.rejects(
      proposeCalibrationRefit({
...
void test("proposeCalibrationRefit: held-out fraction validation rejects 0 and 1", async () => {
  const dir = await makeTmpCurvesDir();
  try {
...
void test("ratifyOrRollback: rolls back when held-out κ floor fails (no signing key required)", async () => {
  const dir = await makeTmpCurvesDir();
  try {
...
void test("ratifyOrRollback: forceRollback path skips signing and writes a sidecar with operator reason", async () => {
  const dir = await makeTmpCurvesDir();
  try {
    const proposal = await proposeCalibrationRefit({
      locale: "DE-DE",
...
void test("ratifyOrRollback: refuses to ratify without a signing key", async () => {
  const dir = await makeTmpCurvesDir();
  try {
    const proposal = await proposeCalibrationRefit({
      locale: "DE-DE",
...
void test("ratifyOrRollback: ratifies a passing proposal, signs it, promotes to production", async () => {
  const dir = await makeTmpCurvesDir();
  try {
    const proposal = await proposeCalibrationRefit({
      locale: "DE-DE",
    ...
    const ratified = await ratifyOrRollback(proposal, {
      curvesDir: dir,
      decidedAt: "2026-05-11T01:00:00.000Z",
      signKeyPath: await operatorKeyPath(),
    });
...
void test("ratifyOrRollback: enforces operator key allowlist", async () => {
  const dir = await makeTmpCurvesDir();
  try {
    const proposal = await proposeCalibrationRefit({
      locale: "DE-DE",
...
void test("verifyCalibrationOperatorSignature: returns false when proposal body has been tampered", async () => {
  const dir = await makeTmpCurvesDir();
  try {
    const proposal = await proposeCalibrationRefit({
      locale: "DE-DE",
...
void test("assertCalibrationRefitSafety: passes on an empty calibration-curves dir", async () => {
  const dir = await makeTmpCurvesDir();
  try {
...
void test("assertCalibrationRefitSafety: passes after a legitimate ratification", async () => {
  const dir = await makeTmpCurvesDir();
  try {
    const proposal = await proposeCalibrationRefit({
      locale: "DE-DE",
...
void test("assertCalibrationRefitSafety: rejects a hand-edited production curve with no backing proposal", async () => {
  const dir = await makeTmpCurvesDir();
  try {
...
void test("assertCalibrationRefitSafety: rejects production curve when its digest disagrees with the ratified proposal", async () => {
  const dir = await makeTmpCurvesDir();
  try {
    const proposal = await proposeCalibrationRefit({
      locale: "DE-DE",
...
void test("loadCalibrationRefitHistory: enumerates production curves, proposals, and rejections", async () => {
  const dir = await makeTmpCurvesDir();
  try {
...
void test("self-improving-calibration: default held-out fraction is the published 20%", () => {
