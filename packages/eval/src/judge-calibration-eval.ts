 ...
 * `packages/core-engine/src/fixtures/judge-calibration/`:
 *
...
export const partitionSamplesByJudge = (
  ...
  Record<JudgeCalibrationJudgeId, ReadonlyArray<JudgeCalibrationSample>>
> => {
  const buckets: Record<JudgeCalibrationJudgeId, JudgeCalibrationSample[]> = {
    logic: [],
    faithfulness: [],
  };
...
export const JUDGE_CALIBRATION_FIXTURE_DIRNAME =
  "packages/core-engine/src/fixtures/judge-calibration" as const;
...
const isVerdictLabel = (
  value: unknown,
): value is JudgeCalibrationVerdictLabel =>
  typeof value === "string" &&
...
const parseGoldVerdict = (
  ...
  if (typeof value["reviewer"] !== "string" || value["reviewer"].length === 0) {
    throw new Error(
      `gold ${fixtureId}: goldVerdicts[${index}].reviewer is required`,
    );
  }
  if (!isVerdictLabel(value["verdict"])) {
    throw new Error(
      `gold ${fixtureId}: goldVerdicts[${index}].verdict is invalid`,
    );
  }
...
const parseAdjudication = (
  ...
  if (typeof value["rationale"] !== "string") {
    throw new Error(
      `gold ${fixtureId}: adjudication.rationale must be a string`,
    );
  }
  if (
    typeof value["timestamp"] !== "string" ||
...
const sortedFindingCodes = (
  codes: ReadonlyArray<string>,
): ReadonlyArray<string> => [...codes].sort();
...
const parseGold = (raw: unknown, fixtureId: string): JudgeCalibrationGold => {
  ...
  if (!isVerdictLabel(mockObj["predictedVerdict"])) {
    throw new Error(
      `gold ${fixtureId}: invalid mockJudgeResponse.predictedVerdict`,
    );
  }
...
const parseInput = (raw: unknown, fixtureId: string): JudgeCalibrationInput => {
  if (typeof raw !== "object" || raw === null) {
...
export const loadAllJudgeCalibrationFixtures = async (): Promise<
  ReadonlyArray<LoadedJudgeCalibrationFixture>
> => {
...
const isHistoryEntry = (
  value: unknown,
): value is JudgeCalibrationHistoryEntry => {
