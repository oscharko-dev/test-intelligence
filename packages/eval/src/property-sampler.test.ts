...
void test("sampler: every Wave-2 active-dataset invariant has a registered factory", () => {
...
void test("sampler: produces deterministic seeds for the active-dataset registry", () => {
  const registry = buildActiveDatasetInvariantRegistry();
...
void test("sampler: seeds carry every Wave-2 active-dataset invariant id", () => {
  const registry = buildActiveDatasetInvariantRegistry();
...
void test("sampler: rejects out-of-range run counts", () => {
  const registry = buildActiveDatasetInvariantRegistry();
...
void test("sampler: skips invariants without a registered factory", () => {
