...
void test("evidence-attestation: statement carries pinned URIs and schema version", () => {
...
void test("evidence-attestation: hard invariants are type-level false on predicate", () => {
  const manifest = fakeManifest();
  const statement = buildWave1ValidationAttestationStatement({
    manifest,
    manifestSha256: ZERO,
...
void test("evidence-attestation: subjects include manifest + every artifact, sorted", () => {
  const manifest = fakeManifest();
  const statement = buildWave1ValidationAttestationStatement({
    manifest,
    manifestSha256: ZERO,
...
void test("evidence-attestation: statement is byte-stable across runs (deterministic)", () => {
  const manifest = fakeManifest();
...
void test("evidence-attestation: rejects manifest with rawScreenshotsIncluded=true", () => {
  const manifest = fakeManifest();
...
void test("evidence-attestation: rejects manifest with imagePayloadSentToTestGeneration=true", () => {
  const manifest = fakeManifest();
  (
...
void test("evidence-attestation: rejects non-sha256 manifest digest", () => {
  const manifest = fakeManifest();
  assert.throws(
    () =>
      buildWave1ValidationAttestationStatement({
...
void test("evidence-attestation: rejects unknown signing mode", () => {
  const manifest = fakeManifest();
  assert.throws(
    () =>
      buildWave1ValidationAttestationStatement({
...
void test("evidence-attestation: predicate carries visual sidecar identity when present", () => {
  const manifest = fakeManifest({
    modelDeployments: {
      testGeneration: "gpt-oss-120b-mock",
      visualPrimary: "llama-4-maverick-vision",
  ...
  assert.equal(
    statement.predicate.visualSidecar.visualFallback,
...
void test("evidence-attestation: predicate omits visual sidecar when absent on manifest", () => {
  const manifest = fakeManifest();
  const statement = buildWave1ValidationAttestationStatement({
    manifest,
    manifestSha256: ZERO,
...
void test("evidence-attestation: encode payload returns canonical-JSON UTF-8 bytes", () => {
  const manifest = fakeManifest();
  const statement = buildWave1ValidationAttestationStatement({
    manifest,
    manifestSha256: ZERO,
...
void test("evidence-attestation: PAE prefix follows DSSEv1 format", () => {
...
void test("evidence-attestation: PAE binds payloadType (different types → different bytes)", () => {
...
void test("evidence-attestation: unsigned envelope has empty signatures and base64 payload", () => {
  const manifest = fakeManifest();
  const statement = buildWave1ValidationAttestationStatement({
    manifest,
    manifestSha256: ZERO,
...
void test("evidence-attestation: envelope digest is byte-stable across runs", () => {
  const manifest = fakeManifest();
  const statement = buildWave1ValidationAttestationStatement({
    manifest,
    manifestSha256: ZERO,
...
void test("evidence-attestation: rejects malformed manifest identity fields", () => {
  for (const field of [
    "promptHash",
    "schemaHash",
    "inputHash",
...
void test("evidence-attestation: verifier fails closed for malformed envelopes", async () => {
  const manifest = fakeManifest();
  const statement = buildWave1ValidationAttestationStatement({
    manifest,
    manifestSha256: ZERO,
  ...
  const cases: Array<{ envelope: unknown; code: string }> = [
    ...
    {
      envelope: {
        payloadType: "application/x-wrong",
        payload: "",
        signatures: [],
      },
  ...
  assert.equal(
    unsignedWithSignature.failures.some(
      (failure) =>
        failure.code === "signature_unsigned_envelope_carries_signatures",
    ),
    true,
  );
