...
void test("evidence-tampering: round-trip success — build + write + verify returns ok", async () => {
...
void test("evidence-tampering: multi-artifact mutation — ALL mutations reported, not just first", async () => {
  await withDir(async (dir) => {
...
void test("evidence-tampering: byte-length resize (additive append) is detected", async () => {
  await withDir(async (dir) => {
    const original = '{"v":1}';
...
void test("evidence-tampering: byte-length resize (truncate) is detected", async () => {
  await withDir(async (dir) => {
    const original =
...
void test("evidence-tampering: manifest modelDeployments mutation is detected on re-verify", async () => {
...
void test("evidence-tampering: valid-looking manifest metadata rewrite is detected by default disk verify", async () => {
  await withDir(async (dir) => {
    const content = '{"v":1}';
    await writeArtifact(dir, "alpha.json", content);
    ...
    const defaultVerification =
      await verifyWave1ValidationEvidenceFromDisk(dir);
    const { result } = await verifyWave1ValidationEvidenceFromDisk(dir, {
...
void test("evidence-tampering: rawScreenshotsIncluded mutation from false to true is detected", async () => {
...
void test("evidence-tampering: filename with null byte is refused", () => {
  assert.throws(
    () =>
      buildWave1ValidationEvidenceManifest(
        baseInput([
...
void test("evidence-tampering: every C0 control character (0x00–0x1F) + DEL (0x7F) in filename is refused", () => {
  const codes: number[] = [];
...
// source.
void test("evidence-tampering: whitespace control chars (tab, LF, CR, VT, FF) in filename are refused", () => {
  for (const ch of ["\t", "\n", "\r", "\v", "\f"]) {
    assert.throws(
      () =>
        buildWave1ValidationEvidenceManifest(
...
void test("evidence-tampering: filename with a lone high surrogate is refused", () => {
  assert.throws(
    () =>
      buildWave1ValidationEvidenceManifest(
        baseInput([
...
void test("evidence-tampering: filename with a lone low surrogate is refused", () => {
  assert.throws(
    () =>
      buildWave1ValidationEvidenceManifest(
        baseInput([
...
void test("evidence-tampering: filename ending with an unpaired high surrogate is refused", () => {
  assert.throws(
    () =>
      buildWave1ValidationEvidenceManifest(
        baseInput([
...
void test("evidence-tampering: filename with a high surrogate followed by a non-low-surrogate is refused", () => {
...
void test("evidence-tampering: filename with a valid astral character (paired surrogates) is accepted", () => {
  const manifest = buildWave1ValidationEvidenceManifest(
    baseInput([
...
// them by hand.
void test("evidence-tampering: property — any unsafe code unit in any position is refused", () => {
  fc.assert(
    fc.property(
      fc.oneof(
...
void test("evidence-tampering: verifier rejects manifests whose artifact entries carry unsafe filenames", async () => {
  await withDir(async (dir) => {
    const content = '{"v":1}';
    await writeArtifact(dir, "alpha.json", content);
...
void test("evidence-tampering: verifier rejects manifests whose artifact entries carry lone surrogates", async () => {
  await withDir(async (dir) => {
    const content = '{"v":1}';
    await writeArtifact(dir, "alpha.json", content);
    const validManifest = buildWave1ValidationEvidenceManifest(
...
void test("evidence-tampering: filename with directory traversal (../) is refused", () => {
  assert.throws(
    () =>
      buildWave1ValidationEvidenceManifest(
        baseInput([
...
void test("evidence-tampering: absolute filename is refused", () => {
...
void test("evidence-tampering: filename longer than 255 bytes is refused", () => {
...
void test("evidence-tampering: identical bytes in two artifacts produce identical sha256", () => {
...
void test("evidence-tampering: different bytes produce different sha256", () => {
  const manifest = buildWave1ValidationEvidenceManifest(
    baseInput([
...
void test("evidence-tampering: rejectUnexpected catches extra file dropped in run dir", async () => {
  await withDir(async (dir) => {
    const content = '{"v":1}';
    await writeArtifact(dir, "alpha.json", content);
...
void test("evidence-tampering: rejectUnexpected=false ignores extra files (default behaviour)", async () => {
  await withDir(async (dir) => {
    const content = '{"v":1}';
    await writeArtifact(dir, "alpha.json", content);
...
void test("evidence-tampering: missing artifact (deleted post-write) is detected", async () => {
  await withDir(async (dir) => {
    const aContent = '{"a":1}';
    const bContent = '{"b":2}';
