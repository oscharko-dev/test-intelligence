...
const synthesizeSelfSignedCertPem = (
  ...
  const cn = enc(
    UTF8STR,
    new TextEncoder().encode("wave1-validation-keyless-test"),
  );
...
const buildStubKeylessCallback = (): {
  ...
  const certPubKeyPem = (
    parsed.publicKey.export({ format: "pem", type: "spki" }) as string
  ).trim();
  assert.equal(
    certPubKeyPem,
    cert.publicKeyPem,
...
void test("evidence-attestation [keyless]: scaffold round-trips with a stub callback", async (t) => {
  ...
  assert.equal(
    signed.envelope.payloadType,
    WAVE1_VALIDATION_ATTESTATION_PAYLOAD_TYPE,
  );
});

void test("evidence-attestation [keyless]: persisted bundle verifies end-to-end via cert chain", async (t) => {
  const fx = await setupScenario();
  t.after(() => fx.cleanup());
  const stub = buildStubKeylessCallback();
  const signer = createKeylessSigstoreSignerScaffold({
...
void test("evidence-attestation [keyless]: invalid certificate-chain content fails closed", async (t) => {
  const fx = await setupScenario();
  t.after(() => fx.cleanup());
  const stub = buildStubKeylessCallback();
  const signer = createKeylessSigstoreSignerScaffold({
...
void test("evidence-attestation [keyless]: scaffold rejects invalid signer reference", () => {
  assert.throws(
    () =>
      createKeylessSigstoreSignerScaffold({
...
void test("evidence-attestation [keyless]: scaffold rejects callback returning non-Uint8Array signature", async () => {
  const stub = buildStubKeylessCallback();
  const signer = createKeylessSigstoreSignerScaffold({
    signerReference: "ci-build-keyless-non-bytes",
    // @ts-expect-error — runtime-only validation: callback returns a
    // non-Uint8Array signature to exercise the runtime guard.
    callback: async () => ({
