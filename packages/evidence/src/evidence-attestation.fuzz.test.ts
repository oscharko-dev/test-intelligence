...
void test("fuzz: PAE encoding starts with literal DSSEv1 prefix for any payload", () => {
  fc.assert(
    fc.property(
      fc.uint8Array({ maxLength: 4096 }),
      fc.string({ maxLength: 64 }).filter(
        // Control characters are deliberately excluded so the generated
        // payloadType is a well-formed DSSE type string.
...
void test("fuzz: PAE length tail equals payload byte length and content", () => {
  fc.assert(
...
void test("fuzz: distinct payloadType inputs always produce distinct PAE bytes", () => {
  fc.assert(
    fc.property(
...
void test("fuzz: base64 round-trip preserves arbitrary bytes (encode→decode)", () => {
  fc.assert(
...
void test("fuzz: signed envelope verifies for any well-formed jobId", async () => {
  ...
  await fc.assert(
    fc.asyncProperty(
      ...
      async (jobId) => {
        ...
        return result.signatureCount === 1 && result.signaturesVerified;
      },
    ),
    { numRuns: 30 },
  );
...
void test("fuzz: any single byte flip in DSSE payload invalidates signature", async () => {
  const { privateKeyPem, publicKeyPem } =
    generateWave1ValidationAttestationKeyPair();
  const signer = createKeyBoundSigstoreSigner({
...
void test("fuzz: unsigned envelope verifier rejects ANY signature insertion", async () => {
  await fc.assert(
    fc.asyncProperty(
      fc
...
void test("fuzz: invalid base64 in signature.sig fails closed (no exception leak)", async () => {
  const { privateKeyPem, publicKeyPem } =
    generateWave1ValidationAttestationKeyPair();
  const signer = createKeyBoundSigstoreSigner({
