...
void test("attestation-tampering: subject digest mismatch when artifact is mutated", async (t) => {
...
void test("attestation-tampering: unsafe artifact filenames fail closed with escaped diagnostics", async (t) => {
  const fx = await setupScenario();
  t.after(() => fx.cleanup());
  const statement = buildWave1ValidationAttestationStatement({
    manifest: fx.manifest,
  ...
  for (const { unsafeFilename, escapedReference } of [
    ...
    const envelope =
      buildUnsignedWave1ValidationAttestationEnvelope(tamperedStatement);
...
void test("attestation-tampering: payload byte mutation invalidates signature (sigstore)", async (t) => {
  const fx = await setupScenario();
  t.after(() => fx.cleanup());
...
void test("attestation-tampering: signature byte flip fails verification", async (t) => {
  const fx = await setupScenario();
  t.after(() => fx.cleanup());
  const { privateKeyPem, publicKeyPem } =
    generateWave1ValidationAttestationKeyPair();
...
void test("attestation-tampering: missing envelope file fails closed", async (t) => {
  const fx = await setupScenario();
  t.after(() => fx.cleanup());
...
void test("attestation-tampering: missing bundle in sigstore mode fails closed", async (t) => {
  const fx = await setupScenario();
  t.after(() => fx.cleanup());
  const { privateKeyPem, publicKeyPem } =
    generateWave1ValidationAttestationKeyPair();
...
void test("attestation-tampering: unparseable envelope JSON fails closed", async (t) => {
  const fx = await setupScenario();
  t.after(() => fx.cleanup());
  const statement = buildWave1ValidationAttestationStatement({
    manifest: fx.manifest,
...
void test("attestation-tampering: predicate jobId rewrite is detected", async (t) => {
  const fx = await setupScenario();
  t.after(() => fx.cleanup());
  const statement = buildWave1ValidationAttestationStatement({
    manifest: fx.manifest,
...
void test("attestation-tampering: predicate hard invariant rewrite (rawScreenshotsIncluded=true) detected", async (t) => {
  const fx = await setupScenario();
  t.after(() => fx.cleanup());
  const statement = buildWave1ValidationAttestationStatement({
    manifest: fx.manifest,
...
void test("attestation-tampering: bundle file replaced with attacker bundle is detected", async (t) => {
  const fx = await setupScenario();
  t.after(() => fx.cleanup());
...
void test("attestation-tampering: requireFullSubjectCoverage catches dropped subject", async (t) => {
  const fx = await setupScenario();
  t.after(() => fx.cleanup());
  const statement = buildWave1ValidationAttestationStatement({
    manifest: fx.manifest,
  ...
  const tamperedEnvelope: Wave1ValidationAttestationDsseEnvelope = {
    ...
    // pinned payloadType remains unchanged under tampering
