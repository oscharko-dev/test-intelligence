...
import { computeWave1ValidationEvidenceManifestDigest } from "@oscharko-dev/ti-evidence";
...
for (const fixtureId of WAVE1_VALIDATION_FIXTURE_IDS) {
  void test(`validation-harness rubric: ${fixtureId} replay is byte-stable`, async () => {
    ...
    try {
      ...
      const aBytes = await readFile(a.selfVerifyRubricArtifactPath!, "utf8");
      const bBytes = await readFile(b.selfVerifyRubricArtifactPath!, "utf8");
...
void test("validation-harness rubric: disabled run produces no rubric artifact (byte-stable disabled path)", async () => {
