...
void test("subprocessor-register: artifact filename is the canonical name", () => {
  assert.equal(
    SUBPROCESSOR_REGISTER_ARTIFACT_FILENAME,
...
void test("subprocessor-register: builder embeds schema + register versions", () => {
...
void test("subprocessor-register: canonical JSON output is byte-stable", () => {
...
void test("subprocessor-register: matches the pinned canonical-JSON fixture", async () => {
...
void test("subprocessor-register: subprocessor list is sorted by stable id", () => {
  const register = buildSubprocessorRegister({
    generatedAt: PINNED_GENERATED_AT,
  });
  ...
  assert.deepEqual(
    ids,
    sorted,
    "subprocessor ids must serialise in sorted order",
  );
});

void test("subprocessor-register: cross-border transfers are sorted by transfer id", () => {
  const register = buildSubprocessorRegister({
    generatedAt: PINNED_GENERATED_AT,
  });
...
void test("subprocessor-register: register entries are deeply frozen", () => {
  const register = buildSubprocessorRegister({
    generatedAt: PINNED_GENERATED_AT,
  });
...
void test("subprocessor-register: every entry covers an Azure / Mistral region from #2099 routing", () => {
  const register = buildSubprocessorRegister({
    generatedAt: PINNED_GENERATED_AT,
  });
...
void test("subprocessor-register: merkle root changes when a subprocessor entry mutates", () => {
...
void test("subprocessor-register: merkle root is a 64-character hex digest", () => {
  const register = buildSubprocessorRegister({
    generatedAt: PINNED_GENERATED_AT,
  });
...
void test("subprocessor-register: Markdown renderer is deterministic", () => {
  const register = buildSubprocessorRegister({
    generatedAt: PINNED_GENERATED_AT,
  });
  assert.equal(
...
void test("subprocessor-register: Markdown rendering matches the canonical TS source", () => {
  const register = buildSubprocessorRegister({
    generatedAt: PINNED_GENERATED_AT,
  });
  ...
  assert.equal(
    `${renderSubprocessorRegisterMarkdown(register)}\n`,
    expected,
    "subprocessor register Markdown rendering drifted from the canonical TS source.",
  );
});
