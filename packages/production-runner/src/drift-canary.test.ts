...
import { loadBaselineArchetypeFixture } from "@oscharko-dev/ti-core-engine";
import {
...
void test("drift-canary: computeBrierScore returns the mean squared error", () => {
  assert.equal(
    computeBrierScore([
...
void test("drift-canary: computeExpectedCalibrationError bins confidence deltas", () => {
  assert.equal(
    computeExpectedCalibrationError([
...
void test("drift-canary: computeExpectedCalibrationError applies debiasing within populated bins", () => {
  const samples = [
...
void test("drift-canary: computeExpectedCalibrationError stays decile-bucketed at a bin boundary", () => {
  const samples = [
...
void test("drift-canary: holdout fixture set is explicitly pinned", () => {
...
void test("drift-canary: provider fingerprint prompts remain five text-safe probes", () => {
  assert.equal(PROVIDER_FINGERPRINT_PROMPTS.length, 5);
  assert.equal(
    PROVIDER_FINGERPRINT_PROMPTS.every((prompt) => !prompt.expectsImageInput),
    true,
  );
});

void test("drift-canary: fixture snapshots preserve pinned holdout intent semantics", async () => {
...
void test("drift-canary: metric drift alerts on >2σ changes and absolute Brier deltas", () => {
...
void test("drift-canary: absolute ECE threshold breaches fail the canary", () => {
  const evaluation = evaluateDriftReport({
...
void test("drift-canary: ECE absolute gate skips classes below the documented sample floor", () => {
  const evaluation = evaluateDriftReport({
    baseline: emptyBaselineState({
      tenantId: "default",
      policyProfileId: "eu-banking-default",
...
void test("drift-canary: ECE absolute gate still fires once the sample floor is reached", () => {
  const evaluation = evaluateDriftReport({
    baseline: emptyBaselineState({
      tenantId: "default",
      policyProfileId: "eu-banking-default",
...
void test("drift-canary: per-risk ECE findings stay risk-scoped and Brier keeps the absolute gate", () => {
  const baseline = appendDriftBaselineRecord(
    appendDriftBaselineRecord(
      emptyBaselineState({
        tenantId: "default",
...
void test("drift-canary: provider fingerprint drift alerts when hashes change under a stable revision", () => {
  const baseline = appendDriftBaselineRecord(
    emptyBaselineState({
      tenantId: "default",
      policyProfileId: "eu-banking-default",
...
void test("drift-canary: classifyCrossFamilyCorrelatedDrift emits a synthesized finding", () => {
...
void test("drift-canary: writes and reloads baseline state atomically", async () => {
...
void test("drift-canary: default alert sink writes drift-alerts.json", async () => {
...
void test("drift-canary: computeDriftCanaryMetrics emits per-locale brier/ece observations when screenLocaleMap is provided", () => {
  ...
  const run: CanaryFixtureRun = {
    ...
    result: {
      generatedTestCases: {
        ...
        testCases: [
          buildMinimalCase("tc-de-1", screenIdDE, 0.8),
          buildMinimalCase("tc-de-2", screenIdDE, 0.7),
          buildMinimalCase("tc-fr-1", screenIdFR, 0.3),
        ] as never,
      },
      policy: {
        decisions: [
        ...
        screens: [
          {
            screenId: screenIdDE,
            screenName: "DE",
            trace: { nodeId: screenIdDE },
          },
          {
            screenId: screenIdFR,
            screenName: "FR",
            trace: { nodeId: screenIdFR },
          },
        ],
        detectedFields: [],
        detectedActions: [],
  ...
  assert.notEqual(
    baseEce,
    undefined,
    "Base ECE observation (no locale) must be emitted",
  );
  ...
  assert.notEqual(
    deDeEce,
    undefined,
    "DE-DE locale ECE observation must be emitted",
  );
  ...
  assert.notEqual(
    frFrEce,
    undefined,
    "FR-FR locale ECE observation must be emitted",
  );
