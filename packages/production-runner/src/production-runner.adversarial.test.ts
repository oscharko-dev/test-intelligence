...
import type { CustomerProfileInput } from "@oscharko-dev/ti-multi-source";
...
void test("production runner adversarial: rejects SSRF/IMDS/RFC1918 URLs and redirect-to-internal fetches", async () => {
...
void test("production runner adversarial: prompt-injection node names stay in quoted user data, never the system prompt", async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "ti-runner-adv-"));
...
void test("production runner adversarial: oversized Figma payloads above the default cap fail closed with a bounded refusal envelope", async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "ti-runner-adv-"));
  try {
    const client = createMockLlmGatewayClient({
      role: "test_generation",
...
void test("production runner adversarial: persisted artifacts never leak figma tokens, query tokens, or bearer strings", async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "ti-runner-adv-"));
  const originalFetch = globalThis.fetch;
...
void test("production runner adversarial: corrupted persistent replay-cache entries are refused and never silently reused", async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "ti-runner-adv-"));
...
void test("production runner adversarial: cross-tenant replay-cache misconfiguration aborts the run with TenantIsolationViolation (Issue #2176)", async () => {
...
void test("production runner adversarial: tenant-isolation-attestation.json is emitted and pinned in provenance.jsonld (Issue #2176)", async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "ti-runner-tiso-"));
  try {
    const client = createMockLlmGatewayClient({
      role: "test_generation",
...
void test("production runner adversarial: cancellation releases the gateway slot and emits a cancelled terminal event", async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "ti-runner-adv-"));
  let dispatches = 0;
  let slotInUse = false;
...
void test("production runner adversarial: customerProfile with HTML injection in glossary is rejected before LLM dispatch", async () => {
...
void test("production runner adversarial: customerProfile PII in glossary definition is redacted, not passed raw to LLM", async () => {
...
void test("production runner adversarial: customerProfile ictRegisterRef inheritance satisfies policy gate", async () => {
