...
void test("export-pipeline: refuses when no test cases are approved", () => {
...
void test("export-pipeline: ACCEPTANCE — unapproved test cases cannot be exported (regression)", () => {
...
void test("export-pipeline: refuses when validation reports schema_invalid", () => {
  const result = runExportPipeline({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    intent: buildIntent(),
...
void test("export-pipeline: semantic override permits export with audit-blocked validation report", () => {
...
void test("export-pipeline: refuses when visual sidecar is blocked", () => {
...
void test("export-pipeline: writes only export-report.json on refusal", async () => {
...
void test("export-pipeline: emits all required artifacts when every case is approved or rejected", async () => {
...
void test("export-pipeline: report has rawScreenshotsIncluded invariant set to false", () => {
  const result = runExportPipeline({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    intent: buildIntent(),
...
void test("export-pipeline: visual evidence hashes are sorted and unique", () => {
  const result = runExportPipeline({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    intent: buildIntent(),
...
void test("export-pipeline: deterministic output across two runs", () => {
  const inputs = {
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    intent: buildIntent(),
