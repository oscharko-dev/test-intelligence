...
interface SealJson {
  ...
  finopsArtifactFilename: string;
  visualEvidenceHashes: Array<{
    screenId: string;
...
const buildFixtureBundle = async (input?: {
  ...
  const finopsReport = {
    ...
    bySource: {
      ...
      visual_fallback: emptyPerSourceCost,
      coverage_planner: emptyPerSourceCost,
      risk_ranker: emptyPerSourceCost,
      adversarial_critic: emptyPerSourceCost,
      gap_finder: emptyPerSourceCost,
      repair_planner: emptyPerSourceCost,
      ir_mutation_oracle: emptyPerSourceCost,
    },
...
void test("seal-verifier: happy path returns OK with merkle root and HMAC", async () => {
  ...
  try {
    ...
    assert.ok(
      ok.length >= 3,
      `expected at least 3 OK artifacts, got ${String(ok.length)}`,
    );
    for (const cc of report.crossChecks) {
      if (
        cc.name === "finops_bySource_hash" ||
        cc.name === "genealogy_dag_hash"
      ) {
...
void test("seal-verifier: tampered artifact yields TAMPERED + artifact_tampered failure", async () => {
  const fixture = await buildFixtureBundle();
  try {
    await writeFile(
    ...
    const code = report.failures.find((f) => f.code === "artifact_tampered");
...
void test("seal-verifier: missing artifact yields MISSING + artifact_missing failure", async () => {
  const fixture = await buildFixtureBundle();
  try {
...
void test("seal-verifier: bundle-extra files reported as EXTRA without failure", async () => {
  const fixture = await buildFixtureBundle();
  try {
    await writeFile(
    ...
    const report = await verifySealBundle({ bundleDir: fixture.runDir });
    assert.equal(report.ok, true, JSON.stringify(report.failures));
    const extra = report.artifacts.find(
      (a) => a.reference === "stray-file.json",
    );
...
void test("seal-verifier: missing seal returns seal_missing failure", async () => {
...
void test("seal-verifier: malformed seal returns seal_unparseable failure", async () => {
...
void test("seal-verifier: --expected-merkle-root mismatch produces merkle_root_mismatch", async () => {
  const fixture = await buildFixtureBundle();
  try {
...
void test("seal-verifier: --expected-hmac mismatch produces hmac_mismatch", async () => {
  const fixture = await buildFixtureBundle();
  try {
    const report = await verifySealBundle({
      bundleDir: fixture.runDir,
...
void test("seal-verifier: explicit key + matching expected-hmac round-trip succeeds", async () => {
  const fixture = await buildFixtureBundle();
  try {
    ...
    const seal = JSON.parse(Buffer.from(sealBytes).toString("utf8")) as unknown;
...
void test("seal-verifier: provenance.jsonld cross-link verifies consistently", async () => {
...
void test("seal-verifier: region-attestations cross-link verifies against FinOps deployment ids", async () => {
...
void test("seal-verifier: nested run-dir inside bundle root is auto-discovered", async () => {
  const fixture = await buildFixtureBundle();
  try {
    const wrapper = await mkdtemp(
      path.join(os.tmpdir(), "seal-verifier-wrap-"),
    );
    try {
      const innerDir = path.join(wrapper, "run-2178");
      await mkdir(innerDir, { recursive: true });
      // Move all fixture files into innerDir.
      const cp = spawnSync("cp", ["-R", `${fixture.runDir}/.`, innerDir], {
        stdio: "ignore",
      });
...
// Skipped in the Issue #9 port: exercises extractSealBundleArchive from test-intelligence-run-cli.ts, a CLI module owned by Issue #20 and outside Issue #9's gateway/routing/FinOps connected component.
void test(
  "seal-verifier: tar.gz archive is extracted and verified",
  {
    skip: "Exercises extractSealBundleArchive from test-intelligence-run-cli.ts; the CLI module is owned by Issue #20 (CLI) and is outside Issue #9's gateway/routing/FinOps scope. Restored when the CLI extraction issue lands.",
  },
  () => {},
);

void test("assertReplayDeterminismVerifiedFromDisk: throws G9 hard-gate error on tamper", async () => {
  const fixture = await buildFixtureBundle();
  try {
    await writeFile(
      path.join(fixture.runDir, fixture.genealogyFilename),
    ...
    assert.match((thrown as Error).message, /G9_REPLAY_DETERMINISM_VERIFIED/);
  } finally {
    await fixture.cleanup();
  }
});

void test("assertReplayDeterminismVerifiedFromDisk: succeeds on unmodified bundle", async () => {
  const fixture = await buildFixtureBundle();
  try {
    const report = await assertReplayDeterminismVerifiedFromDisk(
      fixture.runDir,
    );
...
void test("renderSealVerificationTextReport: includes per-artifact tags + cross-checks", async () => {
  const fixture = await buildFixtureBundle({ includeProvenance: true });
  try {
    const report = await verifySealBundle({ bundleDir: fixture.runDir });
...
void test("renderSealVerificationJsonReport: emits canonical JSON terminating in newline", async () => {
  const fixture = await buildFixtureBundle();
  try {
    const report = await verifySealBundle({ bundleDir: fixture.runDir });
    const text = renderSealVerificationJsonReport(report);
    assert.ok(text.endsWith("\n"));
    const parsed = JSON.parse(text.trimEnd()) as { ok: boolean; jobId: string };
...
void test("DEFAULT_SEAL_VERIFY_KEY_LABEL is the documented sentinel", () => {
  assert.equal(
    DEFAULT_SEAL_VERIFY_KEY_LABEL,
    "test-intelligence:seal-verify:v1",
  );
});

void test("seal-verifier: rejects collapsing-traversal artifact path like 'a/../../escape.json'", async () => {
  const fixture = await buildFixtureBundle();
  try {
...
void test("seal-verifier: rejects unsafe finopsArtifactFilename in cross-checks", async () => {
  const fixture = await buildFixtureBundle();
  try {
    const sealPath = path.join(
      fixture.runDir,
    ...
    assert.match(cc!.detail, /unsafe FinOps path|refusing to read outside/i);
  } finally {
    await fixture.cleanup();
  }
});

void test("seal-verifier: visual sidecar with non-array visualEvidenceRefs fails closed", async () => {
  const fixture = await buildFixtureBundle();
  try {
    const sealPath = path.join(
      fixture.runDir,
...
void test("seal-verifier: rejects seal that names an artifact outside the run dir", async () => {
  const fixture = await buildFixtureBundle();
  try {
    const sealPath = path.join(
      fixture.runDir,
...
void test("seal-verifier: visual-sidecar cross-check detects drift between seal hashes and sidecar refs", async () => {
  const fixture = await buildFixtureBundle();
  try {
    const sealPath = path.join(
      fixture.runDir,
...
void test("seal-verifier: visual-sidecar missing when seal references visuals fails closed", async () => {
  const fixture = await buildFixtureBundle();
  try {
    const sealPath = path.join(
      fixture.runDir,
...
void test("seal-verifier: checked-in fixture bundle verifies clean", async () => {
