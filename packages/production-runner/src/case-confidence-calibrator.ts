...
import type { TestDataOracleReport } from "@oscharko-dev/ti-agentic-harness";
...
export interface LocaleCurveEntry {
  ...
  readonly sampleCountByRiskCategory: Readonly<
...
export interface CaseConfidenceCurveArtifact {
   ...
   */
  readonly localeCurves: Readonly<
    Record<LocaleCalibrationKey, LocaleCurveEntry>
  >;
  /**
...
const PER_LOCALE_ECE_THRESHOLD = 0.1 as const;
...
const buildDefaultLocaleSampleCount = (): Readonly<
  Record<LocaleCalibrationKey, number>
> =>
  Object.fromEntries([
...
const buildAnchorForCase = (
  testCase: GeneratedTestCase,
  runId: string,
): HistoricalAcceptedAnchor => ({
  runId,
...
const deriveOracleResolved = (
  ...
  const projection = report.cases.find(
...
const deriveRagHitStrength = (
  ...
  for (const anchor of acceptedAnchors) {
    ...
    const score = Math.max(
      tokenScore >= 0.7 && (screenScore > 0 || fieldScore > 0) ? 1 : 0,
...
const fitPlattCurve = (input: {
  ...
  const trainingPositives = training.filter(
  ...
  if (trainingPositives === 0 || trainingNegatives === 0) {
    ...
    const earlyReturnLocaleCurves: Record<
  ...
  const evaluationSplit =
    heldOut.length > 0 ? "held_out" : "all_samples_fallback";
  const reliabilityArtifacts = buildReliabilityArtifacts({
    datasetId: input.datasetId,
    generatedAt: input.generatedAt,
    evaluationSplit,
    evaluationSamples: calibrateSamples(
      heldOut.length > 0 ? heldOut : sorted,
      curve,
    ),
  });

  const aggregateTrainingBrierScore = computeBrierScore(
    calibrateSamples(training, curve),
  );
  ...
  return {
    ...
    minimumRiskCategorySampleFloor: CALIBRATION_MIN_SAMPLE_FLOOR,
    heldOutSampleCountByRiskCategory:
      reliabilityArtifacts.sampleCountByRiskCategory,
    eceByRiskCategory: reliabilityArtifacts.eceByRiskCategory,
    eceThresholdByRiskCategory: CALIBRATION_ECE_THRESHOLDS,
    localeCurves,
    perLocaleEceThreshold: PER_LOCALE_ECE_THRESHOLD,
    ...
    ...(heldOut.length > 0
      ? {
          heldOutBrierScore: computeBrierScore(
            calibrateSamples(heldOut, curve),
          ),
        }
      : {}),
  };
...
const listHistoricalRunDirs = async (
  datasetRoot: string,
...
const buildHistoricalCalibrationSamples = (input: {
  ...
  for (const snapshot of input.snapshots) {
    for (const testCase of snapshot.list.testCases) {
      const label =
        input.explicitLabels?.labels.get(
          caseKey(snapshot.runId, testCase.id),
        ) ??
        (snapshot.policy.get(testCase.id) === "approved"
          ? "accepted"
          : "needs_review");
...
const writeReliabilityDiagramArtifacts = async (
  ...
  const artifactDir = dirname(
    resolveCaseConfidenceCurveArtifactPath(datasetRoot),
  );
...
const resolveCaseConfidenceCurveArtifactPath = (
  datasetRoot: string,
): string => {
...
export const loadCaseConfidenceCalibration = async (
  ...
  const samples = buildHistoricalCalibrationSamples({
    ...
    ...(input.screenLocaleMap !== undefined
      ? { screenLocaleMap: input.screenLocaleMap }
      : {}),
  });
  const curve =
    samples.length === 0
  ...
  const localeReliabilityArtifactPaths:
    | Readonly<Partial<Record<LocaleCalibrationKey, string>>>
    | undefined =
    input.screenLocaleMap !== undefined
