...
import { type BaselineArchetypeFixtureId } from "@oscharko-dev/ti-core-engine";
import type { IntentDerivationFigmaInput } from "@oscharko-dev/ti-core-engine";
import { computeFaithfulnessMetrics } from "./faithfulness-eval.js";
import { computeHallucinationMetrics } from "./hallucination-eval.js";
import type { JudgeCalibrationEvalArtifact } from "@oscharko-dev/ti-eval";
...
export const computeDriftCanaryMetrics = (input: {
  ...
  const samplesByRiskLocale = new Map<
    string,
    {
      ...
      locale: LocaleCalibrationKey;
      samples: Array<{ confidence: number; label: 0 | 1 }>;
    }
  >();
  ...
  for (const run of input.runs) {
    ...
    const samples = run.result.generatedTestCases.testCases.map((testCase) => {
      ...
      return {
        ...
        label:
          decisions.get(testCase.id) === "approved"
            ? (1 as const)
            : (0 as const),
        locale,
      };
...
export const loadDriftBaselineState = async (input: {
  ...
  try {
    ...
    if (
      ...
      throw new Error(`drift-canary: baseline identity mismatch at ${path}`);
    }
    return {
      schemaVersion: DRIFT_CANARY_SCHEMA_VERSION,
      tenantId: input.tenantId,
...
export const runProviderFingerprintCanary = async (input: {
  ...
  for (const prompt of input.prompts ?? PROVIDER_FINGERPRINT_PROMPTS) {
    ...
    const request: LlmGenerationRequest = {
      ...
      responseSchemaName: "test-intelligence-drift-canary-fingerprint-v1",
      responseSchema: {
        type: "object",
