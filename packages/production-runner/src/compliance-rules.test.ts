...
void test("registry covers every framework id exactly once", () => {
  const keys = Object.keys(COMPLIANCE_RULE_PACK_REGISTRY).sort();
  const expected = [...COMPLIANCE_FRAMEWORK_IDS].sort();
  assert.deepEqual(keys, expected);
});

void test("each shipped pack passes schema validation and is deep-frozen", () => {
...
void test("validateComplianceRulePack rejects malformed inputs", () => {
  assert.throws(
    () =>
      validateComplianceRulePack({
...
void test("isComplianceFrameworkId is a strict guard", () => {
...
void test("parseComplianceFrameworksFlag accepts both kebab- and snake-case", () => {
...
void test("parseComplianceFrameworksFlag rejects empty / unknown tokens", () => {
  assert.throws(
    () => parseComplianceFrameworksFlag(""),
...
void test("resolveActiveFrameworks defaults from the eu-banking-default profile", () => {
...
void test("resolveActiveFrameworks honours an explicit selection", () => {
  const active = resolveActiveFrameworks(["PSD2", "GDPR"], "eu-banking-default");
  assert.deepEqual([...active], ["PSD2", "GDPR"]);
});

void test("resolveActiveFrameworks returns the full registry on unknown profile", () => {
  const active = resolveActiveFrameworks(undefined, "unknown-profile");
  assert.deepEqual([...active], [...COMPLIANCE_FRAMEWORK_IDS]);
});

void test("resolveActiveFrameworks rejects an empty explicit selection", () => {
  assert.throws(
...
void test("getComplianceRulePack returns the same instance for the same id", () => {
...
void test("PSD2 SCA Article 97 is shipped as an error-severity rule", () => {
