...
void test("role identifier is the documented constant", () => {
  assert.equal(COMPLIANCE_ANNOTATOR_ROLE_ID, "compliance_annotator");
});

void test("annotateTestCases emits stable schemaVersion and frozen entries", () => {
...
void test("PSD2 SCA case applies and satisfies the mandatory test classes", () => {
  const artifact = annotateTestCases({
    jobId: "job-1",
    generatedAt: "2026-01-01T00:00:00Z",
    testCases: [
...
void test("non-matching cases are emitted with empty appliesTo arrays", () => {
  const artifact = annotateTestCases({
    jobId: "job-1",
    generatedAt: "2026-01-01T00:00:00Z",
    testCases: [
...
void test("matches across multiple frameworks are merged on a single case", () => {
  const artifact = annotateTestCases({
    jobId: "job-1",
    generatedAt: "2026-01-01T00:00:00Z",
    testCases: [
