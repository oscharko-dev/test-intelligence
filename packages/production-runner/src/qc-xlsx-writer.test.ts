...
void test("xlsx: produces a valid zip with the expected entries", () => {
...
void test("xlsx: deterministic byte output across two runs with same input", () => {
...
void test("xlsx: header row contains every CSV column in declared order", () => {
  const buffer = renderQcXlsx([baseEntry({})]);
  const entries = parseZip(new Uint8Array(buffer));
...
void test("xlsx: workbook contains a single sheet named TestCases", () => {
  const buffer = renderQcXlsx([baseEntry({})]);
  const entries = parseZip(new Uint8Array(buffer));
...
void test("xlsx: per-entry CRC-32 is non-zero (byte content guard)", () => {
  const buffer = renderQcXlsx([baseEntry({})]);
  const entries = parseZip(new Uint8Array(buffer));
...
void test("neutralizeFormulaLeading prefixes formula-leader values with '", () => {
  const offenders = [
    `=cmd|'/c calc'!A1`,
...
void test("neutralizeFormulaLeading leaves benign values untouched", () => {
  const safe = [
...
void test("xlsx: rendered sheet bytes contain &apos; before a formula-leading cell", () => {
