...
void test("runA11yJudge returns mixed covered/weak/not-covered verdicts across at least four WCAG criteria", async () => {
  ...
  assert.equal(
    result.verdict.criteria.filter(
      (criterion) => criterion.verdict === "covered_weakly",
    ).length,
    1,
  );
  assert.equal(
    result.verdict.criteria.filter(
      (criterion) => criterion.verdict === "not_covered",
    ).length,
    1,
  );
...
void test("runA11yJudge skips cleanly when no a11yJudge slot is configured and preserves cache stability", async () => {
  const figma = await loadFigma("baseline-simple-form");
  const intent = deriveBusinessTestIntentIr({ figma });
  const screenId = intent.detectedFields[0]?.screenId;
  assert.ok(screenId, "fixture must expose a form screen");
...
void test("runA11yJudge fails closed on a hanging gateway call", async () => {
  const figma = await loadFigma("baseline-simple-form");
  const intent = deriveBusinessTestIntentIr({ figma });
  const screenId = intent.detectedFields[0]?.screenId;
  assert.ok(screenId, "fixture must expose a form screen");
