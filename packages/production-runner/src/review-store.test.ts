...
const validPersistedEnvelope = (
  events: unknown[] = [validPersistedReviewEvent()],
) => ({
  schemaVersion: REVIEW_GATE_SCHEMA_VERSION,
  contractVersion: TEST_INTELLIGENCE_CONTRACT_VERSION,
  jobId: "job-1",
...
void test("review-store: seed creates one event per test case + snapshot", async () => {
...
void test("review-store: seed ignores resolved multi-source conflicts when seeding review state", async () => {
...
void test("review-store: refreshPolicyDecisions removes resolved conflict review reasons", async () => {
...
void test("review-store: seed is idempotent on the same job", async () => {
...
void test("review-store: persists artifacts atomically with canonical names", async () => {
...
void test("review-store: rejects tampered persisted event envelopes", async () => {
  await withTempDir("rev-events-tampered", async (dir) => {
    ...
    const variants = [
      ...
      validPersistedEnvelope([{ ...validPersistedReviewEvent(), sequence: 0 }]),
      ...
      validPersistedEnvelope([{ ...validPersistedReviewEvent(), actor: 42 }]),
      validPersistedEnvelope([{ ...validPersistedReviewEvent(), note: 42 }]),
      validPersistedEnvelope([
        {
          ...validPersistedReviewEvent(),
...
void test("review-store: rejects tampered persisted snapshots", async () => {
  await withTempDir("rev-snapshot-tampered", async (dir) => {
    ...
    const variants = [
      { ...validPersistedSnapshot(), pendingSecondaryApprovalCount: 0.5 },
      {
        ...
        fourEyesPolicy: { requiredRiskCategories: ["high"] },
      },
      {
        ...validPersistedSnapshot(),
        perTestCase: [
...
void test("review-store: approve transition appends event and updates snapshot", async () => {
...
void test("review-store: rejects approval transition when policy is blocked", async () => {
...
void test("review-store: refreshPolicyDecisions lets an override-aware policy unblock approval", async () => {
...
void test("review-store: refreshPolicyDecisions preserves missing decisions fail-closed", async () => {
  await withTempDir("rev-refresh-partial", async (dir) => {
    ...
    assert.equal(refreshed.snapshot.perTestCase[0]?.policyDecision, "blocked");
...
void test("review-store: refreshPolicyDecisions rejects mismatched policy reports", async () => {
...
void test("review-store: rejects unknown test case id", async () => {
...
void test("review-store: refuses transition when snapshot missing", async () => {
...
void test("review-store: monotonic sequence across transitions", async () => {
...
void test("review-store: concurrent transitions on same job serialize correctly", async () => {
...
void test("review-store: rejects oversize note", async () => {
