...
void test("incident-classifier: contract enums match Issue #2114 spec", () => {
...
void test("incident-classifier: clean inputs emit zero events and review state ok", () => {
...
void test("incident-classifier: PII outcome on regulated_data case is critical and pauses pipeline", () => {
  const report = classifyIncidents({
    jobId: JOB_ID,
    observedAt: OBSERVED_AT,
...
void test("incident-classifier: PII outcome on a low-risk approved case stays below critical", () => {
  const report = classifyIncidents({
    jobId: JOB_ID,
    observedAt: OBSERVED_AT,
    validationReport: buildValidationReport(1),
...
void test("incident-classifier: judge disagreement outcomes derive judge_disagreement_persistent", () => {
  const report = classifyIncidents({
    jobId: JOB_ID,
    observedAt: OBSERVED_AT,
    validationReport: buildValidationReport(0),
...
void test("incident-classifier: compliance outcomes derive compliance_rule_pack_violation and bump on regulated risk", () => {
  const report = classifyIncidents({
    jobId: JOB_ID,
    observedAt: OBSERVED_AT,
    validationReport: buildValidationReport(0),
...
void test("incident-classifier: policy_gate_bypass signal is always critical and pauses pipeline", () => {
...
void test("incident-classifier: drift_alert / subprocessor_outage / replay_cache_miss signals default to high", () => {
...
void test("incident-classifier: events are deterministically sorted by severity desc, then category, then id", () => {
  const report = classifyIncidents({
    jobId: JOB_ID,
    observedAt: OBSERVED_AT,
...
void test("incident-classifier: rejects mismatched jobId between input and reports", () => {
  assert.throws(() =>
    classifyIncidents({
      jobId: JOB_ID,
      observedAt: OBSERVED_AT,
...
void test("incident-classifier: same input produces byte-identical reports", () => {
  const input = {
    jobId: JOB_ID,
    observedAt: OBSERVED_AT,
    validationReport: buildValidationReport(1),
