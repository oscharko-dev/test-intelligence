...
import { FAITHFULNESS_EVAL_FIXTURE_GENERATED_AT, FAITHFULNESS_EVAL_PROFILE_ID, FAITHFULNESS_EVAL_REPORT_DIRNAME, FAITHFULNESS_EVAL_SCHEMA_VERSION, FAITHFULNESS_PRODUCTION_BASELINE_THRESHOLDS, buildAllFaithfulnessEvalArtifacts, buildFaithfulnessEvalArtifact, computeFaithfulnessMetrics, degradeListForNoRepair, evaluateFaithfulnessVerdict, faithfulnessEvalReportFilename, injectHallucinatedTestCase, writeFaithfulnessEvalArtifact, type FaithfulnessEvalArtifact } from "./faithfulness-eval.js";

void test("faithfulness-eval: thresholds are exported as a frozen production-baseline profile", () => {
  assert.equal(
...
void test("faithfulness-eval: ships ≥ 5 fixture variants spanning forms, validations, and navigation", () => {
  assert.ok(
    (BASELINE_ARCHETYPE_FIXTURE_IDS.length as number) >= 5,
    `expected ≥5 baseline fixtures, got ${BASELINE_ARCHETYPE_FIXTURE_IDS.length}`,
  );
});

void test("faithfulness-eval: every baseline archetype passes the production-baseline gate in repair mode", async () => {
...
void test("faithfulness-eval: every baseline archetype fails the gate when forced into single-pass / no-repair mode", async () => {
  const artifacts = await buildAllFaithfulnessEvalArtifacts({
    mode: "no-repair",
  });
  for (const artifact of artifacts) {
...
void test("faithfulness-eval: hallucinated citations trigger the hallucinatedIdRate gate", async () => {
...
void test("faithfulness-eval: degradeListForNoRepair strips trace nodeIds and trims coverage", async () => {
  const fixture = await loadBaselineArchetypeFixture("baseline-simple-form");
  const intent = deriveBusinessTestIntentIr({ figma: fixture.figma });
...
void test("faithfulness-eval: artifact build is deterministic — same inputs produce byte-identical canonical JSON", async () => {
...
void test("faithfulness-eval: persists per-fixture report under the canonical eval-reports dir layout", async () => {
...
void test("faithfulness-eval: empty IR slots produce a 1.0 ratio (no false negatives)", () => {
  const intent = {
    version: "1.0.0",
    ...
    piiIndicators: [],
    redactions: [],
  } as unknown as Parameters<typeof computeFaithfulnessMetrics>[0]["intent"];
  const generatedList = {
    schemaVersion: "1.0.0",
    jobId: "empty",
    testCases: [],
