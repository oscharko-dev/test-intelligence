...
import type { LlmCircuitBreaker } from "@oscharko-dev/ti-model-gateway";
...
import { isMockLlmGatewayClient } from "@oscharko-dev/ti-model-gateway";
...
export const VISUAL_SIDECAR_RESPONSE_SCHEMA_NAME =
  "test-intelligence-visual-sidecar-v1" as const;
...
const redactRawResponsePii = (input: string): string =>
  redactLongDigitRuns(
    redactUsSsn(redactEmailLike(redactPanLike(redactIbanLike(input)))),
  );

const redactIbanLike = (value: string): string =>
  replaceIbanLike(value, "[REDACTED]");
...
const redactPanLike = (value: string): string =>
  replaceSeparatedDigitRun(value, "[REDACTED]", 13, 19);

const redactLongDigitRuns = (value: string): string =>
  replaceSeparatedDigitRun(value, "[REDACTED]", 11, 11);

const redactUsSsn = (value: string): string => {
  let out = "";
  let cursor = 0;
  while (cursor < value.length) {
    const span = findUsSsnSpan(value, cursor);
    if (span === undefined) {
      out += value.slice(cursor);
      break;
    }
    out += value.slice(cursor, span.start) + "[REDACTED]";
    cursor = span.end;
  }
  return out;
};
...
const findUsSsnSpan = (
  ...
  for (let start = offset; start + 11 <= value.length; start += 1) {
    if (!isTokenBoundary(value[start - 1])) continue;
    if (
      isAsciiDigit(value[start]) &&
      isAsciiDigit(value[start + 1]) &&
      isAsciiDigit(value[start + 2]) &&
      value[start + 3] === "-" &&
      isAsciiDigit(value[start + 4]) &&
      isAsciiDigit(value[start + 5]) &&
      value[start + 6] === "-" &&
      isAsciiDigit(value[start + 7]) &&
      isAsciiDigit(value[start + 8]) &&
      isAsciiDigit(value[start + 9]) &&
      isAsciiDigit(value[start + 10]) &&
      isTokenBoundary(value[start + 11])
    ) {
...
const replaceSeparatedDigitRun = (
  value: string,
  replacement: string,
  minDigits: number,
  maxDigits: number,
  ...
  let cursor = 0;
  while (cursor < value.length) {
    const span = findSeparatedDigitRun(value, cursor, minDigits, maxDigits);
    if (span === undefined) {
      out += value.slice(cursor);
      break;
    }
...
const findSeparatedDigitRun = (
  ...
  for (let start = offset; start < value.length; start += 1) {
    if (!isTokenBoundary(value[start - 1]) || !isAsciiDigit(value[start])) {
      continue;
    }
    let cursor = start;
    let digits = 0;
    ...
    let consumedSeparator = false;
    while (cursor < value.length && digits < maxDigits) {
      const ch = value[cursor];
      if ((ch === " " || ch === "-") && !consumedSeparator) {
        consumedSeparator = true;
        cursor += 1;
    ...
    if (
      digits >= minDigits &&
      digits <= maxDigits &&
      isTokenBoundary(value[lastDigitEnd])
    ) {
      return { start, end: lastDigitEnd };
    }
  }
  return undefined;
};

const redactEmailLike = (value: string): string =>
  replaceEmailLike(value, "[REDACTED]");
...
const isValidDomainLabel = (label: string): boolean => {
  ...
  for (const ch of label) {
    if (!isUpperAsciiAlphaNum(ch.toUpperCase()) && ch !== "-") return false;
  }
  return true;
};
...
const sanitizeDeploymentForFilename = (deployment: string): string => {
  ...
  const trimmed = trimChar(cleaned, "-");
...
const stripMarkdownCodeFence = (value: string): string => {
  ...
  let cursor = 3;
  if (value.slice(cursor, cursor + 4).toLowerCase() === "json") {
    cursor += 4;
  } else if (!isWhitespace(value[cursor])) {
    return value;
  }
  while (cursor < value.length - 3 && isWhitespace(value[cursor])) {
    cursor += 1;
  }
  return value.slice(cursor, value.length - 3).trim();
};
