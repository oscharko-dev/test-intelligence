...
import { computeReplayCacheKeyDigest } from "@oscharko-dev/ti-model-gateway";
import {
...
void test("persistent cache: constants have expected values", () => {
  assert.equal(DEFAULT_PERSISTENT_REPLAY_CACHE_BYTE_BUDGET, 100 * 1024 * 1024);
  assert.equal(
    DEFAULT_PERSISTENT_REPLAY_CACHE_STALE_THRESHOLD_MS,
    10 * 60 * 1000,
  );
...
void test("persistent cache: throws on empty tenant scope segment", () => {
  assert.throws(
    () =>
...
void test("persistent cache: rejects path-traversal tenant scope segments", () => {
...
void test("persistent cache: hit/miss correctness across simulated process restart", async () => {
...
void test("persistent cache: concurrent writes for the same key do not corrupt the entry", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "wsd-persistent-cache-concurrent-"),
  );
  try {
    const tenantScope = {
    ...
    assert.equal(
      result.hit,
      true,
      "entry must be present after concurrent writes",
    );
...
void test("persistent cache: circuit-breaker state is persisted atomically", async () => {
...
void test("persistent cache: tenant-scope isolation — two scopes cannot share entries", async () => {
  ...
  try {
    ...
    const fileA = join(root, "tenant-a", "prod", "default", `${digestA}.json`);
    const fileB = join(root, "tenant-b", "prod", "default", `${digestA}.json`);
...
void test("persistent cache: LRU eviction respects the byte budget", async () => {
...
void test("persistent cache: stale .tmp files are cleaned before a new write", async () => {
