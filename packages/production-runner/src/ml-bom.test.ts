...
void test("buildMlBomDocument emits a CycloneDX 1.7 ML-BOM with citations and data provenance", () => {
...
void test("buildMlBomDocument is deterministic for the same release inputs", () => {
  const first = buildFixture();
  const second = buildFixture();
  assert.equal(JSON.stringify(first), JSON.stringify(second));
});

void test("validateMlBomDocument rejects missing endpoint redaction and missing citations", () => {
  ...
  assert.equal(
    validation.issues.some((issue) => issue.code === "invalid_value"),
    true,
  );
});

void test("writeMlBomArtifact persists the canonical artifact and summary", async () => {
  const root = await mkdtemp(
    path.join(os.tmpdir(), "test-intelligence-ml-bom-"),
  );
  try {
    const document = buildFixture();
