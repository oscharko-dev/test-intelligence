...
import { computeCoverageReport } from "@oscharko-dev/ti-core-engine";
...
void test("regulated risk category triggers needs_review", () => {
...
void test("missing trace blocks the case", () => {
...
void test("PII in test data blocks the case", () => {
...
void test("missing accessibility case for form screens raises a job-level violation", () => {
...
void test("uncovered acceptance criteria raise a job-level policy violation", () => {
  const intent = buildIntent();
  const coveragePlan = buildCoveragePlan({
    minimumCases: [
      {
        requirementId: "REQ-AC-001",
        reasonCode: "acceptance_criterion",
  ...
  const violation = report.jobLevelViolations.find(
    (entry) => entry.outcome === "acceptance_criterion_uncovered",
  );
  assert.equal(report.blocked, true);
  assert.equal(violation?.rule, "policy:acceptance-criteria-coverage");
  assert.match(violation?.reason ?? "", /AC-001/u);
});

void test("Issue #1951: missing screen-reader coverage on a form-screen accessibility case blocks the job", () => {
  const intent = buildIntent({
    detectedFields: [
      {
        id: "f-1",
...
void test("Issue #1794: banking profile blocks when an active model binding is missing ictRegisterRef", () => {
...
void test("Issue #2177: region allow-list data survives a derived profile and does not interfere with ICT register enforcement", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const profile = cloneEuBankingDefaultProfile();
  profile.rules = {
    ...profile.rules,
...
void test("Issue #2069: both_sidecars_failed is a blocking job-level error by default", () => {
  const tc = buildCase({});
  const ctx = harness([tc], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
...
void test("Issue #2069: visual verification required blocks the case when both sidecars fail", () => {
  const tc = buildCase({});
  const ctx = harness([tc], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
...
void test("Issue #2069: fallback-recovered visual sidecar emits info-only policy evidence", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #2100: signed semantic override is accepted when the provider verifies it", () => {
...
void test("Issue #2100: signed semantic override fails closed when the provider is missing", () => {
  const validEntry = buildSignedOverrideEntry();
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #2100: expired signed semantic overrides are rejected", () => {
...
void test("Issue #2100: invalid signed semantic overrides are rejected at least 1000 times via evaluatePolicyGate (fast-check)", () => {
  const validEntry = buildSignedOverrideEntry();
...
void test("resolved multi-source conflicts are pruned before policy blockers are counted", () => {
...
void test("required field with negative coverage satisfies the rule", () => {
  const intent = buildIntent({
    detectedFields: [
      {
        id: "f-iban",
...
void test("missing negative or validation coverage for a required field blocks the job", () => {
  const intent = buildIntent({
    detectedFields: [
      {
        id: "f-iban",
...
void test("Issue #1947: unmet technique quotas block the job at the policy gate", () => {
...
void test("Issue #2068: tier-elastic mode replaces a fixed 12-EP planner quota on a 9-field K0 screen with the field-count floor", () => {
  const ctx = harness(
...
void test("Issue #2068: fixed override on a derived profile preserves the legacy 12-EP minimum", () => {
  const ctx = harness(
    Array.from({ length: 10 }, (_, idx) =>
      buildCase({
        id: `tc-ep-${idx + 1}`,
...
void test("Issue #2026: full field coverage from shared EP cases clears the policy gate", () => {
  const intent = buildIntent({
    detectedFields: [
      {
        id: "s-1.field-1",
...
void test("Issue #1947: policy override can downgrade technique coverage minimum to warning", () => {
  const ctx = harness([buildCase({ id: "tc-use-case" })], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #1949: cross-modal faithfulness score below the gray-zone floor blocks every case", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #1949: cross-modal faithfulness score in the gray zone routes to needs_review", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #2066: stepVerdicts override the case-level score so all-evidence_partial label-only steps clear the 0.80 floor", () => {
  const ctx = harness(
    [
      buildCase({
        steps: [
...
void test("Issue #2066: a positive step mismatch still blocks even when the legacy score is high", () => {
  const ctx = harness(
    [
      buildCase({
        steps: [
...
void test("Issue #1949: policy override threshold can relax the cross-modal faithfulness gate", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #2101: faithfulness judge refusal stays auditable but fail_open keeps cases approved", () => {
...
void test("Issue #2170: partial-majority case fires the warning rule and ships at needs_review", () => {
  const ctx = harness(
    [
      buildCase({
        id: "tc-partial",
...
void test("Issue #2170 audit follow-up: a case with 100 % evidence_partial verdicts fires the partial-majority warning and ships at needs_review (not pass)", () => {
...
void test("Issue #2170: a case with minority evidence_partial verdicts is approved without the warning", () => {
  const ctx = harness(
    [
      buildCase({
        id: "tc-minority",
...
void test("Issue #2170: state_transition technique case clears the gate at the relaxed 0.65 step floor", () => {
  const ctx = harness(
    [
      buildCase({
        id: "tc-workflow",
...
void test("Issue #2101: faithfulness judge refusal fail_closed blocks the job", () => {
  const ctx = harness([buildCase({ id: "tc-pos" })], buildIntent());
  ctx.profile.rules.judgeRefusalPolicy = {
...
void test("low confidence triggers needs_review", () => {
  const tc = buildCase({
    qualitySignals: {
      coveredFieldIds: [],
      coveredActionIds: [],
...
void test("visual sidecar prompt-injection text is propagated as job-level error", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("counters reflect mixed decisions deterministically", () => {
  const cases = [
    buildCase({ id: "tc-1" }),
...
void test("Issue #1412: low-tagged case under regulated intent raises risk_tag_downgrade_detected per-case and job-level", () => {
...
void test("Issue #1412: high-tagged case under regulated intent still flagged (high is outside review-only set)", () => {
  const intent = buildIntent({ risks: ["regulated_data"] });
...
void test("Issue #1412: regulated_data-tagged case does not raise downgrade detection", () => {
  const intent = buildIntent({ risks: ["regulated_data"] });
...
void test("Issue #1412: financial intent classifies a low-tagged case as downgraded", () => {
...
void test("Issue #1412: PII bound to a different screen does not flag the case", () => {
  const intent = buildIntent({
    screens: [
...
void test("Issue #1412: PII bound to the referenced screen flags a low-tagged case", () => {
  const intent = buildIntent({
    screens: [
      { screenId: "s-1", screenName: "Login", trace: { nodeId: "s-1" } },
      { screenId: "s-2", screenName: "Profile", trace: { nodeId: "s-2" } },
...
void test("Issue #1412: PII without a screenId falls through to global (fail-closed)", () => {
  const intent = buildIntent({
    piiIndicators: [
      {
        id: "pii-unbound",
...
void test("Issue #1412: feature flag false suppresses downgrade detection only", () => {
  const intent = buildIntent({ risks: ["regulated_data"] });
  const ctx = harness([buildCase({ riskCategory: "low" })], intent);
...
void test("Issue #1412: job-level downgrade entries are deduplicated per (testCaseId, intentRisk, declaredRisk)", () => {
  const intent = buildIntent({
...
void test("custom context policy signal escalates low-risk cases and records policy report evidence", () => {
...
void test("Issue #1946: ictRegisterRef inherited from customerProfile satisfies policy:ict-register-ref-required", () => {
...
void test("Issue #1946: missing ictRegisterRef still fires when no profile ref is present", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #1948: uncovered p0 risk-class IR element blocks the job", () => {
  const ctx = harness([buildCase({ id: "tc-pos" })], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #1948: live re-run scenario — Banking financial-transaction action uncovered → blocked", () => {
...
void test("Issue #1948: covered p0 element produces no violation", () => {
  const ctx = harness(
    [
      buildCase({
        qualitySignals: {
...
void test("Issue #1948: no false positives on screens with zero p0 elements", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #1948: customerProfile policyOverride downgrades p0 uncovered to warning", () => {
  const ctx = harness([buildCase({ id: "tc-pos" })], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #1951: customerProfile policyOverride can downgrade form-screen accessibility hard-gate to warning", () => {
  const intent = buildIntent({
    detectedFields: [
      {
        id: "f-1",
...
void test("Issue #1951: a11y_judge covered_weakly verdict routes the job to needs_review", () => {
  const intent = buildIntent({
    detectedFields: [
      {
        id: "f-1",
...
void test("Issue #1951: a11y_judge not_covered verdict blocks the job", () => {
  const intent = buildIntent({
    detectedFields: [
      {
        id: "f-1",
...
void test("Issue #1951: disabled form-screen accessibility rule suppresses a11y_judge gating", () => {
  const intent = buildIntent({
    detectedFields: [
      {
        id: "f-1",
...
void test("Issue #2101: default profile routes a11y judge refusals to needs_review", () => {
  const intent = buildIntent({
    detectedFields: [
      {
        id: "f-1",
...
void test("Issue #1950: coverage-baseline drift > 10% emits a warning-severity job-level violation (needs_review, not blocking)", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #1950: seeded baseline (first run) does not emit a coverage-drift violation", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #1950: in-tolerance candidate does not emit a coverage-drift violation", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #2030: job-wide compliance override fires regulated-risk-requires-review when intent has no PII / risks", () => {
...
void test("Issue #2030: job-wide compliance override fires risk-tag-downgrade-detected on low-tagged cases", () => {
  const intent = buildIntent();
  const ctx = harness([buildCase({ riskCategory: "low" })], intent);
  const overrides: ComplianceRiskOverride[] = [
    { riskCategory: "regulated_data", rationale: "MiFID-II" },
...
void test("Issue #2030: compliance override does NOT weaken an already-derived classification", () => {
...
void test("Issue #2030: screen-scoped override only fires for cases on that screen", () => {
  const intent = buildIntent({
    screens: [
...
void test("Issue #2030: override with non-review-only category is ignored", () => {
  const intent = buildIntent();
  const ctx = harness([buildCase({ riskCategory: "low" })], intent);
...
void test("Issue #2116: per-step verdict produces mode=per_step + no fallback rule", () => {
  const ctx = harness(
    [
      buildCase({
        steps: [
...
void test("Issue #2116: legacy verdict (no stepVerdicts) raises a warning fallback rule and records mode=case_level_fallback", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #2116: requirePerStepFaithfulness=true escalates the fallback rule to error and blocks the run", () => {
  const ctx = harness([buildCase({})], buildIntent());
...
void test("Issue #2116: refused verdict records mode=missing without raising an extra evaluation-missing rule", () => {
  const ctx = harness([buildCase({})], buildIntent());
  ctx.profile.rules.judgeRefusalPolicy = {
    faithfulness: "fail_open",
    a11y: "needs_review",
...
void test("Issue #2116: no faithfulness verdict at all preserves the pre-#2116 byte shape (no audit block, no fallback rule)", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #2116: missing faithfulness verdict fails closed when visual sidecar evidence exists", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #2116: case-level-fallback rule severity respects an operator policyOverrides override", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("Issue #2116: policyOverrides cannot downgrade the missing faithfulness verdict hard gate", () => {
  const ctx = harness([buildCase({})], buildIntent());
  const report = evaluatePolicyGate({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
