...
import type { DomainInvariant } from "@oscharko-dev/ti-core-engine";
import {
...
const buildVariant = (input: {
  ...
  return {
    ...
    qualitySignals: {
      coveredFieldIds: [
        input.causeElement.elementId,
        input.effectElement.elementId,
      ],
      coveredActionIds: [],
...
const evaluatePairAssertion = (
  ...
  const sameExpectation =
    ...
    pair.variantA.expectedResults.length ===
      pair.variantB.expectedResults.length;
  if (!sameExpectation) {
...
export const evaluateCounterfactualPairs = (
  ...
  const counts = new Map<string, { generated: number; violated: number }>();
  for (const hypothesis of input.hypotheses) {
