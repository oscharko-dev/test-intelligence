...
import { DOCUMENTED_HALLUCINATION_PATTERNS, HALLUCINATION_EVAL_FIXTURE_GENERATED_AT, HALLUCINATION_EVAL_PROFILE_ID, HALLUCINATION_EVAL_REPORT_DIRNAME, HALLUCINATION_EVAL_SCHEMA_VERSION, HALLUCINATION_PRODUCTION_BASELINE_THRESHOLDS, buildAllHallucinationEvalArtifacts, buildHallucinationEvalArtifact, computeHallucinationMetrics, evaluateHallucinationVerdict, hallucinationEvalReportFilename, injectInventedActionStep, injectInventedButtonStateStep, injectInventedFieldStep, injectInventedScreenStep, injectInventedTraceNodeId, injectInventedValidationCitation, writeHallucinationEvalArtifact, type HallucinationEvalArtifact, type HallucinationPattern } from "./hallucination-eval.js";
...
void test("hallucination-eval: thresholds are exported as a frozen production-baseline profile", () => {
  assert.equal(
...
void test("hallucination-eval: documents at least 6 hallucination patterns covering button, validation, field, screen, trace-id, button-state", () => {
  assert.ok(
...
void test("hallucination-eval: every baseline archetype passes the production-baseline gate with the deterministic synthesiser", async () => {
...
void test("hallucination-eval: adversarial sub-suite — prompt-injection on the IR input does not raise the hallucination rate", async () => {
  const artifacts = await buildAllHallucinationEvalArtifacts({
...
void test("hallucination-eval: invented action label triggers the action-rate hard gate", async () => {
...
void test("hallucination-eval: invented field label trips the field-rate hard gate when above tolerance", async () => {
  const ctx = await synthesizeForFixture(
    "baseline-simple-form",
...
void test("hallucination-eval: invented validation citation is detected as an error finding", async () => {
  const ctx = await synthesizeForFixture(
    "baseline-simple-form",
...
void test("hallucination-eval: invented screen reference is detected as an error finding", async () => {
  const ctx = await synthesizeForFixture(
    "baseline-simple-form",
...
void test("hallucination-eval: invented trace nodeId is detected as an error finding", async () => {
  const ctx = await synthesizeForFixture(
    "baseline-simple-form",
...
void test("hallucination-eval: invented button state is surfaced as a warning, not an error", async () => {
  const ctx = await synthesizeForFixture(
    "baseline-simple-form",
...
void test("hallucination-eval: fuzzy tolerance accepts 1-character typos in field labels", async () => {
  const ctx = await synthesizeForFixture(
    "baseline-simple-form",
    "hallucination-eval-fuzzy",
  );
  const baseField = ctx.intent.detectedFields[0]?.label ?? "Email";
...
void test("hallucination-eval: artifact build is deterministic — same inputs produce byte-identical canonical JSON", async () => {
...
void test("hallucination-eval: persists per-fixture report under the canonical eval-reports dir layout", async () => {
...
void test("hallucination-eval: empty IR + empty test list yields zero rates and a passing verdict (no false negatives)", () => {
  const intent = {
    version: "1.0.0",
    ...
    piiIndicators: [],
    redactions: [],
  } as unknown as Parameters<typeof computeHallucinationMetrics>[0]["intent"];
  const generatedList = {
    schemaVersion: "1.0.0",
    jobId: "empty",
    testCases: [],
...
void test("hallucination-eval: respects custom thresholds passed through evaluateHallucinationVerdict", () => {
  const metrics = {
