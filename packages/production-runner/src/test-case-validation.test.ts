...
void test("Issue #2071: validator rejects oracle-governed testData that lacks deterministic provenance", () => {
...
void test("Issue #2106: validator keeps preserved entries scannable while skipping the oracle-governed synthetic slot", () => {
  const report = validateGeneratedTestCases({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
...
void test("valid input produces a clean report", () => {
  const report = validateGeneratedTestCases({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    list: buildList(),
...
void test("workflow topology ACT ids are accepted as coveredActionIds", () => {
  const report = validateGeneratedTestCases({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    list: buildList([
...
void test("field lifecycle transitions require step anchors and mandatory-tier coverage", () => {
  const report = validateGeneratedTestCases({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    list: buildList(),
...
void test("Issue #2168: recommended field-entry transitions are covered by downstream validation paths", () => {
  // Build a list whose single case anchors the validation-pass path.
  // The follow-up form_commit terminal transition and the preparatory
  // focus/input transitions are considered covered by the downstream
  // positive/negative validation paths. This prevents lifecycle-only
  // warning inflation on real multi-field customer masks.
  const report = validateGeneratedTestCases({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    list: buildList([
...
void test("Issue #2168: state_transition_test_only transitions are silent unless a state_transition case is present", () => {
...
void test("Epic #2167 Q0: mandatory tier aggregates at (screenId, trigger) — one covered field unblocks the whole screen", () => {
...
void test("Epic #2167 Q0: mandatory tier emits ONE error per (screenId, trigger) group when no transition in the group is anchored", () => {
...
void test("field lifecycle transitions pass when every transition is covered by generated steps", () => {
  const report = validateGeneratedTestCases({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    list: buildList([
...
void test("structural schema failures short-circuit semantic checks", () => {
...
void test("missing trace is blocking", () => {
...
void test("trace screen unknown is blocking", () => {
...
void test("missing expected results is blocking", () => {
  const tc = buildCase({
    expectedResults: [],
    steps: [
      { index: 1, action: "Open payment screen" },
...
void test("expected results may live on a step", () => {
  const tc = buildCase({
    expectedResults: [],
    steps: [
      { index: 1, action: "Open" },
...
void test("PII in test data is blocking even when value looks innocuous", () => {
...
void test("redaction tokens in test data are accepted", () => {
  const tc = buildCase({
...
void test("semantic suspicious content in exported step data is blocking", () => {
  const tc = buildCase({
    steps: [
      {
        index: 1,
...
void test("PII in preconditions is blocking", () => {
...
void test("steps must be ordered and sequential", () => {
  const tc = buildCase({
    steps: [
...
void test("step indices must form contiguous 1..N", () => {
  const tc = buildCase({
    steps: [
      { index: 1, action: "Open" },
...
void test("duplicate step index is reported", () => {
  const tc = buildCase({
    steps: [
      { index: 1, action: "Open" },
...
void test("qc mapping with exportable=false requires blocking reasons", () => {
  const tc = buildCase({
...
void test("qc mapping exportable=true with blocking reasons is inconsistent", () => {
  const tc = buildCase({
    qcMappingPreview: {
      exportable: true,
...
void test("ambiguity with auto_approved review state is rejected", () => {
  const tc = buildCase({
...
void test("exact validation details are rejected when the source marks the rule unresolved", () => {
...
void test("generic unresolved validation notes do not block unrelated specified validations", () => {
  const intent = buildIntent();
...
void test("empty-input validation assumptions are rejected when the source marks the rule unresolved", () => {
  const intent = buildIntent();
  intent.detectedFields[0] = {
    ...intent.detectedFields[0]!,
    label: "VAT rate",
...
void test("label-only unresolved validation references downgrade to clarification warnings", () => {
  const intent = buildIntent();
  intent.detectedFields[0] = {
    ...intent.detectedFields[0]!,
...
void test("duplicate test case ids surface as errors", () => {
...
void test("coverage ids that reference unknown intent ids surface as warnings", () => {
  const tc = buildCase({
    qualitySignals: {
...
void test("covered requirement ids are validated against acceptance criteria coverage plan targets", () => {
  ...
  assert.equal(
    report.issues.some(
      (issue) =>
        issue.code === "quality_signals_coverage_unknown_id" &&
        issue.path.endsWith(".coveredRequirementIds"),
    ),
    false,
  );
});

void test("covered requirement ids outside the coverage plan surface as warnings", () => {
  const tc = buildCase({
    qualitySignals: {
      ...buildCase().qualitySignals,
      coveredRequirementIds: ["AC-999"],
    },
  });
  const report = validateGeneratedTestCases({
    jobId: "job-1",
  ...
  assert.equal(
    ...
    "$.testCases[0].qualitySignals.coveredRequirementIds[0]",
  );
});

void test("truncated repair instruction audit metadata surfaces as a warning", () => {
...
void test("report carries deterministic shape stamps", () => {
  const report = validateGeneratedTestCases({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    list: buildList(),
