...
void test("PRODUCTION_RUNNER_HARNESS_MODES exposes the closed mode set", () => {
...
void test("runFigmaToQcTestCases injects an ambiguity probe when unresolved rules would otherwise yield no negative case", async () => {
...
void test("runFigmaToQcTestCases harness mode off writes no harness step artifact", async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "ti-runner-harness-"));
  try {
    const client = createMockLlmGatewayClient({
      role: "test_generation",
...
void test("runFigmaToQcTestCases harness mode shadow_eval persists an accepted step artifact", async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "ti-runner-harness-"));
  try {
    const client = createMockLlmGatewayClient({
      role: "test_generation",
...
void test("runFigmaToQcTestCases harness mode enforced succeeds on a happy path", async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "ti-runner-harness-"));
  try {
    const client = createMockLlmGatewayClient({
      role: "test_generation",
...
void test("runFigmaToQcTestCases harness mode shadow_eval persists an accepted step artifact for dual-pass generation", async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "ti-runner-harness-"));
  try {
    const client = createMockLlmGatewayClient({
      role: "test_generation",
...
void test("runFigmaToQcTestCases harness mode enforced blocks when the logic judge rejects the generated cases", async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "ti-runner-harness-"));
  try {
    const client = createMockLlmGatewayClient({
      role: "test_generation",
...
void test("runFigmaToQcTestCases harness mode enforced surfaces LLM refusal as ProductionRunnerError", async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "ti-runner-harness-"));
  try {
    const client = createMockLlmGatewayClient({
      role: "test_generation",
...
void test("runFigmaToQcTestCases harness mode shadow_eval does not throw on LLM refusal but records rejected step", async () => {
  ...
  try {
    ...
    const parsed = JSON.parse(await readFile(expected, "utf8")) as {
      outcome: string;
    };
    assert.equal(parsed.outcome, "failed_permanent");
  } finally {
...
void test("runFigmaToQcTestCases harness honors a custom roleStepId", async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "ti-runner-harness-"));
  try {
    const client = createMockLlmGatewayClient({
      role: "test_generation",
...
void test("runFigmaToQcTestCases harness modes are exhaustively covered by the integration tests", () => {
...
void test("runFigmaToQcTestCases drives the repair loop when the logic-judge initially asks for repair", async () => {
  ...
  try {
    ...
    const plannerPayload = JSON.parse(
      await readFile(plannerArtifact, "utf8"),
    ) as {
      iteration: number;
      outputs: { repairInstructionCount: number };
    };
    const generatorPayload = JSON.parse(
      await readFile(generatorArtifact, "utf8"),
    ) as { iteration: number; llmGateway: { outcome: string } };
...
void test("runFigmaToQcTestCases skips the repair loop entirely on a clean accept verdict", async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "ti-runner-repair-"));
  try {
    const client = createMockLlmGatewayClient({
      role: "test_generation",
...
void test("runFigmaToQcTestCases drives the repair loop when the initial Logic-Judge verdict is repair and recovers post-repair (Issue #1928)", async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "ti-runner-repair-"));
  try {
