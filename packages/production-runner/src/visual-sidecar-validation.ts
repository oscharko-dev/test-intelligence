...
const asDeployment = (
  ...
  if (trimmed.length === 0 || trimmed.length > SIDECAR_DEPLOYMENT_MAX_LENGTH) {
    return undefined;
  }
  if (
...
const containsInjectionLikeText = (text: string): boolean => {
  ...
  return (
    ...
    containsSystemLabel(lower) ||
    containsRoleTag(lower) ||
    containsSudoCommand(lower)
  );
};
...
const containsSystemLabel = (value: string): boolean => {
  let cursor = value.indexOf("system");
  while (cursor >= 0) {
    let next = cursor + "system".length;
    while (value[next] === " " || value[next] === "\t") next += 1;
    if (value[next] === ":") return true;
    cursor = value.indexOf("system", cursor + 1);
  }
  return false;
};

const containsRoleTag = (value: string): boolean => {
  ...
  while (cursor >= 0) {
    ...
    for (const role of roles) {
      if (!value.startsWith(role, next)) continue;
      const after = value[next + role.length];
...
const containsSudoCommand = (value: string): boolean => {
  let cursor = value.indexOf("sudo");
  while (cursor >= 0) {
    const before = cursor === 0 ? " " : (value[cursor - 1] as string);
    const after = value[cursor + "sudo".length] ?? "";
    if (!isAsciiWord(before) && (after === " " || after === "\t")) return true;
    cursor = value.indexOf("sudo", cursor + 1);
  }
  return false;
};
