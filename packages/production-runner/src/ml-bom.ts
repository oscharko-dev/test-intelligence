/**
 * Branding note (issue #87): every `workspace-dev:*` literal and every
 * `workspace-dev*` BOM-ref in this file is a CycloneDX wire identifier
 * preserved byte-identical against the Workspace Dev reference at pinned SHA
 * `006dabdf0abe30b9cac2b742a7238c6625d8e8c1`. Per intentional delta D-01 in
 * the parity fixture policy, engine-module wire identifiers are
 * preserved verbatim and MUST NOT be rebranded; only operator-facing strings
 * are reskinned to `test-intelligence`.
 */
...
import { listWave1ValidationAttestationArtifactPaths } from "@oscharko-dev/ti-evidence";
import {
...
const buildModelProperties = (input: {
  ...
  sortProperties([
    ...
    {
      name: "workspace-dev:gatewayRelease",
      value: input.binding.gatewayRelease,
    },
    {
...
export const buildMlBomDocument = (
  ...
  const attestationRefs = listWave1ValidationAttestationArtifactPaths(
    input.signingMode,
  );
  const sortedBindings = [...input.modelBindings].sort(
    (left, right) =>
      ML_BOM_ROLE_ORDER.indexOf(left.role) -
      ML_BOM_ROLE_ORDER.indexOf(right.role),
  );
  ...
  const policyComponent: MlBomComponent = {
    ...
    description:
      "Policy bundle provenance for the active test-intelligence gate.",
    ...
    data: [
      {
        ...
        governance: {
          ...
          stewards: [
            { organization: { name: "workspace-dev policy registry" } },
          ],
        },
        contents: {
          properties: sortProperties([
  ...
  for (const role of ML_BOM_ROLE_ORDER) {
    if (
      !document.components.some(
        (component) => component["bom-ref"] === `model:${role}`,
      )
    ) {
      issues.push({
        path: "components",
...
export const summarizeMlBomArtifact = (input: {
  ...
  componentCounts: {
    data: input.document.components.filter(
