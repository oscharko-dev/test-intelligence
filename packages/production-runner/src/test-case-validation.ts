...
import { validateGeneratedTestCaseList } from "@oscharko-dev/ti-core-engine";
import { detectPii } from "@oscharko-dev/ti-security";
import { detectSuspiciousContent } from "@oscharko-dev/ti-security";
 ...
 * tiers stay granular, with one pragmatic aggregation: a field-level
 * `validated → terminal` transition triggered by `form_commit` is
 * considered covered when the same field's positive validation
 * transition is covered; likewise, initial focus/user-input preparation
 * is considered covered when the same field has a downstream
 * validation-pass or validation-fail transition anchored. A submit/commit
 * action and field-entry preparation are shared screen infrastructure;
 * requiring synthetic lifecycle-only steps per field would inflate large
 * customer masks without adding executable test value.
 */
const validateFieldLifecycleCoverage = (
  ...
  const transitionsByFieldId = new Map<
    string,
    WorkflowTopology["fieldLifecycles"][number]["transitions"]
  >();
  for (const lifecycle of lifecycles) {
    transitionsByFieldId.set(lifecycle.fieldId, lifecycle.transitions);
  }
  const isFormCommitTerminalCoveredByPositiveValidation = (
    fieldId: string,
    transition: (typeof allTransitions)[number]["transition"],
  ): boolean => {
    if (
      transition.from !== "validated" ||
      transition.to !== "terminal" ||
      transition.trigger !== "form_commit"
    ) {
      return false;
    }
    return (
      transitionsByFieldId.get(fieldId)?.some(
        (candidate) =>
          candidate.from === "in_progress" &&
          candidate.to === "validated" &&
          coveredTransitionIds.has(candidate.transitionId),
      ) ?? false
    );
  };
  const isFieldPreparationCoveredByDownstreamValidation = (
    fieldId: string,
    transition: (typeof allTransitions)[number]["transition"],
  ): boolean => {
    const isPreparationTransition =
      (transition.from === "initial" &&
        transition.to === "focused" &&
        transition.trigger === "user_focus") ||
      (transition.from === "focused" &&
        transition.to === "in_progress" &&
        transition.trigger === "user_input");
    if (!isPreparationTransition) {
      return false;
    }
    return (
      transitionsByFieldId.get(fieldId)?.some(
        (candidate) =>
          candidate.from === "in_progress" &&
          (candidate.to === "validated" || candidate.to === "error") &&
          coveredTransitionIds.has(candidate.transitionId),
      ) ?? false
    );
  };
  ...
  for (const { transition, fieldId } of allTransitions) {
    ...
    if (tier === "recommended_positive_path") {
      if (
        isFormCommitTerminalCoveredByPositiveValidation(fieldId, transition) ||
        isFieldPreparationCoveredByDownstreamValidation(fieldId, transition)
      ) {
        continue;
      }
      pushIssue(issues, {
 ...
 * fixtures and non-visual topologies degrade to per-field grouping (the
...
const validateQualitySignalsCoverage = (
  ...
  checkIds(
    qs.coveredRequirementIds ?? [],
    "coveredRequirementIds",
    intentIds.requirements,
  );
};
...
export const validateGeneratedTestCasesWithInvariants = (
  ...
  const intentIds = collectIntentIds(
    input.intent,
