...
export interface BuildRiskRankingWithAugmentationInput extends BuildRiskRankingInput {
...
const RISK_RANKER_RESPONSE_SCHEMA_NAME =
  "test-intelligence-risk-ranker-v1" as const;
...
const isAllowedRationale = (value: unknown): value is RiskRankingRationale =>
  typeof value === "string" &&
...
const extractStrictRiskCategories = (
  ...
  for (const candidate of strictRaw) {
    if (
      ...
      allowed.has(candidate as TestCaseRiskCategory)
    ) {
...
export const buildRiskRanking = (input: BuildRiskRankingInput): RiskRanking => {
...
export const buildRiskRankingWithAugmentation = async (
  ...
  const request = {
    jobId: input.jobId,
    systemPrompt: [
      "You are the optional Risk-Ranker augmentation model for Test Intelligence.",
