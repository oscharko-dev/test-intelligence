...
import { buildFaithfulnessTierReport } from "@oscharko-dev/ti-quality";
import {
...
import { collectTechniqueQuotaDeficits } from "@oscharko-dev/ti-model-gateway";
import type { UntrustedContentNormalizationReport } from "./untrusted-content-normalizer.js";

// ---------------------------------------------------------------------------
// Accessibility-criteria helpers (moved from a11y-coverage-eval.ts to break
// the a11y-coverage-eval → validation-harness → validation-pipeline →
// policy-gate → a11y-coverage-eval import cycle).
// ---------------------------------------------------------------------------
...
export interface EvaluatePolicyGateInput {
   ...
   * controlled by the coverage-baseline rebaseline workflow.
   */
...
const resolveVisualSidecarRefusalViolation = (
  visualSidecarRefusal: NonNullable<
    EvaluatePolicyGateInput["visualSidecarRefusal"]
  >,
...
const parseMultiSourceConflictIds = (reason: string): string[] | undefined => {
  const match =
...
const evaluateCase = (
  ...
  for (const issue of caseIssues) {
    const invalidOverride = invalidSemanticOverrideReason(
      issue,
      invalidOverrides,
    );
...
const resolveComplianceRiskOverride = (
  ...
  const matching = overrides.filter(
    (o) => o.screenId === undefined || caseScreenIds.has(o.screenId),
  );
...
const collectMultiSourceConflictIds = (
  ...
  return conflicts
    .filter((conflict) => {
      if (
        conflict.affectedElementIds?.some((elementId) =>
          coveredIds.has(elementId),
        )
      ) {
        return true;
      }
      ...
      return (
        conflict.affectedElementIds === undefined &&
        conflict.affectedScreenIds === undefined
      );
    })
...
const evaluateJobLevel = (
  ...
  if (
    coverage.acceptanceCriteriaCoverage !== undefined &&
    coverage.acceptanceCriteriaCoverage.uncoveredIds.length > 0
  ) {
    violations.push({
      rule: "policy:acceptance-criteria-coverage",
      outcome: "acceptance_criterion_uncovered",
      severity: "error",
      reason:
        `${coverage.acceptanceCriteriaCoverage.uncoveredIds.length} acceptance ` +
        `criterion/criteria have no covering test case: ` +
        coverage.acceptanceCriteriaCoverage.uncoveredIds.join(", "),
    });
  }
...
export type { FaithfulnessEvaluationMode } from "@oscharko-dev/ti-contracts";

export const pruneResolvedMultiSourceConflictViolations = (input: {
  ...
  for (const decision of input.report.decisions) {
    ...
    for (const violation of violations) {
      nextDecision = escalate(
        nextDecision,
        severityToDecision(violation.severity),
      );
    }
    decisions.push({
      ...decision,
