...
import { createLlmCircuitBreaker } from "@oscharko-dev/ti-model-gateway";
import type {
...
void test("primary success: visualPrimary wins, fallback is never called, fallbackReason=none", async () => {
...
void test("fallback success on primary timeout: fallbackReason=primary_unavailable", async () => {
...
void test("fallback on primary rate_limited: fallbackReason=primary_quota_exceeded", async () => {
  const captures = [captureFor("s-1")];
  const primaryResponder: MockResponder = (_request, attempt) => ({
    outcome: "error",
...
void test("both fail: VisualSidecarFailure with both_sidecars_failed and union of attempts", async () => {
  const captures = [captureFor("s-1")];
  const primaryResponder: MockResponder = (_request, attempt) => ({
    outcome: "error",
    errorClass: "timeout",
...
void test("transient visual sidecar failures retry a second primary/fallback round when enabled", async () => {
  const captures = [captureFor("s-1")];
  let primaryCalls = 0;
  const primaryResponder: MockResponder = (request, attempt) => {
    primaryCalls += 1;
    ...
    return buildSuccess(
      request,
      attempt,
      buildEnvelope(captures, PRIMARY_DEPLOYMENT),
      {
        deployment: PRIMARY_DEPLOYMENT,
        modelRevision: `${PRIMARY_DEPLOYMENT}@test`,
        gatewayRelease: "mock",
...
void test("primary circuit breaker skips primary after two protocol failures", async () => {
  const captures = [captureFor("s-1")];
  let primaryCalls = 0;
  let fallbackCalls = 0;
...
void test("invalid sidecar JSON: schema_invalid on primary is safely recovered by fallback success", async () => {
  const captures = [captureFor("s-1")];
  // `schema_invalid_response` is a visual-sidecar attempt class, not a gateway
  // `LlmGatewayErrorClass`; the mock responder emits it verbatim so the client
  // surfaces it on the persisted attempt. The cast keeps the runtime literal
  // available under the standalone strict gate.
  const primaryResponder: MockResponder = (_request, attempt) => ({
    outcome: "error",
    errorClass: "schema_invalid_response" as unknown as LlmGatewayErrorClass,
...
void test("hanging primary gateway call times out locally and falls back", async () => {
  const captures = [captureFor("s-1")];
  let fallbackCalls = 0;
  const primaryResponder: MockResponder = async () =>
    await new Promise(() => {});
  const fallbackResponder: MockResponder = (request, attempt) => {
    fallbackCalls += 1;
    return buildSuccess(
...
void test("repair path coerces schema-near sidecar JSON into a valid result", async () => {
  const captures = [captureFor("s-1")];
  const primaryResponder: MockResponder = (request, attempt) =>
    buildSuccess(
      request,
...
void test("model-emitted deployment aliases normalize to the selected primary deployment before persistence", async () => {
  const captures = [captureFor("s-1")];
  const primaryResponder: MockResponder = (request, attempt) =>
    buildSuccess(
      request,
...
void test("stale model-emitted deployment literals normalize to the selected primary deployment before persistence", async () => {
  const captures = [captureFor("s-1")];
  const primaryResponder: MockResponder = (request, attempt) =>
    buildSuccess(
      request,
...
void test("invalid sidecar JSON: schema_invalid on both attempts hard-fails with schema_invalid_response", async () => {
  const captures = [captureFor("s-1")];
  // See the note above: `schema_invalid_response` is a visual-sidecar attempt
  // class emitted verbatim by the mock; the cast keeps the runtime literal
  // available under the standalone strict gate.
  const primaryResponder: MockResponder = (_request, attempt) => ({
    outcome: "error",
    errorClass: "schema_invalid_response" as unknown as LlmGatewayErrorClass,
...
void test("preflight: image too large → image_payload_too_large; no gateway call", async () => {
...
void test("finops preflight: image budget applies to the whole outbound request", async () => {
...
void test("preflight: unsupported MIME (image/svg+xml) → image_mime_unsupported", () => {
...
void test("preflight: duplicate screenId → duplicate_screen_id", () => {
...
void test("preflight: empty captures → empty_screen_capture_set", () => {
  const result = preflightCaptures([]);
  assert.equal(result.ok, false);
  assert.equal(result.failure.failureClass, "empty_screen_capture_set");
});

void test("test-generation guard: bundle with imageInputSupport=true on testGeneration throws RangeError at entry", async () => {
...
void test("validation gate integration: PII-containing visibleText still surfaces success with possible_pii outcome", async () => {
...
void test("capture identity hashing: SHA-256 matches the decoded bytes; no base64 in identity", async () => {
  const captures = [captureFor("s-1")];
  const primaryResponder: MockResponder = (request, attempt) =>
  ...
  assert.equal(
    (identity as unknown as Record<string, unknown>)["base64Data"],
    undefined,
  );
});

void test("determinism: identical inputs produce byte-identical artifacts (canonical JSON)", async () => {
  const captures = [captureFor("s-1"), captureFor("s-2")];
  ...
  assert.deepEqual(
    parsed.visualEvidenceRefs.map((ref) => ref.screenId),
    ["s-1", "s-2"],
  );
  assert.equal(
...
void test("write artifact: filename matches contract and lands at the requested path", async () => {
  const captures = [captureFor("s-1")];
  const { result } = await describeVisualScreens({
    bundle: buildBundle({
      primary: {
...
void test("response schema: structure is locked; required fields enforce descent into screens", () => {
  ...
  assert.equal(
    VISUAL_SIDECAR_RESPONSE_SCHEMA_NAME,
    "test-intelligence-visual-sidecar-v1",
  );
  assert.match(VISUAL_SIDECAR_RESPONSE_SCHEMA_NAME, /^[a-zA-Z0-9_-]{1,64}$/);
});

void test("allowlists: SVG is intentionally excluded from the MIME allowlist", () => {
  const allowed =
    ALLOWED_VISUAL_SIDECAR_INPUT_MIME_TYPES as ReadonlyArray<string>;
  assert.ok(!allowed.includes("image/svg+xml"));
});

void test("policy_downgrade: forceFallback skips primary entirely", async () => {
  const captures = [captureFor("s-1")];
  let primaryCalls = 0;
