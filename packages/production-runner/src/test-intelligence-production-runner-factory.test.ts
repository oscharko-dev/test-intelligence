/**
 * Unit tests for the auto-wiring helper that lifts the figma_to_qc
 * production runner from CLI-only to server-side.
 *
 * Behavioural contract:
 *   - both gates off  -> factory undefined
 *   - one gate off    -> factory undefined
 *   - both gates on   -> factory returned, builds client lazily on first call
 *   - missing env vars-> ProductionRunnerError(LLM_GATEWAY_FAILED) on first call
 *   - secrets are not echoed in error messages
 */

import assert from "node:assert/strict";
import test from "node:test";

import { ProductionRunnerError } from "./production-runner.js";
import {
  resolveLlmConfigFromEnv,
  resolveTestIntelligenceProductionRunner,
} from "./test-intelligence-production-runner-factory.js";

const ENV_OK = {
  TEST_INTELLIGENCE_MODEL_ENDPOINT: "https://aoai.example/openai/v1",
  TEST_INTELLIGENCE_TESTCASE_MODEL_DEPLOYMENT: "gpt-oss-120b",
  TEST_INTELLIGENCE_LLM_API_KEY: "k-key-secret",
  TEST_INTELLIGENCE_VISUAL_MODEL_ENDPOINT: "https://aoai.example/openai/vision",
  TEST_INTELLIGENCE_VISUAL_PRIMARY_DEPLOYMENT: "llama-4-maverick-vision",
  TEST_INTELLIGENCE_VISUAL_FALLBACK_DEPLOYMENT: "phi-4-multimodal-instruct",
};

void test("resolveTestIntelligenceProductionRunner: undefined when startup gate off", () => {
  const factory = resolveTestIntelligenceProductionRunner({
    startupEnabled: false,
    envEnabled: true,
    env: ENV_OK,
  });
  assert.equal(factory, undefined);
});

void test("resolveTestIntelligenceProductionRunner: undefined when env gate off", () => {
  const factory = resolveTestIntelligenceProductionRunner({
    startupEnabled: true,
    envEnabled: false,
    env: ENV_OK,
  });
  assert.equal(factory, undefined);
});

void test("resolveLlmConfigFromEnv: throws ProductionRunnerError when endpoint missing", () => {
  assert.throws(
    () =>
      resolveLlmConfigFromEnv({
        TEST_INTELLIGENCE_LLM_API_KEY: "k",
      }),
    ProductionRunnerError,
  );
  // Verify the error message names the missing env var, not the secret.
  try {
    resolveLlmConfigFromEnv({
      TEST_INTELLIGENCE_LLM_API_KEY: "k-secret",
    });
    assert.fail("expected throw");
  } catch (err) {
    assert.ok(err instanceof ProductionRunnerError);
    assert.match(err.message, /TEST_INTELLIGENCE_MODEL_ENDPOINT/u);
    assert.doesNotMatch(err.message, /k-secret/u);
  }
});

void test("resolveLlmConfigFromEnv: throws ProductionRunnerError when api-key missing", () => {
  assert.throws(
    () =>
      resolveLlmConfigFromEnv({
        TEST_INTELLIGENCE_MODEL_ENDPOINT: "https://aoai.example/openai/v1",
      }),
    ProductionRunnerError,
  );
});

void test("resolveLlmConfigFromEnv: defaults deployment when not set", () => {
  const config = resolveLlmConfigFromEnv({
    TEST_INTELLIGENCE_MODEL_ENDPOINT: "https://aoai.example/openai/v1",
    TEST_INTELLIGENCE_LLM_API_KEY: "k",
  });
  assert.ok(config.deployment.length > 0);
  assert.equal(config.visualEndpoint, "https://aoai.example/openai/v1");
  assert.equal(config.visualPrimaryDeployment, "llama-4-maverick-vision");
  assert.equal(config.visualFallbackDeployment, "phi-4-multimodal-instruct");
  assert.equal(config.coveragePlannerDeployment, undefined);
});

void test("resolveLlmConfigFromEnv: hydrates optional coverage-planner deployment", () => {
  const config = resolveLlmConfigFromEnv({
    ...ENV_OK,
    TEST_INTELLIGENCE_COVERAGE_PLANNER_DEPLOYMENT: "phi-4-mini-instruct",
  });
  assert.equal(config.coveragePlannerDeployment, "phi-4-mini-instruct");
});

void test("resolveLlmConfigFromEnv: hydrates optional logic-judge deployment", () => {
  const config = resolveLlmConfigFromEnv({
    ...ENV_OK,
    TEST_INTELLIGENCE_LOGIC_JUDGE_DEPLOYMENT: "gpt-oss-120b",
  });
  assert.equal(config.logicJudgeDeployment, "gpt-oss-120b");
});

void test("resolveLlmConfigFromEnv: hydrates optional a11y-judge deployment", () => {
  const config = resolveLlmConfigFromEnv({
    ...ENV_OK,
    TEST_INTELLIGENCE_A11Y_JUDGE_DEPLOYMENT: "phi-4-multimodal-instruct",
  });
  assert.equal(config.a11yJudgeDeployment, "phi-4-multimodal-instruct");
});

void test("resolveTestIntelligenceProductionRunner: factory builds bundle lazily and forwards to runner", async () => {
  let buildCalls = 0;
  let runnerCalls = 0;
  const factory = resolveTestIntelligenceProductionRunner({
    startupEnabled: true,
    envEnabled: true,
    env: ENV_OK,
    buildLlmBundle: () => {
      buildCalls += 1;
      return {
        testGeneration: { tag: "test" },
        visualPrimary: { tag: "primary" },
        visualFallback: { tag: "fallback" },
      } as never;
    },
    runner: async (input) => {
      runnerCalls += 1;
      assert.equal(input.outputRoot, "/tmp/out");
      assert.ok(input.llm.client);
      assert.ok(input.llm.bundle);
      return {
        jobId: input.jobId,
        generatedAt: input.generatedAt,
        fileKey: "abc",
      } as never;
    },
  });
  assert.ok(factory);
  assert.equal(buildCalls, 0);

  await factory({
    jobId: "ti-test-1",
    generatedAt: "2026-05-02T00:00:00.000Z",
    source: { kind: "figma_url", figmaUrl: "x", accessToken: "t" },
    outputRoot: "/tmp/out",
  });
  assert.equal(buildCalls, 1, "builds on first call");
  assert.equal(runnerCalls, 1);

  // Second call reuses cached client.
  await factory({
    jobId: "ti-test-2",
    generatedAt: "2026-05-02T00:00:01.000Z",
    source: { kind: "figma_url", figmaUrl: "y", accessToken: "t" },
    outputRoot: "/tmp/out",
  });
  assert.equal(buildCalls, 1, "client cached after first call");
  assert.equal(runnerCalls, 2);
});

void test("resolveTestIntelligenceProductionRunner: missing env propagates as ProductionRunnerError on first invocation", async () => {
  const factory = resolveTestIntelligenceProductionRunner({
    startupEnabled: true,
    envEnabled: true,
    env: {}, // no envs at all
    runner: async () => {
      assert.fail("runner should not be reached when env is incomplete");
    },
  });
  assert.ok(factory);
  // `TestIntelligenceProductionRunnerFactory` declares a sync-or-async
  // return; `Promise.resolve` normalises the union to the `Promise` that
  // `assert.rejects` requires without changing settlement behaviour.
  await assert.rejects(
    Promise.resolve(
      factory({
        jobId: "ti-test-3",
        generatedAt: "2026-05-02T00:00:02.000Z",
        source: { kind: "figma_url", figmaUrl: "x", accessToken: "t" },
        outputRoot: "/tmp/out",
      }),
    ),
    (err: unknown) =>
      err instanceof ProductionRunnerError &&
      err.failureClass === "LLM_GATEWAY_FAILED",
  );
});
