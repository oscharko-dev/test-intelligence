...
void test("deriveCounterfactualPairs: generates pairs whose values come from the oracle", async () => {
  ...
  for (const pair of pairs) {
    ...
    assert.ok(
      pair.variantA.testData.some((entry) => entry.includes("synthesized")) ||
        pair.variantA.testData.some((entry) => entry.includes("from rule")),
      "variantA testData must reference oracle provenance",
    );
  }
});

void test("deriveCounterfactualPairs: deterministic across replays (byte-equal)", async () => {
  const model = buildModel();
  const invariants = buildActiveDatasetInvariantRegistry().list();
  const hypotheses = buildHypotheses(invariants, model);
...
void test("deriveCounterfactualPairs: synthesized variants carry sha256-hex audit hashes (and are deterministic)", async () => {
  ...
  for (const pair of first) {
    ...
    assert.notEqual(
      pair.variantA.audit.inputHash,
      pair.variantB.audit.inputHash,
    );
    assert.equal(
      pair.variantA.audit.schemaHash,
      pair.variantB.audit.schemaHash,
    );
  }
  ...
  for (const [idx, pair] of first.entries()) {
    ...
    assert.equal(
      replay.variantA.audit.inputHash,
      pair.variantA.audit.inputHash,
    );
    assert.equal(
      replay.variantA.audit.promptHash,
      pair.variantA.audit.promptHash,
    );
    assert.equal(
      replay.variantA.audit.schemaHash,
      pair.variantA.audit.schemaHash,
    );
    assert.equal(
      replay.variantB.audit.inputHash,
      pair.variantB.audit.inputHash,
    );
    assert.equal(
      replay.variantB.audit.promptHash,
      pair.variantB.audit.promptHash,
    );
    assert.equal(
      replay.variantB.audit.schemaHash,
      pair.variantB.audit.schemaHash,
    );
  }
});

void test("deriveCounterfactualPairs: different seeds produce different pairIds", async () => {
  const model = buildModel();
  const invariants = buildActiveDatasetInvariantRegistry().list();
  const hypotheses = buildHypotheses(invariants, model);
  const a = await deriveCounterfactualPairs({
...
void test("deriveCounterfactualPairs: rejects empty seed", async () => {
  const model = buildModel();
  const invariants = buildActiveDatasetInvariantRegistry().list();
  const hypotheses = buildHypotheses(invariants, model);
...
void test("deriveCounterfactualPairs: skips hypotheses whose cause field has no oracle-resolvable validation", async () => {
  const model = buildModel();
...
void test("deriveCounterfactualPairs: respects maxPairsPerHypothesis cap", async () => {
  const model = buildModel();
  const invariants = buildActiveDatasetInvariantRegistry().list();
  const hypotheses = buildHypotheses(invariants, model);
  const pairs = await deriveCounterfactualPairs({
...
void test("evaluateCounterfactualPairs: empty pair list yields zeroed report with correct shape", () => {
...
void test("evaluateCounterfactualPairs: persists per-pair audit rows sorted by pairId with the projected fields", async () => {
  ...
  for (const audit of report.pairs) {
    ...
    assert.ok(
      ...
      `audit row ${audit.pairId} has no source pair`,
    );
    ...
    assert.equal(
      audit.expectedEffectInvariant,
      sourcePair.expectedEffectInvariant,
    );
...
void test("evaluateCounterfactualPairs: counts no-effect violations when valueA == valueB", () => {
...
void test("evaluateCounterfactualPairs: round-trips coverage ratio to six decimals", () => {
  const hypothesis: CausalHypothesis = {
...
void test("summarizeCausalCoverage: projects to the policy-report KPI block", async () => {
  const model = buildModel();
  const invariants = buildActiveDatasetInvariantRegistry().list();
  const hypotheses = buildHypotheses(invariants, model);
  const pairs = await deriveCounterfactualPairs({
...
void test("assertWithinTokenBudget: zero additional tokens always fits", () => {
...
void test("assertWithinTokenBudget: enforces the configured cap", () => {
  const result = assertWithinTokenBudget({
    baselineTokens: 100,
...
void test("assertWithinTokenBudget: zero baseline + non-zero additional fails closed", () => {
  const result = assertWithinTokenBudget({
    baselineTokens: 0,
    additionalTokens: 1,
  });
