...
import {
  ...
  MARK_TRANSLATE_X,
  MARK_VIEWBOX_H,
} from "./customer-markdown-pdf-mappe-mark.js";
...
const FOOTER_TEXT = "Test Intelligence - Customer Dossier";
 ...
 *
 * Supported commands cover everything the product mark uses today:
 ...
 *   - `Z` / `z` (closepath)
 *
 * The SVG outer-group horizontal flip is folded into the rendering:
 * each SVG x becomes `MARK_TRANSLATE_X - x`. The product mark is
 * horizontally symmetric, so this flip is a visual no-op.
 *
...
/**
 * Emit the PDF content-stream snippet that draws the neutral product
 * mark centred at `(originX, originY)` (lower-left of its bounding
 * box) at `size` points wide.
 */
const drawProductMark = (
  originX: number,
  originY: number,
  size: number,
  fillColor: Rgb,
): string => {
  const scale = size / MARK_VIEWBOX_H;
  const ops = svgPathToPdfOps({
    d: MARK_PATH_D,
    viewBoxH: MARK_VIEWBOX_H,
    translateX: MARK_TRANSLATE_X,
    originX,
    originY,
    scale,
  });
...
const layoutCover = (doc: PdfDocument, input: BuildMappeInput): void => {
  ...
  // Neutral product mark, top-left of the cover content area. Drawn
  // as a white-on-green-deep rounded-square plaque with the brand-green
  // product mark inside.
  const LOGO_SIZE = 110;
...
const stripSingleAsteriskMarkdown = (value: string): string => {
  let out = "";
  for (let i = 0; i < value.length; i += 1) {
...
const isTableRow = (raw: string): boolean => {
  const trimmed = raw.trimStart();
  return trimmed.startsWith("|") && trimmed.indexOf("|", 1) >= 0;
};

const isTableSeparatorRow = (raw: string): boolean => {
  const cells = stripOuterTablePipes(raw).split("|");
...
const parseBullet = (raw: string): string | undefined => {
...
const parseLabeledParagraph = (
  ...
  return {
    label: raw.slice(2, close),
    rest: raw.slice(close + 3).trimStart(),
  };
};
...
const parseMarkdown = (md: string): Block[] => {
  ...
  for (let idx = 0; idx < lines.length; idx += 1) {
    ...
    if (bullet !== undefined) {
      flushPara();
      out.push({ kind: "bullet", text: stripInlineMarkdown(bullet) });
      continue;
    }
    ...
    /* Labeled paragraph: a line that begins with `**Label:**` is a
       ...
       heavier weight. */
    const labeled = parseLabeledParagraph(raw);
    if (labeled !== undefined) {
      flushPara();
      const labelText = labeled.label;
      const rest = labeled.rest;
      out.push({
        kind: "labeled",
        text:
          rest.length === 0
