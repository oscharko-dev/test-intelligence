...
void test("property: identical inputs yield identical reports (determinism)", () => {
  fc.assert(
...
void test("property: zero-width count == zero-widths in the input", () => {
  fc.assert(
    fc.property(
      fc.array(fc.constantFrom(...ZERO_WIDTH_CODEPOINTS), {
        minLength: 0,
...
void test("property: sanitised text never contains zero-width characters", () => {
  fc.assert(
    fc.property(arbZeroWidthSpiced, (text) => {
      const result = normalizeUntrustedContent({
        textFields: [{ id: "f", text }],
...
void test("property: per-element cap is never exceeded after normalization", () => {
  fc.assert(
    fc.property(
      fc.string({
        minLength: 0,
...
void test("property: outcome is needs_review iff a critical carrier fired", () => {
  fc.assert(
    fc.property(
      fc.record({
...
void test("property: report is JSON-serialisable and survives a round trip", () => {
  fc.assert(
    fc.property(
      arbSafeText,
      ...
      (text, zw) => {
        ...
        const round: unknown = JSON.parse(JSON.stringify(result.report));
...
void test("property: markdown over byte cap is always truncated to <= cap", () => {
  fc.assert(
    fc.property(
      fc.string({
