...
import type { SupportedLocale } from "@oscharko-dev/ti-eval";
...
void test("buildGeneratedTestCaseConfidenceComponents derives deterministic raw inputs", () => {
...
void test("loadCaseConfidenceCalibration fits a historical fallback curve and persists it under sandbox/calibration", async () => {
  ...
  for (const [
    ...
    await writeJson(join(runDir, "test-data-oracle-report.json"), {
      cases: [
        {
          testCaseId: testCase.id,
          authoritativeTestData: oracleResolved
  ...
  assert.match(
    loaded.artifactPath,
    new RegExp(
      `${CASE_CONFIDENCE_CURVE_ARTIFACT_FILENAME.replace(".", "\\.")}$`,
    ),
  );
  ...
  assert.equal(
    typeof calibrated.testCases[0]?.confidenceComponents?.rawScore,
    "number",
  );
  const summary = summarizeCaseConfidenceDistribution(calibrated);
  assert.notEqual(summary, undefined);
  assert.equal(
    (summary?.confidenceP90 ?? 0) >= (summary?.confidenceP10 ?? 0),
    true,
  );
});

void test("loadCaseConfidenceCalibration persists held-out diagnostics once history reaches ten samples", async () => {
  ...
  assert.equal(loaded.curve.minimumRiskCategorySampleFloor, 50);
    ...
    for (let i = 0; i < count * 2; i++) {
      ...
      cases.push(
        buildCase(`tc-${runDir.slice(-8)}-${offset + i}`, {
          ...
          figmaTraceRefs: [
            { screenId: sid, nodeId: `${sid}-node-${offset + i}` },
          ],
...
void test("loadCaseConfidenceCalibration emits per-locale curves and reliability artifacts when screenLocaleMap supplied", async () => {
  ...
  assert.equal(loaded.curve.perLocaleEceThreshold, 0.1);
  ...
  assert.equal(
    deDeEntry!.fallbackToDefault,
    false,
    "DE-DE should have a genuine fit",
  );
  assert.equal(
    deAtEntry!.fallbackToDefault,
    false,
    "DE-AT should have a genuine fit",
  );
  ...
  assert.equal(
    deChEntry!.fallbackToDefault,
    true,
    "DE-CH should fallback to default",
  );
  ...
  const localeArtifact = JSON.parse(
    await readFile(localeArtifactPath!, "utf8"),
  ) as {
    locale: string;
    sampleCount: number;
    debiasedEce: number;
...
void test("applyCaseConfidenceCalibration uses the per-locale curve when present and falls back to default for unseen locales", async () => {
  const root = await mkdtemp(join(tmpdir(), "case-confidence-locale-apply-"));
  const datasetRoot = join(
    root,
    "sandbox",
    "test-case",
    "locale-dataset-apply",
  );
  ...
  const confidenceWithoutLocale =
    calibratedWithMap.testCases[1]?.confidence ?? -1;
  ...
  const deDeEntry = loaded.curve.localeCurves["DE-DE"];
  if (!deDeEntry.fallbackToDefault) {
  ...
  assert.equal(
    confidenceWithoutLocale >= 0 && confidenceWithoutLocale <= 1,
    true,
  );
});
