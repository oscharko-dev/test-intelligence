...
} from "@oscharko-dev/ti-evidence";
import {
  computeWave1ValidationEvidenceManifestDigest,
  verifyWave1ValidationEvidenceFromDisk,
} from "@oscharko-dev/ti-evidence";
import { validateLbomDocument } from "@oscharko-dev/ti-evidence";
import {
...
import { deriveBusinessTestIntentIr } from "@oscharko-dev/ti-core-engine";
import { loadWave1ValidationFixture } from "@oscharko-dev/ti-core-engine";
import {
...
import { createMemoryReplayCache } from "@oscharko-dev/ti-model-gateway";
...
const lbomPropertyMap = (
  ...
  new Map(
    lbom.metadata.properties.map((property) => [property.name, property.value]),
  );
...
void test("validation-harness: synthetic test generation covers risk labels, actions, navigation, and trace fallbacks", () => {
  ...
  assert.equal(
    list.testCases.every(
      (tc) =>
        typeof tc.polarity === "string" && typeof tc.category === "string",
    ),
    true,
  );
  assert.equal(
    list.testCases.every(
      (tc) => tc.audit !== audit && tc.audit.jobId === audit.jobId,
    ),
    true,
  );
});

void test("validation-harness: unresolved validation rules stay generic and surface open questions", () => {
  const intent: BusinessTestIntentIr = {
    version: BUSINESS_TEST_INTENT_IR_SCHEMA_VERSION,
...
for (const fixtureId of WAVE1_VALIDATION_FIXTURE_IDS) {
  void test(`validation-harness: ${fixtureId} runs end-to-end without external network`, async () => {
    ...
    assert.ok(
      attestedMlBom,
      "manifest must attest the release ML-BOM artifact",
    );
  ...
  void test(`validation-harness: ${fixtureId} produces deterministic artifacts on replay`, async () => {
  ...
  void test(`validation-harness: ${fixtureId} no original PII substrings appear in any persisted artifact`, async () => {
    const runDir = await newRunDir();
    const result = await runWave1Validation({
      fixtureId,
  ...
  void test(`validation-harness: ${fixtureId} verifies clean against verifyWave1ValidationEvidenceFromDisk`, async () => {
    const runDir = await newRunDir();
  ...
  void test(`validation-harness: ${fixtureId} verification fails after deliberate artifact mutation`, async () => {
    const runDir = await newRunDir();
...
void test("validation-harness: export-refused run still emits and attests the release ML-BOM", async () => {
  ...
  assert.ok(
    result.exportArtifacts.refusalCodes.includes(
...
void test("validation-harness: seals request audit proof that test generation received no images", async () => {
  const runDir = await newRunDir();
  const result = await runWave1Validation({
...
void test("validation-harness: sigstore signing returns summary and verifies from disk", async () => {
  const runDir = await newRunDir();
...
void test("validation-harness: post-hoc bySource mutation fails signed attestation verify", async () => {
  const runDir = await newRunDir();
  const { privateKeyPem, publicKeyPem } =
    generateWave1ValidationAttestationKeyPair();
  const signer = createKeyBoundSigstoreSigner({
...
void test("validation-harness: visual mask hash participates in compiled prompt identity", async () => {
  const runDir = await newRunDir();
  const result = await runWave1Validation({
    fixtureId: "validation-payment-auth",
...
void test("validation-harness: visual sidecar gate must not block on shipped fixtures", async () => {
  for (const fixtureId of WAVE1_VALIDATION_FIXTURE_IDS) {
    const runDir = await newRunDir();
    const run = await runWave1Validation({
      fixtureId,
...
void test("validation-harness: structured-test-case generator never receives image payloads", async () => {
...
void test("validation-harness: default execution does not touch live gateway fetch", async () => {
...
void test("validation-harness: FinOps report is returned and persisted with supplied budget and cost rates", async () => {
  const runDir = await newRunDir();
  const budget = {
...
void test("validation-harness: FinOps maxInputTokensPerRequest fails closed before downstream artifacts", async () => {
  const runDir = await newRunDir();
...
void test("validation-harness: aggregate FinOps breaches stop before validation and export", async () => {
  const runDir = await newRunDir();
  await assert.rejects(
    runWave1Validation({
      fixtureId: "validation-onboarding",
...
void test("validation-harness: replay-cache hit skips generation and reports completed_cache_hit", async () => {
...
void test("validation-harness: synthesizer derives banking/insurance regulatory riskCategory from field labels (Issue #2030)", () => {
  const intent: BusinessTestIntentIr = {
    version: BUSINESS_TEST_INTENT_IR_SCHEMA_VERSION,
    source: { kind: "figma_local_json", contentHash: "2".repeat(64) },
    screens: [
    ...
    detectedFields: [
      ...
      // regulated_data matches (PR-N+2 additions)
      {
        id: "f-mifid",
        screenId: "s-2030",
        trace: { nodeId: "n-mifid" },
        provenance: "figma_node",
        confidence: 0.9,
        label: "MiFID-Risikoklasse",
        type: "informative_label",
      },
      {
        id: "f-schufa",
        screenId: "s-2030",
        trace: { nodeId: "n-schufa" },
        provenance: "figma_node",
        confidence: 0.9,
      ...
      },
      {
        ...
        provenance: "figma_node",
        confidence: 0.9,
        label: "MFA fuer alle Mitarbeiter",
        type: "radio_option",
      },
      {
        id: "f-edr",
      ...
      },
      {
        ...
        provenance: "figma_node",
        confidence: 0.9,
        label: "WCAG-2.2-AA Kontrast-Hinweis",
        type: "informative_label",
      },
...
void test("validation-harness: synthesizer keyword list does NOT collide with MA-0 baseline labels (Issue #2030)", () => {
  ...
  for (const [label, expected] of baselineLabelsAndExpectedRisk) {
    ...
    const tc = list.testCases.find((c) =>
      c.id.startsWith("tc-field-functional-"),
    );
    assert.equal(
      tc?.riskCategory,
...
void test("validation-harness: complianceOverrides elevate label-less fields to the sidecar category (Issue #2030)", () => {
  const intent: BusinessTestIntentIr = {
    version: BUSINESS_TEST_INTENT_IR_SCHEMA_VERSION,
...
void test("validation-harness: complianceOverrides elevate action/nav/a11y cases on regulated screens (Issue #2030)", () => {
  const intent: BusinessTestIntentIr = {
    version: BUSINESS_TEST_INTENT_IR_SCHEMA_VERSION,
  ...
  const actionCase = list.testCases.find((tc) =>
    tc.id.startsWith("tc-action-"),
  );
  const navCase = list.testCases.find((tc) =>
    tc.id.startsWith("tc-navigation-"),
  );
...
void test("validation-harness: complianceOverrides preserve hardcoded `low` when no override applies (Issue #2030 / baseline immunity)", () => {
