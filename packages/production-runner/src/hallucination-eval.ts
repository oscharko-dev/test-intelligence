 ...
 * fixtures shipped under `packages/core-engine/src/fixtures/`. The
...
export const computeHallucinationMetrics = (
  ...
  const tolerance = input.fuzzyToleranceLevenshtein ?? DEFAULT_FUZZY_TOLERANCE;
  ...
  for (const testCase of input.generatedList.testCases) {
    for (const step of testCase.steps) {
      ...
      for (const text of texts) {
        ...
        for (const match of extractButtonStateReferences(text)) {
          const state = match.state;
          const buttonReference = match.reference;
...
const extractButtonStateReferences = (
  text: string,
): ReadonlyArray<{ state: string; reference: string }> => {
  const lower = text.toLowerCase();
  const out: Array<{ state: string; reference: string }> = [];
  for (const state of BUTTON_STATES) {
    const prefix = `the ${state} `;
    let cursor = lower.indexOf(prefix);
    while (cursor >= 0) {
      const referenceStart = cursor + prefix.length;
      const buttonEnd = findButtonStateReferenceEnd(lower, referenceStart);
      if (buttonEnd > referenceStart) {
        const reference = text.slice(referenceStart, buttonEnd).trim();
        if (reference.length > 0) out.push({ state, reference });
      }
      cursor = lower.indexOf(prefix, cursor + 1);
    }
  }
  return out;
};

const findButtonStateReferenceEnd = (
  lower: string,
  referenceStart: number,
): number => {
  const button = lower.indexOf(" button", referenceStart);
  const control = lower.indexOf(" control", referenceStart);
  if (button < 0) return control;
  if (control < 0) return button;
  return Math.min(button, control);
};
