...
void test("renderCustomerMarkdown emits a German-format combined document with one heading per case", () => {
...
void test("renderCustomerMarkdown ignores id-derived numbering and renumbers customer labels sequentially", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [
  ...
  assert.deepEqual(
    result.perCaseFiles.map((file) => file.filename),
    ["tc01-antrag-absenden.md", "tc02-felder-validieren.md"],
  );
});

void test("renderCustomerMarkdown renders steps with Beschreibung + Erwartetes Ergebnis", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [
...
void test("renderCustomerMarkdown renders explicit field lifecycle state transitions", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [
...
void test("renderCustomerMarkdown uses persisted polarity when JSON classification is present", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [
...
void test("renderCustomerMarkdown keeps confidence hidden in customer mode by default", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [
...
void test("renderCustomerMarkdown renders confidence when explicitly enabled or in technical mode", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [
  ...
  assert.match(
    customerMode.perCaseFiles[0]?.body ?? "",
    /\*\*Konfidenz:\*\* 0\.93/u,
  );
  assert.match(
    technicalMode.perCaseFiles[0]?.body ?? "",
    /\*\*Konfidenz:\*\* 0\.93/u,
  );
});

void test("renderCustomerMarkdown customer mode consolidates shared clarification questions and strips provenance prefixes", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [
      buildCase({
        ...
        assumptions: [
          "Die Validierungsregel für den Kaufpreis stammt aus dem Fachkonzept.",
        ],
        openQuestions: [
...
void test("renderCustomerMarkdown emits one clarification reference when a case repeats the same question", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [
...
void test("renderCustomerMarkdown produces filename-safe slugs", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [
...
void test("renderCustomerMarkdown gracefully handles an empty test-case list", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [],
...
void test("renderCustomerMarkdown escapes backslashes, pipes, and newlines in overview table cells", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [
...
void test("renderCustomerMarkdown filenames are deterministic for stable input", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [
...
void test("renderCustomerMarkdown surfaces regulatoryRelevance domain + rationale when present (Issue #1735)", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [
...
void test("renderCustomerMarkdown maps enumerated acceptance criteria into the summary and case traceability", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [
      buildCase({
        ...
        objective:
          "Bestätigen, dass der Antrag nach erfolgreicher Validierung abgesendet wird.",
        expectedResults: [
          "Der Antrag wird erfolgreich abgesendet und bestätigt.",
        ],
        steps: [
          {
            index: 1,
      ...
      buildCase({
        ...
        objective:
          "Prüfen, dass fehlende Pflichtfelder eine verständliche Fehlermeldung anzeigen.",
...
void test("renderCustomerMarkdown technical mode preserves internal traceability on request", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [
...
void test("renderCustomerMarkdown adds a Compliance coverage section when supplied (Issue #2042)", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
...
void test("Issue #2170: faithfulnessPartialMajorityCaseIds emits a German evidence-partial footer on flagged cases only", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [buildCase({ id: "tc-flagged" }), buildCase({ id: "tc-clean" })],
  };
  const result = renderCustomerMarkdown({
    list,
    fileName: "x",
...
void test("Issue #2170: omitting faithfulnessPartialMajorityCaseIds leaves every case footer-free", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
...
void test("extractAcceptanceCriteriaFromMarkdown reads enumerated markdown sections", () => {
  const markdown = [
    "# Kontext",
    "",
    "## Akzeptanzkriterien",
