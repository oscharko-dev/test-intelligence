 ...
 *
 * Self-skips unless `TEST_INTELLIGENCE_LIVE_E2E=1`. When enabled,
...
import type { FigmaRestFileSnapshot } from "@oscharko-dev/ti-core-engine";
import { requireLiveSmokeApiKey } from "./visual-sidecar-client.live-env.js";

const LIVE_E2E_FLAG = "TEST_INTELLIGENCE_LIVE_E2E";

type NonAuthRequiredEnv =
  | "TEST_INTELLIGENCE_MODEL_ENDPOINT"
  | "TEST_INTELLIGENCE_TESTCASE_MODEL_DEPLOYMENT";

const FIXTURE_PATH = new URL(
  "../../../packages/core-engine/src/fixtures/live-e2e/banking-antrag.figma.json",
  import.meta.url,
);
...
const LIVE_FIGMA_URL_ENV = "TEST_INTELLIGENCE_FIGMA_URL";
const FIGMA_ACCESS_TOKEN_ENV = "TEST_INTELLIGENCE_FIGMA_ACCESS_TOKEN";

const requireEnv = (name: NonAuthRequiredEnv): string => {
...
void test("live-E2E: production runner generates test cases against Azure for a synthetic banking Antrag", async (t) => {
  ...
  const visualBaseUrl =
    process.env["TEST_INTELLIGENCE_VISUAL_MODEL_ENDPOINT"] ??
    requireEnv("TEST_INTELLIGENCE_MODEL_ENDPOINT");
  const visualPrimaryDeployment =
    process.env["TEST_INTELLIGENCE_VISUAL_PRIMARY_DEPLOYMENT"] ??
    requireEnv("TEST_INTELLIGENCE_TESTCASE_MODEL_DEPLOYMENT");
  const visualFallbackDeployment =
    process.env["TEST_INTELLIGENCE_VISUAL_FALLBACK_DEPLOYMENT"] ??
  ...
  const bundle: LlmGatewayClientBundle = createLlmGatewayClientBundle(
    {
      testGeneration: buildConfig({
        ...
        baseUrl: requireEnv("TEST_INTELLIGENCE_MODEL_ENDPOINT"),
        deployment: requireEnv("TEST_INTELLIGENCE_TESTCASE_MODEL_DEPLOYMENT"),
        imageInputSupport: false,
      }),
