...
import type { AgentSourceLabel } from "@oscharko-dev/ti-model-gateway";

export const REGION_ATTESTATION_SIGNING_KEY_ENV =
  "TEST_INTELLIGENCE_REGION_ATTESTATION_SIGNING_KEY" as const;
export const REGION_ATTESTATION_PINNED_REGION_ENV =
  "TEST_INTELLIGENCE_REGION_ATTESTED_REGION" as const;
const REGION_ATTESTATION_SIGNING_KEY_ENV_ALIASES = [
  REGION_ATTESTATION_SIGNING_KEY_ENV,
  "WORKSPACE_TEST_SPACE_REGION_ATTESTATION_SIGNING_KEY",
] as const;
const REGION_ATTESTATION_PINNED_REGION_ENV_ALIASES = [
...
export const REGION_ATTESTATION_SOVEREIGN_SOURCE_ENV =
  "TEST_INTELLIGENCE_REGION_ATTESTATION_SOVEREIGN_SOURCE" as const;
const REGION_ATTESTATION_SOVEREIGN_SOURCE_ENV_ALIASES = [
...
const MOCK_GATEWAY_ATTESTATION_SIGNING_KEY =
  "test-intelligence-local-mock-region-attestation-key";
...
const parseSupportedRegion = (
  ...
  const matched = supportedRegions.find((candidate) =>
...
const readPinnedRegion = (
  ...
  const envValue = readFirstEnv(REGION_ATTESTATION_PINNED_REGION_ENV_ALIASES);
  return isRegion(envValue) ? envValue : undefined;
};

const readFirstEnv = (names: readonly string[]): string | undefined => {
 ...
 * `TEST_INTELLIGENCE_AIR_GAP_MODE` (a sovereign deployment may run
      ...
      () => {
        ...
        const subjectAltName =
          typeof cert.subjectaltname === "string"
            ? cert.subjectaltname
            : undefined;
        const san = parseRegionFromCertificateText(subjectAltName);
        socket.end();
...
export const resolveRegionAttestationObservation = async (
  ...
  const fromCertificate = await resolveFromTlsCertificate(
...
const readSigningKey = (deploymentId: string): string => {
  const signingKey = readFirstEnv(REGION_ATTESTATION_SIGNING_KEY_ENV_ALIASES);
  ...
  const distinctRegions = Array.from(
    new Set(
      artifacts.flatMap((artifact) =>
        artifact.regionAttestations.map(
          (attestation) => attestation.servedFromRegion,
        ),
      ),
    ),
  ).sort() as RegionAttestationHostingRegion[];
