...
import { loadWave1ValidationCaptureFixture } from "@oscharko-dev/ti-core-engine";
import {
  runWave1Validation,
  Wave1ValidationVisualSidecarFailureError,
} from "./validation-harness.js";
import { verifyWave1ValidationEvidenceFromDisk } from "@oscharko-dev/ti-evidence";
import { type MockResponder } from "@oscharko-dev/ti-model-gateway";
...
const assertFailureManifestAttestsSidecar = async (
  ...
  const { manifest, result } =
    await verifyWave1ValidationEvidenceFromDisk(runDir);
...
void test("validation-harness visualCaptures: happy path — primary succeeds, artifacts land on disk", async () => {
...
void test("validation-harness visualCaptures: fallback path — primary timeout, fallback succeeds", async () => {
  const runDir = await newRunDir();
  try {
    const { captures } = await loadWave1ValidationCaptureFixture(FIXTURE_ID);
...
void test("validation-harness visualCaptures: FinOps image byte budget fails closed before sidecar calls", async () => {
  const runDir = await newRunDir();
  try {
    const { captures } = await loadWave1ValidationCaptureFixture(FIXTURE_ID);
    ...
    assert.equal(
      (
        bundle.visualPrimary as unknown as { callCount: () => number }
      ).callCount(),
      0,
    );
    assert.equal(
      (
        bundle.visualFallback as unknown as { callCount: () => number }
      ).callCount(),
      0,
    );
...
void test("validation-harness visualCaptures: invalid sidecar JSON — harness throws, no test cases produced", async () => {
  const runDir = await newRunDir();
  try {
    const { captures } = await loadWave1ValidationCaptureFixture(FIXTURE_ID);
...
void test("validation-harness visualCaptures: both sidecars fail — harness throws, no test cases produced", async () => {
  const runDir = await newRunDir();
  try {
    const { captures } = await loadWave1ValidationCaptureFixture(FIXTURE_ID);
