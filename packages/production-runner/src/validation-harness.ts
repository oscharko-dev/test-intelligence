 ...
 *
 * Runs the full visual-evidence test-generation chain against an air-gapped
 ...
 *
 *   1. Load a local visual fixture + companion visual sidecar.
...
import { deriveGeneratedTestCaseClassification } from "@oscharko-dev/ti-core-engine";
...
import { cloneOpenTextAlmReferenceProfile } from "@oscharko-dev/ti-integrations";
...
import { type SelfVerifyRubricReplayCache } from "@oscharko-dev/ti-quality";
import {
...
const SYNTHESIZED_RISK_OVERRIDE_PRIORITY: ReadonlyArray<TestCaseRiskCategory> =
  ["regulated_data", "financial_transaction"];
...
export const synthesizeGeneratedTestCases = (input: {
  ...
  const oracleAllTestData = (fieldId: string, fieldLabel: string): string[] => [
    ...
    cases.push(
      buildSyntheticCase({
        ...
        ...(oracleOpenQuestion(field.id) !== undefined
          ? {
              openQuestions: [oracleOpenQuestion(field.id) as string],
            }
          : {}),
        ...stampAudit(input),
      }),
 ...
 * `packages/core-engine/src/fixtures/eval-baseline-*.json` snapshots in
 * the same commit.
 */
  ...
  for (const testCase of input.list.testCases) {
    for (const step of testCase.steps) {
      stepVerdicts.push({
        ...
        verdict: "match",
        message:
          "Deterministic Wave 1 harness fixture matched the visual evidence.",
      });
    }
  }
  return {
