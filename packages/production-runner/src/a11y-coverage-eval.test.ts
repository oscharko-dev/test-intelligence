...
import { GENERATOR_FORM_SCREEN_A11Y_REPAIR_INSTRUCTION } from "@oscharko-dev/ti-model-gateway";
import { canonicalJson } from "@oscharko-dev/ti-security";
import { deriveBusinessTestIntentIr } from "@oscharko-dev/ti-core-engine";
import type { IntentDerivationFigmaInput } from "@oscharko-dev/ti-core-engine";
...
void test("WCAG 2.2 AA pillar list is closed and stable", () => {
  assert.deepEqual([...A11Y_WCAG_22_AA_PILLAR_IDS].sort(), [
    "color-contrast",
    "error-announcements",
    "focus-indicator",
...
void test("hard gate trips when the candidate list has no a11y case anchored to a form screen", async () => {
...
void test("Issue #1951: hard gate trips when the anchored accessibility case omits screen-reader announcements", async () => {
  const fixture = await loadFigma("baseline-simple-form");
  const intent = deriveBusinessTestIntentIr({ figma: fixture });
  const screenId = intent.detectedFields[0]?.screenId;
  assert.ok(screenId, "fixture must carry at least one detected field");
  ...
  const result = computeA11yCoverage({ intent, generatedList: list });
  const failure = result.verdict.failures.find(
    (entry) => entry.reason === "form_screen_missing_accessibility_criterion",
  );
...
void test("hard gate passes for the deterministic synthesiser output of baseline-simple-form", async () => {
  ...
  for (const screen of artifact.perScreen) {
    ...
    assert.deepEqual(screen.coveredCriteria, [
      ...FORM_SCREEN_A11Y_REQUIRED_CRITERIA,
    ]);
    assert.equal(
      screen.expectedPillars.length,
      A11Y_WCAG_22_AA_PILLAR_IDS.length,
    );
  }
});

void test("hard gate passes for the synthesiser output of baseline-complex-mask", async () => {
  const artifact = await buildA11yCoverageEvalArtifactForBaseline({
...
void test("validation-onboarding fixture lands at least one a11y case after the synthesiser pass (Wave 2 + 3 simulation)", async () => {
...
void test("isFormScreenA11yCase only matches anchored accessibility cases", () => {
...
void test("Issue #1951: listCoveredFormScreenA11yCriteria detects the three required sub-criteria", () => {
...
void test("Issue #1951: Unicode hyphen in Screen-Reader still satisfies screen-reader criterion", () => {
  const criteria = listCoveredFormScreenA11yCriteria(
    buildCase({
...
void test("buildA11yCoverageRepairInstruction renders the canonical template", () => {
  ...
  assert.equal(
    ri.instruction,
    GENERATOR_FORM_SCREEN_A11Y_REPAIR_INSTRUCTION.replace(
      "{screenId}",
      "s-loan",
    ),
  );
...
void test("computeA11yCoverage is deterministic and byte-stable", async () => {
...
void test("buildAllBaselineA11yCoverageEvalArtifacts covers every baseline archetype", async () => {
...
void test("writeA11yCoverageEvalArtifact persists canonical-JSON output and round-trips", async () => {
  const artifact = await buildA11yCoverageEvalArtifactForBaseline({
    archetypeId: "baseline-simple-form",
  });
...
void test("a11yCoverageEvalReportFilename strips the baseline- prefix and uses `a11y-<id>.json`", () => {
  assert.equal(
    a11yCoverageEvalReportFilename({
...
void test("default thresholds match documented production-baseline values", () => {
  assert.equal(
...
void test("report directory constant is the documented eval-reports path", () => {
  assert.equal(
    A11Y_COVERAGE_EVAL_REPORT_DIRNAME,
