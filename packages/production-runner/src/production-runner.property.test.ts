    ...
    await fc.assert(
      fc.asyncProperty(fileModelArb, async (model) => {
        const tempRoot = await mkdtemp(
          path.join(os.tmpdir(), "ti-runner-prop-"),
        );
        try {
    ...
    await fc.assert(
      fc.asyncProperty(fileModelArb, async (model) => {
        ...
        try {
          ...
          const secondHashes = {
            compiledPrompt: await sha256File(
              second.artifactPaths.compiledPrompt,
            ),
            ...
            coverageReport: await sha256File(
              second.artifactPaths.coverageReport,
            ),
          };
          const third = await run();
          const thirdHashes = {
            compiledPrompt: await sha256File(
              third.artifactPaths.compiledPrompt,
            ),
            generatedTestCases: await sha256File(
            ...
            coverageReport: await sha256File(
              third.artifactPaths.coverageReport,
            ),
          };
