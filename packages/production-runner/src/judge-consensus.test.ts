...
import type { JudgeConsensusAgreementShape } from "@oscharko-dev/ti-contracts";

interface ConsensusScenario {
  readonly name: string;
  readonly panel: readonly ("accept" | "repair" | "reject")[];
  readonly verdict: "accept" | "repair" | "reject";
  readonly agreementShape: JudgeConsensusAgreementShape;
  readonly confidenceByIndex?: Readonly<Record<number, number>>;
  readonly judgeIds?: readonly string[];
  readonly schemaRepairAt?: number;
  readonly mismatchAt?: number;
  readonly transientRejectAt?: number;
...
const buildFaithfulnessVerdict = (
  ...
  options?: Partial<Pick<FaithfulnessVerdict, "hallucinations" | "mismatches">>,
): FaithfulnessVerdict => ({
...
void test("buildJudgeConsensus accepts a single accept judge unchanged", () => {
...
void test("buildJudgeConsensus preserves a logic schema-class veto as repair and surfaces vetoBy", () => {
  const consensus = buildJudgeConsensus({
    jobId: "job-consensus",
    generatedAt: "2026-05-06T00:00:00.000Z",
    panel: [
...
void test("buildJudgeConsensus preserves a faithfulness mismatch veto and unions repair instructions", () => {
  const consensus = buildJudgeConsensus({
    jobId: "job-consensus",
    generatedAt: "2026-05-06T00:00:00.000Z",
    panel: [
...
void test("buildJudgeConsensus keeps a low-confidence accept versus reject tie as a split", () => {
  const consensus = buildJudgeConsensus({
    jobId: "job-consensus",
    generatedAt: "2026-05-06T00:00:00.000Z",
    panel: [
...
void test("Issue #2102: consensus voting protocol covers 30 handcrafted disagreement cases", () => {
  ...
  const cases: readonly ConsensusScenario[] = [
    {
      name: "unanimous accept",
      panel: ["accept", "accept", "accept"],
      verdict: "accept",
      agreementShape: "unanimous",
    },
    {
      name: "unanimous repair",
      panel: ["repair", "repair", "repair"],
      verdict: "repair",
      agreementShape: "unanimous",
    },
    {
      name: "majority repair",
      panel: ["repair", "repair", "accept"],
      verdict: "repair",
      agreementShape: "majority",
    },
    {
      name: "majority accept no dissenting repair",
      panel: ["accept", "accept", "accept"],
      verdict: "accept",
      agreementShape: "unanimous",
    },
    {
      name: "majority accept with one repair",
      panel: ["accept", "accept", "repair"],
      verdict: "repair",
      agreementShape: "majority",
    },
    {
      name: "low confidence a11y reject becomes repair against accept majority",
      panel: ["accept", "accept", "reject"],
      verdict: "repair",
      agreementShape: "majority",
      confidenceByIndex: { 2: 0.4 },
    },
    {
      name: "high confidence reject vetoes accept majority",
      panel: ["accept", "accept", "reject"],
      verdict: "reject",
      agreementShape: "vetoed",
      judgeIds: ["logic_judge", "faithfulness_judge", "logic_judge"],
    },
    {
      name: "high confidence reject vetoes repair majority",
      panel: ["repair", "repair", "reject"],
      verdict: "reject",
      agreementShape: "vetoed",
      judgeIds: ["logic_judge", "faithfulness_judge", "logic_judge"],
    },
    {
      name: "logic repair schema veto stays vetoed",
      panel: ["repair", "accept"],
      verdict: "repair",
      agreementShape: "vetoed",
      schemaRepairAt: 0,
    ...
    {
      name: "faithfulness mismatch veto stays vetoed",
      panel: ["accept", "repair"],
      verdict: "repair",
      agreementShape: "vetoed",
      mismatchAt: 1,
    ...
    {
      name: "accept repair low-confidence a11y reject resolves as majority repair",
      panel: ["accept", "repair", "reject"],
      verdict: "repair",
      agreementShape: "majority",
      confidenceByIndex: { 2: 0.5 },
    },
    {
      name: "two-judge accept repair is split",
      panel: ["accept", "repair"],
      verdict: "repair",
      agreementShape: "split",
    },
    {
      name: "two-judge accept reject low confidence is split",
      panel: ["accept", "reject"],
      verdict: "repair",
      agreementShape: "split",
      confidenceByIndex: { 1: 0.5 },
    },
    {
      name: "two-judge accept reject high confidence vetoes",
      panel: ["accept", "reject"],
      verdict: "reject",
      agreementShape: "vetoed",
      judgeIds: ["logic_judge", "logic_judge"],
    },
    {
      name: "a11y reject normalizes to repair",
      panel: ["accept", "accept", "reject"],
      verdict: "repair",
      agreementShape: "majority",
      confidenceByIndex: { 2: 0.5 },
      judgeIds: ["logic_judge", "faithfulness_judge", "a11y_judge"],
    },
    {
      name: "transient a11y reject does not veto but still normalizes to repair",
      panel: ["accept", "accept", "reject"],
      verdict: "repair",
      agreementShape: "majority",
      transientRejectAt: 2,
    ...
    {
      name: "transient two-judge reject splits",
      panel: ["accept", "reject"],
      verdict: "repair",
      agreementShape: "split",
      transientRejectAt: 1,
    ...
    {
      name: "weighted majority accept with repair still repairs",
      panel: ["accept", "accept", "repair"],
      verdict: "repair",
      agreementShape: "majority",
      weights: { 0: 2, 1: 2, 2: 1 },
    ...
    {
      name: "weighted majority repair stays repair",
      panel: ["repair", "accept", "repair"],
      verdict: "repair",
      agreementShape: "majority",
      weights: { 0: 2, 1: 1, 2: 2 },
    },
    {
      name: "weighted split stays split",
      panel: ["accept", "repair"],
      verdict: "repair",
      agreementShape: "split",
      weights: { 0: 2, 1: 2 },
    ...
    {
      name: "single low confidence reject still splitless unanimous reject",
      panel: ["reject"],
      verdict: "reject",
      agreementShape: "unanimous",
      confidenceByIndex: { 0: 0.4 },
    },
    {
      name: "single high confidence reject vetoes",
      panel: ["reject"],
      verdict: "reject",
      agreementShape: "vetoed",
    },
    {
      name: "faithfulness reject high confidence vetoes",
      panel: ["accept", "reject"],
      verdict: "reject",
      agreementShape: "vetoed",
      judgeIds: ["logic_judge", "faithfulness_judge"],
    },
    {
      name: "logic reject low confidence with repair panel can normalize to unanimous repair",
      panel: ["repair", "repair", "reject"],
      verdict: "repair",
      agreementShape: "unanimous",
      confidenceByIndex: { 2: 0.4 },
    ...
    {
      name: "logic reject low confidence with accept majority accepts",
      panel: ["accept", "accept", "reject"],
      verdict: "accept",
      agreementShape: "majority",
      confidenceByIndex: { 2: 0.4 },
      judgeIds: ["faithfulness_judge", "a11y_judge", "logic_judge"],
    },
    {
      name: "repair accept reject low confidence split",
      panel: ["repair", "accept", "reject"],
      verdict: "repair",
      agreementShape: "split",
      confidenceByIndex: { 2: 0.2 },
      judgeIds: ["faithfulness_judge", "a11y_judge", "logic_judge"],
    },
    {
      name: "accept accept repair no veto codes",
      panel: ["accept", "accept", "repair"],
      verdict: "repair",
      agreementShape: "majority",
      judgeIds: ["logic_judge", "a11y_judge", "faithfulness_judge"],
    },
    {
      name: "repair accept accept with low confidence reject absent",
      panel: ["repair", "accept", "accept"],
      verdict: "repair",
      agreementShape: "majority",
    },
    {
      name: "split with four judges and no strict majority",
      panel: ["accept", "accept", "repair", "repair"],
      verdict: "repair",
      agreementShape: "split",
    },
    {
      name: "four-judge high confidence reject vetoes",
      panel: ["accept", "accept", "repair", "reject"],
      verdict: "reject",
      agreementShape: "vetoed",
      judgeIds: [
        "logic_judge",
      ...
      const findingCode =
        scenario.transientRejectAt === index
          ...
          : scenario.mismatchAt === index
            ...
            : undefined;
      const confidence = scenario.confidenceByIndex?.[index];
...
void test("buildJudgeConsensus records repaired-success history separately from active findings", () => {
...
void test("buildJudgeConsensus normalizes a11y and coverage rejects down to repair", () => {
  const consensus = buildJudgeConsensus({
    jobId: "job-consensus",
    generatedAt: "2026-05-06T00:00:00.000Z",
    panel: [
...
void test("buildJudgeConsensus marks repaired history separately from active findings", () => {
  const consensus = buildJudgeConsensus({
    jobId: "job-consensus",
    generatedAt: "2026-05-06T00:00:00.000Z",
    panel: [buildLogicJudgeConsensusEntry(buildLogicVerdict("accept"))],
...
void test("buildA11yJudgeConsensusEntry projects structured a11y findings into the consensus panel", () => {
      ...
      repairInstructions: [
        {
          ...
          instruction:
            "Add an explicit focus-visible assertion for each tab stop.",
        },
      ],
    }),
  );
...
void test("buildJudgeConsensus emits crossFamily summary when every judge declares a family", () => {
  const consensus = buildJudgeConsensus({
...
void test("buildJudgeConsensus omits crossFamily summary when every judge shares the same family", () => {
  const consensus = buildJudgeConsensus({
...
void test("buildJudgeConsensus refuses an unknown family marker on a panel entry", () => {
  assert.throws(
    () =>
      buildJudgeConsensus({
...
void test("buildJudgeConsensus refuses a malformed humanReview decision", () => {
  assert.throws(
    () =>
      buildJudgeConsensus({
...
void test("buildJudgeConsensus forwards crossFamilyOptions.mostTrustedFamily into the summary", () => {
  const consensus = buildJudgeConsensus({
...
void test("buildJudgeConsensus emits a crossFamily summary when only family is supplied", () => {
  const consensus = buildJudgeConsensus({
...
void test("buildJudgeConsensus refuses an empty modelId on a panel entry", () => {
  assert.throws(
    () =>
      buildJudgeConsensus({
...
void test("buildJudgeConsensus attaches a humanReview decision verbatim", () => {
  const consensus = buildJudgeConsensus({
