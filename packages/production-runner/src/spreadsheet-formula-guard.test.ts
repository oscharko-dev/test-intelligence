...
void test("neutralizeFormulaLeading: prefixes the OWASP CWE-1236 leader characters", () => {
...
void test("neutralizeFormulaLeading: leaves benign values byte-for-byte intact", () => {
  for (const value of [
...
void test("neutralizeFormulaLeading: is idempotent — applying twice equals applying once", () => {
...
void test("neutralizeFormulaLeading: empty string returns empty string", () => {
  assert.equal(neutralizeFormulaLeading(""), "");
});

void test("neutralizeFormulaLeading: only the FIRST character is treated as a leader", () => {
