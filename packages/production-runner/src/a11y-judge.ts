...
const SYSTEM_PROMPT = [
  "You are the screenshot accessibility coverage judge for Test Intelligence.",
...
export interface A11yJudgeReplayCache {
  lookup(
    ...
    { hit: false; key: string } | { hit: true; entry: A11yJudgeCacheEntry }
  >;
  ...
  if (criteria.length === 0) {
    return {
      verdict: buildSkippedVerdict({
        ...
        message:
          "No form-screen accessibility criteria were derived for this run.",
      }),
      cacheHit: false,
      promptArtifact,
    };
  ...
  if (client === undefined) {
    return {
      verdict: buildSkippedVerdict({
        ...
        message:
          "Accessibility judge deployment is not configured for this run.",
      }),
      cacheHit: false,
      promptArtifact,
    };
  ...
  for (const row of rows) {
    ...
    if (
      ...
      return {
        ok: false,
        message:
          "criteria entries must include non-empty criterionId, verdict, and rationale fields",
      };
    }
    ...
    if (
      !ALLOWED_A11Y_CRITERION_VERDICTS.includes(
        verdict as A11yCriterionVerdictLabel,
      )
    ) {
    ...
    const repairInstruction = readOptionalNonEmptyString(
      row,
      "repairInstruction",
    );
    normalized.push({
...
const buildA11yVerdict = (input: {
  ...
  return {
    ...
    verdict:
      findings.length === 0
        ? ALLOWED_A11Y_VERDICTS[0]
        : ALLOWED_A11Y_VERDICTS[1],
    criteria,
    findings,
    repairInstructions,
