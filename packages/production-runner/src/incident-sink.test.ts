...
void test("incident-sink: writes incidents.json into <destinationDir>/<jobId>/ atomically", async () => {
  ...
  try {
    ...
    assert.equal(
      result.bytesWritten,
      Buffer.byteLength(canonicalJson(report), "utf8"),
    );
  } finally {
...
void test("incident-sink: persisted bytes are byte-stable for identical reports (canonical JSON)", async () => {
  const dir = await mkTempDir();
  try {
    const sink = createFileSystemIncidentSink({ destinationDir: dir });
    const report = buildReport();
...
void test("incident-sink: rejects empty destinationDir at construction time", () => {
  assert.throws(
...
void test("incident-sink: rejects empty jobId at write time", async () => {
  const dir = await mkTempDir();
  try {
    const sink = createFileSystemIncidentSink({ destinationDir: dir });
...
void test("incident-sink: rejects path-traversal jobId values", async () => {
  const dir = await mkTempDir();
  try {
    const sink = createFileSystemIncidentSink({ destinationDir: dir });
    await assert.rejects(
...
void test("incident-sink: allows writing beneath the filesystem root", async () => {
  const dir = await mkTempDir();
