...
void test("isAirGapModeEnabled accepts 1/true/yes case-insensitively and only those", () => {
...
void test("readAirGapAllowedHosts trims, lowercases, drops empties", () => {
  assert.deepEqual(
    readAirGapAllowedHosts({
...
void test("guard is a transparent pass-through when air-gap mode is off", async () => {
  let invoked = false;
...
void test("guard rejects hosts outside the explicit allow-list", async () => {
...
void test("guard permits hosts inside the explicit allow-list", async () => {
  let receivedUrl = "";
...
void test("guard falls back to env allow-list when no explicit hosts are provided", async () => {
  const inner: typeof fetch = async () => new Response("ok", { status: 200 });
  const guarded = createAirGapFetchGuard({
    fetchImpl: inner,
...
void test("guard rejects requests with no parseable host", async () => {
  const inner: typeof fetch = async () => new Response("ok", { status: 200 });
  const guarded = createAirGapFetchGuard({
    fetchImpl: inner,
    env: enabledEnv(),
...
void test("guard permits data: URIs because they emit no network traffic", async () => {
  let invoked = false;
  const inner: typeof fetch = async () => {
    invoked = true;
    return new Response("ok", { status: 200 });
...
void test("guard refuses URL-object inputs whose host is not allow-listed", async () => {
  const inner: typeof fetch = async () => new Response("ok", { status: 200 });
  const guarded = createAirGapFetchGuard({
    fetchImpl: inner,
    env: enabledEnv(),
...
void test("assertLocalFilesystemPath is a no-op outside air-gap mode", () => {
  assert.doesNotThrow(() =>
    assertLocalFilesystemPath("s3://bucket/key", { env: {} }),
  );
});

void test("assertLocalFilesystemPath rejects every remote scheme under air-gap mode", () => {
...
void test("assertLocalFilesystemPath accepts absolute and relative local paths", () => {
  const env = enabledEnv();
  assert.doesNotThrow(() =>
    assertLocalFilesystemPath("/var/lib/test-intelligence/cache", { env }),
  );
  assert.doesNotThrow(() => assertLocalFilesystemPath("./cache", { env }));
  assert.doesNotThrow(() =>
    assertLocalFilesystemPath("C:\\workspace\\cache", { env }),
  );
});

void test("assertLocalFilesystemPath rejects empty path under air-gap mode", () => {
  assert.throws(
