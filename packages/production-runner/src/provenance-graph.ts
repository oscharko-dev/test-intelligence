/**
 * Branding note (issue #87): the `workspace-dev.local` namespace URI, the
 * `urn:workspace-dev:test-intelligence:` URN prefix, and the
 * `workspace-dev` PROV agent id/label in this file are PROV-O / URN wire
 * identifiers preserved byte-identical against the Workspace Dev reference
 * at pinned SHA `006dabdf0abe30b9cac2b742a7238c6625d8e8c1`. Per intentional
 * the parity fixture policy, engine-module wire
 * identifiers are preserved verbatim and MUST NOT be rebranded; only
 * operator-facing strings are reskinned to `test-intelligence`.
 */
...
import type { AdversarialCriticFinding } from "@oscharko-dev/ti-agentic-harness";
...
export const buildRunProvenanceGraph = async (
  ...
  for (const observation of (input.regionAttestations ?? [])
    .slice()
    .sort(
      (left, right) =>
    ...
    upsertNode(nodes, node);
    const existing =
      regionAttestationIdsBySource.get(observation.sourceLabel) ?? [];
    existing.push(nodeId);
    regionAttestationIdsBySource.set(observation.sourceLabel, existing);
  }
  const regionAttestationIdsForSources = (
    ...sources: readonly string[]
  ): string[] =>
    Array.from(
      new Set(
        sources.flatMap(
          (source) => regionAttestationIdsBySource.get(source) ?? [],
        ),
      ),
    ).sort((left, right) => left.localeCompare(right));
