...
import type { FinOpsBudgetReport } from "@oscharko-dev/ti-contracts";
import {
...
void test("buildFinOpsTimeSeriesRecord groups generator, judge, and visual tokens", () => {
...
void test("resolveFinOpsFixtureId namespaces runner file keys for latency tracking", () => {
  assert.equal(resolveFinOpsFixtureId({ fileKey: "ABC" }), "figma:ABC");
});

void test("appendFinOpsTimeSeriesRecord deduplicates by jobId and retains only recent records", () => {
  const older = {
  ...
  assert.deepEqual(
    updated.records.map((record) => record.jobId),
    ["job-current"],
  );
});

void test("evaluateFinOpsSlo passes for in-budget token and latency history", () => {
  const store = {
...
void test("evaluateFinOpsSlo fails when role tokens, fixture latency, and savings regress", () => {
  const store = {
    schemaVersion: "1.0.0" as const,
    records: [
      {
...
void test("time-series store and SLO report persist canonically and alerts reuse the drift sink", async () => {
  ...
  try {
    ...
    assert.equal(
      reportPath.endsWith(FINOPS_SLO_REPORT_ARTIFACT_FILENAME),
      true,
    );
...
void test("appendFinOpsTimeSeriesRecordOnDisk preserves concurrent writers", async () => {
  ...
  try {
    ...
    const loaded = await loadFinOpsTimeSeriesStore(storePath);
    assert.deepEqual(loaded.records.map((record) => record.jobId).sort(), [
      "job-1",
      "job-2",
    ]);
  } finally {
