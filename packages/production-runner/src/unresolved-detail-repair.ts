...
const compactRepairTitleSuffix = (value: string): string =>
  collapseWhitespace(stripStepActionRefPrefix(value))
    .slice(0, REPAIR_TITLE_SUFFIX_MAX_LENGTH)
    .trim();

const fieldTargetFromStepAction = (value: string): string | undefined => {
  const normalized = stripStepActionRefPrefix(value).trim();
  const target = extractFieldTarget(normalized);
  if (target === undefined) return undefined;
  const suffix = compactRepairTitleSuffix(target);
  return suffix.length === 0 ? undefined : suffix;
};

const stripStepActionRefPrefix = (value: string): string => {
  const trimmed = value.trimStart();
  if (!trimmed.startsWith("[")) return trimmed;
  const close = trimmed.indexOf("]");
  if (close < 0) return trimmed;
  return trimmed.slice(close + 1).trimStart();
};
...
const extractFieldTarget = (value: string): string | undefined => {
  const lower = value.toLowerCase();
  for (const keyword of ["field", "feld"] as const) {
    let index = lower.indexOf(keyword);
    while (index >= 0) {
      if (isWordBoundary(lower, index, keyword.length)) {
        const parsed = readFieldTargetAfterKeyword(
          value,
...
const isWordBoundary = (
  ...
  const before = index === 0 ? "" : (value[index - 1] as string);
  const after = value[index + length] ?? "";
  return !isAsciiWord(before) && !isAsciiWord(after);
};

const readFieldTargetAfterKeyword = (
  value: string,
  offset: number,
): string | undefined => {
  let cursor = offset;
  ...
  let target = value.slice(start, cursor).trim();
  let suffixCursor = quoted ? cursor + 1 : cursor;
  while (value[suffixCursor] === " " || value[suffixCursor] === "\t") {
    suffixCursor += 1;
  }
  if (value[suffixCursor] === "(") {
    const suffixEnd = value.indexOf(")", suffixCursor + 1);
    if (suffixEnd > suffixCursor) {
      target += value.slice(suffixCursor, suffixEnd + 1);
    }
  }
...
const buildGenericUnresolvedStepAction = (
  ...
  const suffix =
    fieldTargetFromStepAction(originalAction) ??
    fallbackTargetFromCase(testCase);
  return suffix === undefined || suffix.length === 0
...
const buildModel = (input: RepairUnresolvedDetailsInput): TestDesignModel =>
