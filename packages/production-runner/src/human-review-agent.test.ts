...
void test("buildDryRunHumanReviewMarker emits a deterministic offline envelope", () => {
...
void test("buildHumanReviewDecision hashes principalId with sha256", () => {
...
void test("buildHumanReviewDecision accepts a pre-computed principalHash", () => {
...
void test("buildHumanReviewDecision rejects bad principalHash", () => {
  assert.throws(
    () =>
      buildHumanReviewDecision({
        reviewerKind: "principal",
...
void test("buildHumanReviewDecision rejects rationale with line endings", () => {
  assert.throws(
    () =>
      buildHumanReviewDecision({
...
void test("buildHumanReviewDecision rejects U+2028 / U+2029 line separators", () => {
  for (const sep of ["\u2028", "\u2029"]) {
    assert.throws(
      () =>
        buildHumanReviewDecision({
...
void test("buildHumanReviewDecision rejects empty rationale", () => {
  assert.throws(
    () =>
      buildHumanReviewDecision({
        reviewerKind: "dry_run_marker",
...
void test("buildHumanReviewDecision caps rationale length at HUMAN_REVIEW_RATIONALE_MAX_CHARS", () => {
...
void test("buildHumanReviewDecision rejects non-ISO-8601 decidedAt", () => {
  assert.throws(
    () =>
      buildHumanReviewDecision({
        reviewerKind: "dry_run_marker",
...
void test("buildHumanReviewDecision refuses principal without id or hash", () => {
  assert.throws(
    () =>
      buildHumanReviewDecision({
        reviewerKind: "principal",
...
void test("assertHumanReviewDecisionInvariants validates a reloaded record", () => {
  const decision = buildHumanReviewDecision({
    reviewerKind: "dry_run_marker",
    verdict: "deferred",
    rationale: "valid",
...
void test("hashPrincipalId yields a stable sha256", () => {
