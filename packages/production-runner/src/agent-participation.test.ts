...
import {
  ...
  type AgentParticipationArtifact,
} from "./agent-participation.js";
import { classifyTaskBatch } from "@oscharko-dev/ti-model-gateway";

void test("schema: bumped to 1.2.0 for routing decisions plus explicit role views (#2028, #2043)", () => {
  assert.equal(AGENT_PARTICIPATION_SCHEMA_VERSION, "1.2.0");
});

void test("roles: task_classifier registered alongside the existing roles (#2043)", () => {
  assert.ok(
...
void test("buildAgentParticipationArtifact: omits routingDecisions when absent (legacy byte-shape)", () => {
...
void test("buildAgentParticipationArtifact: role views separate workflow roles from deployed sidecars", () => {
  const artifact = buildAgentParticipationArtifact({
    jobId: "job-views",
    generatedAt: "2026-05-08T00:00:00Z",
    roles: [
...
void test("buildAgentParticipationArtifact: persists routing decisions sorted by taskId", () => {
...
void test("buildAgentParticipationArtifact: zero-length routing decisions is the same as undefined", () => {
  const artifact = buildAgentParticipationArtifact({
    jobId: "job-empty",
    generatedAt: "2026-05-08T00:00:00Z",
    roles: [],
...
void test("buildAgentParticipationArtifact: byte-stable for identical input", () => {
  const decisions = classifyTaskBatch([
    {
      taskId: "a",
      taskKind: "simple_ui_validation",
...
void test("writeAgentParticipationArtifact: persists with routing decisions", async () => {
  const decisions = classifyTaskBatch([
    {
      taskId: "t-1",
      taskKind: "simple_ui_validation",
  ...
  try {
    ...
    const parsed = JSON.parse(persisted) as AgentParticipationArtifact;
