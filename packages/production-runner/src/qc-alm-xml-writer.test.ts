...
import { OPENTEXT_ALM_REFERENCE_PROFILE } from "@oscharko-dev/ti-integrations";
...
void test("alm-xml: empty preview produces well-formed empty envelope", () => {
...
void test("alm-xml: special characters in attributes are escaped", () => {
  const xml = renderQcAlmXml({
...
void test("alm-xml: CDATA wraps description when profile.cdataDescription=true", () => {
  const xml = renderQcAlmXml({
...
void test("alm-xml: CDATA defends against `]]>` in payload", () => {
  const xml = renderQcAlmXml({
...
void test("alm-xml: deterministic output across two renders", () => {
...
void test("alm-xml: provenance element renders only when visual provenance present", () => {
...
void test("alm-xml: blocking reasons element appears only when populated", () => {
...
void test("alm-xml: exportable attribute reflects entry flag", () => {
  const xml = renderQcAlmXml({
