...
const TEST_CASE_NUMBER_PATTERN = /\bTC[\s_-]*0*(\d{1,4})\b/iu;
...
export const extractAcceptanceCriteriaFromMarkdown = (
  ...
  for (const line of lines) {
    ...
    const prefixed = parseAcceptanceCriterionPrefix(trimmed);
    if (prefixed !== undefined) {
      extracted.push(prefixed);
      continue;
    }
    if (!inAcceptanceSection) continue;
    const listItem = parseMarkdownListItem(trimmed);
    if (listItem !== undefined) {
      extracted.push(listItem);
      continue;
    }
...
const isAcceptanceCriteriaHeading = (line: string): boolean => {
  const heading = markdownHeadingText(line);
  if (heading === undefined) return false;
  const lower = heading.toLowerCase();
  return (
    lower.startsWith("acceptance criteria") ||
    lower.startsWith("akzeptanzkriterien")
  );
};

const isMarkdownHeading = (line: string): boolean =>
  markdownHeadingText(line) !== undefined;

const markdownHeadingText = (line: string): string | undefined => {
  let level = 0;
  while (level < line.length && line[level] === "#") level += 1;
  if (level < 1 || level > 6 || !isWhitespace(line[level] ?? "")) {
    return undefined;
  }
  return line.slice(level).trim();
};

const parseAcceptanceCriterionPrefix = (line: string): string | undefined => {
  const separator = findPrefixSeparator(line);
  if (separator < 0) return undefined;
  const prefix = line.slice(0, separator).trim();
  if (!isAcceptanceCriterionLabel(prefix)) return undefined;
  const rest = line.slice(separator + 1).trim();
  return rest.length > 0 ? rest : undefined;
};

const findPrefixSeparator = (line: string): number => {
...
const isAcceptanceCriterionLabel = (prefix: string): boolean => {
  const compact = prefix
    .toLowerCase()
    .split("")
    .filter((ch) => ch !== " " && ch !== "." && ch !== "_" && ch !== "-")
    .join("");
  if (compact.startsWith("ac")) {
    return hasDigit(compact.slice(2));
  }
  if (compact.startsWith("akzeptanzkriterium")) {
    return hasDigit(compact.slice("akzeptanzkriterium".length));
  }
  if (compact.startsWith("acceptancecriterion")) {
    return hasDigit(compact.slice("acceptancecriterion".length));
  }
  return false;
};

const hasDigit = (value: string): boolean => {
  for (let i = 0; i < value.length; i += 1) {
    if (isDigit(value[i] as string)) return true;
  }
  return false;
};

const parseMarkdownListItem = (line: string): string | undefined => {
  ...
  if (line.startsWith("[") && line.length >= 4) {
    ...
    if (marker === "x" || marker === "X" || marker === " ") cursor += 1;
    if (line[cursor] === " ") cursor += 1;
    if (line[cursor] === "]" && isWhitespace(line[cursor + 1] ?? "")) {
      return line.slice(cursor + 2).trim();
    }
  }
  let cursor = 0;
...
const renderCombined = (
  ...
  fieldLifecycleTransitions: ReadonlyMap<
    string,
    WorkflowFieldLifecycleTransition
  >,
  showConfidence: boolean,
  ...
  lines.push(`Exportmodus: ${mode === "customer" ? "customer" : "technical"}`);
  lines.push("");
  lines.push("---");
  lines.push("");
  ...
  lines.push("");
  lines.push("| Testfall | Zweck | Klasse | Abdeckung | Hinweise |");
  lines.push("| --- | --- | --- | --- | --- |");
  ...
  if (acceptanceCriteria.length > 0) {
    ...
    for (let i = 0; i < acceptanceCriteria.length; i += 1) {
      ...
      const matchedCases = preparedCases
        .filter((entry) =>
          entry.matchedAcceptanceCriteria.some((value) =>
            value.startsWith(acLabel),
          ),
        )
...
const renderSingleCase = (
  ...
  if (
    mode === "technical" &&
    (tc.assumptions.length > 0 || tc.openQuestions.length > 0)
  ) {
...
const inferCoverageTheme = (tc: GeneratedTestCase): string => {
  ...
  if (tc.type === "boundary" || tc.technique === "boundary_value_analysis") {
    return "Grenzwerte";
  }
...
const buildPreparedFilename = (
  ...
  const base = titleSlug.length > 0 ? `${prefix}-${titleSlug}` : prefix;
...
const compareCustomerCases = (
  ...
  const titleCompare = left.customerTitle.localeCompare(
    right.customerTitle,
    "de",
  );
...
const extractExplicitCaseNumber = (
  tc: GeneratedTestCase,
): number | undefined => {
...
const classifyCustomerCase = (tc: GeneratedTestCase): string => {
  const polarity =
    tc.polarity ?? deriveGeneratedTestCaseClassification(tc).polarity;
...
const matchAcceptanceCriteria = (
  ...
  for (let i = 0; i < acceptanceCriteria.length; i += 1) {
    ...
    const overlap = criterionTokens.filter((token) =>
      caseTokens.includes(token),
    );
    if (overlap.length < Math.min(2, criterionTokens.length)) continue;
    matched.push(`${formatAcceptanceCriterionLabel(i + 1)}: ${criterion}`);
  }
  return matched;
};
