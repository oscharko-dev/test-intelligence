...
import type { QcMappingPreviewEntry } from "@oscharko-dev/ti-contracts";
...
void test("csv: header row matches QC_CSV_COLUMNS", () => {
  const out = renderQcCsv([]);
  const firstLine = out.split("\r\n")[0];
  assert.equal(firstLine, QC_CSV_COLUMNS.join(","));
});

void test("csv: empty body has only header followed by CRLF", () => {
  const out = renderQcCsv([]);
  assert.equal(out, QC_CSV_COLUMNS.join(",") + "\r\n");
});

void test("csv: cells with comma are quoted", () => {
  const out = renderQcCsv([baseEntry({ testName: "Hello, world" })]);
  assert.match(out, /"Hello, world"/);
});

void test("csv: cells with embedded double quotes get them doubled", () => {
  const out = renderQcCsv([baseEntry({ testName: 'A "quoted" value' })]);
  assert.match(out, /"A ""quoted"" value"/);
});

void test("csv: cells with embedded newline are quoted", () => {
  const out = renderQcCsv([baseEntry({ objective: "first\nsecond" })]);
  assert.match(out, /"first\nsecond"/);
});

void test("csv: deterministic output across two renders with sorted entries", () => {
  const a = renderQcCsv([
...
void test("csv: one row per step; case without steps still has one row", () => {
  const out = renderQcCsv([
    baseEntry({
      testCaseId: "stepless",
      designSteps: [],
...
void test("csv: visual provenance columns default to none / 0.000000 / 0", () => {
...
void test("csv: visual provenance round-trip when present", () => {
  const out = renderQcCsv([
    baseEntry({
      visualProvenance: {
...
void test("csv: formula-leading values get a single-quote prefix", () => {
...
void test("csv: values without a formula leader are emitted unchanged", () => {
