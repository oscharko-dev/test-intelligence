...
const buildAccessibilityCase = (
  ...
  buildCase({
    type: "accessibility",
    title:
      "Keyboard navigation, focus order, and screen-reader announcements are covered",
    objective:
...
void test("pipeline runs in-memory without filesystem touch", () => {
...
void test("workflow topology ACT ids flow through the validation pipeline without unknown-id findings", () => {
  const result = runValidationPipeline({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    list: {
...
void test("blocking flows propagate end-to-end", () => {
  const list = richList();
  list.testCases[0] = {
    ...list.testCases[0]!,
...
void test("Issue #1947: coverage-plan technique quota minimum flows through validation pipeline", () => {
  const result = runValidationPipeline({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    list: richList(),
...
void test("Issue #2068: technique-quota report is emitted in-memory and persisted next to policy-report", async () => {
...
void test("Issue #2071: pipeline reconciles oracle-governed testData and persists the oracle report", async () => {
...
void test("Issue #2101: validation report records judgeAvailability and persists it canonically", async () => {
...
void test("Issue #2071: oracle reconciliation preserves unrelated testData entries on multi-field cases", () => {
  const list: GeneratedTestCaseList = {
    schemaVersion: GENERATED_TEST_CASE_SCHEMA_VERSION,
    jobId: "job-1",
    testCases: [
...
void test("Issue #2071: mixed resolved and unresolved rules preserve oracle data and add an open question", () => {
  ...
  assert.equal(
    result.testDataOracleReport!.cases[0]!.oracleResolvedFields[0]!
      .oracleProvenance![0]!.note,
    "synthesized by deterministic oracle",
  );
});

void test("Issue #2071: required oracle open questions survive the soft cap", () => {
  const result = runValidationPipeline({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    list: {
...
void test("Issue #2068: pipeline omits technique-quota artifact when no CoveragePlan is supplied", () => {
  const result = runValidationPipeline({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    list: richList(),
...
void test("Issue #1947: policyOverrides downgrade technique quota minimum inside validation pipeline", () => {
  const result = runValidationPipeline({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    list: richList(),
...
void test("Issue #1949: cross-modal faithfulness threshold override flows through validation pipeline", () => {
  const result = runValidationPipeline({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    list: richList(),
  ...
  assert.equal(
    result.policy.jobLevelViolations.some(
      (violation) => violation.rule === "policy:cross-modal-faithfulness-score",
    ),
    false,
  );
  assert.equal(result.policy.blocked, false);
...
void test("Issue #1951: a11y_judge verdict flows through validation pipeline and blocks uncovered form-screen criteria", () => {
  const result = runValidationPipeline({
    jobId: "job-1",
    generatedAt: GENERATED_AT,
    list: richList(),
      ...
      criteria: [
        {
          ...
          verdict: "not_covered",
          rationale:
            "No case verifies screen-reader announcements for validation changes.",
        },
      ],
    }),
  });
...
void test("semantic override clears effective pipeline block while preserving validation artifact", () => {
  const list = richList();
  list.testCases[0] = {
    ...list.testCases[0]!,
    steps: [
  ...
  assert.equal(result.policy.blocked, false);
  assert.equal(result.blocked, false);
  const decision = result.policy.decisions.find(
    (d) => d.testCaseId === "tc-pos",
  );
...
void test("schema-invalid generated output returns blocked diagnostics", () => {
  const list = {
    schemaVersion: "wrong",
    jobId: "job-1",
    testCases: "not-array",
...
void test("blocking visual sidecar findings propagate end-to-end", () => {
...
void test("artifacts persist as canonical JSON in the supplied directory", async () => {
...
void test("visual sidecar artifact is persisted only when input is provided", async () => {
...
void test("running the pipeline twice produces byte-identical artifacts", () => {
  const list = richList();
