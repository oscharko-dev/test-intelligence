...
void test("disabled rubric pipeline matches the synchronous baseline byte-for-byte", async () => {
...
void test("enabled rubric pipeline populates rubric report + coverage rubricScore", async () => {
...
void test("enabled rubric pipeline propagates a valid low score into coverage", async () => {
  const list = buildList([buildCase({ id: "tc-1" })]);
  const intent = buildIntent();
...
void test("Issue #1951: self-verify pipeline honors policyOverrides for a11y hard-gate downgrades", async () => {
...
void test("enabled rubric pipeline scores validated visual subscores end-to-end", async () => {
  const list = buildList([buildCase({ id: "tc-1" })]);
  const intent = buildIntent();
...
void test("blocked visual sidecar data is not sent to the rubric LLM", async () => {
  const list = buildList([buildCase({ id: "tc-1" })]);
  const intent = buildIntent();
...
void test("runAndPersistValidationPipelineWithSelfVerify writes the rubric report under testcases/", async () => {
...
void test("rubric pipeline preserves byte-stability when refusal occurs (no rubricScore on cases)", async () => {
  const list = buildList([buildCase({ id: "tc-1" })]);
  const intent = buildIntent();
...
void test("structurally-invalid validation skips the rubric pass entirely", async () => {
