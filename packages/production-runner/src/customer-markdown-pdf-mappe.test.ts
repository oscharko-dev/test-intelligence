...
void test("buildCustomerMarkdownMappe returns a PDF with the magic header and EOF marker", () => {
...
void test("buildCustomerMarkdownMappe embeds the screenshot as a DeviceRGB image XObject", () => {
  const pdf = buildCustomerMarkdownMappe(fixedInput());
...
void test("buildCustomerMarkdownMappe places the cover background and section headings", () => {
  const pdf = buildCustomerMarkdownMappe(fixedInput());
  const bin = pdf.toString("binary");
...
void test("buildCustomerMarkdownMappe falls back to a placeholder when jira story is empty", () => {
...
void test("buildCustomerMarkdownMappe handles zero screenshots without crashing", () => {
...
void test("decodePngToRgb decodes a minimal RGBA PNG to a packed RGB buffer", () => {
...
void test("decodePngToRgb rejects a non-PNG buffer", () => {
  assert.throws(() => decodePngToRgb(Buffer.from("not a png")));
});

void test("decodePngToRgb rejects a PNG truncated mid-chunk with a deterministic error", () => {
...
void test("decodePngToRgb rejects a PNG with a truncated chunk header", () => {
...
void test("extractJiraStoryFromCustomContext returns undefined for a body that has no Jira-story heading", () => {
...
void test("buildCustomerMarkdownMappe truncates a multi-line cover title to two lines (ellipsis lands in the content stream)", () => {
  const longTitle =
