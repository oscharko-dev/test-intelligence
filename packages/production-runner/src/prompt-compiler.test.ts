...
import { buildGeneratedTestCaseListJsonSchema } from "@oscharko-dev/ti-core-engine";
import { reconcileSources } from "@oscharko-dev/ti-core-engine";
...
void test("compiler: produces stable inputHash, promptHash, schemaHash, cacheKey", async () => {
...
void test("compiler: replay identity ignores volatile execution jobId", async () => {
  const { intent, visual } = await loadFixture();
  const a = compilePrompt({
    jobId: "ti-cli-111",
    intent,
...
void test("compiler: replay identity ignores volatile coverage visual refs", async () => {
...
void test("compiler: replay identity ignores visual-only model drift", async () => {
  const intent = await loadBaselineSimpleFormIntent();
...
void test("compiler: hash differs when modelRevision changes", async () => {
  const { intent, visual } = await loadFixture();
  const a = compilePrompt({
    jobId: "job-1",
    intent,
...
void test("compiler: hash differs when policy bundle version changes", async () => {
  const { intent, visual } = await loadFixture();
  const a = compilePrompt({
    jobId: "job-1",
    intent,
...
void test("compiler: replay identity ignores optional visual sidecar deployment changes", async () => {
  const { intent, visual } = await loadFixture();
...
void test("compiler: replay identity ignores optional visual fixture hash changes", async () => {
  const { intent, visual } = await loadFixture();
  const a = compilePrompt({
    jobId: "job-1",
    intent,
...
void test("compiler: includes the unresolved-validation anti-fabrication rule in the prompt preamble", () => {
  assert.match(
    COMPILED_USER_PROMPT_PREAMBLE,
...
void test("compiler: includes reviewer-approved agent lessons in the AgentLessons prompt section", async () => {
  const { intent, visual } = await loadFixture();
  ...
  assert.match(
    ...
    /Always reject malformed IBAN values\./u,
  );
});

void test("compiler: refuses agent lessons that are not reviewer-approved", async () => {
  const { intent, visual } = await loadFixture();
...
void test("compiler: includes sanitized custom context in prompt and replay identity", async () => {
  const { intent, visual } = await loadFixture();
  ...
  assert.match(withContext.request.userPrompt, /\[5\] CustomerDomainContext/u);
  assert.match(
...
void test("compiler: suffix-only changes do not change cacheablePrefixHash", async () => {
  const { intent, visual } = await loadFixture();
  const a = compilePrompt({
...
void test("compiler: canonical section markers inside text suffixes stay out of the prefix", async () => {
  const intent = await loadBaselineSimpleFormIntent();
  ...
  const fullPrompt = [
    guarded.artifacts.promptLayout.prefix,
    guarded.artifacts.promptLayout.suffix,
  ].join("\n\n");
  assert.equal(
...
void test("compiler: untrusted Figma spans are structurally wrapped in the prompt", async () => {
  const { intent, visual } = await loadFixture();
...
void test("compiler: finding sections reject raw body strings", async () => {
  const { intent, visual } = await loadFixture();
  assert.throws(
    () =>
      compilePrompt({
...
void test("compiler: hybrid-source open questions are wrapped as untrusted prompt data", async () => {
  const { intent, visual } = await loadFixture();
  const result = compilePrompt({
...
void test("compiler: active context-budget analysis changes the cache key and emits a per-role-step report", async () => {
  const { intent, visual } = await loadFixture();
  const customContext: CompiledPromptCustomContext = {
    markdownSections: [
      {
  ...
  assert.notEqual(
    raw.request.hashes.cacheKey,
    compacted.request.hashes.cacheKey,
  );
  ...
  if (compacted.contextBudgetReport!.action === "compact_prompt_payload") {
    assert.ok(
      compacted.contextBudgetReport!.compactedFromArtifactHashes.length > 0,
    );
  }
  assert.match(
...
void test("compiler: context-budget analysis with no customer markdown leaves cacheablePrefixHash stable", async () => {
  const { intent, visual } = await loadFixture();
  const raw = compilePrompt({
...
void test("compiler: artifacts contain only redacted PII (golden snapshot guard)", async () => {
  const { intent, visual } = await loadFixture();
  const result = compilePrompt({
    jobId: "job-1",
    intent,
...
void test("compiler: redacts PII-like values from visual sidecar prompts and artifacts", async () => {
  const { intent, visual } = await loadFixture();
...
void test("compiler: strips unexpected visual sidecar properties from prompts and artifacts", async () => {
  const { intent, visual } = await loadFixture();
...
void test("compiler: includes versioned breadcrumbs in user prompt body", async () => {
  const { intent, visual } = await loadFixture();
...
void test("compiler: renders numbered prompt sections exactly once in ascending order", async () => {
  const intent = await loadBaselineSimpleFormIntent();
...
void test("compiler: customer markdown is promoted to a dedicated [5] CustomerDomainContext section ahead of Customer Rubric (Issue #1941)", async () => {
  const intent = await loadBaselineSimpleFormIntent();
  const customContext: CompiledPromptCustomContext = {
    markdownSections: [
      {
...
void test("compiler: customer markdown promotion stays on the cacheable prefix (Issue #1941)", async () => {
  const intent = await loadBaselineSimpleFormIntent();
  const customContext: CompiledPromptCustomContext = {
    markdownSections: [
      {
  ...
  assert.equal(
    compiled.artifacts.promptLayout.suffix.includes(
      "[5] CustomerDomainContext",
    ),
    false,
  );
...
void test("compiler: prompt section assembly is idempotent across repeated compilation", async () => {
  const intent = await loadBaselineSimpleFormIntent();
  const compile = () =>
    compilePrompt({
...
void test("compiler: artifacts pin the contract and schema versions", async () => {
  const { intent, visual } = await loadFixture();
...
void test("compiler: surfaces explicit technique quota guidance and serialization for the generator", async () => {
  const intent = await loadBaselineSimpleFormIntent();
  const { request } = compilePrompt({
...
void test("compiler: serializes every flattened technique quota entry across screens", async () => {
  const intent = await loadBaselineSimpleFormIntent();
...
void test("compiler: normalizes visual binding screen count from redacted visual batch", async () => {
  const { intent, visual } = await loadFixture();
  const { artifacts } = compilePrompt({
    jobId: "job-1",
    intent,
...
void test("compiler: omits seed and fixtureImageHash from request when absent", async () => {
  const { intent, visual } = await loadFixture();
...
void test("compiler: response schema name is stable", async () => {
  ...
  assert.equal(
    ...
    `test-intelligence-generated-test-case-list-v${majorVersion}`,
  );
  assert.match(request.responseSchemaName, /^[a-zA-Z0-9_-]{1,64}$/);
});

void test("compiler: request schema matches generated test case JSON schema", async () => {
  const { intent } = await loadFixture();
  const { request } = compilePrompt({
    jobId: "job-1",
    intent,
...
void test("Issue #1946: customer profile glossary rendered in [5] CustomerDomainContext", async () => {
  const { intent } = await loadFixture();
  const customContext: CompiledPromptCustomContext = {
    markdownSections: [
      {
...
void test("Issue #1946: customer profile few-shot examples rendered in [5] CustomerDomainContext", async () => {
  const { intent } = await loadFixture();
  const customContext: CompiledPromptCustomContext = {
    markdownSections: [
      {
...
void test("Issue #1946: identical customer profile sections produce identical cache key", async () => {
  const { intent } = await loadFixture();
...
void test("Issue #1946: risk taxonomy and policy overrides rendered in [5] CustomerDomainContext", async () => {
  const { intent } = await loadFixture();
  const customContext: CompiledPromptCustomContext = {
    markdownSections: [
      {
