...
void test("normalizeUntrustedContent returns ok outcome with zero counts on empty input", () => {
...
void test("strips Figma layers with visible=false and counts the drop", () => {
  const document = {
    id: "0:0",
    type: "DOCUMENT",
    children: [
...
void test("strips zero-opacity layers", () => {
...
void test("strips off-canvas layers (bbox outside parent)", () => {
  const document = figmaNode({
    id: "root",
    type: "FRAME",
...
void test("strips zero font-size layers", () => {
  const document = figmaNode({
    id: "root",
    type: "FRAME",
    absoluteBoundingBox: { x: 0, y: 0, width: 100, height: 100 },
...
void test("sentinel-named layers are dropped and route to needs_review (critical)", () => {
  const document = figmaNode({
    id: "root",
    type: "FRAME",
    absoluteBoundingBox: { x: 0, y: 0, width: 100, height: 100 },
...
void test("strips zero-width Unicode from text fields and counts each codepoint", () => {
...
void test("zero-widths are stripped from Figma TEXT.characters too", () => {
  const document = figmaNode({
    id: "root",
    type: "FRAME",
    absoluteBoundingBox: { x: 0, y: 0, width: 100, height: 100 },
...
void test("ADF outside the parser allow-list is collapsed and counted", () => {
...
void test("ADF inside the allow-list passes through with sanitised plaintext", () => {
...
void test("Markdown injection patterns increment the count", () => {
...
void test("PII matches in text fields are counted (no raw match exposure)", () => {
...
void test("secret-shaped strings flip the secret_match counter and outcome", () => {
  const text =
...
void test("per-element cap truncates over-large text fields and counts the truncation", () => {
...
void test("Markdown over the byte cap is truncated before scanning", () => {
...
void test("UTF-8 truncation never emits a half-encoded codepoint", () => {
...
void test("normalizeUntrustedContent is pure: input objects are not mutated", () => {
  const document = figmaNode({
    id: "root",
    type: "FRAME",
    absoluteBoundingBox: { x: 0, y: 0, width: 100, height: 100 },
...
void test("normalizeUntrustedContent is deterministic for identical input", () => {
  const input = {
    figma: {
      document: figmaNode({
        id: "root",
...
void test("writeUntrustedContentNormalizationReport persists canonical-JSON drop counts", async () => {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { outcome: string; counts: { sentinelLayerNames: number } };
...
void test("report contains no raw stripped content (counts only)", () => {
  const result = normalizeUntrustedContent({
    figma: {
      document: figmaNode({
        id: "r",
...
void test("does not over-traverse pathological depth (no stack blow-up)", () => {
