...
import { SUPPORTED_REGION_ATTESTATION_HOSTING_REGIONS } from "@oscharko-dev/ti-contracts";
...
void describe("carbon-footprint: published table integrity", () => {
  void test("reference table passes its own validator", () => {
    validateEnergyCoefficientTable(REFERENCE_ENERGY_COEFFICIENT_TABLE);
  });

  void test("reference table cites a source for every entry", () => {
    for (const entry of REFERENCE_ENERGY_COEFFICIENT_TABLE.entries) {
      ...
      assert.ok(
        entry.origin === "estimated" || entry.origin === "published_paper",
      );
    }
  });

  void test("schema versions are stable constants", () => {
...
void describe("carbon-footprint: validators", () => {
  void test("validateEnergyCoefficientTable rejects unknown origin", () => {
  ...
  void test("validateEnergyCoefficientTable rejects negative coefficient", () => {
    const bad: EnergyCoefficientTable = {
      schemaVersion: ENERGY_COEFFICIENT_TABLE_SCHEMA_VERSION,
      publishedAt: "2026-05-11",
      entries: [
  ...
  void test("validateEnergyCoefficientTable rejects duplicate deployment", () => {
    const bad: EnergyCoefficientTable = {
      schemaVersion: ENERGY_COEFFICIENT_TABLE_SCHEMA_VERSION,
      publishedAt: "2026-05-11",
      entries: [
  ...
  void test("validateGridCarbonIntensityTable accepts our fixture", () => {
    validateGridCarbonIntensityTable(REGION_TABLE);
  });

  void test("requireEnergyCoefficient throws on unknown deployment", () => {
    assert.throws(
      () =>
  ...
  void test("requireGridCarbonIntensity throws on unknown region", () => {
    assert.throws(
...
void describe("carbon-footprint: freshness ceiling", () => {
  void test("age in days is computed correctly", () => {
  ...
  void test("assertGridCarbonIntensityTableFresh passes within the window", () => {
    assertGridCarbonIntensityTableFresh(REGION_TABLE, FIXED_NOW);
  });

  void test("assertGridCarbonIntensityTableFresh rejects > 35 days", () => {
  ...
  void test("max-age constant matches AC monthly cadence (~35d)", () => {
    assert.equal(GRID_CARBON_INTENSITY_MAX_AGE_DAYS, 35);
  });
});

void describe("carbon-footprint: buildCarbonFootprintReport", () => {
  const report = buildCarbonFootprintReport(BUILD_INPUT);

  void test("stamps schemaVersion and methodology disclaimer", () => {
  ...
  void test("carries energyKwh and co2eGrams on the manifest", () => {
  ...
  void test("energyKwh equals the sum of per-role energy", () => {
    const sum = report.perRole.reduce((acc, line) => acc + line.energyKwh, 0);
    assert.ok(Math.abs(report.energyKwh - sum) < 1e-9);
  });

  void test("co2eGrams equals energyKwh × region intensity to ≤1e-3 g", () => {
  ...
  void test("per-role lines are sorted by role for byte-stable hashing", () => {
    const roles = report.perRole.map((line) => line.role);
    const sorted = [...roles].sort();
    assert.deepEqual(roles, sorted);
  });

  void test("type-level invariants are stamped false", () => {
    assert.equal(report.secretsIncluded, false);
    assert.equal(report.rawPromptsIncluded, false);
  });

  void test("methodology pins the cited grid intensity row", () => {
  ...
  void test("identical inputs produce byte-identical reports", () => {
  ...
  void test("zero-token job produces a zero-footprint report (no NaN)", () => {
  ...
  void test("rejects unknown deployment with clear error", () => {
    assert.throws(
      () =>
        buildCarbonFootprintReport({
          ...BUILD_INPUT,
  ...
  void test("rejects duplicate role entry", () => {
    assert.throws(
      () =>
        buildCarbonFootprintReport({
          ...BUILD_INPUT,
  ...
  void test("rejects negative token counts", () => {
    assert.throws(
      () =>
        buildCarbonFootprintReport({
          ...BUILD_INPUT,
  ...
  void test("low-carbon region produces lower co2eGrams than coal-heavy region", () => {
...
void describe("carbon-footprint: writeCarbonFootprintReport", () => {
  void test("writes the artifact under <runDir>/carbon/", async () => {
...
void describe("carbon-footprint: aggregateCarbonFootprint", () => {
  void test("groups by (customerId, month) and totals", () => {
  ...
  void test("rows are sorted by (customerId, month)", () => {
    const base = buildCarbonFootprintReport(BUILD_INPUT);
  ...
  void test("unattributed bucket appears when customerId is omitted", () => {
    const { customerId: _omittedCustomerId, ...inputWithoutCustomer } =
      BUILD_INPUT;
    void _omittedCustomerId;
    const noCustomer = buildCarbonFootprintReport(inputWithoutCustomer);
    const out = aggregateCarbonFootprint({
      reports: [noCustomer],
      generatedAt: FIXED_NOW,
    });
...
void describe("carbon-footprint: rankCandidatesByCarbon (routing optimizer hook)", () => {
  void test("ranks low-carbon regions first", () => {
  ...
  void test("smaller models beat larger ones on the same region", () => {
    const ranked = rankCandidatesByCarbon({
      candidates: [
        { deployment: "azure-openai-gpt-4o", region: "westeurope" },
  ...
  void test("unknown deployment / region surfaces in `skipped`", () => {
    const ranked = rankCandidatesByCarbon({
      candidates: [
  ...
  void test("higher weight wins on a CO2e tie", () => {
  ...
  void test("custom input/output mix changes the ranking signal", () => {
...
void describe("carbon-footprint: production-runner wiring (Issue #2129 Wave B.3)", () => {
  void test("REFERENCE_GRID_CARBON_INTENSITY_TABLE passes the standard validator", () => {
    validateGridCarbonIntensityTable(REFERENCE_GRID_CARBON_INTENSITY_TABLE);
  });

  void test("REFERENCE_GRID_CARBON_INTENSITY_TABLE covers every region admitted by SUPPORTED_REGION_ATTESTATION_HOSTING_REGIONS", () => {
  ...
  void test("REFERENCE_GRID_CARBON_INTENSITY_TABLE also covers the legacy Azure-style aliases", () => {
    const covered = new Set(
      REFERENCE_GRID_CARBON_INTENSITY_TABLE.entries.map(
        (entry) => entry.region,
      ),
  ...
  void test("carbonRoleUsageFromFinOpsRoles drops rows with no deployment label and rows with zero attempts", () => {
  ...
  void test("pickDominantCarbonRegion picks the most-frequent region and ties break alphabetically", () => {
    assert.equal(
      pickDominantCarbonRegion([
        { region: "eu-west-1" },
        { region: "eu-central-1" },
  ...
  void test("buildCarbonFootprintReport works end-to-end with the baked-in reference tables", () => {
