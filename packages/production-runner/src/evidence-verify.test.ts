...
void test("verifyJobEvidence: untouched validation run verifies clean (ok=true, no failures)", async () => {
...
void test("verifyJobEvidence: tampered manifest field surfaces digest_witness failure", async () => {
...
void test("verifyJobEvidence: schema-mismatched manifest returns manifest_unparseable", async () => {
...
void test("verifyJobEvidence: digest witness failure does not mark manifest metadata failed", async () => {
  ...
  try {
    ...
    assert.equal(
      result.body.failures.some((f) => f.code === "manifest_metadata_invalid"),
      false,
    );
  } finally {
    await seed.cleanup();
...
void test("verifyJobEvidence: malformed versioned manifest returns ok=false instead of throwing", async () => {
...
void test("verifyJobEvidence: metadata failure with a matching digest witness is classified independently", async () => {
...
void test("verifyJobEvidence: tampered artifact surfaces both resized + mutated", async () => {
...
void test("verifyJobEvidence: missing artifact surfaces artifact_missing", async () => {
...
void test("verifyJobEvidence: empty job dir with no manifest returns no_evidence", async () => {
...
void test("verifyJobEvidence: nonexistent jobId returns job_not_found", async () => {
...
void test("verifyJobEvidence: sigstore-signed run reports signaturesVerified=true", async () => {
  ...
  try {
    ...
    assert.equal(result.body.attestation.signaturesVerified, true);
    assert.ok(result.body.attestation.signatureCount >= 1);
  } finally {
    await seed.cleanup();
  }
});

void test("verifyJobEvidence: tampered signed bundle surfaces a verification failure", async () => {
...
void test("verifyJobEvidence: visual_sidecar_evidence_missing when manifest lacks visualSidecar but cases reference screens", async () => {
  ...
  try {
    ...
    const { canonicalJson } = await import("@oscharko-dev/ti-security");
...
void test("verifyJobEvidence: two consecutive verifications produce byte-identical bodies (modulo verifiedAt)", async () => {
...
void test("verifyJobEvidence: response body never carries absolute paths or fake bearer tokens (fast-check)", async () => {
...
void test("verifyJobEvidence: response shape matches the documented EvidenceVerifyResponse schema (golden)", async () => {
