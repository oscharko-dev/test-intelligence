...
import {
  ...
  type FormScreenA11yCriterionId,
...
> = Object.freeze({
  ...
  "focus-indicator": Object.freeze({
    ...
    successCriterion:
      "WCAG 2.4.7 Focus Visible (Level AA) + 2.4.13 Focus Appearance",
    description:
  ...
  "error-announcements": Object.freeze({
    ...
    successCriterion:
      "WCAG 3.3.1 Error Identification (Level A) + 4.1.3 Status Messages",
    description:
  ...
  "color-contrast": Object.freeze({
    ...
    successCriterion:
      "WCAG 1.4.3 Contrast (Minimum) (Level AA) + 1.4.11 Non-text Contrast",
    description:
...
// Moved to policy-gate.ts to break the import cycle:
// a11y-coverage-eval → validation-harness → validation-pipeline →
// policy-gate → a11y-coverage-eval. Re-exported here for backward compat.
export {
  FORM_SCREEN_A11Y_REQUIRED_CRITERIA,
  type FormScreenA11yCriterionId,
  listCoveredFormScreenA11yCriteria,
...
export const computeA11yCoverage = (
  ...
  const thresholds =
    input.thresholds ?? A11Y_COVERAGE_PRODUCTION_BASELINE_THRESHOLDS;
...
export type A11yCoverageEvalSourceId =
  | {
      readonly kind: "baseline-archetype";
      readonly id: BaselineArchetypeFixtureId;
    }
...
export const buildA11yCoverageEvalArtifactForBaseline = async (
  ...
  const generatedAt =
    input.generatedAt ?? A11Y_COVERAGE_EVAL_FIXTURE_GENERATED_AT;
  const thresholds =
    input.thresholds ?? A11Y_COVERAGE_PRODUCTION_BASELINE_THRESHOLDS;
