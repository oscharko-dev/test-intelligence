...
import { verifyProvenanceFromDisk } from "@oscharko-dev/ti-production-runner";
import {
  verifySealBundle,
...
import { validateFinOpsBudgetEnvelope } from "@oscharko-dev/ti-model-gateway";
import type { AgentHarnessTestDepth } from "@oscharko-dev/ti-agentic-harness";
import type { FigmaRestNode } from "@oscharko-dev/ti-core-engine";
import type {
...
const DEPRECATED_TOPOLOGY_DEPLOYMENT_ENV_ALIASES = [
  {
    ...
    canonicalEnv: "TEST_INTELLIGENCE_TESTCASE_MODEL_DEPLOYMENT",
    deprecatedEnv:
      "TEST_INTELLIGENCE_AZURE_AI_FOUNDRY_TEST_GENERATION_DEPLOYMENT",
  },
  {
    role: "visual_primary" as const,
    canonicalEnv: "TEST_INTELLIGENCE_VISUAL_PRIMARY_DEPLOYMENT",
    deprecatedEnv:
      "TEST_INTELLIGENCE_AZURE_AI_FOUNDRY_VISUAL_PRIMARY_DEPLOYMENT",
  },
  {
    role: "visual_fallback" as const,
    canonicalEnv: "TEST_INTELLIGENCE_VISUAL_FALLBACK_DEPLOYMENT",
    deprecatedEnv:
      "TEST_INTELLIGENCE_AZURE_AI_FOUNDRY_VISUAL_FALLBACK_DEPLOYMENT",
  },
] as const;
...
export interface TestIntelligenceRunOptions {
   ...
   *   2. `TEST_INTELLIGENCE_LOGIC_JUDGE_DEPLOYMENT` env var.
...
export const parseTestIntelligenceRunArgs = (
  ...
  let modelEndpoint: string | undefined =
    env.TEST_INTELLIGENCE_MODEL_ENDPOINT?.trim() || undefined;
  let modelDeployment: string =
    env.TEST_INTELLIGENCE_TESTCASE_MODEL_DEPLOYMENT?.trim() ||
    PRODUCTION_RUNNER_TEST_GENERATION_DEPLOYMENT;
  ...
  let ictRegisterRef: string | undefined =
    env.TEST_INTELLIGENCE_ICT_REGISTER_REF?.trim() || undefined;
  ...
  let requireMultiAgentTopology = isTruthyFlag(
    env.TEST_INTELLIGENCE_REQUIRE_MULTI_AGENT_TOPOLOGY,
  );
  ...
  let maxFigmaPayloadBytes: number | undefined = parsePositiveIntegerEnv(
    env.TEST_INTELLIGENCE_MAX_FIGMA_PAYLOAD_BYTES,
  );
  if (
    ...
    throw new TestIntelligenceRunOperatorError(
      `TEST_INTELLIGENCE_MAX_FIGMA_PAYLOAD_BYTES=${maxFigmaPayloadBytes} exceeds the security hard ceiling of ${MAX_FIGMA_PAYLOAD_BYTES_CEILING} bytes (128 MiB). Streaming larger payloads is tracked as a follow-up; until then 128 MiB is the audited safe ceiling.`,
    );
  }
  ...
  let coverageBaselineTenantId: string =
    env.TEST_INTELLIGENCE_TENANT_ID?.trim() || "default";
  ...
  const topologyInputSources: TopologyInputSources = {
    modelDeployment:
      readTrimmedEnv(env, "TEST_INTELLIGENCE_TESTCASE_MODEL_DEPLOYMENT") !==
      undefined
        ? "env"
        : "default",
    ...
    coveragePlannerDeployment:
      readTrimmedEnv(env, "TEST_INTELLIGENCE_COVERAGE_PLANNER_DEPLOYMENT") !==
      undefined
        ? "env"
        : "default",
    riskRankerDeployment:
      readTrimmedEnv(env, "TEST_INTELLIGENCE_RISK_RANKER_DEPLOYMENT") !==
      undefined
        ? "env"
        : "default",
    visualPrimaryDeployment:
      readTrimmedEnv(env, "TEST_INTELLIGENCE_VISUAL_PRIMARY_DEPLOYMENT") !==
      undefined
        ? "env"
        : "default",
    visualFallbackDeployment:
      readTrimmedEnv(env, "TEST_INTELLIGENCE_VISUAL_FALLBACK_DEPLOYMENT") !==
      undefined
        ? "env"
        : "default",
    a11yJudgeDeployment:
      readTrimmedEnv(env, "TEST_INTELLIGENCE_A11Y_JUDGE_DEPLOYMENT") !==
      undefined
        ? "env"
        : "default",
  };
...
/**
 * Pure parser for `test-intelligence doctor`. Mirrors the
...
export const parseTestIntelligenceVerifyProvenanceArgs = (
  ...
  throw new TestIntelligenceRunOperatorError(
    "usage: test-intelligence verify-provenance <run-dir> or test-intelligence --verify-provenance <run-dir>",
  );
};
...
export const parseTestIntelligenceAuditDossierArgs = (
  ...
  let signKeyPath = env.TEST_INTELLIGENCE_AUDIT_SIGN_KEY?.trim() || undefined;
  ...
  if (!runDir) {
    throw new TestIntelligenceRunOperatorError(
      "usage: test-intelligence audit-dossier --run-dir <path> --output <dir> [--sign-key <path>]",
    );
  }
  if (!outputDir) {
    throw new TestIntelligenceRunOperatorError(
  ...
  if (!signKeyPath) {
    throw new TestIntelligenceRunOperatorError(
      "Audit dossier signing key is required via --sign-key or TEST_INTELLIGENCE_AUDIT_SIGN_KEY.",
    );
  }
...
export const parseTestIntelligenceAuditVerifyArgs = (
  ...
  throw new TestIntelligenceRunOperatorError(
    "usage: test-intelligence audit-verify <bundle-prefix-or-json> or test-intelligence audit-verify --bundle <bundle-prefix-or-json>",
  );
};
...
export const parseTestIntelligenceVerifySealArgs = (
  ...
  if (!bundle) {
    throw new TestIntelligenceRunOperatorError(
      "usage: test-intelligence verify-seal --bundle <path> [--key <path>] [--expected-hmac <hex>] [--expected-merkle-root <hex>] [--json] [--output <path>]",
    );
  }
  return {
    bundle,
...
export const buildLiveLogicJudgeClient = (
  ...
  if (!options.modelEndpoint) {
    throw new TestIntelligenceRunOperatorError(
      "--model-endpoint or TEST_INTELLIGENCE_MODEL_ENDPOINT is required for mode=deterministic_llm",
    );
  }
  if (!options.modelApiKey) {
    throw new TestIntelligenceRunOperatorError(
      "--model-api-key or TEST_INTELLIGENCE_LLM_API_KEY is required for mode=deterministic_llm",
    );
  }
...
const resolveModelApiKeyFromEnv = (
  ...
  return readTrimmedEnv(env, "TEST_INTELLIGENCE_LLM_API_KEY");
};
...
const requireVisualSidecarEnv = (
  ...
  throw new TestIntelligenceRunOperatorError(
    "--enable-visual-sidecar requires TEST_INTELLIGENCE_VISUAL_MODEL_ENDPOINT, TEST_INTELLIGENCE_VISUAL_PRIMARY_DEPLOYMENT, TEST_INTELLIGENCE_VISUAL_FALLBACK_DEPLOYMENT",
  );
};
...
const requireVisualSidecarDeployments = (
  ...
  if (
    readTrimmedEnv(env, "TEST_INTELLIGENCE_VISUAL_MODEL_ENDPOINT") === undefined
  ) {
    missing.push("TEST_INTELLIGENCE_VISUAL_MODEL_ENDPOINT");
  }
  if (visualPrimaryDeployment === undefined) {
    missing.push(
      "--visual-primary-deployment or TEST_INTELLIGENCE_VISUAL_PRIMARY_DEPLOYMENT",
    );
  }
  if (visualFallbackDeployment === undefined) {
    missing.push(
      "--visual-fallback-deployment or TEST_INTELLIGENCE_VISUAL_FALLBACK_DEPLOYMENT",
    );
  }
  if (missing.length > 0) {
    throw new TestIntelligenceRunOperatorError(
...
const buildTopologyPreflightReport = ({
  ...
  const strictModeEnabled =
    ...
    isTruthyFlag(env.TEST_INTELLIGENCE_REQUIRE_MULTI_AGENT_TOPOLOGY);
  ...
  const visualPrimarySource =
    ...
    deploymentSourceFromEnv(env, "TEST_INTELLIGENCE_VISUAL_PRIMARY_DEPLOYMENT");
  ...
  const a11ySource =
    ...
    deploymentSourceFromEnv(env, "TEST_INTELLIGENCE_A11Y_JUDGE_DEPLOYMENT");
  ...
  if (!visualSidecarEnabled) {
    ...
    if (visualPrimaryDeployment === undefined) {
      roles.push({
        ...
        source: visualPrimarySource,
        status: "disabled",
        skipReason: "TEST_INTELLIGENCE_VISUAL_PRIMARY_DEPLOYMENT is unset",
      });
      errors.push(
    ...
    if (visualFallbackDeployment === undefined) {
      roles.push({
        ...
        source: visualFallbackSource,
        status: "disabled",
        skipReason: "TEST_INTELLIGENCE_VISUAL_FALLBACK_DEPLOYMENT is unset",
      });
      errors.push(
...
const buildDoctorReport = (
  ...
  if (options.logicJudgeDeployment === undefined) {
    pushRole({
      ...
      fix: joinFixes(
        `set TEST_INTELLIGENCE_LOGIC_JUDGE_DEPLOYMENT=${TEST_INTELLIGENCE_LOGIC_JUDGE_RECOMMENDED_DEPLOYMENT}`,
  ...
  pushOptionalTextDoctorRole({
    ...
    envVar: "TEST_INTELLIGENCE_COVERAGE_PLANNER_DEPLOYMENT",
  ...
  pushOptionalTextDoctorRole({
    ...
    envVar: "TEST_INTELLIGENCE_RISK_RANKER_DEPLOYMENT",
  ...
  pushVisualRole({
    ...
    recommendedDeployment:
      TEST_INTELLIGENCE_VISUAL_PRIMARY_RECOMMENDED_DEPLOYMENT,
    envVar: "TEST_INTELLIGENCE_VISUAL_PRIMARY_DEPLOYMENT",
    unsetSummary:
  ...
  pushVisualRole({
    ...
    recommendedDeployment:
      TEST_INTELLIGENCE_VISUAL_FALLBACK_RECOMMENDED_DEPLOYMENT,
    envVar: "TEST_INTELLIGENCE_VISUAL_FALLBACK_DEPLOYMENT",
    unsetSummary:
  ...
  pushVisualRole({
    ...
    envVar: "TEST_INTELLIGENCE_A11Y_JUDGE_DEPLOYMENT",
    unsetSummary:
  ...
  if (
    ...
    if (fallbackRole && doctorStatusRank(fallbackRole.status) < 2) {
      ...
      fallbackRole.fix = `set TEST_INTELLIGENCE_VISUAL_FALLBACK_DEPLOYMENT=${TEST_INTELLIGENCE_VISUAL_FALLBACK_RECOMMENDED_DEPLOYMENT}`;
    }
  }
...
export const runTestIntelligenceAuditDossierCommand = async (
  ...
  try {
    ...
    const result = await generateAuditDossier({
      ...
      ...(runtime.env?.TEST_INTELLIGENCE_ICT_REGISTER_REF?.trim()
        ? {
            ictRegisterRef:
              runtime.env.TEST_INTELLIGENCE_ICT_REGISTER_REF.trim(),
          }
...
export const runTestIntelligenceCommand = async (
  ...
  if (!resolveTestIntelligenceEnabled(env)) {
    sink.stderr(
      `error: ${TEST_INTELLIGENCE_ENV}=1 must be set to use "test-intelligence run"\n`,
    );
    return 1;
  }
        ...
        `  visual sidecar: ${
          options.noVisualSidecar
            ...
            : options.enableVisualSidecar
              ...
              : "disabled (default; set --enable-visual-sidecar or TEST_INTELLIGENCE_ENABLE_VISUAL_SIDECAR=1)"
        }`,
        "  resolved roles:",
        ...topologyPreflightReport.roles.map(formatTopologyRoleLine),
  ...
  const cliProgressSink = (event: ProductionRunnerEvent): void => {
    ...
    sink.stderr(formatCliProgressEvent(event, cliProgressFirstEventTimestamp));
  };
...
test-intelligence run - drive the figma_to_qc_test_cases pipeline

Usage:
  test-intelligence run [options]
...
LLM (defaults from environment):
  --model-endpoint <url>     default: env TEST_INTELLIGENCE_MODEL_ENDPOINT
  ...
  --logic-judge-deployment <name>
                             ...
                             Default: env TEST_INTELLIGENCE_LOGIC_JUDGE_DEPLOYMENT
  ...
  --risk-ranker-deployment <name>
                             ...
                             when unset).
  --model-api-key <key>      default: env TEST_INTELLIGENCE_LLM_API_KEY
...
Figma payload:
  --max-figma-payload-bytes <n>
                             ...
                             settable via env
                             TEST_INTELLIGENCE_MAX_FIGMA_PAYLOAD_BYTES.
...
Mutation-killing eval (Issue #2041):
  --enable-mutation-eval     Run the mutation-killing eval after the
                             ...
                             Default: off (env override:
                             TEST_INTELLIGENCE_ENABLE_MUTATION_EVAL=1).
...
Coverage-baseline drift (Issue #1950):
  ...
  --coverage-baseline-tenant <id>
                             ...
                             TEST_INTELLIGENCE_TENANT_ID, then "default").
...
Feature gate:
  TEST_INTELLIGENCE_ENABLED=1 must be set.
...
test-intelligence doctor - inspect the local Test Intelligence topology

Usage:
  test-intelligence doctor [options]
...
Deployments (defaults from environment):
  ...
  --visual-fallback-deployment <name>   default: env TEST_INTELLIGENCE_VISUAL_FALLBACK_DEPLOYMENT
  --a11y-judge-deployment <name>        default: env TEST_INTELLIGENCE_A11Y_JUDGE_DEPLOYMENT
...
test-intelligence verify-provenance - verify provenance.jsonld against a run directory

Usage:
  test-intelligence verify-provenance <run-dir>
  test-intelligence --verify-provenance <run-dir>
...
test-intelligence audit-dossier - generate a signed audit-dossier bundle from one run directory

Usage:
  test-intelligence audit-dossier --run-dir <path> --output <dir> [--sign-key <path>]

Behavior:
  - Requires a signing key via --sign-key or TEST_INTELLIGENCE_AUDIT_SIGN_KEY.
...
test-intelligence audit-verify - verify an audit-dossier bundle

Usage:
  test-intelligence audit-verify <bundle-prefix-or-json>
  test-intelligence audit-verify --bundle <bundle-prefix-or-json>
...
test-intelligence verify-seal - verify a production-runner reproducibility seal independently of the original run dir

Usage:
  test-intelligence verify-seal --bundle <path>
                                            [--key <path>]
                                            [--expected-hmac <hex>]
...
Usage:
  test-intelligence run [options]
  test-intelligence doctor [options]
  test-intelligence audit-dossier --run-dir <path> --output <dir>
  test-intelligence audit-verify <bundle-prefix-or-json>
  test-intelligence verify-provenance <run-dir>
  test-intelligence verify-seal --bundle <path> [--key <path>]
  test-intelligence review <list|get|decide> [options]
  test-intelligence calibration-refit [options]
  test-intelligence tms-push --run-dir <path> --tms <id> --project <id>
  test-intelligence onboard --tenant-id <id> --legal-name <name> --policy-profile <id> --output-root <dir>
  test-intelligence onboard --doctor --tenant-id <id> --output-root <dir>

Run "test-intelligence run --help" for the live-run flags.
Run "test-intelligence doctor --help" for the topology doctor flags.
Run "test-intelligence audit-dossier --help" for bundle generation.
Run "test-intelligence audit-verify --help" for bundle verification.
Run "test-intelligence verify-provenance --help" for provenance verification.
Run "test-intelligence verify-seal --help" for self-contained seal verification.
Run "test-intelligence review --help" for the human-oversight queue (Issue #2179).
Run "test-intelligence calibration-refit --help" for the self-improving calibration loop (Issue #2182).
Run "test-intelligence tms-push --help" for the production-grade TMS adapters (Issue #2183).
Run "test-intelligence onboard --help" for the self-service customer onboarding CLI (Issue #2185).
`;
