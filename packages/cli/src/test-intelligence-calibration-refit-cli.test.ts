...
import type { CalibrationGoldEntry } from "@oscharko-dev/ti-eval";

let cachedOperatorKeyPath: string | undefined;
const operatorKeyPath = async (): Promise<string> => {
  if (cachedOperatorKeyPath !== undefined) {
    return cachedOperatorKeyPath;
  }
  const { privateKey } = generateKeyPairSync("ed25519");
  const dir = await mkdtemp(join(tmpdir(), "ti-calibration-key-"));
  const keyPath = join(dir, "operator-ed25519.private-key.pem");
  await writeFile(
    keyPath,
    privateKey.export({ format: "pem", type: "pkcs8" }) as string,
    "utf8",
  );
  cachedOperatorKeyPath = keyPath;
  return keyPath;
};

const buildSeparableEntries = (
  count: number,
...
void test("calibration-refit CLI: rejects unknown locale", () => {
  assert.throws(
    () =>
      parseTestIntelligenceCalibrationRefitArgs([
        "--locale",
...
void test("calibration-refit CLI: rejects unknown risk-class", () => {
  assert.throws(
    () =>
      parseTestIntelligenceCalibrationRefitArgs([
        "--locale",
...
void test("calibration-refit CLI: dry-run does not require sign key", () => {
...
void test("calibration-refit CLI: ratification requires sign-key + decided-at", () => {
  assert.throws(
    () =>
      parseTestIntelligenceCalibrationRefitArgs([
        "--locale",
...
void test("calibration-refit CLI: HELP text mentions hard rollback safety", () => {
  assert.match(
    TEST_INTELLIGENCE_CALIBRATION_REFIT_HELP,
...
void test("calibration-refit CLI: dry-run writes a proposal record and exits 0", async () => {
...
void test("calibration-refit CLI: ratify path signs the proposal and promotes the curve", async () => {
  const goldPath = await writeGoldFile(buildSeparableEntries(60));
  const curvesDir = await mkdtemp(join(tmpdir(), "ti-cli-refit-curves-"));
  const sink = collectingSink();
  try {
