...
const buildSignedRawEvidence = (
  ...
  const sig = cryptoSign(
    null,
    signingBytes,
    privateKey as Parameters<typeof cryptoSign>[2],
  );
  return {
    ...body,
...
void test("parseTestIntelligenceExecutionPullArgs: requires every mandatory flag", () => {
  assert.throws(
    () => parseTestIntelligenceExecutionPullArgs([]),
    TestIntelligenceExecutionPullOperatorError,
  );
...
void test("parseTestIntelligenceExecutionPullArgs: rejects unknown flag", () => {
  assert.throws(
    () =>
      parseTestIntelligenceExecutionPullArgs([
        "--tms",
...
void test("parseTestIntelligenceExecutionPullArgs: rejects malformed --since", () => {
  assert.throws(
    () =>
      parseTestIntelligenceExecutionPullArgs([
        "--tms",
...
void test("parseTestIntelligenceExecutionPullArgs: parses the canonical happy path", () => {
...
void test("runTestIntelligenceExecutionPullCommand: end-to-end happy path writes report", async () => {
  ...
  const code = await runTestIntelligenceExecutionPullCommand({
    ...
    env: {
      TEST_INTELLIGENCE_TMS_XRAY_TOKEN: "test-token",
    },
    adapterFactory: () => stub,
...
void test("runTestIntelligenceExecutionPullCommand: --strict-signature exits 2 on bad sig", async () => {
  ...
  assert.match(
    stderrChunks.join(""),
    new RegExp(G12_EXECUTION_EVIDENCE_SIGNED),
  );
});

void test("runTestIntelligenceExecutionPullCommand: missing tenant dir → exit 1", async () => {
...
void test("runTestIntelligenceExecutionPullCommand: missing TMS credentials → exit 1", async () => {
  const { publicKey } = generateKeyPairSync("ed25519");
  const publicKeyPem = publicKey.export({
    type: "spki",
    format: "pem",
