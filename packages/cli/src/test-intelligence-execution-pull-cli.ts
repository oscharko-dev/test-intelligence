/**
 * `test-intelligence execution-pull` sub-command
 * (Issue #2186, W8-4).
 *
...
export const runTestIntelligenceExecutionPullCommand = async (
  ...
  if (!tenantDirExists) {
    input.sink.stderr(
      `error: tenant directory does not exist: ${tenantDir}\n` +
        `       run \`test-intelligence onboard --tenant-id ${input.options.tenantId} ` +
  ...
  if (
    ...
    const gateError = new ExecutionEvidenceSignatureGateError(
      signatureRejections,
    );
    input.sink.stderr(
...
test-intelligence execution-pull — pull TMS execution evidence

Usage:
  test-intelligence execution-pull \\
    --tms <xray|alm|qtest|polarion> \\
    --project <id> \\
    --since <iso-time> \\
    --tenant <id> \\
