/**
 * `test-intelligence tms-push` sub-command (Issue #2183).
 *
 ...
 * `loadTmsCredentialsFromEnv` (`TEST_INTELLIGENCE_TMS_<NAME>_TOKEN`,
...
export const parseTestIntelligenceTmsPushArgs = (
  ...
  if (!runDir) {
    throw new TestIntelligenceTmsPushOperatorError(
      "usage: test-intelligence tms-push --run-dir <path> --tms <xray|alm|qtest|polarion> --project <id> [--endpoint <alias>] [--tenant <id>] [--run-id <id>] [--batch-size <n>] [--dry-run]",
    );
  }
  if (!tms) {
    throw new TestIntelligenceTmsPushOperatorError(
...
test-intelligence tms-push — push approved test cases to a TMS

Usage:
  test-intelligence tms-push \\
    --run-dir <path> \\
    --tms <xray|alm|qtest|polarion> \\
    --project <id> \\
    [--endpoint <alias>] \\
...
Credentials are read from the env (NEVER printed):
  TEST_INTELLIGENCE_TMS_<NAME>_TOKEN              (PAT)
  TEST_INTELLIGENCE_TMS_<NAME>_OAUTH_ACCESS_TOKEN (OAuth 2.0)
  TEST_INTELLIGENCE_TMS_<NAME>_BEARER             (Bearer)
