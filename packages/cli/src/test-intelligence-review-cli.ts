/**
 * `test-intelligence review …` sub-commands (Issue #2179).
 *
...
const resolveQueueRoot = (override?: string): string => {
  ...
  const fromEnv = process.env["TEST_INTELLIGENCE_REVIEW_ROOT"];
...
export const parseTestIntelligenceReviewDecideArgs = (
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--tenant") {
      ...
      if (
        !(HUMAN_REVIEW_QUEUE_VERDICT_LABELS as readonly string[]).includes(
          value,
        )
      ) {
        throw new TestIntelligenceReviewOperatorError(
  ...
  if (typeof out.rationaleFile !== "string" || out.rationaleFile.length === 0) {
    throw new TestIntelligenceReviewOperatorError(
  ...
  return {
    ...
    ...(out.revisedTcFile !== undefined
      ? { revisedTcFile: out.revisedTcFile }
      : {}),
...
export const runTestIntelligenceReviewDecideCommand = async (
  ...
  try {
    ...
    const principalId =
      options.reviewerPrincipalId ?? sha256OfKeyPath(options.signKeyPath);
    ...
    return failReviewError(
      error,
      sink,
      "Failed to record human-review verdict.",
    );
  }
};
...
const sha256OfKeyPath = (keyPath: string): string =>
  sha256Hex(resolve(keyPath));
...
export const TEST_INTELLIGENCE_REVIEW_HELP = `test-intelligence review - human-oversight queue + decision capture (DSGVO Art. 22 / EU AI Act Art. 14)

Usage:
  test-intelligence review list   --tenant <id> [--profile <id>] [--sla-due-by <iso-8601>] [--root <dir>]
  test-intelligence review get    <item-id> --tenant <id> [--root <dir>]
  test-intelligence review decide <item-id> --tenant <id>
                                                --verdict <approved|rejected|revised>
                                                --rationale <md-file>
                                                [--revised-tc <json-file>]
                                                --sign-key <pem>
...
Options:
  ...
  --root <dir>               Override the queue root directory.
                             Default: $TEST_INTELLIGENCE_REVIEW_ROOT or "<cwd>/.ti-review"
