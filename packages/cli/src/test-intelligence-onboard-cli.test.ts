...
void test("parseTestIntelligenceOnboardArgs accepts the documented flag set in provision mode", () => {
...
void test("parseTestIntelligenceOnboardArgs accepts --doctor with the doctor-mode flag set", () => {
  const parsed = parseTestIntelligenceOnboardArgs([
    "--doctor",
    "--tenant-id",
    "acme-bank",
...
void test("parseTestIntelligenceOnboardArgs rejects unknown flags and missing required flags", () => {
  assert.throws(
    () =>
      parseTestIntelligenceOnboardArgs([
        "--tenant-id",
...
void test("parseTestIntelligenceOnboardArgs rejects duplicate flags", () => {
  assert.throws(
    () =>
      parseTestIntelligenceOnboardArgs([
        "--tenant-id",
...
void test("runTestIntelligenceOnboardCommand exits 0 on a clean provision and prints next-step commands", async () => {
...
void test("runTestIntelligenceOnboardCommand returns exit code 1 on operator error and 2 on doctor failure", async () => {
  const outputRoot = await makeRoot();
  try {
...
void test("TEST_INTELLIGENCE_ONBOARD_HELP documents the documented flag set", () => {
