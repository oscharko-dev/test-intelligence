...
void test("parseTestIntelligenceTmsPushArgs: requires --run-dir", () => {
  assert.throws(
    () => parseTestIntelligenceTmsPushArgs([]),
    TestIntelligenceTmsPushOperatorError,
  );
...
void test("parseTestIntelligenceTmsPushArgs: rejects unknown --tms value", () => {
  assert.throws(
    () =>
      parseTestIntelligenceTmsPushArgs([
        "--run-dir",
...
void test("parseTestIntelligenceTmsPushArgs: rejects unknown flags", () => {
  assert.throws(
    () =>
      parseTestIntelligenceTmsPushArgs([
        "--run-dir",
...
void test("parseTestIntelligenceTmsPushArgs: parses minimal required flags", () => {
...
void test("runTestIntelligenceTmsPushCommand: missing credentials → exit 1", async () => {
...
void test("runTestIntelligenceTmsPushCommand: writes report and exits 0 on success", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "tms-cli-"));
  try {
    ...
    const code = await runTestIntelligenceTmsPushCommand({
      options: opts,
      sink,
      env: { TEST_INTELLIGENCE_TMS_XRAY_TOKEN: "tok" },
    ...
    const persisted = JSON.parse(
      await readFile(join(tempDir, TMS_PUSH_REPORT_ARTIFACT_FILENAME), "utf8"),
...
void test("runTestIntelligenceTmsPushCommand: --dry-run propagates", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "tms-cli-"));
  try {
    await mkdir(tempDir, { recursive: true });
    await writeFile(
