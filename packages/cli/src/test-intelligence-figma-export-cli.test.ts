...
void test("parseFigmaExportArgs requires --figma-url, --output, and a token", () => {
  assert.throws(() => parseFigmaExportArgs([], {}), /--figma-url is required/u);
  assert.throws(
    () =>
      parseFigmaExportArgs(["--figma-url", "https://figma.com/design/abc"], {}),
...
void test("parseFigmaExportArgs takes the token from FIGMA_ACCESS_TOKEN env var", () => {
...
void test("parseFigmaExportArgs prefers --figma-token over the env var", () => {
  const parsed = parseFigmaExportArgs(
    [
      "--figma-url",
      "https://figma.com/design/abc",
...
void test("parseFigmaExportArgs rejects unknown flags and HELP sentinel", () => {
  assert.throws(() => parseFigmaExportArgs(["--unknown"], {}), /Unknown flag/u);
  assert.throws(
...
void test("parseFigmaExportArgs parses --timeout-ms when positive integer", () => {
  const parsed = parseFigmaExportArgs(
    [
      "--figma-url",
      "https://figma.com/design/abc",
...
void test("runFigmaExport packages the snapshot under the canonical schema and exits 0", async () => {
...
void test("runFigmaExport writes into a directory when --output is a folder path", async () => {
...
void test("runFigmaExportCli returns exit code 1 on operator error", async () => {
  const stderr: string[] = [];
...
void test("runFigmaExportCli surfaces --help with exit code 0", async () => {
  const stdout: string[] = [];
