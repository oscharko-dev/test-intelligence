 ...
 * generation test mints a fresh keypair at runtime. The audit dossier
 * embeds the derived public key in the signed manifest, so a bundle signed
 * with the ephemeral key still verifies end-to-end.
 */
...
void test("audit-dossier: generates a verifiable bundle for the accepted run fixture", async () => {
...
void test("audit-dossier: verify passes for the checked-in expected bundle", async () => {
...
void test("contracts: region attestation report is canonical-json stable", () => {
...
void test("audit-dossier: verify fails when the Merkle proof is tampered", async () => {
  const tempDir = await mkdtemp(
    path.join(os.tmpdir(), "audit-dossier-verify-"),
  );
  try {
    ...
    assert.ok(
      result.failures.some(
        (failure) => failure.code === "merkle_proof_mismatch",
      ),
    );
  } finally {
    await rm(tempDir, { recursive: true, force: true });
...
void test("audit-dossier: verify fails when the Merkle root is tampered", async () => {
  ...
  try {
    ...
    assert.ok(
      result.failures.some(
        (failure) => failure.code === "merkle_root_mismatch",
      ),
    );
  } finally {
    await rm(tempDir, { recursive: true, force: true });
...
void test("audit-dossier: verify fails when the PDF is missing", async () => {
...
void test("audit-dossier: verify returns manifest_unparseable for invalid manifest shape", async () => {
...
void test("audit-dossier: generation fails clearly when a required artifact is missing", async () => {
  const tempDir = await mkdtemp(
    path.join(os.tmpdir(), "audit-dossier-missing-"),
  );
  const runDir = path.join(tempDir, "run");
  await mkdir(runDir, { recursive: true });
  await cp(acceptedRunDir, runDir, { recursive: true });
  await rm(path.join(runDir, "incidents.json"));
  const signKeyPath = await writeEphemeralSigningKey(tempDir);
...
void test("audit-dossier: generation fails when the provenance root does not match the attested leaf set", async () => {
  const tempDir = await mkdtemp(
    path.join(os.tmpdir(), "audit-dossier-root-gen-"),
  );
  const runDir = path.join(tempDir, "run");
  await mkdir(runDir, { recursive: true });
  await cp(acceptedRunDir, runDir, { recursive: true });
...
void test("audit-dossier: embeds the self-improving calibration refit history when proposals exist (Issue #2182)", async () => {
  const tempDir = await mkdtemp(
...
void test("audit-dossier: embeds the resolved tenant bundle when present (Issue #2184)", async () => {
  const tempDir = await mkdtemp(
  ...
  try {
    ...
    assert.equal(
      customerBundle.inheritsFromPolicyProfile,
      "eu-banking-default",
    );
...
void test("audit-dossier: manifest excludes raw prompts and screenshots", async () => {
  const manifest = JSON.parse(
