...
void test("parseTestIntelligenceReviewListArgs: requires --tenant", () => {
  assert.throws(
    () => parseTestIntelligenceReviewListArgs([]),
    TestIntelligenceReviewOperatorError,
  );
...
void test("parseTestIntelligenceReviewListArgs: optional flags", () => {
...
void test("parseTestIntelligenceReviewListArgs: rejects unknown flag", () => {
  assert.throws(
...
void test("parseTestIntelligenceReviewGetArgs: requires positional itemId and --tenant", () => {
  assert.throws(
...
void test("parseTestIntelligenceReviewDecideArgs: validates verdict + flag dependencies", () => {
  assert.throws(
    () =>
      parseTestIntelligenceReviewDecideArgs([
        "the-id",
...
void test("runTestIntelligenceReviewListCommand: emits empty items list when queue is empty", async () => {
...
void test("runTestIntelligenceReviewGetCommand: 404-style stderr when item missing", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "ti-review-cli-"));
  try {
...
void test("runTestIntelligenceReviewDecideCommand: end-to-end signs + persists with a generated ed25519 key", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "ti-review-cli-"));
  try {
