...
void test("schema constants are pinned", () => {
...
void test("isAgentHarnessCheckpointStatus accepts only the closed list", () => {
...
void test("appendAgentHarnessCheckpoint derives parentHash + chainIndex", () => {
...
void test("appendAgentHarnessCheckpoint propagates LangSmith-adapter fields", () => {
  const [root, middle, tail] = buildThreeStepChain();
  assert.ok(root && middle && tail);
...
void test("appendAgentHarnessCheckpoint is byte-stable for byte-identical input", () => {
...
void test("appendAgentHarnessCheckpoint sorts nextRoleStepIds and rejects duplicates", () => {
  const root = appendAgentHarnessCheckpoint(null, {
    jobId: "wd-sort",
...
void test("appendAgentHarnessCheckpoint rejects jobId mismatch with predecessor", () => {
...
void test("assertAgentHarnessCheckpointInvariants rejects bad fields", () => {
...
void test("assertAgentHarnessCheckpointInvariants rejects missing/invalid runId with checkpoint_schema_violation", () => {
...
void test("assertAgentHarnessCheckpointInvariants rejects missing/invalid completedAt with checkpoint_schema_violation", () => {
  const [root] = buildThreeStepChain();
  assert.ok(root);
  assert.throws(
    () =>
...
void test("assertAgentHarnessCheckpointInvariants rejects bad parentRunId at root (must be null)", () => {
  const [root] = buildThreeStepChain();
  assert.ok(root);
  assert.throws(
    () =>
...
void test("assertAgentHarnessCheckpointInvariants rejects parentRunId === runId (self-reference)", () => {
...
void test("verifier rejects new non-root runId with null parentRunId", () => {
...
void test("verifier rejects same runId with conflicting parentRunId across snapshots", () => {
...
void test("assertAgentHarnessCheckpointInvariants rejects negative or non-integer token counts", () => {
  const [root] = buildThreeStepChain();
  assert.ok(root);
  assert.throws(
...
void test("appendAgentHarnessCheckpoint rejects malformed runId at root", () => {
  assert.throws(
    () =>
      appendAgentHarnessCheckpoint(null, {
        jobId: "wd-bad-runid",
...
void test("verifier surfaces checkpoint_schema_violation when runId is structurally invalid", () => {
  const chain = buildThreeStepChain();
...
void test("verifier surfaces checkpoint_schema_violation when parentRunId references unknown runId", () => {
  const chain = buildThreeStepChain();
  const orphan: readonly AgentHarnessCheckpoint[] = [
    chain[0] as AgentHarnessCheckpoint,
    chain[1] as AgentHarnessCheckpoint,
...
void test("verifier accepts a snapshot pair sharing one runId (started + completed for same role-step)", () => {
...
void test("plausibility: per-step (promptTokens + completionTokens) maps 1:1 to a notional cost entry", () => {
...
void test("verifyAgentHarnessCheckpointChain accepts a clean chain and reports head + length", () => {
  const chain = buildThreeStepChain();
  const result = verifyAgentHarnessCheckpointChain(chain);
  assert.equal(result.ok, true);
...
void test("verifyAgentHarnessCheckpointChain returns zero-hash + length 0 for empty chain", () => {
...
void test("summarizeAgentHarnessCheckpointChain returns headOfChainHash + chainLength", () => {
  const chain = buildThreeStepChain();
...
void test("summarizeAgentHarnessCheckpointChain throws on tampered chain", () => {
  const chain = buildThreeStepChain();
  const tampered = [
    chain[0] as AgentHarnessCheckpoint,
  ...
  assert.throws(
    () => summarizeAgentHarnessCheckpointChain(tampered),
    /chain_break/,
  );
});
...
void test("AT-027: tampering with a middle checkpoint reports chain_break at the first breaking chainIndex", () => {
  const chain = buildThreeStepChain();
...
void test("verifier reports missing_root when the root carries a non-zero parentHash", () => {
  const chain = buildThreeStepChain();
...
void test("verifier reports chain_index_mismatch when checkpoints are reordered", () => {
  const chain = buildThreeStepChain();
...
void test("verifier reports duplicate_chain_index when the same index appears twice", () => {
  const chain = buildThreeStepChain();
...
void test("verifier reports schema_invalid for a structurally bad checkpoint", () => {
  const chain = buildThreeStepChain();
  const bad: readonly AgentHarnessCheckpoint[] = [
    chain[0] as AgentHarnessCheckpoint,
    {
...
void test("write + read round-trips a chain on disk and yields byte-stable artifacts", async () => {
...
void test("readAgentHarnessCheckpointChain returns [] when no chain exists", async () => {
  await withRunDir(async (runDir) => {
...
void test("AT-027 disk round-trip: mutating a middle checkpoint on disk fails verification at the first breaking chainIndex", async () => {
  await withRunDir(async (runDir) => {
...
void test("disk round-trip: deleting a middle checkpoint surfaces chain_index_mismatch", async () => {
  await withRunDir(async (runDir) => {
...
void test("writeAgentHarnessCheckpoint rejects unsafe jobIds for path traversal protection", async () => {
  await withRunDir(async (runDir) => {
    const root = appendAgentHarnessCheckpoint(null, {
      jobId: "../../etc/passwd",
      roleStepId: "x",
...
void test("writeAgentHarnessCheckpoint never persists raw prompts or secrets", async () => {
  await withRunDir(async (runDir) => {
