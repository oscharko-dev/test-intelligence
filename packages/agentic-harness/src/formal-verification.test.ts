...
const PSD2_SPEC_PATH =
  "packages/agentic-harness/src/formal-verification/specs/psd2-sca-art-97.smv";
const MIFID_SPEC_PATH =
  "packages/agentic-harness/src/formal-verification/specs/mifid-ii-art-25.smv";
...
void test("artifact filename + schema version constants are stable", () => {
  assert.equal(
    FORMAL_VERIFICATION_REPORT_ARTIFACT_FILENAME,
...
void test("passing fixture verifies", async () => {
...
void test("PSD2 SCA spec verifies all three properties", async () => {
...
void test("MiFID II suitability spec verifies all three properties", async () => {
...
void test("failing fixture is detected and reports a counterexample", async () => {
...
void test("buildFormalVerificationReport is byte-deterministic for fixed inputs", async () => {
...
void test("buildFormalVerificationReport sorts specs alphabetically by specPath", async () => {
  const specs = [
...
void test("report summary matches per-formula totals", async () => {
  const specs = await Promise.all([
    loadSpec(PSD2_SPEC_PATH),
    loadSpec(MIFID_SPEC_PATH),
  ]);
...
void test("assertFormalVerificationPass throws when any formula fails", async () => {
...
void test("assertFormalVerificationPass is a no-op when verdict is pass", async () => {
...
void test("renderFormalVerificationReportText includes per-spec verdict header", async () => {
  const specs = await Promise.all([
    loadSpec(PSD2_SPEC_PATH),
    loadSpec(MIFID_SPEC_PATH),
  ]);
...
void test("parser rejects an empty spec", () => {
  assert.throws(
    () =>
      verifyFormalVerificationSpec({
        specPath: "empty.smv",
...
void test("parser rejects a spec missing LTLSPEC / CTLSPEC", () => {
...
void test("parser supports CTLSPEC with AG / AF operators", () => {
  const source = `MODULE main
VAR state : { a, b };
ASSIGN
  init(state) := a;
...
void test("parser supports CTLSPEC with E[...U...] / A[...U...]", () => {
  const source = `MODULE main
VAR state : { a, b, c };
ASSIGN
  init(state) := a;
...
void test("counterexample trace contains valid reachable states", async () => {
  const failingSpec = await loadSpec(FAILING_FIXTURE_PATH);
...
void test("spec source SHA-256 is exposed in the per-spec result", async () => {
  const spec = await loadSpec(PASSING_FIXTURE_PATH);
  const result = verifyFormalVerificationSpec(spec);
  assert.match(result.specSha256, /^[0-9a-f]{64}$/u);
});

void test("spec exceeding max bytes is rejected at parse time", () => {
...
void test("parser rejects DEFINE (reserved but unsupported in pilot)", () => {
  const source = `MODULE main
VAR x : { a, b };
DEFINE p := x = a;
ASSIGN init(x) := a;
...
void test("LTL formula with X operator translates to AX under CTL semantics", () => {
  const source = `MODULE main
VAR state : { a, b };
ASSIGN
  init(state) := a;
...
void test("ranged-integer variables are accepted and bounded", () => {
  const source = `MODULE main
VAR
  counter : 0 .. 2;
  state   : { a, b };
...
void test("renderFormalVerificationReportJson emits canonical newline-terminated JSON", async () => {
  const spec = await loadSpec(PASSING_FIXTURE_PATH);
  const report = buildFormalVerificationReport({
    specs: [spec],
    generatedAt: "2026-05-10T00:00:00.000Z",
...
void test("buildFormalVerificationReport rejects malformed generatedAt", async () => {
  const spec = await loadSpec(PASSING_FIXTURE_PATH);
  assert.throws(
    () =>
      buildFormalVerificationReport({
