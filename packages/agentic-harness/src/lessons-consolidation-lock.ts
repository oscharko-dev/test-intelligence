...
import { recordActiveTenantRead } from "@oscharko-dev/ti-tenant";
...
const tryWriteLockBodyExclusive = async (
  path: string,
  body: ConsolidationLockBody,
): Promise<boolean> => {
  const tempPath = `${path}.${process.pid}.${randomUUID()}.tmp`;
  try {
    await writeFile(tempPath, canonicalJson(body), {
      encoding: "utf8",
      flag: "wx",
    });
...
export const tryAcquireConsolidationLock = async (
  ...
  if (existing === null) {
    ...
    const created = await tryWriteLockBodyExclusive(path, heldBody);
    if (!created) {
      return tryAcquireConsolidationLock(input);
    }
