...
void test("case-merger: merges disjoint passes and labels provenance per pass", () => {
  ...
  assert.deepEqual(merged.testCases.map((c) => c.id).sort(), [
    "tc-a-1",
    "tc-b-1",
  ]);
...
void test("case-merger: full overlap collapses with positive bias toward pass A", () => {
...
void test("case-merger: partial overlap merges shared cases and keeps unique ones", () => {
  const runA = buildList([
...
void test("case-merger: conflict-resolution rule 1 prefers the un-repaired side", () => {
...
void test("case-merger: when both sides carry repair instructions, falls back to positive bias", () => {
  const runA = buildList([
    buildCase("tc-a-iban", { coveredFieldIds: ["iban"] }),
  ]);
  const runB = buildList([
...
void test("case-merger: conflict path absorbs coverage ids from the dropped pass", () => {
...
void test("case-merger: empty inputs produce an empty merged list and zeroed totals", () => {
...
void test("case-merger: rejects mixed jobIds", () => {
...
void test("case-merger: signature includes screenId and technique so different screens never collapse", () => {
  const runA = buildList([
    buildCase("tc-a", {
      screenId: "s-payment",
      coveredFieldIds: ["amount"],
...
void test("case-merger: signature is order-insensitive across coveredFieldIds and coveredActionIds", () => {
...
void test("case-merger: deterministic — equivalent inputs in different orders produce byte-identical artifacts", () => {
...
void test("case-merger: property — N runs over permuted inputs converge on the same canonical merged list", () => {
  const cases = [
...
void test("case-merger: writeCaseMergerReport persists deterministic JSON with the canonical filename", async () => {
  ...
  try {
    ...
    assert.ok(
      result.artifactPath.endsWith(CASE_MERGER_REPORT_ARTIFACT_FILENAME),
    );
...
void test("case-merger: per-case provenance log is sorted by testCaseId for stable diffs", () => {
  const runA = buildList([
...
void test("case-merger: signature exposes the canonical screenId from figmaTraceRefs", () => {
...
void test("case-merger: legacy mergeGeneratedTestCaseLists wrapper preserves the public API", () => {
