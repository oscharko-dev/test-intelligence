...
void test("test-data-oracle: numeric range emits min/mid/max + below-min/above-max invalids", () => {
  ...
  for (const value of r.valid) {
    assert.equal(value.synthetic, true);
  }
  for (const value of r.invalid) {
    assert.equal(value.synthetic, true);
  }
});

void test("test-data-oracle: integer >= 1 boundary uses integer step (no decimals)", () => {
  const r = resolveTestData({
    fieldLabel: "Stueck",
...
void test("test-data-oracle: numeric > 0 emits 0.01 as boundary_min and 0 as invalid", () => {
  const r = resolveTestData({
...
void test("test-data-oracle: fixed length emits exact-len valid and len-1 / len+1 invalids", () => {
  const r = resolveTestData({
    fieldLabel: "PLZ",
...
void test("test-data-oracle: ISO date <= today emits today + 1y past valid + tomorrow invalid", () => {
  const r = resolveTestData({
...
void test("test-data-oracle: IBAN format emits documentation IBAN (not invented)", () => {
  const r = resolveTestData({
    fieldLabel: "IBAN",
  ...
  assert.ok(r.provenance.some((p) => p.includes("public documentation IBAN")));
});

void test("test-data-oracle: ISIN format emits documentation ISIN", () => {
  const r = resolveTestData({
    fieldLabel: "ISIN",
...
void test("test-data-oracle: presence-only rules without bounds return openQuestion (no inventing)", () => {
  const r = resolveTestData({
    fieldLabel: "Beruf",
...
void test("test-data-oracle: conditional 'Required if X is Yes' is unresolvable (no concrete value invented)", () => {
  const r = resolveTestData({
...
void test("test-data-oracle: computed-value rules surface as openQuestion (rubric forbids inventing concrete numbers)", () => {
  const r = resolveTestData({
...
void test("test-data-oracle: deterministic across calls (same inputs -> byte-identical outputs)", () => {
  const args = {
    fieldLabel: "Kreditbetrag",
    validations: ["Required", "Numeric in range 1000..50000"],
    now: ANCHOR,
...
void test("test-data-oracle: formatOracleValueAsTestDataEntry produces deterministic provenance string", () => {
  const r = resolveTestData({
    fieldLabel: "Kreditbetrag",
  ...
  const entry = formatOracleValueAsTestDataEntry("Kreditbetrag", r.valid[0]!);
  assert.equal(
    entry,
...
void test("test-data-oracle: ISO datetime + Date >= today + 1 day combination resolves to documented future date", () => {
  const r = resolveTestData({
    fieldLabel: "Termin-Slot",
    validations: [
  ...
  const boundaryMin = r.valid.find(
    (v) => v.rule === "Date >= today + 1 day" && v.category === "boundary_min",
  );
  assert.equal(boundaryMin?.value, "2026-05-10");
});

void test("test-data-oracle: long Max-N-characters values are chunked so they cannot trip the base64 heuristic (issue #2087)", () => {
  ...
  const aboveMax = r.invalid.find(
    (v) =>
      v.rule === "Max 500 characters" && v.category === "above_max_invalid",
  );
  assert.equal(aboveMax?.value.length, 501);
  assert.doesNotMatch(aboveMax.value, /[A-Za-z0-9+/]{64,}/u);
});

void test("test-data-oracle: short fixed-length values stay as a contiguous run (no whitespace splice for len <= 16)", () => {
...
void test("test-data-oracle: documentation-example test-data entries carry a redaction-token sentinel so the PII validator skips them", () => {
...
void test("test-data-oracle: ISO-date / datetime entries carry the redaction-token sentinel so DOB-label collisions do not trip the PII validator", () => {
