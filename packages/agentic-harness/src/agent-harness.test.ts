...
void test("outcome vocabulary is closed and maps onto existing job statuses", () => {
  assert.deepEqual(
    [...AGENT_HARNESS_OUTCOMES],
    [
      "accepted",
  ...
  assert.equal(
    AGENT_HARNESS_OUTCOME_TO_JOB_STATUS["failed_retryable"],
    "failed",
  );
  assert.equal(
    AGENT_HARNESS_OUTCOME_TO_JOB_STATUS["failed_permanent"],
    "failed",
  );
...
void test("resolveMaxRepairIterations returns the bounded budgets", () => {
  assert.equal(
    resolveMaxRepairIterations("standard"),
    DEFAULT_REPAIR_ITERATIONS,
  );
  assert.equal(
    resolveMaxRepairIterations("exhaustive"),
    EXHAUSTIVE_REPAIR_ITERATIONS,
  );
...
void test("accepted outcome on first judge accept short-circuits the loop", async () => {
...
void test("blocked outcome short-circuits on policy_block (terminal, partial status)", async () => {
  await withRunDir(async (runDir) => {
    const result = await runAgentHarnessStep({
      runDir,
      jobId: "job-1780",
...
void test("failed_permanent short-circuits and never re-iterates", async () => {
  await withRunDir(async (runDir) => {
    let calls = 0;
    const result = await runAgentHarnessStep({
      runDir,
...
void test("failed_retryable when budget exhausts on retryable errors with no judge rejections", async () => {
  await withRunDir(async (runDir) => {
    const calls: number[] = [];
    const result = await runAgentHarnessStep({
      runDir,
...
void test("needs_review when budget exhausts on judge rejections (max-iteration exhaustion ≠ accepted)", async () => {
  await withRunDir(async (runDir) => {
    let calls = 0;
    const result = await runAgentHarnessStep({
      runDir,
...
void test("needs_review wins over failed_retryable when any attempt was a judge rejection", async () => {
  await withRunDir(async (runDir) => {
    const result = await runAgentHarnessStep({
      runDir,
      jobId: "job-1780",
...
void test("acceptance after a retry uses the late accept", async () => {
  await withRunDir(async (runDir) => {
    const result = await runAgentHarnessStep({
      runDir,
      jobId: "job-1780",
...
void test("standard testDepth caps attempts at 1 + DEFAULT_REPAIR_ITERATIONS, role profile cap dominates when smaller", async () => {
  await withRunDir(async (runDir) => {
    let calls = 0;
...
void test("exhaustive depth uses the +1 attempt when the profile permits it", async () => {
  await withRunDir(async (runDir) => {
...
void test("per-step artifact is written under <runDir>/agent-role-runs/<roleStepId>.json with no secrets", async () => {
  await withRunDir(async (runDir) => {
    const result = await runAgentHarnessStep({
      runDir,
      jobId: "job-1780",
...
void test("rejects malformed hashes from the attempt callback", async () => {
  await withRunDir(async (runDir) => {
...
void test("integration: 5-outcome matrix produces distinct status mappings", async () => {
  await withRunDir(async (runDir) => {
...
void test("rejects unknown role and empty arguments", async () => {
  await withRunDir(async (runDir) => {
    await assert.rejects(
      runAgentHarnessStep({
        runDir,
