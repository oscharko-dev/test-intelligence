...
const withRunDir = async (
  ...
  const runDir = await mkdtemp(join(tmpdir(), "wd-lesson-memdir-test-1789-"));
  try {
    await fn(runDir);
  } finally {
...
void test("lessonsStorage memdir is the default; flat_json is deprecated", () => {
...
void test("validateLessonWritePath refuses parent-directory traversal", async () => {
...
void test("validateLessonWritePath refuses null bytes", async () => {
  await withRunDir(async (runDir) => {
    const dir = await ensureLessonsDir(runDir);
    const result = await validateLessonWritePath({
      lessonsDir: dir,
...
void test("validateLessonWritePath refuses %2e (percent-encoded dot)", async () => {
  await withRunDir(async (runDir) => {
    const dir = await ensureLessonsDir(runDir);
    const result = await validateLessonWritePath({
      lessonsDir: dir,
...
void test("validateLessonWritePath refuses full-width Unicode dot/slash", async () => {
  await withRunDir(async (runDir) => {
    const dir = await ensureLessonsDir(runDir);
    const result = await validateLessonWritePath({
      lessonsDir: dir,
...
void test("validateLessonWritePath refuses backslashes", async () => {
  await withRunDir(async (runDir) => {
    const dir = await ensureLessonsDir(runDir);
    const result = await validateLessonWritePath({
      lessonsDir: dir,
...
void test("validateLessonWritePath refuses absolute paths", async () => {
  await withRunDir(async (runDir) => {
    const dir = await ensureLessonsDir(runDir);
    const result = await validateLessonWritePath({
      lessonsDir: dir,
...
void test("validateLessonWritePath refuses windows-style drive prefix", async () => {
  await withRunDir(async (runDir) => {
    const dir = await ensureLessonsDir(runDir);
    const result = await validateLessonWritePath({
      lessonsDir: dir,
...
void test("validateLessonWritePath refuses empty names", async () => {
  await withRunDir(async (runDir) => {
    const dir = await ensureLessonsDir(runDir);
...
void test("validateLessonWritePath catches symlink escape via realpath walk", async () => {
  await withRunDir(async (runDir) => {
    const dir = await ensureLessonsDir(runDir);
...
void test("validateLessonWritePath accepts well-formed names", async () => {
  await withRunDir(async (runDir) => {
    const dir = await ensureLessonsDir(runDir);
    const result = await validateLessonWritePath({
      lessonsDir: dir,
...
void test("frontmatter round-trip is byte-stable", () => {
  ...
  assert.equal(reserialised, serializeAgentLessonFrontmatter(frontmatter));
});

void test("frontmatter parser rejects missing closing fence", () => {
  assert.throws(() => {
    parseAgentLessonFrontmatter(`---\nid: foo\n`);
  }, /missing closing/);
});

void test("frontmatter parser rejects unknown schemaVersion", () => {
  const text = [
    "---",
    'schemaVersion: "9.0.0"',
    "id: x",
...
void test("frontmatter parser rejects CR characters", () => {
  assert.throws(() => {
    parseAgentLessonFrontmatter(`---\r\nid: x\r\n---\r\n`);
  }, /CR characters/);
});
...
void test("writeAgentLesson persists a lesson and scanLessons reads it back", async () => {
  await withRunDir(async (runDir) => {
    ...
    assert.ok(
      result.filePath.endsWith(lexical.replace(/^\/private/, "")) ||
...
void test("scanLessons reads at most 30 lines per file", async () => {
  await withRunDir(async (runDir) => {
...
void test("writing the same name twice without supersedes is refused", async () => {
  await withRunDir(async (runDir) => {
...
void test("writing the same name with the wrong supersedes is refused", async () => {
  await withRunDir(async (runDir) => {
    const first = await writeAgentLesson({
      runDir,
      ...baseInput("lesson-001", "auth-rule", "first body\n"),
...
void test("writing the same name with the correct supersedes succeeds", async () => {
  await withRunDir(async (runDir) => {
    const first = await writeAgentLesson({
      runDir,
      ...baseInput("lesson-001", "auth-rule", "first body\n"),
...
void test("freshnessNote is undefined for fresh lessons (<24h)", () => {
  const now = NOW;
  assert.equal(freshnessNote({ mtimeMs: now - 1000, nowMs: now }), undefined);
  assert.equal(
    freshnessNote({
...
void test("freshnessNote wraps lessons older than 24h", () => {
  const now = NOW;
...
void test("scanLessons attaches freshnessNote when stale", async () => {
  await withRunDir(async (runDir) => {
...
void test("selectRelevantLessons is deterministic and respects max", async () => {
  await withRunDir(async (runDir) => {
...
void test("selectRelevantLessons filters by policyProfileId when provided", async () => {
  await withRunDir(async (runDir) => {
...
void test("selectRelevantLessons excludes policy-matching lessons when query overlap is zero", async () => {
  await withRunDir(async (runDir) => {
...
void test("AT-033: writeAgentLesson refuses traversal-named id", async () => {
  await withRunDir(async (runDir) => {
    // Force an unsafe id to confirm the lessonFilename guard fires.
    await assert.rejects(async () => {
      await writeAgentLesson({
        runDir,
        ...baseInput("../../../etc/passwd", "name-x", "body\n"),
      });
    }, /unsafe characters/);
  });
});
