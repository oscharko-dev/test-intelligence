...
void test("first acquire creates the lock and emits a chain root event", async () => {
...
void test("release advances lastConsolidatedAt mtime", async () => {
  await withLessonsDir(async (dir) => {
    ...
    assert.ok(last !== null);
    assert.ok(Math.abs(last - (NOW + 60_000)) < 1000);
    const inspected = await inspectConsolidationLock(dir);
    assert.equal(inspected?.state, "released");
  });
});

void test("release refuses when the holderId does not match", async () => {
  await withLessonsDir(async (dir) => {
    const acquire = await tryAcquireConsolidationLock({
      lessonsDir: dir,
      holderId: "h-correct",
...
void test("second acquire while held by live holder is refused", async () => {
  await withLessonsDir(async (dir) => {
...
void test("concurrent first acquires are serialized by exclusive lock creation", async () => {
  await withLessonsDir(async (dir) => {
    ...
    assert.equal(inspected.state, "held");
    assert.ok(
      inspected.holderId === "first" || inspected.holderId === "second",
    );
  });
});
...
void test("AT-034: stale-by-age takeover emits a Merkle-chained lock_takeover event", async () => {
  await withLessonsDir(async (dir) => {
    const first = await tryAcquireConsolidationLock({
      lessonsDir: dir,
...
void test("dead-PID takeover succeeds even within the holder timeout", async () => {
  await withLessonsDir(async (dir) => {
    const first = await tryAcquireConsolidationLock({
      lessonsDir: dir,
...
void test("takeover preserves prior lastConsolidatedAt mtime", async () => {
  await withLessonsDir(async (dir) => {
    const first = await tryAcquireConsolidationLock({
      lessonsDir: dir,
      holderId: "h1",
    ...
    assert.ok(Math.abs(lastAfterReacquire - lastAfterFirst) < 1000);
  });
});
...
void test("rollback restores the prior mtime and does not advance lastConsolidatedAt", async () => {
  await withLessonsDir(async (dir) => {
    const first = await tryAcquireConsolidationLock({
      lessonsDir: dir,
      holderId: "h1",
...
void test("crash-replay: a held lock persists, and a fresh acquirer waits for staleness", async () => {
  await withLessonsDir(async (dir) => {
...
void test("mutating a middle lock event surfaces chain_break at the affected index", async () => {
  await withLessonsDir(async (dir) => {
    await tryAcquireConsolidationLock({
      lessonsDir: dir,
      holderId: "h1",
...
void test("verifier accepts an empty chain", () => {
...
void test("lock file lives at <lessonsDir>/.consolidate-lock", async () => {
  await withLessonsDir(async (dir) => {
    await tryAcquireConsolidationLock({
      lessonsDir: dir,
      holderId: "h",
