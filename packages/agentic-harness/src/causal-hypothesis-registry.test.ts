...
import type { TestDesignModel } from "@oscharko-dev/ti-contracts";
import {
...
void test("semanticFieldId: rejects components containing '#'", () => {
  assert.throws(
    () => semanticFieldId("s#bad", "e1"),
    CausalValidationFrameworkError,
  );
  assert.throws(
    () => semanticFieldId("s1", "e#bad"),
    CausalValidationFrameworkError,
  );
});

void test("semanticFieldId: rejects empty components", () => {
  assert.throws(
    () => semanticFieldId("", "e1"),
    CausalValidationFrameworkError,
  );
...
void test("semanticFieldId + parseSemanticFieldId: round-trip stable", () => {
...
void test("buildCausalHypothesisRegistry: derives hypotheses from registered invariants", () => {
  ...
  assert.ok(
    hypotheses.length >= 2,
    `expected >= 2 hypotheses, got ${hypotheses.length}`,
  );
...
void test("buildCausalHypothesisRegistry: deterministic ordering by hypothesisId", () => {
  const model = buildModelWithVatAndFinancingNeed();
  const invariants = buildActiveDatasetInvariantRegistry().list();
  ...
  assert.deepEqual(
    [...ids].sort((l, r) => l.localeCompare(r)),
    ids,
  );
});

void test("buildCausalHypothesisRegistry: skips catalog rows whose fields are absent in the model", () => {
...
void test("buildCausalHypothesisRegistry: merges operator-declared hypotheses with invariant-derived rows", () => {
  const model = buildModelWithVatAndFinancingNeed();
...
void test("buildCausalHypothesisRegistry: rejects operator hypotheses referencing unknown screens", () => {
  const model = buildModelWithVatAndFinancingNeed();
  const operatorHypotheses: readonly CausalHypothesis[] = [
    {
      hypothesisId: "OP-002",
...
void test("buildCausalHypothesisRegistry: rejects self-referential hypotheses", () => {
  const model = buildModelWithVatAndFinancingNeed();
  const operatorHypotheses: readonly CausalHypothesis[] = [
    {
      hypothesisId: "OP-003",
...
void test("buildCausalHypothesisRegistryFromRegistry: forwards registry list()", () => {
  const model = buildModelWithVatAndFinancingNeed();
  ...
  const wrapped = buildCausalHypothesisRegistryFromRegistry({
    registry,
    model,
  });
...
void test("loadOperatorHypotheses: accepts a top-level array", () => {
  const payload = [
    {
      hypothesisId: "OP-100",
      cause: "s-loan#e-vat",
...
void test("loadOperatorHypotheses: accepts the { hypotheses: [...] } envelope", () => {
  const payload = {
    hypotheses: [
      {
        hypothesisId: "OP-101",
  ...
  assert.equal(
    ...
    "VAT rate is excluded from the financing-need calculation.",
  );
});

void test("loadOperatorHypotheses: rejects malformed payload kinds", () => {
  assert.throws(
...
void test("loadOperatorHypotheses: rejects unknown relationship", () => {
  const payload = [
    {
      hypothesisId: "OP-200",
      cause: "s-loan#e-vat",
...
void test("loadOperatorHypotheses: rejects non-ISO declaredAt timestamps", () => {
  const payload = [
    {
      hypothesisId: "OP-201",
      cause: "s-loan#e-vat",
...
void test("loadOperatorHypotheses: rejects duplicate hypothesis ids", () => {
  const payload = [
    {
      hypothesisId: "OP-300",
      cause: "s-loan#e-vat",
...
void test("loadOperatorHypotheses: rejects malformed cause / effect that are not in <screen>#<element> form", () => {
  const payload = [
    {
      hypothesisId: "OP-400",
