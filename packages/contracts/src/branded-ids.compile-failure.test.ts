...
void test("branded ids compile-fail when RoleStepId is passed where JobId is expected", async () => {
  const tempRoot = await mkdtemp(
    join(tmpdir(), "test-intelligence-branded-ids-"),
  );
  try {
    ...
    await writeFile(
      ...
      [
        ...
        'const roleStepId = toRoleStepId("ti-test-generation-0123456789abcdef");',
        "if (roleStepId !== null) {",
