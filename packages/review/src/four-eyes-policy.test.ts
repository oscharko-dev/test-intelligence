...
import {
  ...
  type ALLOWED_VISUAL_SIDECAR_VALIDATION_OUTCOMES,
...
void test("four-eyes-policy: EU banking default lists payment/regulated/high (sorted)", () => {
...
void test("four-eyes-policy: cloneFourEyesPolicy dedupes and sorts", () => {
...
void test("four-eyes-policy: resolveFourEyesPolicy applies defaults when input is undefined", () => {
...
void test("four-eyes-policy: resolveFourEyesPolicy honours empty arrays as DISABLED", () => {
...
void test("four-eyes-policy: resolveFourEyesPolicy drops unknown values silently", () => {
  const policy = resolveFourEyesPolicy({
    fourEyesRequiredRiskCategories: [
      "regulated_data",
...
void test("four-eyes-policy: validateFourEyesPolicy reports unknown values", () => {
...
void test("four-eyes-policy: evaluate enforces by risk_category", () => {
...
void test("four-eyes-policy: evaluate does not enforce for non-listed risk", () => {
  const result = evaluateFourEyesEnforcement({
...
void test("four-eyes-policy: evaluate enforces by visual sidecar low_confidence", () => {
...
void test("four-eyes-policy: evaluate maps each visual outcome to its reason", () => {
  const outcomes = [
    "fallback_used",
    "possible_pii",
    "prompt_injection_like_text",
...
void test("four-eyes-policy: evaluate combines risk + visual reasons (sorted)", () => {
  const visual: VisualSidecarValidationReport = {
    schemaVersion: VISUAL_SIDECAR_VALIDATION_REPORT_SCHEMA_VERSION,
    contractVersion: TEST_INTELLIGENCE_CONTRACT_VERSION,
    visualSidecarSchemaVersion: VISUAL_SIDECAR_SCHEMA_VERSION,
...
void test("four-eyes-policy: evaluate ignores visual records for unrelated screens", () => {
  const visual: VisualSidecarValidationReport = {
    schemaVersion: VISUAL_SIDECAR_VALIDATION_REPORT_SCHEMA_VERSION,
    contractVersion: TEST_INTELLIGENCE_CONTRACT_VERSION,
    visualSidecarSchemaVersion: VISUAL_SIDECAR_SCHEMA_VERSION,
...
void test("four-eyes-policy: isFourEyesEnforcementReason narrows correctly", () => {
