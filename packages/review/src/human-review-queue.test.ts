...
void test("computeHumanReviewItemId is deterministic and stable", () => {
...
void test("hashReviewerPrincipalId is sha256 of the input string", () => {
...
void test("queue item invariants reject malformed segments and SLA inversion", () => {
  assert.throws(
    () =>
      assertHumanReviewQueueItemInvariants(
        buildItem({
          tenantId: "../etc/passwd",
        } as Partial<HumanReviewQueueItem>),
      ),
...
void test("verdict invariants reject bad fields and signature shape", () => {
...
void test("enqueue → fetch → record verdict round-trips on disk and verifies signature", async () => {
  await withTempRoot(async (root) => {
    ...
    await assert.rejects(
      ...
      (err: unknown) =>
        ...
        err.code === "E_QUEUE_ITEM_ALREADY_EXISTS",
    );
    ...
    const verdictsForRun = await loadHumanReviewVerdictsForRun(
      root,
      TENANT,
      RUN,
    );
...
void test("recordHumanReviewVerdict refuses tampered signatures", async () => {
  await withTempRoot(async (root) => {
    const km = generateEd25519();
    const item = buildItem();
    await enqueueHumanReview(root, item);
...
void test("recordHumanReviewVerdict refuses fingerprint mismatch", async () => {
  await withTempRoot(async (root) => {
    const km = generateEd25519();
...
void test("recordHumanReviewVerdict requires the caller to target the owning tenant partition", async () => {
  await withTempRoot(async (root) => {
    const km = generateEd25519();
    const item = buildItem();
    await enqueueHumanReview(root, item);
    ...
    await assert.rejects(
      () =>
        recordHumanReviewVerdict(
          root,
          buildVerdict(item.itemId, km),
          "other-tenant",
        ),
      (err: unknown) =>
        err instanceof HumanReviewQueueError &&
        err.code === "E_QUEUE_ITEM_NOT_FOUND",
    );
  });
});

void test("findHumanReviewSlaBreaches surfaces only past-due items without verdicts", async () => {
  await withTempRoot(async (root) => {
    const km = generateEd25519();
    ...
    await recordHumanReviewVerdict(
      root,
      buildVerdict(decidedItem.itemId, km),
      TENANT,
    );
...
void test("buildHumanReviewLog assembles a canonical, byte-stable per-run log", async () => {
  await withTempRoot(async (root) => {
    const km = generateEd25519();
    const item = buildItem();
    await enqueueHumanReview(root, item);
...
void test("revised verdicts must include a revisedTestCase JSON body", async () => {
  await withTempRoot(async (root) => {
    const km = generateEd25519();
    const item = buildItem();
    await enqueueHumanReview(root, item);
...
void test("CLI key files written under temp dirs are accepted by the runner", async () => {
