...
import { createFileSystemReviewStore } from "@oscharko-dev/ti-production-runner";
import {
...
void test("detector: empty string returns null", () => {
  assert.equal(detectSuspiciousContent(""), null);
});

void test("detector: whitespace-only string returns null", () => {
  assert.equal(detectSuspiciousContent("   \t \n  "), null);
});

void test("detector: benign step text returns null", () => {
  for (const benign of [
    "Submit",
...
void test("detector: shell metacharacter shapes flagged as shell_metacharacters", () => {
  for (const value of [
    "rm -rf /",
    "rm -rf /var/lib/app",
    "echo bad | sh",
...
void test("detector: command substitution shapes flagged as command_substitution", () => {
  for (const value of [
    "$(whoami)",
...
void test("detector: JNDI/log4shell payloads flagged as jndi_log4shell", () => {
  for (const value of [
...
void test("detector: long base64 run flagged as encoded_payload_base64", () => {
  const longBase64 =
...
void test("detector: short base64-shape passes (<64 chars)", () => {
  assert.equal(detectSuspiciousContent("dGVzdA=="), null);
  assert.equal(detectSuspiciousContent("ABC123"), null);
});

void test("detector: long hex run flagged as encoded_payload_hex", () => {
...
void test("detector: short hex passes (<40 chars)", () => {
  assert.equal(detectSuspiciousContent("abcdef0123"), null);
});

void test("detector: <script> tag flagged as script_tag", () => {
  for (const value of [
...
void test("detector: inline event handler flagged as html_event_handler", () => {
  for (const value of [
    "<div onclick=alert(1)>",
...
void test("detector: dangerous URL schemes flagged as dangerous_url_scheme", () => {
  for (const value of [
...
void test("detector: detection order — JNDI/script_tag take precedence over URL scheme", () => {
...
void test("detector: matchedSnippet is bounded to 64 chars", () => {
...
void test("detector: input longer than 16k is clipped without crash", () => {
...
void test("detector: SEMANTIC_SUSPICION_CATEGORIES is the closed list", () => {
...
const buildEvent = (
  overrides: Partial<ReviewEvent> & {
    testCaseId: string;
    metadata: NonNullable<ReviewEvent["metadata"]>;
    actor: string;
    id: string;
    sequence: number;
    at: string;
...
void test("recordSemanticContentOverride: refuses empty actor / justification / path / testCaseId", async () => {
...
void test("extractSemanticContentOverrides: ignores non-override notes; later events supersede earlier ones", () => {
...
void test("extractSemanticContentOverrides: keeps replayable signed events and drops malformed path/category metadata", () => {
...
void test("effectiveSemanticContentBlock: covered semantic finding stops blocking", () => {
...
void test("effectiveSemanticContentBlock: uncovered path keeps blocking", () => {
  const report = buildValidationReport([
    semanticIssue("$.testCases[0].steps[0].action"),
...
void test("effectiveSemanticContentBlock: non-semantic error always keeps blocking", () => {
  const report = buildValidationReport([
    {
      testCaseId: "tc-1",
...
void test("partitionSemanticContentOverridesForValidation: keeps only verified semantic matches", () => {
  const report = buildValidationReport([
    semanticIssue("$.testCases[0].steps[0].action"),
    {
      testCaseId: "tc-1",
  ...
  const stale = new Map([
    [
      "tc-1",
      new Map([
        ["$.testCases[0].steps[0].action", buildSignedOverrideEntry()],
        [
          "$.testCases[0].title",
...
void test("effectiveSemanticContentBlock: category-mismatched replay override stays blocking", () => {
  const report = buildValidationReport([
    semanticIssue("$.testCases[0].steps[0].action"),
  ]);
...
void test("policy-gate: respects semanticContentOverrides — overridden case downgraded to needs_review with annotated rule", () => {
  ...
  const overrides = new Map<
    string,
    Map<string, ReturnType<typeof createSignedSemanticContentOverrideEntry>>
...
void test("policy-gate: override map without matching path leaves the case blocked", () => {
  const list = buildListForOverride();
  const intent: BusinessTestIntentIr = {
    version: "1.0.0",
    source: { kind: "figma_local_json", contentHash: "0".repeat(64) },
...
void test("policy-gate: missing authority provider rejects otherwise signed overrides fail-closed", () => {
  const validation = buildValidationReport([
    semanticIssue("$.testCases[0].steps[0].action"),
  ]);
  const result = evaluatePolicyGate({
...
void test("policy-gate: expired override entries are rejected fail-closed", () => {
  const validation = buildValidationReport([
    semanticIssue("$.testCases[0].steps[0].action"),
  ]);
  const result = evaluatePolicyGate({
...
void test("policy-gate: property-based invalid signatures are all rejected", () => {
  const validation = buildValidationReport([
    semanticIssue("$.testCases[0].steps[0].action"),
  ]);
...
void test("end-to-end: reviewer override note recorded via store flips effective block", async () => {
