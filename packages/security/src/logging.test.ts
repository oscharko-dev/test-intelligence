...
void test("redactLogMessage redacts shared high-risk secret shapes", async (t) => {
  const cases = [
    {
...
void test("redactLogMessage preserves benign prose around secret-like words", async (t) => {
  const cases = [
...
void test("createWorkspaceLogger emits newline-delimited JSON records with correlation fields", () => {
...
void test("createWorkspaceLogger keeps text mode human-readable with correlation prefixes", () => {
  const stdoutLines: string[] = [];
  const logger = createWorkspaceLogger({
    format: "text",
    stdoutWriter: (line) => {
  ...
  assert.equal(
    stdoutLines[0],
    "[test-intelligence][job=job-abc][stage=figma.source][request=req-abc][event=workspace.sync.applied][method=POST][path=/workspace/jobs/job-abc/sync][status=200] Completed stage 'figma.source'.\n",
  );
});

void test("createWorkspaceLogger routes warnings to stderr in text mode", () => {
  const stderrLines: string[] = [];
  const logger = createWorkspaceLogger({
    format: "text",
    stderrWriter: (line) => {
  ...
  assert.equal(stderrLines[0], "[test-intelligence][warn] token=[REDACTED]\n");
});

void test("resolveWorkspaceLogLevel falls back to info for unknown values", () => {
...
void test("createWorkspaceLogger suppresses entries below the configured minimum level", () => {
  const stdoutLines: string[] = [];
  const stderrLines: string[] = [];
  const logger = createWorkspaceLogger({
    format: "text",
  ...
  assert.deepEqual(stderrLines, ["[test-intelligence][warn] Shown warning\n"]);
});
