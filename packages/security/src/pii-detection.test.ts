...
void test("detectPii flags a valid IBAN (DE89 3704 0044 0532 0130 00)", () => {
...
void test("detectPii rejects an IBAN with a bad checksum", () => {
  const result = detectPii("DE89 3704 0044 0532 0130 01");
  assert.equal(result, null);
});

void test("detectPii flags a BIC/SWIFT-style bank identifier", () => {
...
void test("detectPii flags a lowercase BIC/SWIFT identifier", () => {
  const result = detectPii("ingddeffxxx");
  assert.notEqual(result, null);
  assert.equal(result?.kind, "bic");
  assert.equal(result.redacted, "[REDACTED:BIC]");
...
void test("detectPii flags a mixed-case BIC/SWIFT identifier", () => {
  const result = detectPii("IngdDeffXXX");
  assert.notEqual(result, null);
  assert.equal(result?.kind, "bic");
  assert.equal(result.redacted, "[REDACTED:BIC]");
...
void test("detectPii rejects clean action labels", () => {
  assert.equal(detectPii("Submit"), null);
  assert.equal(detectPii("Cancel"), null);
  assert.equal(detectPii(""), null);
});

void test("detectPii redaction tokens never contain the original value", () => {
  const cases = [
...
void test("detectPii is idempotent across repeated calls (no stateful regex lastIndex)", () => {
...
void test("redactPii returns the stable token per kind", () => {
...
void test("special_category: 'cancer' must NOT trigger inside 'cancellation'", () => {
  for (const benign of [
...
void test("special_category: 'krebs' must NOT trigger inside 'krebsforschung'", () => {
  for (const benign of [
...
void test("special_category: extra word-boundary cases for compound and adjacent forms", () => {
...
void test("detectPii: refuses input > 16 KiB and returns null in well under 50ms (ReDoS guard)", () => {
...
void test("detectPii: 32 KiB adversarial postal-address-shaped input completes in <50ms (ReDoS guard)", () => {
