import type { PiiKind } from "@oscharko-dev/ti-contracts";
...
const detectIban = (input: string): PiiMatch | null => {
  let offset = 0;
  for (;;) {
    const span = findIbanCandidateSpan(input, offset);
    if (span === undefined) break;
    offset = span.end;
    const normalized = stripIbanSeparators(input.slice(span.start, span.end));
...
const stripIbanSeparators = (value: string): string => {
  let out = "";
  for (const ch of value) {
    if (ch !== " " && ch !== "-") out += ch.toUpperCase();
  }
  return out;
};

const containsEmailLike = (value: string): boolean => {
  let cursor = 0;
  while (cursor < value.length) {
    ...
    if (isEmailLike(value.slice(start, end))) return true;
    cursor = at + 1;
  }
  return false;
};
