...
void test("error sanitization redacts PAN-like values that pass the Luhn checksum", async (t) => {
...
void test("error sanitization preserves long numeric values that fail the Luhn checksum", async (t) => {
...
void test("error sanitization preserves Luhn-valid non-PAN identifiers", async (t) => {
...
void test("error sanitization redacts multiple PANs in a single message", () => {
  const message = sanitizeErrorMessage({
...
void test("error sanitization redacts PAN at message boundaries", async (t) => {
...
void test("error sanitization redacts mixed sensitive content without over-redacting non-pan long numbers", () => {
  const message = sanitizeErrorMessage({
    error: new Error(
...
void test("error sanitization redacts shared high-risk secret shapes", async (t) => {
  const cases = [
    {
...
void test("error sanitization redacts JSON-serialized secrets", async (t) => {
  const cases = [
    {
...
void test("error sanitization preserves benign prose around secret-like words", async (t) => {
  const cases = [
...
void test("error sanitization returns fallback for non-error input", () => {
  const message = sanitizeErrorMessage({
...
void test("error sanitization truncates long error messages", () => {
...
void test("error sanitization falls back when sanitized message is empty", () => {
  const message = sanitizeErrorMessage({
    error: new Error("   "),
    fallback: "fallback",
  });
...
void test("redactErrorChain returns empty string for null or undefined input", () => {
  assert.equal(redactErrorChain(null), "");
  assert.equal(redactErrorChain(undefined), "");
});

void test("redactErrorChain renders a single error with no cause", () => {
...
void test("redactErrorChain redacts bearer tokens in the root message", () => {
...
void test("redactErrorChain walks a 3-level cause chain and redacts each level", () => {
...
void test("redactErrorChain detects direct self-referential circular causes", () => {
...
void test("redactErrorChain detects two-node circular causes", () => {
...
void test("redactErrorChain enforces the default depth limit of 8", () => {
...
void test("redactErrorChain respects a custom depthMax", () => {
...
void test("redactErrorChain renders a string cause", () => {
...
void test("redactErrorChain renders a plain object cause", () => {
...
void test("redactErrorChain handles an error with missing message", () => {
  const err = new Error("");
  const out = redactErrorChain(err);
...
void test("redactErrorChain includes and redacts the stack when present", () => {
...
void test("redactErrorChain tolerates an error with no stack property", () => {
...
void test("redactErrorChain redacts JSON-serialized secrets in nested causes", () => {
...
void test("redactErrorChain redacts PAN-like values in nested causes", () => {
...
void test("redactErrorChain redacts secrets inside AggregateError.errors[]", () => {
...
void test("redactErrorChain handles AggregateError nested in cause chain", () => {
...
// Issue #40: a distinct-object AggregateError tree deeper than the depth cap
// must be truncated, not fully walked — the WeakSet guards cycles but not trees.
void test("redactErrorChain truncates nested AggregateError trees beyond the depth cap", () => {
  // Build a chain of N=DEFAULT_CAUSE_DEPTH_MAX+2 nested AggregateErrors so
  // the depth budget is exhausted before the tree ends.
  const depth = 12; // > DEFAULT_CAUSE_DEPTH_MAX (8)
  let inner: AggregateError = new AggregateError([], `level-${depth}`);
  for (let index = depth - 1; index >= 0; index -= 1) {
    inner = new AggregateError([inner], `level-${index}`);
  }

  const out = redactErrorChain(inner);

  // The output must mention the depth-cap truncation marker or simply not
  // contain the deepest level — either proves the walk was bounded.
  const containsDeepest = out.includes(`level-${depth}`);
  assert.equal(
    containsDeepest,
    false,
    "output must not contain level beyond the depth cap",
  );
});

void test("sanitizeErrorMessage uses the cause chain when cause is present", () => {
...
void test("redactErrorChain scrubs POSIX home directory prefixes from stack frames", () => {
  const err = new Error("boom");
  err.stack = [
    "Error: boom",
    "    at fetchPng (/Users/alice/Projects/test-intelligence/src/job-engine/figma.ts:262:34)",
...
void test("redactErrorChain scrubs Windows home directory prefixes from stack frames", () => {
  const err = new Error("boom");
  err.stack = [
    "Error: boom",
...
void test("redactErrorChain scrubs absolute node_modules paths", () => {
  const err = new Error("boom");
  err.stack = [
    "Error: boom",
...
void test("redactErrorChain preserves the readable portion of the stack frame", () => {
  const err = new Error("boom");
  err.stack = [
    "Error: boom",
