#!/usr/bin/env node
/**
 * Air-gap smoke runner (Issue #30).
 *
 * Verifies the sovereign-cloud / air-gap deployment profile fails closed
 * under the strict env flag:
 *
 *  1. `air-gap-guard.fetchGuard` refuses every host outside the explicit
 ...
 *  4. The figma-export CLI parser still validates required flags so the
 *     connected-machine workflow is testable in CI even without an
 *     internet egress.
 *
 * The runner sets `TEST_INTELLIGENCE_AIR_GAP_MODE=1` for every
...
const tests = [
  ...
  "packages/cli/src/test-intelligence-figma-export-cli.test.ts",
];

const env = {
  ...process.env,
  TEST_INTELLIGENCE_AIR_GAP_MODE: "1",
  ...
  TEST_INTELLIGENCE_AIR_GAP_ALLOWED_HOSTS: "",
};

const child = spawn("pnpm", ["exec", "tsx", "--test", ...tests], {
  cwd: repoRoot,
  env,
  stdio: "inherit",
});
