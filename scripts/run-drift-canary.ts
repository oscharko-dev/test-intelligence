...
import { buildDriftCanaryFixtureSnapshot } from "@oscharko-dev/ti-core-engine";
import {
...
import { runFaithfulnessJudge } from "@oscharko-dev/ti-quality";
import { runLogicJudge } from "@oscharko-dev/ti-quality";
import type { LlmGatewayRuntime } from "@oscharko-dev/ti-model-gateway";

const DEFAULT_OUTPUT_ROOT = "artifacts/testing/drift-canary";
const DEFAULT_RUNTIME_ROOT = ".test-intelligence";
...
const main = async (): Promise<void> => {
  ...
  const apiKey = readRequiredEnv("TEST_INTELLIGENCE_LLM_API_KEY");
  const endpoint = readRequiredEnv("TEST_INTELLIGENCE_MODEL_ENDPOINT");
  ...
  const crossFamilyDeployment = readOptionalEnv(
    "TEST_INTELLIGENCE_DRIFT_CANARY_CROSS_FAMILY_DEPLOYMENT",
  ...
  for (const fixture of calibrationFixtures) {
    if (fixture.judge === "logic") {
      const input = fixture.input.input as {
        ...
        generatedTestCases: Parameters<
          typeof runLogicJudge
        >[0]["generatedTestCases"];
      };
    ...
    faithfulnessSamples.push({
      ...
      humanVerdict: fixture.gold.humanVerdict,
      humanFindingCodes: fixture.gold.humanFindingCodes,
      predictedVerdict: result.verdict.refusal
        ? "reject"
