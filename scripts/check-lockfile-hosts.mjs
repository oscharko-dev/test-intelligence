...
/**
 * Lockfile host allowlist gate.
 *
 * Scans tracked `pnpm-lock.yaml` files for resolver URLs and fails closed if
 * any host outside the allowlist is observed. Catches accidental introduction
 * of `localhost`/`127.0.0.1`/private-registry dependencies.
 *
 * Scoped to a single tracked lockfile; the standalone package has no template
 * sub-packages.
 */
...
export const parseArgs = (args) => {
  ...
  const overrideHosts =
    allowHostValues.length > 0
      ? normalizeHosts(allowHostValues, "--allow-hosts")
      : null;
  return {
...
const parseHostnameFromUrlCandidate = (value, context) => {
  ...
  const normalizedUrl = normalizedValue.startsWith("git+https://")
  ...
  try {
    ...
    throw new Error(
      `Malformed URL-like resolver content in ${context}: ${normalizedValue}`,
    );
  }
};
...
const collectInlineResolverHosts = (value, context, hosts) => {
  ...
  for (const match of value.matchAll(URL_LIKE_START_PATTERN)) {
    ...
    const isCoveredByParsedUrl = matchedUrlRanges.some(
      (range) => start >= range.start && start < range.end,
    );
    if (!isCoveredByParsedUrl && URL_LIKE_PATTERN.test(match[0])) {
      throw new Error(
        `Malformed URL-like resolver content in ${context}: ${value.trim()}`,
      );
    }
  }
};
...
export const extractHosts = (content) => {
  ...
  const resolverFieldPattern =
    /^[ \t]*(resolution|resolved|repository):\s*(.+)$/gm;
...
export const scanLockfileHosts = async ({
...
export const runLockfileHostAllowlist = async ({
  ...
  try {
    ...
    stdout(
      `[lockfile-host-allowlist] Effective allowlist: ${formatHosts(allowedHosts)}`,
    );
    ...
    const observedHosts = await scanLockfileHosts({
      lockfilePaths,
      readTextFile,
    });

    const unexpectedHosts = [...observedHosts]
      .filter((host) => !allowedHosts.has(host))
      .sort();
    if (unexpectedHosts.length > 0) {
      stderr(
        "[lockfile-host-allowlist] Unexpected hosts found in tracked lockfiles:",
      );
      for (const host of unexpectedHosts) {
        stderr(` - ${host}`);
      }
      stderr(
        `[lockfile-host-allowlist] Allowed hosts: ${formatHosts(allowedHosts)}`,
      );
      return 1;
    }
    ...
    stderr(
      `[lockfile-host-allowlist] Failed: ${error instanceof Error ? error.message : error}`,
    );
    return 1;
  }
};
