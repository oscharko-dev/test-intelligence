...
test("buildApiUrl derives the project endpoint from GITHUB_REPOSITORY", () => {
  assert.equal(
    buildApiUrl({
      env: {
        GITHUB_REPOSITORY: "oscharko-dev/test-intelligence",
      },
    }),
    "https://api.securityscorecards.dev/projects/github.com/oscharko-dev/test-intelligence",
  );
});
...
test("evaluateScorecardResult requests a retry when the published commit lags behind", () => {
  const result = evaluateScorecardResult({
    payload: {
      score: 6.2,
      repo: {
        name: "github.com/oscharko-dev/test-intelligence",
        commit: "older-commit",
      },
    ...
    expectedRepositoryUri: "github.com/oscharko-dev/test-intelligence",
  });
