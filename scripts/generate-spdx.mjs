...
/**
 * SPDX 2.3 SBOM generator for `@oscharko-dev/test-intelligence`.
 *
 * Scoped to a single npm artifact per ADR-0006. Uses
 * `@cyclonedx/cyclonedx-npm` to walk the runtime dependency tree (which
 * correctly handles pnpm-resolved layouts, peer/optional deps, link:
 * specifiers) and then transforms the components into SPDX `packages[]`
 * with DEPENDS_ON relationships to the root package.
 *
 * Output: `artifacts/sbom/test-intelligence.spdx.json` (SPDX 2.3, JSON).
 *
 * Reproducible: when `SOURCE_DATE_EPOCH` is set the `creationInfo.created`
 * timestamp is derived from it instead of `Date.now()`.
 *
 * Usage:
 *   node scripts/generate-spdx.mjs [--package-root <dir>] [<outputPath>]
 *
 * Defaults:
 *   <outputPath>            artifacts/sbom/test-intelligence.spdx.json
...
const DEFAULT_OUTPUT_PATH = "artifacts/sbom/test-intelligence.spdx.json";
const SPDX_GENERATOR_TOOL_NAME = "test-intelligence-spdx-generator@1.0.0";
...
const collectCycloneDxPackages = async (packageRoot, packageJson) => {
  ...
  const tempDirectoryPath = await mkdtemp(
    path.join(os.tmpdir(), "test-intelligence-spdx-"),
  );
  ...
  try {
    ...
    const cycloneDxDocument = JSON.parse(
      await readFile(tempCycloneDxPath, "utf8"),
    );
    const packageByKey = new Map();
    const components = Array.isArray(cycloneDxDocument.components)
      ...
      packageByKey.set(packageKeyFromCycloneDxComponent(component), {
        ...
        packagePurl:
          typeof component.purl === "string" && component.purl.length > 0
            ...
            : normalizePackageKey(
                `pkg:npm/${packageName}@${component.version}`,
              ),
    ...
    return [...packageByKey.values()].sort((first, second) => {
      const nameComparison = first.packageName.localeCompare(
        second.packageName,
      );
  ...
  return {
    ...
    relationships: packages.map((dependencyPackage) => ({
      ...
      relatedSpdxElement: toSpdxId(
        ...
        dependencyPackage.packageVersion,
      ),
    })),
  };
};
...
const main = async () => {
  ...
  const dependencyGraph = await collectRuntimeDependencyGraph(
    packageRoot,
    packageJson,
  );
  const rootPackageId = toSpdxId(
    String(packageJson.name),
    String(packageJson.version),
  );
  const packages = [
    {
      ...
      primaryPackagePurpose:
        packageJson.private === true ? "APPLICATION" : "LIBRARY",
      externalRefs: [
        {
  ...
  const document = {
    ...
    creationInfo: {
      created: timestamp,
      creators: [`Tool: ${SPDX_GENERATOR_TOOL_NAME}`],
    },
  ...
  console.log(
    `[sbom] SPDX written to ${absoluteOutputPath} (packageRoot=${packageRoot})`,
  );
};
