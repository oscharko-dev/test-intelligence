#!/usr/bin/env node
/**
 * FIPS smoke check (Issue #30).
 *
 * Probes `node --enable-fips` on the host. When FIPS is available it
 * imports `dist/index.js` under FIPS to confirm the standalone runtime
 * loads cleanly. When the host OpenSSL FIPS module is unavailable, the
 * check exits 0 with a `[fips] Skipping...` log line so the smoke can
 * run on stock `ubuntu-latest` runners without flaking.
 */
...
const main = () => {
  const probe = spawnSync(
    ...
    [
      ...
      "--input-type=module",
      "-e",
      "import { getFips } from 'node:crypto'; console.log(getFips());",
    ],
    {
      cwd: packageRoot,
      encoding: "utf8",
  ...
  if (isUnsupported) {
    console.log(
      "[fips] Skipping smoke check: host OpenSSL FIPS module is not available.",
    );
    return;
  }
