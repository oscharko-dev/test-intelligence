...
const runGenerator = async ({
  ...
  const tempRoot = await mkdtemp(
    path.join(os.tmpdir(), "test-intelligence-spdx-generation-"),
  );
  ...
  try {
    ...
    const fileEntries = Object.entries(installTree ?? {}).sort(
    ...
    const result = await new Promise<{
      ...
      const child = spawn(
        process.execPath,
        [scriptPath, outputPath, "--package-root", projectRoot],
        {
          cwd: packageRoot,
    ...
    return {
      ...
      document:
        result.code === 0
          ? (JSON.parse(await readFile(outputPath, "utf8")) as Record<
              string,
              unknown
            >)
          : null,
...
const packageKeysFromDocument = (
  document: Record<string, unknown>,
): string[] => {
  const packages: unknown[] = Array.isArray(document.packages)
    ? document.packages
    : [];
  return packages
    .filter(
      (packageEntry): packageEntry is { name: string; versionInfo: string } =>
        Boolean(
          ...
          typeof (packageEntry as { name: unknown }).name === "string" &&
          typeof (packageEntry as { versionInfo: unknown }).versionInfo ===
            "string",
        ),
    )
...
void test("SPDX generator resolves transitive runtime dependencies from a pnpm-style symlink layout", async () => {
  const result = await runGenerator({
    ...
    installTree: {
      "node_modules/.pnpm/allowed-parent@1.2.0/node_modules/allowed-parent/package.json":
        createPackageJson({
          name: "allowed-parent",
          version: "1.2.0",
          dependencies: {
      ...
      "node_modules/.pnpm/allowed-child@2.0.0/node_modules/allowed-child/package.json":
        createPackageJson({
          name: "allowed-child",
          version: "2.0.0",
        }),
...
void test("SPDX generator ignores an inherited npm_execpath that points at pnpm", async () => {
  const result = await runGenerator({
    manifest: createPackageJson({
      name: "figma-generated-app",
      dependencies: {
...
void test("SPDX generator fails when runtime dependencies are declared but node_modules is missing", async () => {
  const result = await runGenerator({
    manifest: createPackageJson({
      name: "figma-generated-app",
      dependencies: {
  ...
  assert.equal(
    result.code,
    1,
    `Expected install-tree failure, got stdout:\n${result.stdout}`,
  );
  assert.equal(result.document, null);
  assert.match(result.stderr, /node_modules is missing/);
  assert.match(
    result.stderr,
    /Install runtime dependencies before generating SPDX/,
  );
});
