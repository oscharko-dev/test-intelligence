 ...
 *
 * Branding note (issue #87): every `workspace-dev:*` assertion target in
 * this test file mirrors a CycloneDX property-namespace WIRE IDENTIFIER
 * emitted by `packages/evidence/src/lbom-emitter.ts` and preserved
 * byte-identical against the Workspace Dev reference at pinned SHA
 * `006dabdf0abe30b9cac2b742a7238c6625d8e8c1`. Per intentional delta D-01 in
 * the parity fixture policy, wire identifiers are preserved
 * verbatim and MUST NOT be rebranded.
 *
 * Covers:
...
void test("buildLbomDocument: emits a CycloneDX 1.6 ML-BOM with all required roots", () => {
...
void test("buildLbomDocument: emits exactly the three required model roles + two data components", () => {
  const doc = buildLbomDocument(baseInput());
...
void test("buildLbomDocument: model components carry deployment + role + image-input + fallback properties", () => {
  const doc = buildLbomDocument(baseInput());
...
void test("buildLbomDocument: optional visual model identity and weights flow into components", () => {
...
void test("buildLbomDocument: few-shot bundle component carries promptHash + schemaHash", () => {
  const doc = buildLbomDocument(baseInput());
  ...
  if (bundle.type === "data") {
    ...
    for (const h of bundle.hashes) assert.equal(h.alg, "SHA-256");
  } else {
    assert.fail("expected few-shot-bundle data component");
  }
});

void test("buildLbomDocument: policy-profile component hashes the canonical profile object", () => {
...
void test("buildLbomDocument: dependencies form a closed graph rooted at the job subject", () => {
  const doc = buildLbomDocument(baseInput());
...
void test("buildLbomDocument: serialNumber is byte-stable across rebuilds with identical inputs", () => {
...
void test("buildLbomDocument: serialNumber differs when cacheKeyDigest changes", () => {
  const docA = buildLbomDocument(baseInput());
...
void test("buildLbomDocument: visual sidecar success with fallback flips metadata + component flag", () => {
...
void test("buildLbomDocument: rejects non-hex hash inputs", () => {
  assert.throws(
    () =>
      buildLbomDocument(
...
void test("validateLbomDocument: passes on a freshly built document", () => {
  const doc = buildLbomDocument(baseInput());
...
void test("validateLbomDocument: catches invalid bomFormat", () => {
...
void test("validateLbomDocument: catches invalid specVersion", () => {
  const doc = buildLbomDocument(baseInput());
...
void test("validateLbomDocument: catches invalid serialNumber", () => {
  const doc = buildLbomDocument(baseInput());
...
void test("validateLbomDocument: catches non-hex hash content", () => {
  const doc = buildLbomDocument(baseInput()) as Wave1ValidationLbomDocument;
...
void test("validateLbomDocument: catches duplicate bom-ref entries", () => {
  const doc = buildLbomDocument(baseInput());
...
void test("validateLbomDocument: catches dangling dependency refs", () => {
  const doc = buildLbomDocument(baseInput());
  const dependencies = [
    {
...
void test("validateLbomDocument: catches a flipped invariant", () => {
  const doc = buildLbomDocument(baseInput());
  const tampered = {
    ...doc,
    metadata: {
...
void test("validateLbomDocument: catches raw `contents` payload on a data component", () => {
  const doc = buildLbomDocument(baseInput());
...
void test("validateLbomDocument: catches a high-risk secret pattern in a property value", () => {
  const doc = buildLbomDocument(baseInput()) as Wave1ValidationLbomDocument;
  const components = doc.components.map((c) => ({
    ...c,
...
void test("buildLbomDocument: redacts a deployment label that smells like a secret", () => {
  const doc = buildLbomDocument(
    baseInput({
      modelDeployments: {
        testGeneration: "gpt-oss-120b-mock",
...
void test("writeLbomArtifact: persists canonical JSON under runDir/lbom and validates", async () => {
...
void test("writeLbomArtifact: refuses to persist an invalid document", async () => {
...
void test("summarizeLbomArtifact: counts components and reports the visual fallback flag", () => {
  const doc = buildLbomDocument(baseInput());
...
void test("lbomDataKindFromBomRef: maps known data-component bom-refs", () => {
  assert.equal(
...
void test("isAllowedVisualFallbackReason: matches the contract enum", () => {
...
void test("buildLbomDocument: properties are sorted by name (deterministic)", () => {
  const doc = buildLbomDocument(baseInput());
...
void test("buildLbomDocument: matches a fresh policy profile clone", () => {
