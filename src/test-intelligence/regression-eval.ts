 ...
 *
 * Approve workflow mirrors the golden approve workflow:
 * setting `TEST_INTELLIGENCE_REGRESSION_APPROVE=true` rewrites every
...
export const buildRegressionSnapshot = async (
  ...
  const generatedAt = input.generatedAt ?? REGRESSION_EVAL_FIXTURE_GENERATED_AT;
    ...
    evalOutcomes: {
      ...
      a11yCoverage: toEvalOutcome(
        ...
        a11y.verdict.failures
          ...
          .filter(
            (f: { severity: "error" | "warning" }) => f.severity === "error",
          )
...
export const diffRegressionSnapshot = (
  ...
  for (const evalKey of [
    ...
    if (
      ...
      baselineReasons.some(
        (reason, index) => reason !== candidateReasons[index],
      )
    ) {
      findings.push({
        archetypeId,
...
export const renderDriftReport = (input: RenderDriftReportInput): string => {
  ...
  lines.push(
    `- Eval outcomes: identical \`passed\` flag and failure-reason set`,
  );
  lines.push("");
  ...
  for (const diff of driftedArchetypes) {
    ...
    lines.push("");
    lines.push("| Path | Before | After | Δ (abs) | Δ (%) | Tolerance |");
    lines.push("| --- | --- | --- | --- | --- | --- |");
  ...
  lines.push("```sh");
  lines.push(
    "TEST_INTELLIGENCE_REGRESSION_APPROVE=true pnpm run test:ti-regression",
  );
  lines.push("```");
  lines.push("");
  lines.push(
    "CI rejects `TEST_INTELLIGENCE_REGRESSION_APPROVE` so approved snapshots only enter the repo via PR review.",
  );
  return `${lines.join("\n")}\n`;
};
...
export const isRegressionApproveModeEnabled = (
  ...
  const raw = env.TEST_INTELLIGENCE_REGRESSION_APPROVE?.trim().toLowerCase();
