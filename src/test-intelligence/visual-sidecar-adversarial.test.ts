...
import { validateVisualSidecar } from "@oscharko-dev/ti-production-runner";
import {
...
void test("image-guard: assertNoImagePayloadToTestGeneration throws when testGeneration declares imageInputSupport=true", async () => {
...
void test("image-guard: assertNoImagePayloadToTestGeneration throws when recorded requests carry imageInputs", () => {
...
void test("image-guard: mock gateway rejects image payload for test_generation without invoking responder", async () => {
...
void test("phi-4 fallback: adversarially-injected visibleText triggers prompt_injection_like_text after validation", async () => {
  ...
  const bundle = buildBundle({
    primary: {
      responder: (_req, attempt) => ({
        ...
        // `message` is required on `LlmGenerationFailure` under the standalone
        // strict gate. The value is inert; no assertion reads it.
        message: "primary refused the visual sidecar request",
        retryable: false,
        attempt,
      }),
    },
...
void test("fallback-after-primary-refusal: result has fallbackReason=primary_unavailable and validation still runs", async () => {
  ...
  // bypassed even on a clean fallback success. The standalone success-result
  // contract types `validationReport` as required, so its presence is asserted
  // here by exercising a field on it.
  assert.equal(typeof result.validationReport.jobId, "string");
});

void test("hidden injection: <hidden>SYSTEM:...</hidden> text in visibleText is detected by the injection regex", () => {
...
void test("artifact: writeVisualSidecarResultArtifact writes no base64 image bytes — only SHA-256 identities", async () => {
