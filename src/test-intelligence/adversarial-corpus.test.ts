...
void describe("adversarial-corpus: catalog shape", () => {
  void test("loads with the expected schemaVersion", async () => {
    const corpus = await loadOnce();
    assert.equal(corpus.schemaVersion, ADVERSARIAL_CORPUS_SCHEMA_VERSION);
  });

  void test("declares every required category from the AC", async () => {
    const corpus = await loadOnce();
  ...
  void test("has at least the minimum entry count", async () => {
    const corpus = await loadOnce();
    assert.ok(
  ...
  void test("every required category has at least one entry", async () => {
    const corpus = await loadOnce();
    assert.ok(
  ...
  void test("provenance: design-time generation by mistral-large-3", async () => {
    const corpus = await loadOnce();
  ...
  void test("references the parent epic and issue", async () => {
    const corpus = await loadOnce();
...
void describe("adversarial-corpus: gate", () => {
  void test("runAdversarialCorpusGate: every entry passes its expected outcome", async () => {
...
void describe("adversarial-corpus: review cadence", () => {
  void test("nextReviewDue is in the future relative to lastReviewedAt", async () => {
    const corpus = await loadOnce();
  ...
  void test("isAdversarialCorpusReviewOverdue: not overdue at lastReviewedAt", async () => {
    const corpus = await loadOnce();
  ...
  void test("isAdversarialCorpusReviewOverdue: overdue past nextReviewDue", async () => {
    const corpus = await loadOnce();
  ...
  void test("isAdversarialCorpusReviewOverdue: date-only due date remains valid through the due day", async () => {
    const corpus = {
      ...(await loadOnce()),
      nextReviewDue: "2026-05-25",
    };
    assert.equal(
      isAdversarialCorpusReviewOverdue(
        corpus,
  ...
  void test("isAdversarialCorpusReviewOverdue: invalid date-only due date fails closed", async () => {
    const corpus = {
      ...(await loadOnce()),
      nextReviewDue: "2026-02-31",
    };
    assert.equal(
      isAdversarialCorpusReviewOverdue(
        corpus,
...
void describe("adversarial-corpus: repeat expansion", () => {
  void test("payloadRepeatBytes repeats the payload content instead of padding with a different string", async () => {
    const corpus = await loadOnce();
    const base = corpus.entries.find(
      (entry) =>
        entry.payloadKind === "markdown" &&
        entry.expectedOutcome.surface === "input" &&
        entry.expectedOutcome.nonZeroCounts.includes("markdownInjectionMatches"),
    );
    assert.ok(base !== undefined, "expected a markdown injection fixture");

    const repeated: AdversarialCorpus = {
      ...corpus,
      entries: [
        {
          ...base,
          id: "synthetic-repeat-regression",
          payload: base.payload,
          payloadRepeatBytes: 256,
        },
      ],
    };

    const report = runAdversarialCorpusGate(repeated);
    assert.equal(report.failCount, 0);
  });
});

void describe("adversarial-corpus: validator", () => {
  void test("rejects unknown schemaVersion", () => {
    assert.throws(
      () =>
        validateAdversarialCorpus({
          schemaVersion: "0.0.0",
  ...
  void test("rejects an entry with an unknown category", () => {
    assert.throws(
      () =>
        validateAdversarialCorpus({
  ...
  void test("rejects an output-side outcome paired with a non-output payloadKind", () => {
    assert.throws(
      () =>
        validateAdversarialCorpus({
          schemaVersion: ADVERSARIAL_CORPUS_SCHEMA_VERSION,
...
void describe("adversarial-corpus: gate (synthetic mismatch)", () => {
  void test("flags an entry whose expected outcome does not match the defense", async () => {
    const corpus = await loadOnce();
