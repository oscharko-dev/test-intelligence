...
void test("end-to-end repair: empty coveredFieldIds triggers and resolves a repair iteration", async () => {
  ...
  const repairedList = buildList([
    ...
    buildTestCase("tc-a11y", { coveredFieldIds: ["fld-a"] }, "accessibility"),
  ]);
...
void test("end-to-end repair: hallucinated id is replaced with a real IR id across one iteration", async () => {
  const testDesignModel = buildTestDesignModel();
  const initialList = buildList([
...
void test("end-to-end repair: unmet technique quota triggers repair before acceptance", async () => {
  const testDesignModel = buildTestDesignModel();
