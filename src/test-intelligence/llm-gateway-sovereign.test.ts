...
import type { LlmGatewayClientConfig } from "@oscharko-dev/ti-contracts";
import { AIR_GAP_MODE_ENV } from "@oscharko-dev/ti-production-runner";
import {
...
void test("createSovereignLlmGatewayClient pins the baseUrl host into the allow-list", async () => {
  const calls: string[] = [];
...
void test("createSovereignLlmGatewayClient rejects a baseUrl that is not a valid URL", () => {
  assert.throws(
    () =>
      createSovereignLlmGatewayClient(
...
void test("sovereign gateway refuses public-cloud hosts under air-gap mode via guarded fetch", async () => {
...
void test("additionalAllowedHosts extend the seeded allow-list with extra entries", async () => {
  const innerFetch: typeof fetch = async () =>
    new Response("{}", {
      status: 200,
      headers: { "content-type": "application/json" },
...
void test("generate() under strict air-gap mode reaches the baseUrl host via the seeded allow-list", async () => {
...
void test("sovereign client is a transparent wrapper when air-gap mode is off", async () => {
  const innerFetch: typeof fetch = async () =>
    new Response(
      JSON.stringify({
        id: "x",
