...
const SYNTHETIC_BASIC_AUTH_HEADER = `Authorization: Basic ${Buffer.from(
  "synthetic-user:synthetic-secret",
  "utf8",
).toString("base64")}`;

void test("jira-token-leakage: Jira response tokens are redacted from every persisted Jira artifact", async () => {
  ...
  try {
    const fixture = JSON.parse(
      await readFile(
        new URL(
          "../../packages/core-engine/src/fixtures/adversarial-jira-token-leak.response.json",
          import.meta.url,
        ),
        "utf8",
      ),
    ...
    for (const raw of persisted) {
      ...
      assert.equal(raw.includes(SYNTHETIC_BASIC_AUTH_HEADER), false);
...
void test("jira-token-leakage: property redacts supported secret shapes deterministically", () => {
  fc.assert(
    fc.property(
      fc.constantFrom(
        "Bearer jira-token-value-123",
        SYNTHETIC_BASIC_AUTH_HEADER,
...
void test("jira-token-leakage: property redacts bare token shapes (no label) — #1667", () => {
