...
import { computeIntentDelta } from "@oscharko-dev/ti-core-engine";
...
void test("property: computeIntentDelta is deterministic (byte-identical for same inputs)", () => {
  fc.assert(
...
void test("property: identical inputs always yield zero entries", () => {
  fc.assert(
...
void test("property: removing a field always surfaces a `removed` entry for that id", () => {
  fc.assert(
    fc.property(
