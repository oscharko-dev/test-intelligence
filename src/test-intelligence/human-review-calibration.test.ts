...
void test("human-review-calibration: multi-agent harness is better calibrated against human truth", () => {
  ...
  assert.equal(
    singlePass.schemaVersion,
    HUMAN_REVIEW_CALIBRATION_SCHEMA_VERSION,
  );
  assert.equal(
    multiAgent.schemaVersion,
    HUMAN_REVIEW_CALIBRATION_SCHEMA_VERSION,
  );
...
void test("human-review-calibration: bias controls are explicit and stable", () => {
...
void test("human-review-calibration: repeated reviewed examples per criterion aggregate without duplicate-key failures", () => {
  const report = buildHumanReviewCalibrationReport({
    archetypeId: SAMPLE_ARCTYPE,
    pipelineId: "multi_agent_harness",
    samples: [
