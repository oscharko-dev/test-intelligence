...
import { WAVE1_VALIDATION_FIXTURE_IDS } from "@oscharko-dev/ti-contracts";
import {
  runWave1Validation,
...
void test("validation-eval: default thresholds pass for both shipped fixtures", async () => {
...
void test("validation-eval: report is sorted by fixtureId for byte-stability", async () => {
  const runs = await Promise.all(
    WAVE1_VALIDATION_FIXTURE_IDS.map((id) => runFixture(id)),
  );
  const report = evaluateWave1Validation({
...
void test("validation-eval: tightening minTraceCoverageFields above measured value fails", async () => {
...
void test("validation-eval: lowering maxDuplicateSimilarity below observed maximum fails", async () => {
...
void test("validation-eval: report.pass is the AND of every fixture.pass", async () => {
  ...
  assert.ok(failing.fixtures.every((f) => !f.pass));
});

void test("validation-eval: each threshold rule fails when its metric is degraded", async () => {
  const run = await runFixture("validation-payment-auth");
  ...
  for (const testCase of cases) {
    ...
    assert.equal(fixture.pass, false, `expected ${testCase.rule} to fail`);
    assert.ok(
