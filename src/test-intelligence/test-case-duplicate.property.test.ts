...
void test("jaccard similarity is symmetric", () => {
  fc.assert(
    fc.property(
...
void test("jaccard similarity is bounded in [0, 1]", () => {
  fc.assert(
    fc.property(
...
void test("a case is always identical to itself", () => {
  fc.assert(
...
void test("duplicate detector emits pairs in sorted (left, right) lex order", () => {
  fc.assert(
    fc.property(
...
void test("threshold 0 produces all C(n,2) pairs", () => {
  const cases = [
...
void test("threshold > 1 raises RangeError", () => {
  assert.throws(() =>
