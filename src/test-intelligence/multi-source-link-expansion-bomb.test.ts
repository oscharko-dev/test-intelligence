...
void test("multi-source-link-expansion-bomb: Jira links are not recursively expanded or persisted by default", async () => {
  ...
  try {
    const client = createJiraGatewayClient(CONFIG, {
      fetchImpl: (async (url: string, init?: RequestInit) => {
        ...
        searchBody = JSON.parse(init?.body as string) as Record<
          string,
          unknown
        >;
        return new Response(
    ...
    assert.equal(
      (searchBody?.["fields"] as string[]).includes("issuelinks"),
      false,
    );
