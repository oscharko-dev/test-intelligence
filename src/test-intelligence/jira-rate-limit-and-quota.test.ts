...
void test("jira-rate-limit-and-quota: numeric Retry-After retries within budget", async () => {
  let searchCalls = 0;
  let slept = 0;
...
void test("jira-rate-limit-and-quota: malformed or over-budget Retry-After fails closed", async () => {
...
void test("jira-rate-limit-and-quota: oversized responses persist no partial Jira IR", async () => {
