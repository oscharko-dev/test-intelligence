...
import { buildMultiSourceTestIntentEnvelope } from "@oscharko-dev/ti-multi-source";
import {
  reconcileMultiSourceIntent,
  writeMultiSourceReconciliationReport,
} from "@oscharko-dev/ti-multi-source";
import { evaluatePolicyGate } from "@oscharko-dev/ti-production-runner";
...
import { validateGeneratedTestCases } from "@oscharko-dev/ti-production-runner";
...
const buildCase = (): GeneratedTestCase => ({
  ...
  testData: [],
  steps: [
    { index: 1, action: "Submit invalid IBAN", expected: "Validation shown" },
  ],
...
void test("priority policy prefers the higher-priority Jira label and records the loser as a conflict", () => {
...
void test("reviewer_decides conflicts escalate the downstream policy gate to needs_review", () => {
  const issue = jiraIssue(
    "jira-review",
    'The field "IBAN field" must be required.',
  );
...
void test("jira statements about unspecified validation rules propagate into open questions", () => {
  const issue = jiraIssue(
...
void test("jira financing-rule statements propagate VAT-exclusion assumptions", () => {
  const issue = jiraIssue(
    "jira-financing-vat",
...
void test("non-priority reconciliation is stable under source reordering", () => {
  const issue = jiraIssue(
    "jira-stable",
    'The field "IBAN field" must be required.',
  );
  ...
  assert.equal(JSON.stringify(left.report), JSON.stringify(right.report));
});

void test("jira-only reconciliation emits a merged IR without synthetic figma provenance", () => {
  const issue = jiraIssue("jira-only", 'The field "Email" must be required.');
  const envelope = buildMultiSourceTestIntentEnvelope({
    sources: [
      {
        sourceId: "jira.0",
...
void test("detects validation, risk, test-data, duplicate-criterion, and paste-collision conflicts", () => {
  ...
  const kinds = new Set(
    result.report.conflicts.map((conflict) => conflict.kind),
  );
...
void test("keep_both emits deterministic sibling IR elements for conflicting Jira alternatives", () => {
  const issue = jiraIssue(
    "jira-keep-both",
    [
...
void test("figma-only reconciliation remains conflict-free and uses only Figma provenance", () => {
  const envelope = buildMultiSourceTestIntentEnvelope({
    sources: [
      {
        sourceId: "figma.0",
...
void test("figma-only reconciliation deep-clones rich optional provenance", () => {
  const figmaRef = {
    sourceId: "figma.rich",
    ...
    detectedActions: [
      {
        ...
        kind: "primary_button",
        ambiguity: {
          reason: "primary submit button inferred from visual emphasis",
        },
        sourceRefs: [figmaRef],
      },
    ],
    ...
    detectedNavigation: [
      {
        ...
        ambiguity: {
          reason: "prototype link inferred from component metadata",
        },
        sourceRefs: [figmaRef],
      },
    ],
    inferredBusinessObjects: [
      {
        ...
        ambiguity: {
          reason: "card object inferred from masked PAN-like content",
        },
        sourceRefs: [figmaRef],
      },
    ],
...
void test("figma reconciliation handles sparse optional provenance and unmatched custom sources", () => {
  const figmaRef = {
  ...
  assert.deepEqual(
    result.mergedIntent.detectedFields[0]?.sourceRefs?.map(
      (ref) => ref.sourceId,
    ),
    ["figma.sparse"],
  );
  assert.equal(
    result.mergedIntent.risks.includes("financial_transaction"),
    true,
  );
...
void test("jira sources with unmatched hashes stay unmatched without synthetic fields", () => {
  const issue = jiraIssue(
    "jira-hash-mismatch",
...
void test("matching Jira field, validation, and example merge into sparse Figma fields", () => {
  const intent = figmaIntent();
  intent.detectedFields[0] = {
    ...intent.detectedFields[0]!,
    label: "Email",
  ...
  delete intent.detectedFields[0].sourceRefs;
  intent.detectedValidations[0] = {
    ...intent.detectedValidations[0]!,
    rule: "must be required",
  };
  delete intent.detectedValidations[0].targetFieldId;
  delete intent.detectedValidations[0].sourceRefs;
  const issue = jiraIssue(
    "jira-merge",
    'The field "Email" must be required. The field "Email" example is "ada@example.test".',
  );
...
void test("keep_both retains alternative Jira examples for matching fields", () => {
  const intent = figmaIntent();
  intent.detectedFields[0] = {
    ...intent.detectedFields[0]!,
    label: "IBAN",
...
void test("adversarial edge reconciliation covers sparse priority and source metadata branches", () => {
  ...
  assert.equal(
    priority.mergedIntent.detectedFields[0]?.defaultValue,
    "DE00 FIGMA VALUE",
  );
  assert.equal(
...
void test("jira plus custom reconciliation records custom risk without Figma placeholders", () => {
...
void test("jira-only reconciliation extracts adversarial semantic labels and custom risk signals", () => {
  const issue = jiraIssue(
    "jira-semantic-sweep",
    [
  ...
  const labels = new Set(
    result.mergedIntent.detectedFields.map((field) => field.label),
  );
  for (const label of [
    "Email field",
    "PAN field",
  ...
  assert.equal(
    result.mergedIntent.detectedFields.some(
      (field) =>
        field.label === "Email field" && field.defaultValue === "ada@example",
    ),
    true,
  );
  assert.equal(result.mergedIntent.risks.includes("regulated_data"), true);
...
void test("present custom text with no deterministic contribution is unmatched but absent families are silent", () => {
  const issue = jiraIssue(
    "jira-unmatched",
...
void test("writeMultiSourceReconciliationReport writes canonical byte-stable JSON", async () => {
  const issue = jiraIssue(
    "jira-write",
    'The field "IBAN field" must be required.',
  );
