...
import { compilePrompt } from "@oscharko-dev/ti-production-runner";
import {
  planSourceMix,
  writeSourceMixPlan,
...
void test("AC1: jira_rest_only plan has not_applicable visual sidecar requirement", () => {
...
void test("AC2: jira_paste_only plan has equivalent shape to jira_rest_only", () => {
...
void test("AC3: figma-only plan is backward-compatible with single-source envelope", () => {
...
void test("AC3: prompt-compiler without sourceMixPlan is unchanged (backward compat)", () => {
...
void test("AC4: figma + jira plan includes merged source IDs and reconciliation_report", () => {
...
void test("AC5: custom_markdown-only job fails closed with primary_source_required", () => {
...
void test("AC5: empty sources envelope returns primary_source_required", () => {
...
void test("AC6: duplicate Jira REST+paste for same key raises duplicate_jira_issue_key", () => {
  const envelope = buildEnvelope([
...
void test("AC7: prompt-compiler cache key includes sourceMixPlanHash when plan is present", () => {
...
void test("AC7: prompt-compiler auto-plans when intent carries a source envelope", () => {
...
void test("AC7: prompt-compiler auto-planning allows Jira duplicate only with paste-collision evidence", () => {
  const envelope = buildEnvelope([
    figmaRef("fig.0"),
  ...
  assert.equal(
    compiled.artifacts.payload.sourceMixPlan?.kind,
    "figma_jira_mixed",
  );
});

void test("AC7: identical source mix + content produces identical cache keys", () => {
...
void test("AC7: different source mix produces different cache key", () => {
...
void test("AC8: rawJiraResponsePersisted is false on every plan type", () => {
  const envelopes = [
...
void test("AC9: Jira-only compiled prompt does not contain FIGMA_INTENT section", () => {
...
void test("AC9: Figma-only compiled prompt contains FIGMA_INTENT section", () => {
  const envelope = buildEnvelope([figmaRef("fig.0")]);
  const r = planSourceMix(envelope);
  assert.equal(r.ok, true);
...
void test("AC10: prompt compiled without sourceMixPlan still includes Business Test Intent IR", () => {
  const compiled = compilePrompt({
    jobId: "job-bc-002",
    intent: stubIntent(),
    modelBinding: stubModelBinding(),
...
void test("AC11: jira_rest + custom_markdown enriches plan with custom_context_markdown section", () => {
  const envelope = buildEnvelope([
...
void test("AC11: Jira REST + custom markdown compiled prompt keeps Jira and custom sections while omitting FIGMA_INTENT", () => {
  const envelope = buildEnvelope([
...
void test("AC11: Markdown-only still fails with primary_source_required", () => {
  const envelope = buildEnvelope([customMarkdownRef("md.0")]);
  const result = planSourceMix(envelope);
  assert.equal(result.ok, false);
  assert.ok(result.issues.some((i) => i.code === "primary_source_required"));
...
void test("writeSourceMixPlan: Jira-only plan persists and re-reads correctly", async () => {
