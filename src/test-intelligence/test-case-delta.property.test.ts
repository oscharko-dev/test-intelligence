...
import { classifyTestCaseDelta } from "@oscharko-dev/ti-core-engine";
...
void test("property: stable fingerprint AND no IR delta touch yields `unchanged`", () => {
  fc.assert(
...
void test("property: case absent from current with all trace screens absent from IR is `obsolete`", () => {
  fc.assert(
    fc.property(caseArb, (tc) => {
      const out = classifyTestCaseDelta({
        jobId: "job-1",
...
void test("property: classifier output is deterministic", () => {
  fc.assert(
    fc.property(
...
void test("property: every row has an allowed verdict + each verdict's reasons all in the allowlist", () => {
  fc.assert(
    fc.property(
