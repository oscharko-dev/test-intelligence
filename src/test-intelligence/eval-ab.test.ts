...
void test("eval-ab: registers a canonical eval-ab file for each archetype", () => {
...
void test("eval-ab: checked-in artifacts match the deterministic builder", async () => {
...
void test("eval-ab: every checked-in artifact is canonical JSON", async () => {
  for (const archetypeId of BASELINE_ARCHETYPE_FIXTURE_IDS) {
...
void test("eval-ab: harness wins on required metrics for every archetype", async () => {
  ...
  for (const artifact of artifacts) {
    ...
    assert.ok(harness.metrics.genericExpectedResultDelta.deltaVsSinglePass < 0);
...
void test("eval-ab: write helper is byte-stable on the second write", async () => {
...
void test("eval-ab: winner resolution can report a single-pass win", async () => {
  const artifact = await buildEvalAbArtifact({
    archetypeId: "baseline-simple-form",
    generatedAt: EVAL_AB_FIXTURE_GENERATED_AT,
  });
