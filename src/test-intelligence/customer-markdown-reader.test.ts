...
void test("readCustomerMarkdownArtifact returns the combined Markdown when present", async () => {
...
void test("readCustomerMarkdownArtifact returns not_found when the file does not exist", async () => {
  const { root, cleanup } = await setup();
  try {
    const result = await readCustomerMarkdownArtifact({
      artifactRoot: root,
...
void test("readCustomerMarkdownArtifact returns not_found when the path is a directory", async () => {
  const { root, cleanup } = await setup();
  try {
...
void test("readCustomerMarkdownArtifact refuses paths outside the artifact root", async () => {
  const { root, cleanup } = await setup();
  try {
    const result = await readCustomerMarkdownArtifact({
      artifactRoot: root,
...
void test("buildCustomerMarkdownAttachmentName sanitises the job id", () => {
  assert.equal(
