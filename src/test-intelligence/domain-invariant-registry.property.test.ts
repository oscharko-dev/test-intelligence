...
void test("property: well-formed Netto-only expectation never violates INV-NETTO-BRUTTO-01", () => {
...
void test("property: optional-cost expectation without selection always violates INV-OPTIONAL-COST-01", () => {
  const registry = buildActiveDatasetInvariantRegistry();
  const context = buildContext();
  fc.assert(
...
void test("property: optional-cost expectation with declared selection never violates INV-OPTIONAL-COST-01", () => {
  const registry = buildActiveDatasetInvariantRegistry();
  const context = buildContext();
  fc.assert(
    fc.property(NETTO_AMOUNT, OPTIONAL_FEE, (base, fee) => {
...
void test("property: evaluator is deterministic — same input yields byte-identical output", () => {
  const registry = buildActiveDatasetInvariantRegistry();
  const context = buildContext();
  fc.assert(
    fc.property(NETTO_AMOUNT, (amount) => {
