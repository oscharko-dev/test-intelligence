...
import { parseJiraAdfDocument } from "@oscharko-dev/ti-multi-source";
import { createJiraGatewayClient } from "@oscharko-dev/ti-multi-source";
import {
  buildJiraIssueIr,
...
import { reconcileMultiSourceIntent } from "@oscharko-dev/ti-multi-source";
...
void test("wave4 edge coverage: ADF parser handles safe rich nodes and fail-closed shape variants", () => {
  const rich = parseJiraAdfDocument(
    adf([
      ...
      {
        type: "blockquote",
        content: [{ type: "paragraph", content: [textNode("quote")] }],
      },
        ...
        content: [
          {
            ...
            content: [
              {
                type: "tableHeader",
                content: [{ type: "paragraph", content: [textNode("A")] }],
              },
              {
                type: "tableCell",
                content: [{ type: "paragraph", content: [textNode("B")] }],
              },
            ],
          },
        ],
      ...
      {
        type: "mediaSingle",
        content: [{ type: "media", attrs: { alt: "safe.png" } }],
      },
      {
        type: "mediaGroup",
        content: [{ type: "media", attrs: { alt: "../bad.png" } }],
      },
      { type: "rule" },
  ...
  const invalidDocs = [
    [
      { type: "doc", version: 1, content: "nope" },
      "jira_adf_node_shape_invalid",
    ],
    [
      { type: "doc", version: 1, content: [null] },
      "jira_adf_node_shape_invalid",
    ],
    [
      {
    ...
    [
      {
        ...
        version: 1,
        content: [
          { type: "paragraph", content: [textNode("x", { marks: "bad" })] },
        ],
      },
      "jira_adf_node_shape_invalid",
    ],
...
void test("wave4 edge coverage: Jira IR rejects malformed metadata and preserves minimized optional collections", () => {
  const refusals: Array<[Partial<BuildJiraIssueIrInput>, string]> = [
    ...
    [
      { fieldSelection: { customFieldAllowList: ["bad"] } },
    ...
    [
      {
        ...
        comments: [{ createdAt: "bad", body: { kind: "plain", text: "x" } }],
      },
      "jira_comment_invalid",
    ],
    [
      {
        fieldSelection: { includeAttachments: true },
    ...
    [
      {
        fieldSelection: { includeLinks: true },
        links: [{ targetIssueKey: "bad", relationship: "blocks" }],
      },
      "jira_link_invalid",
    ],
    [
      {
        fieldSelection: { includeLinks: true },
        links: [{ targetIssueKey: "PAY-2", relationship: "" }],
      },
      "jira_link_invalid",
    ],
  ];
              ...
              content: [
                {
                  ...
                  content: [
                    {
                      type: "paragraph",
                      content: [textNode("Reject script tags")],
                    },
                  ],
                },
              ],
      ...
      comments: [
        {
          ...
          createdAt: ISO,
          body: { kind: "plain", text: "Looks safe" },
        },
...
void test("wave4 edge coverage: Jira gateway rejects bad config/request shapes and redacts replay failures", async () => {
  const badConfigs: JiraGatewayConfig[] = [
    ...
    {
      ...gatewayConfig,
      auth: { kind: "oauth2_3lo", accessToken: "" },
  ...
  const badRequests = [
    ...
    {
      query: { kind: "jql" as const, jql: "project = PAY", maxResults: 1 },
      fieldSelection: { customFieldAllowList: ["bad"] },
    },
    {
      query: { kind: "jql" as const, jql: "project = PAY", maxResults: 1 },
  ...
  assert.equal(
    replayUnconfigured.diagnostic?.code,
    "jira_replay_cache_unconfigured",
  );
...
void test("wave4 edge coverage: Jira gateway status, retry, response, and usage branches fail closed", async () => {
  const usage: string[] = [];
  ...
  for (const status of statuses) {
    ...
    const client = createJiraGatewayClient(
      ...
      },
      {
        ...
        retryBackoffMs: [100],
        onUsageEvent: (event) =>
          usage.push(`${event.diagnosticCode ?? "ok"}:${event.attempts}`),
        fetchImpl: (async (url: string) => {
          calls += 1;
          ...
          if (status === 429) {
            return new Response("", {
              status,
              headers: {
                "Retry-After": "not-number",
                "RateLimit-Reason": "Bearer secret-token-value",
              },
            });
          }
  ...
  const malformedResponses = [
    ...
    new Response(JSON.stringify({ issues: [{ key: "bad", fields: {} }] }), {
      status: 200,
    }),
  ];
  ...
  assert.ok(
    usage.some((entry) => entry.startsWith("jira_retry_budget_exceeded")),
  );
  assert.ok(usage.some((entry) => entry.startsWith("jira_rate_limited")));
});

void test("wave4 edge coverage: paste ingestion and source envelopes fail closed on malformed source claims", () => {
  ...
  assert.equal(unsupported.code, "paste_format_invalid");

  const missingSummary = ingestJiraPaste({
    request: {
      jobId: "job",
      format: "plain_text",
      body: "Summary: Checkout\nStatus: Open",
    },
    authorHandle: "reviewer",
    capturedAt: ISO,
  });
...
void test("wave4 edge coverage: reconciliation records unmatched and fallback source families", () => {
  const jira = buildJiraIssueIr(
    baseIssue({
      summary: "Transfer amount",
      description: {
        kind: "plain",
        text: "IBAN payment amount must be reviewed.",
      },
  ...
  assert.ok(result.report.unmatchedSources.length >= 0);
});

void test("wave4 edge coverage: multi-source mode gate covers disabled and throw paths", () => {
