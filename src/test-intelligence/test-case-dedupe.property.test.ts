...
void test("property: cosineSimilarity always in [0, 1] for finite vectors", () => {
...
void test("property: identical vector pair → cosineSimilarity = 1 (within fp epsilon) for non-zero inputs", () => {
...
void test("property: detectTestCaseDuplicatesExtended is deterministic across two runs", () => {
  void fc.assert(
    fc.asyncProperty(
      fc.uniqueArray(
        fc
...
void test("property: when no provider/no probe, totals.duplicates equals lexical-flagged perCase entries", () => {
  void fc.assert(
    fc.asyncProperty(
      fc.uniqueArray(
        fc
...
void test("property: with a constant-vector embedding provider, all >=2 cases are flagged as duplicates above threshold 0.5", () => {
