...
void test("issue #2017: protocol failure on primary persists a diagnostic with errorClass and gateway message", async () => {
...
void test("issue #2017: schema-invalid fallback persists a diagnostic with normalized parser error and bounded raw text", async () => {
  const captures = [captureFor("s-1")];
  ...
  // `LlmGatewayErrorClass`; the mock emits it verbatim so the client surfaces
  // it on the persisted attempt. The cast keeps the runtime literal available
  // under the standalone strict gate.
  const primaryResponder: MockResponder = (_request, attempt) => ({
    outcome: "error",
...
void test("issue #2017: pre-flight failure produces no diagnostics (no gateway round-trip occurred)", async () => {
...
void test("issue #2017: PII inside a malformed response is redacted before it lands in the diagnostic", async () => {
  const captures = [captureFor("s-1")];
