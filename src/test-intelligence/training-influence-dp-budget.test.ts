...
void test("constants: filename appears in the harness artifact filename allow-list", () => {
  assert.equal(
...
void test("constants: schema and decision tokens are well-formed", () => {
...
void test("estimateJobDpCharge: linear in input tokens, deterministic", () => {
...
void test("estimateJobDpCharge: respects custom coefficients", () => {
...
void test("estimateJobDpCharge: rejects invalid inputs", () => {
...
void test("estimateJobDpCharge: throws when epsilon overflows to Infinity", () => {
  assert.throws(
    () =>
      estimateJobDpCharge({
...
void test("createTenantDpBudgetState: stamps schema + contract versions, zero consumed", () => {
...
void test("createTenantDpBudgetState: rejects invalid inputs", () => {
  assert.throws(
    () =>
      createTenantDpBudgetState({
        tenantId: "",
...
void test("applyDpCharge (disabled): skipped_disabled, state unchanged", () => {
...
void test("applyDpCharge (enabled): accepted accumulates consumed totals", () => {
...
void test("applyDpCharge: rejects when epsilon would exceed cap", () => {
...
void test("applyDpCharge: rejects when delta would exceed cap", () => {
...
void test("applyDpCharge: throws when config caps drift from persisted state caps", () => {
  const state = freshState(ENABLED_CONFIG);
...
void test("applyDpCharge: charge exactly at cap is accepted, next is rejected", () => {
  const config: TrainingInfluenceDpBudgetConfig = {
    enabled: true,
    perTokenEpsilon: 1,
    deltaPerJob: 0,
...
void test("applyDpCharge: zero-token job is accepted as a no-op for epsilon", () => {
  const state = freshState(ENABLED_CONFIG);
...
void test("resetTenantDpBudgetCycle: zeroes consumed but preserves tenant + budgets", () => {
  let state = freshState(ENABLED_CONFIG);
...
void test("resetTenantDpBudgetCycle: rejects same cycleId, invalid ISO, invalid config", () => {
  const state = freshState(ENABLED_CONFIG);
  assert.throws(
    () =>
...
void test("buildDpBudgetConsumedManifest: validator accepts well-formed manifest", () => {
  const state = freshState(ENABLED_CONFIG);
  const result = applyDpCharge(state, {
    config: ENABLED_CONFIG,
    inputTokens: 1000,
...
void test("buildDpBudgetConsumedManifest: rejected jobs still emit an audit-ready manifest", () => {
  const config: TrainingInfluenceDpBudgetConfig = {
    enabled: true,
    perTokenEpsilon: 1,
    deltaPerJob: 0,
...
void test("isDpBudgetConsumedManifest: rejects malformed payloads", () => {
  const state = freshState(ENABLED_CONFIG);
  const result = applyDpCharge(state, {
    config: ENABLED_CONFIG,
    inputTokens: 1000,
...
void test("serializeDpBudgetConsumedManifest: canonical, trailing newline, byte-stable", () => {
  const state = freshState(ENABLED_CONFIG);
  const result = applyDpCharge(state, {
    config: ENABLED_CONFIG,
    inputTokens: 1000,
