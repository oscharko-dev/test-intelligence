...
const TEST_JIRA_CONFIG = {
  ...
  userAgent: "test-intelligence-test/1.0",
  maxRetries: 0,
} as const;
...
const createMockClient = (
  ...
  const lookups: Map<string, JiraSubTaskLookupResult> =
    options.lookupResults ?? new Map<string, JiraSubTaskLookupResult>();
  const creates: Map<string, JiraSubTaskCreateResult> =
    options.createResults ?? new Map<string, JiraSubTaskCreateResult>();
  const defaultCreate =
    options.defaultCreate ??
    ((_fields: JiraSubTaskFields): JiraSubTaskCreateResult => {
      void _fields;
      return {
        ok: true,
...
void test("feature_gate_disabled refuses without invoking the client", async () => {
...
void test("admin_gate_disabled refuses without invoking the client", async () => {
  await withTempDir(async (runDir) => {
    const client = createMockClient();
...
void test("bearer_token_missing fires when no token configured", async () => {
  await withTempDir(async (runDir) => {
    const client = createMockClient();
...
void test("invalid_parent_issue_key fires for malformed parent", async () => {
  await withTempDir(async (runDir) => {
    const client = createMockClient();
...
void test("no_approved_test_cases fires when no case is approved", async () => {
  await withTempDir(async (runDir) => {
    const client = createMockClient();
...
void test("policy_blocked_cases_present fires when any case is blocked", async () => {
  await withTempDir(async (runDir) => {
    const client = createMockClient();
...
void test("schema_invalid_cases_present fires when validation has errors", async () => {
  await withTempDir(async (runDir) => {
    const client = createMockClient();
    const input = buildInput(runDir, {
...
void test("visual_sidecar_blocked fires when sidecar report is blocked", async () => {
  await withTempDir(async (runDir) => {
    const client = createMockClient();
    const input = buildInput(runDir, {
      visualPresent: true,
...
void test("multiple gate violations are all collected and reported", async () => {
  await withTempDir(async (runDir) => {
    const client = createMockClient();
    const input = buildInput(runDir, {
...
void test("every refusal code is part of the published allow-list", () => {
...
void test("happy path creates one sub-task per approved test case", async () => {
  await withTempDir(async (runDir) => {
    const cases = [
...
void test("dryRun records dry_run outcomes without invoking the client", async () => {
  await withTempDir(async (runDir) => {
    const client = createMockClient();
    const cases = [
...
void test("idempotency: lookup hit short-circuits to skipped_duplicate", async () => {
  await withTempDir(async (runDir) => {
    ...
    assert.equal(client.createCalls.length, 0);
    const created = await readCreated(runDir);
    const subtask0 = created.subtasks[0];
    assert.ok(subtask0);
    assert.equal(subtask0.jiraIssueKey, "PROJ-999");
    assert.equal(subtask0.outcome, "skipped_duplicate");
  });
});
...
void test("per-case failure does not abort subsequent cases", async () => {
  await withTempDir(async (runDir) => {
    const cases = [
      buildTestCase({ id: "tc-a" }),
      buildTestCase({ id: "tc-b" }),
...
void test("thrown exception on createSubTask is caught as transport_error", async () => {
  await withTempDir(async (runDir) => {
    const cases = [
      buildTestCase({ id: "tc-a" }),
      buildTestCase({ id: "tc-b" }),
...
void test("live client retries retryable lookup responses before creating", async () => {
  await withTempDir(async (runDir) => {
...
void test("live client sends Idempotency-Key header on create POST", async () => {
  await withTempDir(async (runDir) => {
...
void test("Idempotency-Key is stable across re-runs for the same job/case/parent", async () => {
  await withTempDir(async (runDir) => {
...
void test("live client does not retry ambiguous create responses directly", async () => {
  await withTempDir(async (runDir) => {
...
void test("live client does not retry non-retryable create responses", async () => {
  await withTempDir(async (runDir) => {
    const calls: Array<{ method: string | undefined }> = [];
    const fetchImpl: typeof fetch = async (_url, init) => {
      calls.push({ method: init?.method });
...
void test("lookup retry exhaustion fails the case without attempting create", async () => {
  await withTempDir(async (runDir) => {
    const calls: Array<{ method: string | undefined }> = [];
    const fetchImpl: typeof fetch = async (_url, init) => {
      calls.push({ method: init?.method });
...
void test("externalId is deterministic across runs", () => {
...
void test("externalId differs when any input changes", () => {
...
void test("artifacts are written atomically under the run dir", async () => {
  await withTempDir(async (runDir) => {
    const client = createMockClient();
    const input = buildInput(runDir, {
...
void test("markdown output path trims custom input and falls back to the default path", async () => {
  await withTempDir(async (runDir) => {
    const client = createMockClient();
    const cases = [buildTestCase({ id: "tc-a" })];
...
void test("no bearer token, password, or auth fragment leaks into artifacts", async () => {
  await withTempDir(async (runDir) => {
    const client = createMockClient();
    const input = buildInput(runDir, {
...
void test("createUnconfiguredJiraWriteClient returns provider_not_implemented on create", async () => {
  await withTempDir(async (runDir) => {
