 ...
 *   - export-report.json
 *
 * Re-record by running with TEST_INTELLIGENCE_GOLDEN_APPROVE=1.
 */
...
void test("golden: TEST_INTELLIGENCE_PROMPT_TEMPLATE_VERSION snapshot", () => {
  assert.equal(
    TEST_INTELLIGENCE_PROMPT_TEMPLATE_VERSION,
    "1.7.3",
    "Prompt template version changed - bump the literal in this snapshot and update the prompt-template version fixture.",
  );
});

void test("golden: export pipeline emits byte-identical artifacts", async () => {
  ...
  for (const artifact of ARTIFACTS) {
    ...
    assert.equal(
      serialized,
      expected,
      `golden ${artifact.name} drifted — re-run with TEST_INTELLIGENCE_GOLDEN_APPROVE=1`,
    );
  }
  ...
  if (APPROVE) {
    ...
    assert.equal(
      reportSerialized,
      expected,
      "golden export-report.json drifted — re-run with TEST_INTELLIGENCE_GOLDEN_APPROVE=1",
    );
  }
...
void test("golden: pipeline refuses if any of the cases is unapproved", () => {
  const result = runExportPipeline({
    jobId: "job-1365-golden",
    generatedAt: GENERATED_AT,
    intent: buildIntent(),
