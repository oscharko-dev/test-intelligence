...
import { createAirGapFetchGuard } from "@oscharko-dev/ti-production-runner";
import {
...
export interface SovereignLlmGatewayOptions {
   ...
   * with `TEST_INTELLIGENCE_AIR_GAP_MODE` set without mutating the
   * real environment).
   */
 ...
 * `TEST_INTELLIGENCE_AIR_GAP_MODE`, the guard is a transparent
 ...
 */
export { isAirGapModeEnabled } from "@oscharko-dev/ti-production-runner";

/**
 * Re-export the typed error so callers can `instanceof`-narrow without
 * pulling in the air-gap module by name.
 */
export { AirGapNetworkPolicyError } from "@oscharko-dev/ti-production-runner";
