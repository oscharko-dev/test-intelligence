...
const assertParityTriple = (
  ...
  const shadowViolations: readonly TestCasePolicyViolation[] =
    collectAllPolicyViolations(shadow.policy);
  const enforcedViolations: readonly TestCasePolicyViolation[] =
    collectAllPolicyViolations(enforced.policy);
...
void test("ReplayCacheKey contract excludes any harness-mode field so cache hits are mode-independent", () => {
