...
import { synthesizeGeneratedTestCases } from "@oscharko-dev/ti-production-runner";
...
void test("regression-eval: tolerances are exported as a frozen production-baseline profile", () => {
...
void test("regression-eval: ships snapshots covering all 7 baseline archetypes", () => {
  assert.equal(BASELINE_ARCHETYPE_FIXTURE_IDS.length, 7);
});

void test("regression-eval: snapshots are deterministic across rebuilds", async () => {
  const first = await buildAllRegressionSnapshots();
  const second = await buildAllRegressionSnapshots();
  assert.equal(canonicalJson(first), canonicalJson(second));
});

void test("regression-eval: every committed snapshot matches a fresh re-run", async () => {
...
void test("regression-eval: every snapshot covers every documented riskCategory and technique key", async () => {
  for (const archetypeId of BASELINE_ARCHETYPE_FIXTURE_IDS) {
    const baseline = await loadRegressionSnapshot(archetypeId);
...
void test("regression-eval: drift detection fires when a candidate run drops a test case", async () => {
...
void test("regression-eval: drift detection fires when the faithfulness verdict flips", async () => {
  const archetypeId = BASELINE_ARCHETYPE_FIXTURE_IDS[0];
  const baseline = await buildRegressionSnapshot({ archetypeId });
...
void test("regression-eval: tolerances absorb sub-threshold case-count perturbations", async () => {
  const archetypeId = BASELINE_ARCHETYPE_FIXTURE_IDS[0];
  const baseline = await buildRegressionSnapshot({ archetypeId });
  const fixture = await loadBaselineArchetypeFixture(archetypeId);
  const intent = deriveBusinessTestIntentIr({ figma: fixture.figma });
...
void test("regression-eval: writeRegressionSnapshot is atomic and round-trips canonical JSON", async () => {
  const archetypeId = BASELINE_ARCHETYPE_FIXTURE_IDS[0];
  const tempDir = await mkdtemp(join(tmpdir(), "regression-eval-"));
  const snapshot = await buildRegressionSnapshot({ archetypeId });
  const outputPath = join(tempDir, regressionSnapshotFilename(archetypeId));
...
void test("regression-eval: drift report renders identifying metadata and tolerance footer", async () => {
  const archetypeId = BASELINE_ARCHETYPE_FIXTURE_IDS[0];
  const baseline = await buildRegressionSnapshot({ archetypeId });
  ...
  assert.match(report, /TEST_INTELLIGENCE_REGRESSION_APPROVE/);
});

void test("regression-eval: writeDriftReport persists Markdown to a stable filename", async () => {
  ...
  assert.equal(
    written.endsWith(regressionDriftReportFilename(generatedAt)),
    true,
  );
...
void test("regression-eval: approve mode is gated by TEST_INTELLIGENCE_REGRESSION_APPROVE and CI runtime", () => {
  assert.equal(isRegressionApproveModeEnabled({}), false);
  assert.equal(
    isRegressionApproveModeEnabled({
      TEST_INTELLIGENCE_REGRESSION_APPROVE: "true",
    }),
    true,
  );
  assert.equal(
    isRegressionApproveModeEnabled({
      TEST_INTELLIGENCE_REGRESSION_APPROVE: "1",
    }),
    true,
  );
  assert.equal(
    isRegressionApproveModeEnabled({
      TEST_INTELLIGENCE_REGRESSION_APPROVE: "false",
    }),
    false,
  );
...
void test("regression-eval: diffRegressionSnapshot rejects archetypeId mismatch", async () => {
...
void test("regression-eval: tampered snapshot file fails the drift gate even when JSON parses", async () => {
  const archetypeId = BASELINE_ARCHETYPE_FIXTURE_IDS[0];
  ...
  const baselinePath = join(tempDir, regressionSnapshotFilename(archetypeId));
...
void test("regression-eval (#1950): runtime coverage-baseline seeds on the first run per archetype", async () => {
...
void test("regression-eval (#1950): drift > 10% on any tracked axis trips the runtime gate", async () => {
  const runtimeRoot = await mkdtempBaseline(
...
void test("regression-eval (#1950): operator-driven update mode rebases the runtime baseline", async () => {
  const runtimeRoot = await mkdtempBaseline(
