...
import type { UntrustedContentNormalizationOutcome } from "@oscharko-dev/ti-contracts";
import {
...
const inflateRepeat = (
  ...
  if (payloadBytes === 0) return "X".repeat(targetBytes);
  return payload.repeat(Math.ceil(targetBytes / payloadBytes));
};
...
export const isAdversarialCorpusReviewOverdue = (
  ...
  const due = parseReviewDueExclusiveMs(corpus.nextReviewDue);
...
const parseReviewDueExclusiveMs = (value: string): number => {
  ...
  if (dateOnly !== null) {
    ...
    const d = Number(day);
    const startOfDueDay = Date.UTC(y, m - 1, d);
    const normalized = new Date(startOfDueDay);
    if (
    ...
    return startOfDueDay + 86_400_000;
  }
