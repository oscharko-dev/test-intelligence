...
void test("handler: returns 503 when bearer token is not configured", async () => {
...
void test("handler: returns 401 when authorization header missing", async () => {
  await withTempDir(async (dir) => {
    const store = await seedStore(dir);
    const res = await handleReviewRequest(
    ...
    assert.equal(
      res.wwwAuthenticate,
      'Bearer realm="test-intelligence-review"',
    );
  });
});

void test("handler: returns 401 when bearer token mismatches", async () => {
  await withTempDir(async (dir) => {
    const store = await seedStore(dir);
    const res = await handleReviewRequest(
...
void test("handler: accepts case-insensitive scheme + whitespace", async () => {
  await withTempDir(async (dir) => {
    const store = await seedStore(dir);
    const res = await handleReviewRequest(
      baseEnvelope({
...
void test("handler: approve transitions state and persists event", async () => {
  await withTempDir(async (dir) => {
    const store = await seedStore(dir);
    const res = await handleReviewRequest(
...
void test("handler: reject transitions state to rejected", async () => {
  await withTempDir(async (dir) => {
    const store = await seedStore(dir);
    const res = await handleReviewRequest(
...
void test("handler: refuses approval when policy is blocked", async () => {
  await withTempDir(async (dir) => {
...
void test("handler: GET state requires no auth and returns snapshot + events", async () => {
  await withTempDir(async (dir) => {
    const store = await seedStore(dir);
    const res = await handleReviewRequest(
      baseEnvelope({
...
void test("handler: GET state returns 404 when job missing", async () => {
  await withTempDir(async (dir) => {
    const store = createFileSystemReviewStore({ destinationDir: dir });
    const res = await handleReviewRequest(
      baseEnvelope({
...
void test("handler: unknown write action returns 404", async () => {
  await withTempDir(async (dir) => {
    const store = await seedStore(dir);
    const res = await handleReviewRequest(
...
void test("handler: PUT method returns 405", async () => {
  await withTempDir(async (dir) => {
    const store = await seedStore(dir);
    const res = await handleReviewRequest(
...
void test("parseReviewRoute: extracts jobId/action/testCaseId from path", () => {
...
void test("parseReviewRoute: rejects unknown prefix", () => {
  const r = parseReviewRoute("/elsewhere/foo");
  assert.equal(r.ok, false);
});

void test("parseReviewRoute: rejects too many segments", () => {
  const r = parseReviewRoute(
...
void test("handler: approve action on a four-eyes case routes to primary then secondary", async () => {
  await withTempDir(async (dir) => {
...
void test("handler: legacy bearer is one principal, so it cannot satisfy four-eyes twice", async () => {
  await withTempDir(async (dir) => {
...
void test("handler: request body actor cannot impersonate the authenticated principal", async () => {
  await withTempDir(async (dir) => {
    const store = await seedFourEyesStore(dir, "regulated_data");
    const res = await handleReviewRequest(
...
void test("handler: explicit primary-approve action is accepted on four-eyes cases", async () => {
  await withTempDir(async (dir) => {
...
void test("handler: secondary-approve before primary returns 409 primary_approval_required", async () => {
  await withTempDir(async (dir) => {
    const store = await seedFourEyesStore(dir, "high");
    const res = await handleReviewRequest(
...
void test("handler: primary-approve without any configured principal returns 503", async () => {
  await withTempDir(async (dir) => {
    const store = await seedFourEyesStore(dir, "regulated_data");
    const res = await handleReviewRequest(
      baseEnvelope({
