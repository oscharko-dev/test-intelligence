...
import { describeVisualScreens } from "@oscharko-dev/ti-production-runner";
...
void test("sanitizeErrorMessage strips a Bearer token from an error message", () => {
...
void test("sanitizeErrorMessage strips an Authorization header value from an error message", () => {
...
void test("redactHighRiskSecrets strips figmaAccessToken from a debug log line", () => {
...
// -----------------------------------------------------------------------
// Jira gateway: replay cache secret isolation
// -----------------------------------------------------------------------

void test("Jira gateway persists replay cache without token-shaped raw response strings", async () => {
...
void test("visual sidecar failure: failureMessage is sanitized before persisting", async () => {
...
void test("sanitizeFolderResolutionEvidence via redactHighRiskSecrets strips x-figma-token header", () => {
...
void test("evidence manifest: modelDeployments field only contains allowed deployment strings, not free-form tokens", () => {
...
void test("evidence manifest: write + read does not introduce unexpected token-shaped fields", async () => {
...
void test("property: redactHighRiskSecrets strips any Bearer <token> where token matches /[A-Za-z0-9._-]{8,}/", () => {
