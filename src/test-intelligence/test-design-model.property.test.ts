...
void test("property: sourceHash is stable for byte-identical input", () => {
  fc.assert(
...
void test("property: canonical serialization is stable for repeated projection", () => {
  fc.assert(
    fc.property(intentArb, visualArb, envelopeArb, (intent, visual, sourceEnvelope) => {
      const first = buildTestDesignModel({
        jobId: "job-a",
...
void test("property: envelope changes participate in sourceHash", () => {
  fc.assert(
