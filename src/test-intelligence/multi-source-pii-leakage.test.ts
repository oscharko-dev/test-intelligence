...
import { ingestAndPersistJiraPaste } from "@oscharko-dev/ti-multi-source";
import { compilePrompt } from "@oscharko-dev/ti-production-runner";

void test("multi-source-pii-leakage: Jira + custom context artifacts and compiled prompt never persist raw PII", async () => {
  ...
  const compiled = compilePrompt({
    ...
    intent: {
      ...
      source: {
        kind: "jira_paste",
        contentHash: jira.result.jiraIssueIr.contentHash,
      },
      screens: [
        {
          screenId: "s-1",
...
void test("multi-source-pii-leakage: property redacts email-shaped custom context values deterministically", async () => {
  await fc.assert(
    fc.asyncProperty(
      fc
        .tuple(
