...
void test("recordPromptState returns no previous on first call for a querySource", async () => {
...
void test("recordPromptState returns previous snapshot on second call for same querySource", async () => {
  const h = await buildHarness();
  try {
...
void test("checkResponseForCacheBreak does nothing without a previous snapshot", async () => {
  const h = await buildHarness();
  try {
    const a = h.detector.recordPromptState({
      ...baseRecord,
...
void test("checkResponseForCacheBreak does NOT fire when read ratio is healthy", async () => {
  const h = await buildHarness();
  try {
...
void test("checkResponseForCacheBreak does NOT fire when cacheCreationTokens is below the floor", async () => {
  const h = await buildHarness();
  try {
    h.detector.recordPromptState({
      ...baseRecord,
...
void test("checkResponseForCacheBreak fires when both thresholds breach", async () => {
  const h = await buildHarness();
  try {
    h.detector.recordPromptState({
      ...baseRecord,
...
void test("notifyCompaction suppresses exactly one subsequent break for the same jobId", async () => {
  const h = await buildHarness();
  try {
    h.detector.recordPromptState({
      ...baseRecord,
...
void test("notifyCacheDeletion suppression is one-shot — next break fires normally", async () => {
  const h = await buildHarness();
  try {
    h.detector.recordPromptState({
      ...baseRecord,
...
void test("LRU caps at the configured snapshot count", async () => {
...
void test("recording the same querySource twice keeps LRU size at 1", async () => {
...
void test("AT-031: stable-prefix reorder between two iterations triggers cache_break + redacted diff artifact", async () => {
  const h = await buildHarness();
  try {
...
void test("diff artifact redacts high-risk secrets pasted into a prompt body", async () => {
  const h = await buildHarness();
  try {
    h.detector.recordPromptState({
      ...baseRecord,
...
void test("diff artifact strips zero-width prompt-injection chars before persistence", async () => {
  const h = await buildHarness();
  try {
    h.detector.recordPromptState({
      ...baseRecord,
...
void test("diff artifact filename uses the detector clock's millisecond timestamp", async () => {
  const h = await buildHarness();
  try {
    h.detector.recordPromptState({
      ...baseRecord,
...
void test("`cache_break` is in PRODUCTION_RUNNER_EVENT_PHASES", async () => {
  const mod = await import("@oscharko-dev/ti-production-runner");
  assert.ok(
...
void test("recordPromptState validates required string + numeric fields", async () => {
  const h = await buildHarness();
  try {
...
void test("checkResponseForCacheBreak rejects non-finite or negative token counts", async () => {
  const h = await buildHarness();
  try {
    const a = h.detector.recordPromptState({
      ...baseRecord,
