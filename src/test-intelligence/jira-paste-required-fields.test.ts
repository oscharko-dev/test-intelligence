...
void test("jira-paste-required-fields: missing issue key is refused fail-closed", () => {
  ...
  assert.equal(result.code, "paste_issue_key_missing");
});

void test("jira-paste-required-fields: Jira-shaped JSON without a summary fails closed during IR validation", () => {
  ...
  assert.equal(missingSummary.code, "paste_jira_ir_invalid");
});
