...
import { runExportPipeline } from "@oscharko-dev/ti-production-runner";
import {
...
void test("four-eyes: snapshot stamps fourEyesEnforced + reasons for high-risk case", async () => {
...
void test("four-eyes: custom-context regulated escalation enforces when regulated_data is configured", async () => {
...
void test("four-eyes: multi-source conflict policy outcome stamps the dedicated enforcement reason", async () => {
  await withTempDir("four-eyes-multi-source-conflict", async (dir) => {
    ...
    assert.equal(entry?.fourEyesEnforced, true);
    assert.deepEqual(entry.fourEyesReasons, ["multi_source_conflict_present"]);
  });
});

void test("four-eyes: high-risk case cannot reach approved with a single approver", async () => {
...
void test("four-eyes: two distinct approvers transition to approved with both identities recorded", async () => {
...
void test("four-eyes: self-approval (same actor twice) is rejected with a structured code", async () => {
...
void test("four-eyes: explicit secondary-approve action with same actor as primary is refused", async () => {
...
void test("four-eyes: duplicate explicit primary approval is refused with duplicate_principal_refused", async () => {
...
void test("four-eyes: secondary_approved without a primary is refused", async () => {
...
void test("four-eyes: primary_approved without an actor is refused", async () => {
...
void test("four-eyes: explicit primary_approved on a non-enforced case is refused", async () => {
...
void test("four-eyes: approving one's own edit is refused", async () => {
...
void test("four-eyes: non-high-risk case retains the one-reviewer flow from #1365", async () => {
...
void test("four-eyes: visual-sidecar low_confidence on the case's screen triggers four-eyes", async () => {
...
void test("four-eyes: re-edit invalidates the in-progress approval chain", async () => {
...
void test("four-eyes: snapshot persists fourEyesPolicy on disk", async () => {
...
void test("four-eyes: export pipeline blocks an enforced case stuck at pending_secondary_approval", async () => {
...
void test("four-eyes: export pipeline refuses forged approved state without two reviewers", () => {
  const tc = buildCase({
    id: "tc-fin",
    riskCategory: "financial_transaction",
  });
...
void test("four-eyes: export pipeline refuses forged approved state with missing secondary timestamp", () => {
  const tc = buildCase({
    id: "tc-fin",
    riskCategory: "financial_transaction",
  });
