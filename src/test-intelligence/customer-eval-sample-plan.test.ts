...
void test("customer-eval sample plan documents the required governance fields", async () => {
...
void test("customer-eval sample plan traces both rubrics back to explicit source families", async () => {
  const samplePlan = await readRepoFile(
    "fixtures/test-intelligence/customer-evals/SAMPLE-PLAN.md",
  );
...
void test("customer-eval sample-plan non-update note defines the explicit override path", async () => {
