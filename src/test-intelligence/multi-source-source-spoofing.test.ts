...
import { validateMultiSourceTestIntentEnvelope } from "@oscharko-dev/ti-multi-source";
...
void test("multi-source-source-spoofing: authenticated principal overrides body actor", async () => {
...
void test("multi-source-source-spoofing: forged sourceId and contentHash are refused by envelope validation", () => {
  ...
  assert.equal(
    validation.issues.some((issue) => issue.code === "invalid_source_id"),
    true,
  );
  assert.equal(
    validation.issues.some((issue) => issue.code === "invalid_content_hash"),
    true,
  );
});
