...
const buildLlmVerdict = (verdict: LogicJudgeVerdictLabel): JudgeVerdict => ({
...
void test("hard-gate emits empty_coverage_signals + upgrades accept to repair", () => {
...
void test("hard-gate emits hallucinated_id with the fabricated id in repair instruction", () => {
  const list = buildList([
...
void test("hard-gate accepts workflow ACT ids that are declared via coverage-plan targets", () => {
  const list = buildList([
...
void test("hard-gate rejects workflow ACT ids that are not declared via coverage-plan targets", () => {
  const list = buildList([
...
void test("hard-gate rejects exact validation details when the source marks validation behavior unresolved", () => {
...
void test("hard-gate does not reject label-only unresolved validation checks", () => {
  const model = buildTestDesignModel();
  model.screens[0]!.elements[0] = {
    elementId: "fld-a",
...
void test("hard-gate rejects VAT-inclusive financing need expectations when VAT is excluded", () => {
  const model = buildTestDesignModel();
  model.screens[0] = {
    screenId: "financing",
    name: "Financing Need",
  ...
  assert.match(
    repair.instruction,
    /Remove VAT from the financing-need formula\/result/u,
  );
  assert.equal(augmented.verdict, "repair");
});

void test("hard-gate emits insufficient_coverage_breadth at $job when below thresholds", () => {
...
void test("hard-gate emits weak_trace as warning and does NOT upgrade accept to repair", () => {
  const list = buildList([
...
void test("hard-gate leaves a reject verdict terminal even when adding error findings", () => {
  const list = buildList([
    buildTestCase("tc-empty", {
      qualitySignals: {
        coveredFieldIds: [],
...
void test("hard-gate passes through refusal verdicts untouched", () => {
...
void test("hard-gate accepts a fully covered, IR-faithful test case set with no findings", () => {
  const list = buildList([
...
void test("hard-gate skips navigation existence check when knownNavigationIds is undefined", () => {
  const list = buildList([
...
void test("hard-gate flags hallucinated navigation id when knownNavigationIds is supplied", () => {
  const list = buildList([
...
void test("runLogicJudge applies the hard-gate to a cache hit and a cache miss byte-identically", async () => {
...
void test("hard-gate emits no findings when the IR is empty (vacuous job)", () => {
