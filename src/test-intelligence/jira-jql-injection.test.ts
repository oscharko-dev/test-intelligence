...
void test("jira-jql-injection: curated malicious JQL fragments are rejected before fetch", async () => {
  const payloads = [
...
void test("jira-jql-injection: property rejects injection vectors with deterministic seed", async () => {
  await fc.assert(
    fc.asyncProperty(
      fc.oneof(
        ...
        fc
          .string({ minLength: 513, maxLength: 700 })
