...
import { ADVERSARIAL_NEGATIVE_RATIO_IMPROVEMENT_THRESHOLD } from "@oscharko-dev/ti-agentic-harness";
import {
...
void test("default profile carries the canonical id and version", () => {
  assert.equal(
...
void test("regulated_data and financial_transaction are review-only", () => {
  assert.ok(
...
void test("clone is mutable and isolated from the frozen default", () => {
...
void test("default profile enables risk-tag downgrade detection (Issue #1412)", () => {
  assert.equal(
...
void test("default profile is deep-frozen", () => {
...
void test("Issue #2053: default profile enforces G-NEG-CASE at 0.30 with deep-frozen config", () => {
...
void test("Issue #2053: clone round-trips negativeCaseLift and isolates it from the frozen default", () => {
  const a = cloneEuBankingDefaultProfile();
  const b = cloneEuBankingDefaultProfile();
...
void test("Issue #2068: default profile selects tier-elastic technique-coverage minimum", () => {
  const config =
...
void test("Issue #2171: clone round-trips techniqueCoverageMinimum tiers and isolates the override", () => {
  const a = cloneEuBankingDefaultProfile();
  const b = cloneEuBankingDefaultProfile();
  ...
  assert.ok(
    a.rules.techniqueCoverageMinimum.tiers,
...
void test("Issue #2053: default threshold matches the in-module agent constant", () => {
...
void test("Issue #2070: default profile enables 3-sample self-consistency voting", () => {
...
void test("Issue #2101: default profile routes faithfulness and a11y judge refusals to needs_review", () => {
  assert.deepEqual(
...
void test("Issue #2101: clone round-trips judgeRefusalPolicy and isolates overrides", () => {
  const a = cloneEuBankingDefaultProfile();
  const b = cloneEuBankingDefaultProfile();
...
void test("Issue #2070: clone round-trips selfConsistency and isolates overrides", () => {
  const a = cloneEuBankingDefaultProfile();
  const b = cloneEuBankingDefaultProfile();
...
void test("Issue #2116: secure default keeps requirePerStepFaithfulness off (warn-only)", () => {
...
void test("Issue #2116: clone round-trips requirePerStepFaithfulness and isolates overrides", () => {
  const a = cloneEuBankingDefaultProfile();
  const b = cloneEuBankingDefaultProfile();
...
void test("Issue #2169: default profile exposes the elastic FinOps wall-clock coefficients", () => {
  assert.deepEqual(
...
void test("Issue #2169: clone round-trips finopsWallClockBudget and isolates overrides", () => {
