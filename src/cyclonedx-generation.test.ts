...
const runGenerator = async ({
  ...
  const tempRoot = await mkdtemp(
    path.join(os.tmpdir(), "test-intelligence-cyclonedx-generation-"),
  );
  ...
  try {
    ...
    const result = await new Promise<{
      ...
      child.on("close", (code, signal) => {
        if (signal) {
          reject(
            new Error(`generate-cyclonedx exited via signal '${signal}'.`),
          );
          return;
        }
        resolve({
...
void test("CycloneDX generator ignores an inherited npm_execpath that points at pnpm", async () => {
  ...
  const components: unknown[] = Array.isArray(result.document.components)
    ? result.document.components
    : [];
  const packageKeys = components
    .filter((component): component is { name: string; version: string } =>
      Boolean(
        component &&
        ...
        typeof (component as { version: unknown }).version === "string",
      ),
    )
    .map((component) => `${component.name}@${component.version}`)
    .sort((first, second) => first.localeCompare(second));

  assert.deepEqual(packageKeys, [
    "allowed-child@2.0.0",
    "allowed-parent@1.2.0",
  ]);
});

void test("CycloneDX generator fails when npm reports dependency resolution errors", async () => {
  const result = await runGenerator({
    manifest: createPackageJson({
      name: "broken-project",
      dependencies: {
  ...
  assert.notEqual(
    result.code,
    0,
    "Expected generation to fail when dependencies are unresolved",
  );
  assert.equal(result.document, null);
  assert.match(result.stderr, /Command failed with exit code/);
});

void test("CycloneDX generator can explicitly ignore npm dependency resolution errors", async () => {
  const result = await runGenerator({
