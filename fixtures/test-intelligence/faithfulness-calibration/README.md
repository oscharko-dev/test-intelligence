# Faithfulness calibration anchors
...
The corpus mirrors the current faithfulness calibration acceptance criteria:
...
- **Anchors 061-075 — `label_only` partial-evidence-only.** Fifteen
  additional reviewer-confirmed `evidence_partial`
  ...
  required number for the v3 rubric recalibration.
- **Anchors 076-085 — `state_transition`.** Ten workflow steps from
  `technique === "state_transition"` test cases.
...
The anchors are loaded by the offline calibration harness. They are NOT used
to auto-tune thresholds at runtime - calibration is offline-only.
