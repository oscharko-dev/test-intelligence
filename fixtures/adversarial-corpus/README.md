...
test-intelligence stack. It extends the `adversarial-2025` fixture suite
with broader coverage across the full prompt-injection attack surface,
...
`mistral-large-3` and reviewed by the test-intelligence and security
maintainers before landing. The generation step is **design-time only**:
the corpus itself is committed JSON, and no model is invoked at gate time.
